/**
	...
	* API code for job QryWznmVisVis1NSheet (implementation)
...
QryWznmVisVis1NSheet::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmVisVis1NSheet::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmVisVis1NSheet");
	else
...
set<uint> QryWznmVisVis1NSheet::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmVisVis1NSheet::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmVisVis1NSheet::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmVisVis1NSheet");
	else
		basefound = checkXPath(docctx, basexpath);
