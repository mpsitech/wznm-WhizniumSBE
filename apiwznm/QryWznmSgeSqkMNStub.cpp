/**
	* \file QryWznmSgeSqkMNStub.cpp
	* API code for job QryWznmSgeSqkMNStub (implementation)
...
QryWznmSgeSqkMNStub::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmSgeSqkMNStub::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmSgeSqkMNStub");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmSgeSqkMNStub";
...
set<uint> QryWznmSgeSqkMNStub::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmSgeSqkMNStub::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmSgeSqkMNStub::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmSgeSqkMNStub");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmSgeSqkMNStub";
