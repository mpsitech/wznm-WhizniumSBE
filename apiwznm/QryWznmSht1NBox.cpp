/**
	...
	* API code for job QryWznmSht1NBox (implementation)
...
QryWznmSht1NBox::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmSht1NBox::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmSht1NBox");
	else
...
set<uint> QryWznmSht1NBox::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmSht1NBox::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmSht1NBox::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmSht1NBox");
	else
		basefound = checkXPath(docctx, basexpath);
