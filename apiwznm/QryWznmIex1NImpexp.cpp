/**
	...
	* API code for job QryWznmIex1NImpexp (implementation)
...
QryWznmIex1NImpexp::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmIex1NImpexp::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmIex1NImpexp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmIex1NImpexp";
...
set<uint> QryWznmIex1NImpexp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmIex1NImpexp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmIex1NImpexp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmIex1NImpexp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmIex1NImpexp";
