/**
	...
	* API code for job QryWznmTco1NQuerycol (implementation)
...
QryWznmTco1NQuerycol::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTco1NQuerycol::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTco1NQuerycol");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmTco1NQuerycol";
...
set<uint> QryWznmTco1NQuerycol::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTco1NQuerycol::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTco1NQuerycol::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTco1NQuerycol");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmTco1NQuerycol";
