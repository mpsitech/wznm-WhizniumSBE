/**
	...
	* API code for job QryWznmTblMNVector (implementation)
...
QryWznmTblMNVector::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTblMNVector::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTblMNVector");
	else
...
set<uint> QryWznmTblMNVector::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTblMNVector::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTblMNVector::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTblMNVector");
	else
		basefound = checkXPath(docctx, basexpath);
