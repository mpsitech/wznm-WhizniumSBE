/**
	...
	* API code for job QryWznmRelRef1NDialog (implementation)
...
QryWznmRelRef1NDialog::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmRelRef1NDialog::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmRelRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmRelRef1NDialog";
...
set<uint> QryWznmRelRef1NDialog::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmRelRef1NDialog::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmRelRef1NDialog::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmRelRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmRelRef1NDialog";
