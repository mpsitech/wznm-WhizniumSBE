/**
	...
	* API code for job QryWznmVecSrc1NFeed (implementation)
...
QryWznmVecSrc1NFeed::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmVecSrc1NFeed::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmVecSrc1NFeed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmVecSrc1NFeed";
...
set<uint> QryWznmVecSrc1NFeed::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmVecSrc1NFeed::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmVecSrc1NFeed::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmVecSrc1NFeed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmVecSrc1NFeed";
