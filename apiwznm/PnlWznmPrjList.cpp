/**
	...
	* API code for job PnlWznmPrjList (implementation)
...
bool PnlWznmPrjList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmPrjList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWznmPrjList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmPrjList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmPrjList::Tag::Tag(
			...
			, TcoGrl(TcoGrl)
		{
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOGRP, TCOOWN, TCOSHO, TCOTIT, TCOVER, TCOGRL};
};

bool PnlWznmPrjList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmPrjList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmPrjList::DpchAppData::DpchAppData(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMPRJLISTDATA, scrJref)
		{
...
void PnlWznmPrjList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmPrjListData");
...
PnlWznmPrjList::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMPRJLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWznmPrjList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmPrjListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWznmPrjList::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMPRJLISTDATA)
		{
...
void PnlWznmPrjList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmPrjListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWznmPrjList::StatApp();
		statshrqry = QryWznmPrjList::StatShr();
