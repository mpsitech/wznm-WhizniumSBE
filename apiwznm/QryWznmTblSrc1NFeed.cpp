/**
	...
	* API code for job QryWznmTblSrc1NFeed (implementation)
...
QryWznmTblSrc1NFeed::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTblSrc1NFeed::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTblSrc1NFeed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmTblSrc1NFeed";
...
set<uint> QryWznmTblSrc1NFeed::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTblSrc1NFeed::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTblSrc1NFeed::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTblSrc1NFeed");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmTblSrc1NFeed";
