/**
	...
	* API code for job QryWznmTcoRef1NControl (implementation)
...
QryWznmTcoRef1NControl::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTcoRef1NControl::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTcoRef1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmTcoRef1NControl";
...
set<uint> QryWznmTcoRef1NControl::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTcoRef1NControl::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTcoRef1NControl::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTcoRef1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmTcoRef1NControl";
