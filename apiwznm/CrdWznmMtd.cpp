/**
	* \file CrdWznmMtd.cpp
	* API code for job CrdWznmMtd (implementation)
...
bool CrdWznmMtd::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmMtd");
	else
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdMtd", MtxCrdMtd)) add(MTXCRDMTD);
	};
...
bool CrdWznmMtd::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWznmMtd");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> CrdWznmMtd::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdWznmMtd::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdWznmMtd::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmMtd");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool CrdWznmMtd::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmMtd");
	else
		basefound = checkXPath(docctx, basexpath);
...
CrdWznmMtd::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMMTDDO, scrJref)
		{
...
void CrdWznmMtd::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmMtdDo");
...
CrdWznmMtd::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMMTDDATA)
		{
...
void CrdWznmMtd::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmMtdData");
	else
		basefound = checkXPath(docctx, basexpath);
