/**
	...
	* API code for job QryWznmCmpMNOppack (implementation)
...
QryWznmCmpMNOppack::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmCmpMNOppack::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmCmpMNOppack");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmCmpMNOppack";
...
set<uint> QryWznmCmpMNOppack::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmCmpMNOppack::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmCmpMNOppack::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmCmpMNOppack");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmCmpMNOppack";
