/**
	...
	* API code for job QryWznmVisRef1NFile (implementation)
...
QryWznmVisRef1NFile::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmVisRef1NFile::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmVisRef1NFile");
	else
...
set<uint> QryWznmVisRef1NFile::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmVisRef1NFile::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmVisRef1NFile::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmVisRef1NFile");
	else
		basefound = checkXPath(docctx, basexpath);
