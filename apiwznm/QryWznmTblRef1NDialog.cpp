/**
	...
	* API code for job QryWznmTblRef1NDialog (implementation)
...
QryWznmTblRef1NDialog::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTblRef1NDialog::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTblRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmTblRef1NDialog";
...
set<uint> QryWznmTblRef1NDialog::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTblRef1NDialog::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTblRef1NDialog::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTblRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmTblRef1NDialog";
