/**
	...
	* API code for job QryWznmIexRef1NDialog (implementation)
...
QryWznmIexRef1NDialog::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmIexRef1NDialog::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmIexRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmIexRef1NDialog";
...
set<uint> QryWznmIexRef1NDialog::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmIexRef1NDialog::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmIexRef1NDialog::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmIexRef1NDialog");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmIexRef1NDialog";
