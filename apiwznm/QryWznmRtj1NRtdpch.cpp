/**
	...
	* API code for job QryWznmRtj1NRtdpch (implementation)
...
QryWznmRtj1NRtdpch::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmRtj1NRtdpch::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmRtj1NRtdpch");
	else
...
set<uint> QryWznmRtj1NRtdpch::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmRtj1NRtdpch::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmRtj1NRtdpch::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmRtj1NRtdpch");
	else
		basefound = checkXPath(docctx, basexpath);
