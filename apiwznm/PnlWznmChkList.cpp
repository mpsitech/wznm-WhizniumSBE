/**
	...
	* API code for job PnlWznmChkList (implementation)
...
bool PnlWznmChkList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmChkList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWznmChkList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmChkList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmChkList::Tag::Tag(
	...
	mask = {CPT, TXTFOR, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOTYP, TCOTBL, TCOTCO};
};

bool PnlWznmChkList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmChkList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmChkList::DpchAppData::DpchAppData(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMCHKLISTDATA, scrJref)
		{
...
void PnlWznmChkList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmChkListData");
...
PnlWznmChkList::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMCHKLISTDO, scrJref)
		{
...
void PnlWznmChkList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmChkListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWznmChkList::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMCHKLISTDATA)
		{
...
void PnlWznmChkList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmChkListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWznmChkList::StatApp();
		statshrqry = QryWznmChkList::StatShr();
