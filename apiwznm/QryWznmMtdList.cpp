/**
	...
	* API code for job QryWznmMtdList (implementation)
...
QryWznmMtdList::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmMtdList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmMtdList");
	else
...
set<uint> QryWznmMtdList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmMtdList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmMtdList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmMtdList");
	else
		basefound = checkXPath(docctx, basexpath);
