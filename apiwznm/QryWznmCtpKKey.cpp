/**
	...
	* API code for job QryWznmCtpKKey (implementation)
...
QryWznmCtpKKey::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmCtpKKey::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmCtpKKey");
	else
...
set<uint> QryWznmCtpKKey::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmCtpKKey::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmCtpKKey::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmCtpKKey");
	else
		basefound = checkXPath(docctx, basexpath);
