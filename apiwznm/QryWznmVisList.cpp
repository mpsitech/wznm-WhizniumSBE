/**
	...
	* API code for job QryWznmVisList (implementation)
...
QryWznmVisList::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmVisList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmVisList");
	else
...
set<uint> QryWznmVisList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmVisList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmVisList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmVisList");
	else
		basefound = checkXPath(docctx, basexpath);
