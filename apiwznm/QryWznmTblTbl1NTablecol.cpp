/**
	...
	* API code for job QryWznmTblTbl1NTablecol (implementation)
...
QryWznmTblTbl1NTablecol::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTblTbl1NTablecol::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTblTbl1NTablecol");
	else
...
set<uint> QryWznmTblTbl1NTablecol::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTblTbl1NTablecol::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTblTbl1NTablecol::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTblTbl1NTablecol");
	else
		basefound = checkXPath(docctx, basexpath);
