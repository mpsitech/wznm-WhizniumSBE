/**
	...
	* API code for job QryWznmApp1NRtjob (implementation)
...
QryWznmApp1NRtjob::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmApp1NRtjob::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmApp1NRtjob");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmApp1NRtjob";
...
set<uint> QryWznmApp1NRtjob::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmApp1NRtjob::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmApp1NRtjob::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmApp1NRtjob");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmApp1NRtjob";
