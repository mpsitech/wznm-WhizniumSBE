/**
	...
	* API code for job QryWznmVer1NImpexpcplx (implementation)
...
QryWznmVer1NImpexpcplx::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmVer1NImpexpcplx::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmVer1NImpexpcplx");
	else
...
set<uint> QryWznmVer1NImpexpcplx::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmVer1NImpexpcplx::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmVer1NImpexpcplx::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmVer1NImpexpcplx");
	else
		basefound = checkXPath(docctx, basexpath);
