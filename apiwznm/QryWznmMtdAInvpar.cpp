/**
	...
	* API code for job QryWznmMtdAInvpar (implementation)
...
QryWznmMtdAInvpar::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmMtdAInvpar::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmMtdAInvpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmMtdAInvpar";
...
set<uint> QryWznmMtdAInvpar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmMtdAInvpar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmMtdAInvpar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmMtdAInvpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmMtdAInvpar";
