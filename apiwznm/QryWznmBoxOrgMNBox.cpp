/**
	...
	* API code for job QryWznmBoxOrgMNBox (implementation)
...
QryWznmBoxOrgMNBox::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmBoxOrgMNBox::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmBoxOrgMNBox");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmBoxOrgMNBox";
...
set<uint> QryWznmBoxOrgMNBox::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmBoxOrgMNBox::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmBoxOrgMNBox::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmBoxOrgMNBox");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmBoxOrgMNBox";
