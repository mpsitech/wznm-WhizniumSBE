/**
	...
	* API code for job PnlWznmLocRec (implementation)
...
bool PnlWznmLocRec::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmLocRec");
	else
...
bool PnlWznmLocRec::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWznmLocRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWznmLocRec::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmLocRec::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {INITDONEDETAIL, INITDONEMNVERSION};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool PnlWznmLocRec::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmLocRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWznmLocRec::StatShr::diff(
	...
	diffitems = {IXWZNMVEXPSTATE, SCRJREFDETAIL, SCRJREFMNVERSION, BUTREGULARIZEACTIVE};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool PnlWznmLocRec::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmLocRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
void PnlWznmLocRec::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmLocRecDo");
...
PnlWznmLocRec::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMLOCRECDATA)
		{
};
...
void PnlWznmLocRec::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmLocRecData");
	else
		basefound = checkXPath(docctx, basexpath);
