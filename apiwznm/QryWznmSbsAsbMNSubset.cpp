/**
	...
	* API code for job QryWznmSbsAsbMNSubset (implementation)
...
QryWznmSbsAsbMNSubset::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmSbsAsbMNSubset::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmSbsAsbMNSubset");
	else
...
set<uint> QryWznmSbsAsbMNSubset::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmSbsAsbMNSubset::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmSbsAsbMNSubset::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmSbsAsbMNSubset");
	else
		basefound = checkXPath(docctx, basexpath);
