/**
	* \file PnlWznmMtdARetpar.cpp
	* API code for job PnlWznmMtdARetpar (implementation)
...
bool PnlWznmMtdARetpar::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmMtdARetpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfWznmMtdARetpar";
...
PnlWznmMtdARetpar::StatApp::StatApp(
...
bool PnlWznmMtdARetpar::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWznmMtdARetpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppWznmMtdARetpar";
...
set<uint> PnlWznmMtdARetpar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmMtdARetpar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWznmMtdARetpar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmMtdARetpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrWznmMtdARetpar";
...
PnlWznmMtdARetpar::Tag::Tag(
	...
	mask = {CPT, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOVTY, TCOVEC, TCOLEN, TCOCMT};
};

bool PnlWznmMtdARetpar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmMtdARetpar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWznmMtdARetpar";
...
PnlWznmMtdARetpar::DpchAppData::DpchAppData(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMMTDARETPARDATA, scrJref)
		{
...
void PnlWznmMtdARetpar::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmMtdARetparData");
...
PnlWznmMtdARetpar::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMMTDARETPARDO, scrJref)
		{
...
void PnlWznmMtdARetpar::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmMtdARetparDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWznmMtdARetpar::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMMTDARETPARDATA)
		{
...
void PnlWznmMtdARetpar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmMtdARetparData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWznmMtdARetpar::StatApp();
		statshrqry = QryWznmMtdARetpar::StatShr();
