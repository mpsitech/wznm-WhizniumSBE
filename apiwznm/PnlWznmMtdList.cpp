/**
	...
	* API code for job PnlWznmMtdList (implementation)
...
bool PnlWznmMtdList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmMtdList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWznmMtdList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmMtdList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmMtdList::Tag::Tag(
			...
			, TcoExs(TcoExs)
		{
	mask = {CPT, TXTFOR, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSRF, TCOJOB, TCOEXS};
};

bool PnlWznmMtdList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmMtdList");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoExs", TcoExs)) add(TCOEXS);
	};
...
PnlWznmMtdList::DpchAppData::DpchAppData(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMMTDLISTDATA, scrJref)
		{
...
void PnlWznmMtdList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmMtdListData");
...
PnlWznmMtdList::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMMTDLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWznmMtdList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmMtdListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWznmMtdList::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMMTDLISTDATA)
		{
...
void PnlWznmMtdList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmMtdListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWznmMtdList::StatApp();
		statshrqry = QryWznmMtdList::StatShr();
