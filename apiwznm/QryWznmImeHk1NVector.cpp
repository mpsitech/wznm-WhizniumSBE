/**
	...
	* API code for job QryWznmImeHk1NVector (implementation)
...
QryWznmImeHk1NVector::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmImeHk1NVector::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmImeHk1NVector");
	else
...
set<uint> QryWznmImeHk1NVector::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmImeHk1NVector::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmImeHk1NVector::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmImeHk1NVector");
	else
		basefound = checkXPath(docctx, basexpath);
