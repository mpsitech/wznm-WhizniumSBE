/**
	...
	* API code for job QryWznmSeqSeq1NState (implementation)
...
QryWznmSeqSeq1NState::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmSeqSeq1NState::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmSeqSeq1NState");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmSeqSeq1NState";
...
set<uint> QryWznmSeqSeq1NState::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmSeqSeq1NState::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmSeqSeq1NState::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmSeqSeq1NState");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmSeqSeq1NState";
