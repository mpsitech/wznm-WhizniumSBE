/**
	...
	* API code for job QryWznmConSup1NControl (implementation)
...
QryWznmConSup1NControl::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmConSup1NControl::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmConSup1NControl");
	else
...
set<uint> QryWznmConSup1NControl::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmConSup1NControl::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmConSup1NControl::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmConSup1NControl");
	else
		basefound = checkXPath(docctx, basexpath);
