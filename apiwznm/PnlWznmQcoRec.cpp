/**
	...
	* API code for job PnlWznmQcoRec (implementation)
...
bool PnlWznmQcoRec::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmQcoRec");
	else
...
bool PnlWznmQcoRec::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWznmQcoRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWznmQcoRec::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmQcoRec::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {INITDONEDETAIL, INITDONEREF1NCONTROL};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool PnlWznmQcoRec::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmQcoRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlWznmQcoRec::StatShr::diff(
	...
	diffitems = {IXWZNMVEXPSTATE, SCRJREFDETAIL, SCRJREFREF1NCONTROL, BUTREGULARIZEACTIVE};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
bool PnlWznmQcoRec::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmQcoRec");
	else
		basefound = checkXPath(docctx, basexpath);
...
void PnlWznmQcoRec::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmQcoRecDo");
...
PnlWznmQcoRec::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMQCORECDATA)
		{
};
...
void PnlWznmQcoRec::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmQcoRecData");
	else
		basefound = checkXPath(docctx, basexpath);
