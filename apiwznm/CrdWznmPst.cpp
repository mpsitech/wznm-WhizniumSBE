/**
	* \file CrdWznmPst.cpp
	* API code for job CrdWznmPst (implementation)
...
bool CrdWznmPst::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmPst");
	else
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdPst", MtxCrdPst)) add(MTXCRDPST);
	};
...
bool CrdWznmPst::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppWznmPst");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> CrdWznmPst::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdWznmPst::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdWznmPst::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmPst");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool CrdWznmPst::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmPst");
	else
		basefound = checkXPath(docctx, basexpath);
...
CrdWznmPst::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMPSTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void CrdWznmPst::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmPstDo");
...
CrdWznmPst::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMPSTDATA)
		{
...
void CrdWznmPst::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmPstData");
	else
		basefound = checkXPath(docctx, basexpath);
