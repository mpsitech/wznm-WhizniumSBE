/**
	...
	* API code for job QryWznmJobJob1NStage (implementation)
...
QryWznmJobJob1NStage::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmJobJob1NStage::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmJobJob1NStage");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmJobJob1NStage";
...
set<uint> QryWznmJobJob1NStage::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmJobJob1NStage::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmJobJob1NStage::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmJobJob1NStage");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmJobJob1NStage";
