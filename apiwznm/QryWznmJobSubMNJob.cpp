/**
	...
	* API code for job QryWznmJobSubMNJob (implementation)
...
QryWznmJobSubMNJob::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmJobSubMNJob::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmJobSubMNJob");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmJobSubMNJob";
...
set<uint> QryWznmJobSubMNJob::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmJobSubMNJob::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmJobSubMNJob::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmJobSubMNJob");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmJobSubMNJob";
