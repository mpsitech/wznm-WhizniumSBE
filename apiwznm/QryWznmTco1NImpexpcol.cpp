/**
	...
	* API code for job QryWznmTco1NImpexpcol (implementation)
...
QryWznmTco1NImpexpcol::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTco1NImpexpcol::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTco1NImpexpcol");
	else
...
set<uint> QryWznmTco1NImpexpcol::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTco1NImpexpcol::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTco1NImpexpcol::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTco1NImpexpcol");
	else
		basefound = checkXPath(docctx, basexpath);
