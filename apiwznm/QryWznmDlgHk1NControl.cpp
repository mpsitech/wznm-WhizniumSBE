/**
	...
	* API code for job QryWznmDlgHk1NControl (implementation)
...
QryWznmDlgHk1NControl::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmDlgHk1NControl::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmDlgHk1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmDlgHk1NControl";
...
set<uint> QryWznmDlgHk1NControl::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmDlgHk1NControl::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmDlgHk1NControl::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmDlgHk1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmDlgHk1NControl";
