/**
	...
	* API code for job QryWznmDlgMNQuery (implementation)
...
QryWznmDlgMNQuery::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmDlgMNQuery::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmDlgMNQuery");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmDlgMNQuery";
...
set<uint> QryWznmDlgMNQuery::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmDlgMNQuery::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmDlgMNQuery::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmDlgMNQuery");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmDlgMNQuery";
