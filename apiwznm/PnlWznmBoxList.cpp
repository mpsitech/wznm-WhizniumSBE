/**
	...
	* API code for job PnlWznmBoxList (implementation)
...
bool PnlWznmBoxList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfWznmBoxList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlWznmBoxList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrWznmBoxList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmBoxList::Tag::Tag(
			...
			, TcoUnv(TcoUnv)
		{
	mask = {CPT, TXTFOR, TXTRECORD1, TXTRECORD2, TRS, TXTSHOWING1, TXTSHOWING2, TCOSHT, TCOMTB, TCOUNV};
};

bool PnlWznmBoxList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmBoxList");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoSht", TcoSht)) add(TCOSHT);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoMtb", TcoMtb)) add(TCOMTB);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoUnv", TcoUnv)) add(TCOUNV);
	};
...
PnlWznmBoxList::DpchAppData::DpchAppData(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMBOXLISTDATA, scrJref)
		{
...
void PnlWznmBoxList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmBoxListData");
...
PnlWznmBoxList::DpchAppDo::DpchAppDo(
			...
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMBOXLISTDO, scrJref)
			, ixVDo(ixVDo)
		{
...
void PnlWznmBoxList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppWznmBoxListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlWznmBoxList::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMBOXLISTDATA)
		{
...
void PnlWznmBoxList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmBoxListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryWznmBoxList::StatApp();
		statshrqry = QryWznmBoxList::StatShr();
