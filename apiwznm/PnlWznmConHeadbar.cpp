/**
	* \file PnlWznmConHeadbar.cpp
	* API code for job PnlWznmConHeadbar (implementation)
...
PnlWznmConHeadbar::StgInf::StgInf(
...
bool PnlWznmConHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWznmConHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWznmConHeadbar";
...
set<uint> PnlWznmConHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmConHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWznmConHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmConHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWznmConHeadbar";
...
PnlWznmConHeadbar::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMCONHEADBARDATA)
		{
};
...
void PnlWznmConHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmConHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
