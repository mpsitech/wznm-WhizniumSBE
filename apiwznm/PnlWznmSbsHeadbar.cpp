/**
	* \file PnlWznmSbsHeadbar.cpp
	* API code for job PnlWznmSbsHeadbar (implementation)
...
PnlWznmSbsHeadbar::StgInf::StgInf(
...
bool PnlWznmSbsHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWznmSbsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWznmSbsHeadbar";
...
set<uint> PnlWznmSbsHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmSbsHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWznmSbsHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmSbsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWznmSbsHeadbar";
...
PnlWznmSbsHeadbar::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMSBSHEADBARDATA)
		{
};
...
void PnlWznmSbsHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmSbsHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
