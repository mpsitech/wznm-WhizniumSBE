/**
	...
	* API code for job QryWznmSbsBsbMNSubset (implementation)
...
QryWznmSbsBsbMNSubset::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmSbsBsbMNSubset::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmSbsBsbMNSubset");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmSbsBsbMNSubset";
...
set<uint> QryWznmSbsBsbMNSubset::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmSbsBsbMNSubset::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmSbsBsbMNSubset::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmSbsBsbMNSubset");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmSbsBsbMNSubset";
