/**
	...
	* API code for job QryWznmOpxSqkMNStub (implementation)
...
QryWznmOpxSqkMNStub::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmOpxSqkMNStub::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmOpxSqkMNStub");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmOpxSqkMNStub";
...
set<uint> QryWznmOpxSqkMNStub::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmOpxSqkMNStub::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmOpxSqkMNStub::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmOpxSqkMNStub");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmOpxSqkMNStub";
