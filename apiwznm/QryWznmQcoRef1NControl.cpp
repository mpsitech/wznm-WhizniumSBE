/**
	...
	* API code for job QryWznmQcoRef1NControl (implementation)
...
QryWznmQcoRef1NControl::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmQcoRef1NControl::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmQcoRef1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmQcoRef1NControl";
...
set<uint> QryWznmQcoRef1NControl::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmQcoRef1NControl::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmQcoRef1NControl::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmQcoRef1NControl");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmQcoRef1NControl";
