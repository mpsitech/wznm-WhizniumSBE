/**
	...
	* API code for job QryWznmTbl1NCheck (implementation)
...
QryWznmTbl1NCheck::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmTbl1NCheck::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmTbl1NCheck");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmTbl1NCheck";
...
set<uint> QryWznmTbl1NCheck::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmTbl1NCheck::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmTbl1NCheck::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmTbl1NCheck");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmTbl1NCheck";
