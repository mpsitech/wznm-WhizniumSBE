/**
	* \file PnlWznmLibHeadbar.cpp
	* API code for job PnlWznmLibHeadbar (implementation)
...
PnlWznmLibHeadbar::StgInf::StgInf(
...
bool PnlWznmLibHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWznmLibHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWznmLibHeadbar";
...
set<uint> PnlWznmLibHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmLibHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWznmLibHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmLibHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWznmLibHeadbar";
...
PnlWznmLibHeadbar::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMLIBHEADBARDATA)
		{
};
...
void PnlWznmLibHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmLibHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
