/**
	...
	* API code for job QryWznmOpkAInvarg (implementation)
...
QryWznmOpkAInvarg::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmOpkAInvarg::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmOpkAInvarg");
	else
...
set<uint> QryWznmOpkAInvarg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmOpkAInvarg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmOpkAInvarg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmOpkAInvarg");
	else
		basefound = checkXPath(docctx, basexpath);
