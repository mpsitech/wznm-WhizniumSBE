/**
	...
	* API code for job QryWznmBoxList (implementation)
...
QryWznmBoxList::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmBoxList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmBoxList");
	else
...
set<uint> QryWznmBoxList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmBoxList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmBoxList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmBoxList");
	else
		basefound = checkXPath(docctx, basexpath);
