/**
	...
	* API code for job QryWznmShtList (implementation)
...
QryWznmShtList::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmShtList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmShtList");
	else
...
set<uint> QryWznmShtList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmShtList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmShtList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmShtList");
	else
		basefound = checkXPath(docctx, basexpath);
