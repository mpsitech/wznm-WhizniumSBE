/**
	* \file PnlWznmRlsHeadbar.cpp
	* API code for job PnlWznmRlsHeadbar (implementation)
...
PnlWznmRlsHeadbar::StgInf::StgInf(
...
bool PnlWznmRlsHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfWznmRlsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfWznmRlsHeadbar";
...
set<uint> PnlWznmRlsHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmRlsHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlWznmRlsHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagWznmRlsHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemWznmRlsHeadbar";
...
PnlWznmRlsHeadbar::DpchEngData::DpchEngData() :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMRLSHEADBARDATA)
		{
};
...
void PnlWznmRlsHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngWznmRlsHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
