/**
	...
	* API code for job QryWznmQryQry1NQuerycol (implementation)
...
QryWznmQryQry1NQuerycol::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmQryQry1NQuerycol::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmQryQry1NQuerycol");
	else
...
set<uint> QryWznmQryQry1NQuerycol::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmQryQry1NQuerycol::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmQryQry1NQuerycol::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmQryQry1NQuerycol");
	else
		basefound = checkXPath(docctx, basexpath);
