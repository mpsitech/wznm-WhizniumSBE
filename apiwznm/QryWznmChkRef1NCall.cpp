/**
	...
	* API code for job QryWznmChkRef1NCall (implementation)
...
QryWznmChkRef1NCall::StatApp::StatApp(
			const uint firstcol
...
bool QryWznmChkRef1NCall::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryWznmChkRef1NCall");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryWznmChkRef1NCall";
...
set<uint> QryWznmChkRef1NCall::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryWznmChkRef1NCall::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryWznmChkRef1NCall::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryWznmChkRef1NCall");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryWznmChkRef1NCall";
