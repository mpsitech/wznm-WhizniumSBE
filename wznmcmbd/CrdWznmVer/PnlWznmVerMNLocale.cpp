/**
	...
	* job handler for job PnlWznmVerMNLocale (implementation)
...
#include "PnlWznmVerMNLocale_blks.cpp"
#include "PnlWznmVerMNLocale_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerMNLocale
 ******************************************************************************/

PnlWznmVerMNLocale::PnlWznmVerMNLocale(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERMNLOCALE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerMNLocale(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerMNLocale::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerMNLocale::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVerMNLocale::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerMNLocale::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERMNLOCALEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERMNLOCALEDO) {
...
void PnlWznmVerMNLocale::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerMNLocale::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerMNLocale::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerMNLocale::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerMNLocale* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerMNLocale::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerMNLocale::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerMNLocale::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmlocale->loadRecByRef(recSelNew->mref, &_recLoc)) {
					recLoc = *_recLoc;
					delete _recLoc;
				};
			};
...
void PnlWznmVerMNLocale::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCLOC, jref)) {
			sref = "CrdWznmLoc";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recLoc.ref, jrefNew);
		};
...
void PnlWznmVerMNLocale::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerMNLocale::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerMNLocale::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerMNLocale::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerMNLocale::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
