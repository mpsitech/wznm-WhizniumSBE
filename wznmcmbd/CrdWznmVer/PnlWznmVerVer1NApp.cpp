/**
	...
	* job handler for job PnlWznmVerVer1NApp (implementation)
...
#include "PnlWznmVerVer1NApp_blks.cpp"
#include "PnlWznmVerVer1NApp_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerVer1NApp
 ******************************************************************************/

PnlWznmVerVer1NApp::PnlWznmVerVer1NApp(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERVER1NAPP, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerVer1NApp(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerVer1NApp::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerVer1NApp::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVerVer1NApp::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NApp::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NAPPDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NAPPDO) {
...
void PnlWznmVerVer1NApp::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NApp::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NApp::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerVer1NApp::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerVer1NApp* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerVer1NApp::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerVer1NApp::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerVer1NApp::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recApp = WznmMApp();

			if (recSelNew) {
				if (dbswznm->tblwznmmapp->loadRecByRef(recSelNew->ref, &_recApp)) {
					recApp = *_recApp;
					delete _recApp;
				};
			};
...
void PnlWznmVerVer1NApp::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NApp::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NApp::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCAPP, jref)) {
			sref = "CrdWznmApp";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recApp.ref, jrefNew);
		};
...
void PnlWznmVerVer1NApp::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCAPP, jref) & VecWznmWAccess::EDIT) != 0) {
			sref = "CrdWznmApp";
...
void PnlWznmVerVer1NApp::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NApp::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerVer1NApp::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerVer1NApp::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
