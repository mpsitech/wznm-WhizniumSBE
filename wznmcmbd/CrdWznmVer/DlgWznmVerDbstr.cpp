/**
	...
	* job handler for job DlgWznmVerDbstr (implementation)
...
#include "DlgWznmVerDbstr_blks.cpp"
#include "DlgWznmVerDbstr_evals.cpp"
...
/******************************************************************************
 class DlgWznmVerDbstr
 ******************************************************************************/

DlgWznmVerDbstr::DlgWznmVerDbstr(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::DLGWZNMVERDBSTR, jrefSup, ixWznmVLocale)
		{
	...
	iex = new JobWznmIexDbs(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* DlgWznmVerDbstr::getNewDpchEng(
			set<uint> items
		) {
...
void DlgWznmVerDbstr::refreshIfi(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void DlgWznmVerDbstr::refreshImp(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void DlgWznmVerDbstr::refreshPpr(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void DlgWznmVerDbstr::refreshLfi(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void DlgWznmVerDbstr::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPDLGWZNMVERDBSTRDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPDLGWZNMVERDBSTRDO) {
...
void DlgWznmVerDbstr::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void DlgWznmVerDbstr::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppDoButDneClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppDoImpButRunClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppDoImpButStoClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppDoPprButRunClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppDoPprButStoClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void DlgWznmVerDbstr::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
void DlgWznmVerDbstr::handleDpchRetWznm(
			DbsWznm* dbswznm
			, DpchRetWznm* dpchret
		) {
...
void DlgWznmVerDbstr::handleUploadInSgeIdle(
			DbsWznm* dbswznm
...
string DlgWznmVerDbstr::handleDownloadInSgeDone(
			DbsWznm* dbswznm
		) {
...
void DlgWznmVerDbstr::handleTimerWithSrefMonInSgeImport(
			DbsWznm* dbswznm
		) {
...
void DlgWznmVerDbstr::changeStage(
			DbsWznm* dbswznm
			, uint _ixVSge
			, DpchEngWznm** dpcheng
		) {
...
string DlgWznmVerDbstr::getSquawk(
			DbsWznm* dbswznm
		) {
	string retval;
	// IP getSquawk --- RBEGIN
	if ( (ixVSge == VecVSge::PARSE) || (ixVSge == VecVSge::ALRWZNMPER) || (ixVSge == VecVSge::PRSDONE) || (ixVSge == VecVSge::IMPORT) || (ixVSge == VecVSge::ALRWZNMIER) || (ixVSge == VecVSge::IMPDONE) || (ixVSge == VecVSge::POSTPRC) ) {
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			...
			else if (ixVSge == VecVSge::IMPORT) retval = "importing database structure (" + to_string(iex->impcnt) + " records added)";
...
uint DlgWznmVerDbstr::enterSgeIdle(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint DlgWznmVerDbstr::enterSgePrsidle(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgePrsidle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgePrsidle --- INSERT
};

uint DlgWznmVerDbstr::enterSgeParse(
			DbsWznm* dbswznm
			, const bool reenter
		) {
	uint retval;
	...
	if (iex->ixVSge != JobWznmIexDbs::VecVSge::PRSDONE) {
		if (reqCmd) {
			if (iex->ixVSge == JobWznmIexDbs::VecVSge::PRSERR) cout << "\t" << iex->getSquawk(dbswznm) << endl;
...
void DlgWznmVerDbstr::leaveSgeParse(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeParse --- INSERT
};

uint DlgWznmVerDbstr::enterSgeAlrwznmper(
			DbsWznm* dbswznm
			, const bool reenter
		) {
	...
	if (iex->ixVSge == JobWznmIexDbs::VecVSge::PRSERR) continf.TxtMsg1 = iex->getSquawk(dbswznm);
...
void DlgWznmVerDbstr::leaveSgeAlrwznmper(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeAlrwznmper --- INSERT
};

uint DlgWznmVerDbstr::enterSgePrsdone(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgePrsdone(
			DbsWznm* dbswznm
		) {
	// IP leaveSgePrsdone --- INSERT
};

uint DlgWznmVerDbstr::enterSgeImpidle(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgeImpidle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeImpidle --- INSERT
};

uint DlgWznmVerDbstr::enterSgeImport(
			DbsWznm* dbswznm
			, const bool reenter
		) {
	uint retval;
	...
	if (iex->ixVSge == JobWznmIexDbs::VecVSge::IMPERR) retval = nextIxVSgeFailure;
...
void DlgWznmVerDbstr::leaveSgeImport(
			DbsWznm* dbswznm
		) {
	invalidateWakeups();
...
uint DlgWznmVerDbstr::enterSgeAlrwznmier(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgeAlrwznmier(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeAlrwznmier --- INSERT
};

uint DlgWznmVerDbstr::enterSgeImpdone(
			DbsWznm* dbswznm
			, const bool reenter
		) {
	...
	if (ifitxt) {
		Filename = "IexWznmDbs_" + StrMod::lc(Prjshort) + ".txt";
		srefKMimetype = "txt";
	} else if (ifixml) {
		Filename = "IexWznmDbs_" + StrMod::lc(Prjshort) + ".xml";
		srefKMimetype = "xml";
	};
...
void DlgWznmVerDbstr::leaveSgeImpdone(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeImpdone --- INSERT
};

uint DlgWznmVerDbstr::enterSgePostprc(
			DbsWznm* dbswznm
			, const bool reenter
		) {
	...
	addInv(new DpchInvWznmComplDbs(0, 0, refWznmMVersion, Prjshort));
...
void DlgWznmVerDbstr::leaveSgePostprc(
			DbsWznm* dbswznm
		) {
	// IP leaveSgePostprc --- IBEGIN
	ubigint refWznmMVersion = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFVER, jref);

	Wznm::updateVerste(dbswznm, refWznmMVersion, VecWznmVMVersionState::DSNDBSTR);
	// IP leaveSgePostprc --- IEND
};

uint DlgWznmVerDbstr::enterSgeDone(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void DlgWznmVerDbstr::leaveSgeDone(
			DbsWznm* dbswznm
		) {
