/**
	...
	* job handler for job PnlWznmVer1NOppack (implementation)
...
#include "PnlWznmVer1NOppack_blks.cpp"
#include "PnlWznmVer1NOppack_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NOppack
 ******************************************************************************/

PnlWznmVer1NOppack::PnlWznmVer1NOppack(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NOPPACK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NOppack(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NOppack::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NOppack::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVer1NOppack::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NOppack::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NOPPACKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NOPPACKDO) {
...
void PnlWznmVer1NOppack::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NOppack::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NOppack::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NOppack::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVer1NOppack* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NOppack::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NOppack::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NOppack::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOpk = WznmMOppack();

			if (recSelNew) {
				if (dbswznm->tblwznmmoppack->loadRecByRef(recSelNew->ref, &_recOpk)) {
					recOpk = *_recOpk;
					delete _recOpk;
				};
			};
...
void PnlWznmVer1NOppack::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmVer1NOppack::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPK, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmOpk";
...
void PnlWznmVer1NOppack::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NOppack::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NOppack::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NOppack::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
