/**
	...
	* job handler for job PnlWznmVerVer1NError (implementation)
...
#include "PnlWznmVerVer1NError_blks.cpp"
#include "PnlWznmVerVer1NError_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerVer1NError
 ******************************************************************************/

PnlWznmVerVer1NError::PnlWznmVerVer1NError(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERVER1NERROR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerVer1NError(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerVer1NError::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerVer1NError::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVerVer1NError::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NError::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NERRORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NERRORDO) {
...
void PnlWznmVerVer1NError::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NError::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NError::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerVer1NError::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerVer1NError* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerVer1NError::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerVer1NError::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerVer1NError::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recErr = WznmMError();

			if (recSelNew) {
				if (dbswznm->tblwznmmerror->loadRecByRef(recSelNew->ref, &_recErr)) {
					recErr = *_recErr;
					delete _recErr;
				};
			};
...
void PnlWznmVerVer1NError::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NError::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NError::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCERR, jref)) if (refVer != 0) {
			sref = "CrdWznmErr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recErr.ref, jrefNew);
		};
...
void PnlWznmVerVer1NError::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCERR, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmErr";
...
void PnlWznmVerVer1NError::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NError::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerVer1NError::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerVer1NError::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
