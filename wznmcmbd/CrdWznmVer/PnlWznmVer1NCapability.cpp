/**
	...
	* job handler for job PnlWznmVer1NCapability (implementation)
...
#include "PnlWznmVer1NCapability_blks.cpp"
#include "PnlWznmVer1NCapability_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NCapability
 ******************************************************************************/

PnlWznmVer1NCapability::PnlWznmVer1NCapability(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NCAPABILITY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NCapability(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NCapability::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NCapability::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVer1NCapability::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NCapability::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCAPABILITYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCAPABILITYDO) {
...
void PnlWznmVer1NCapability::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NCapability::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NCapability::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NCapability::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVer1NCapability* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NCapability::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NCapability::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NCapability::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCpb = WznmMCapability();

			if (recSelNew) {
				if (dbswznm->tblwznmmcapability->loadRecByRef(recSelNew->ref, &_recCpb)) {
					recCpb = *_recCpb;
...
void PnlWznmVer1NCapability::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAP, jref)) if ((ixWSubsetCpb & VecWznmWMCapabilitySubset::SBSWZNMBMCAPABILITYCAP) != 0) if (refVer != 0) {
			sref = "CrdWznmCap";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recCpb.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCTP, jref)) if ((ixWSubsetCpb & VecWznmWMCapabilitySubset::SBSWZNMBMCAPABILITYCTP) != 0) {
				sref = "CrdWznmCtp";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recCpb.ref, jrefNew);
			};
		};
...
void PnlWznmVer1NCapability::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmVer1NCapability::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NCapability::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NCapability::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NCapability::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
