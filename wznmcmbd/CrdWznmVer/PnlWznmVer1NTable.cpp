/**
	...
	* job handler for job PnlWznmVer1NTable (implementation)
...
#include "PnlWznmVer1NTable_blks.cpp"
#include "PnlWznmVer1NTable_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NTable
 ******************************************************************************/

PnlWznmVer1NTable::PnlWznmVer1NTable(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NTABLE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NTable(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NTable::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NTable::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVer1NTable::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NTable::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NTABLEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NTABLEDO) {
...
void PnlWznmVer1NTable::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NTable::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NTable::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NTable::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVer1NTable* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NTable::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NTable::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NTable::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recTbl = WznmMTable();

			if (recSelNew) {
				if (dbswznm->tblwznmmtable->loadRecByRef(recSelNew->ref, &_recTbl)) {
					recTbl = *_recTbl;
...
void PnlWznmVer1NTable::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCTBL, jref)) if (refVer != 0) {
			sref = "CrdWznmTbl";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recTbl.ref, jrefNew);
		};
...
void PnlWznmVer1NTable::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmVer1NTable::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NTable::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NTable::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NTable::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
