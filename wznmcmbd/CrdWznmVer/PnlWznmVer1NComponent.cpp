/**
	...
	* job handler for job PnlWznmVer1NComponent (implementation)
...
#include "PnlWznmVer1NComponent_blks.cpp"
#include "PnlWznmVer1NComponent_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NComponent
 ******************************************************************************/

PnlWznmVer1NComponent::PnlWznmVer1NComponent(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NCOMPONENT, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NComponent(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NComponent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NComponent::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVer1NComponent::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NComponent::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCOMPONENTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCOMPONENTDO) {
...
void PnlWznmVer1NComponent::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NComponent::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NComponent::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NComponent::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVer1NComponent* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NComponent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NComponent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NComponent::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCmp = WznmMComponent();

			if (recSelNew) {
				if (dbswznm->tblwznmmcomponent->loadRecByRef(recSelNew->ref, &_recCmp)) {
					recCmp = *_recCmp;
...
void PnlWznmVer1NComponent::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCMP, jref)) if (refVer != 0) {
			sref = "CrdWznmCmp";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recCmp.ref, jrefNew);
		};
...
void PnlWznmVer1NComponent::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCMP, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmCmp";
...
void PnlWznmVer1NComponent::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NComponent::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NComponent::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NComponent::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
