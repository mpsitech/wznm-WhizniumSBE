/**
	* \file PnlWznmVerBvr1NVersion.cpp
	* job handler for job PnlWznmVerBvr1NVersion (implementation)
...
#include "PnlWznmVerBvr1NVersion_blks.cpp"
#include "PnlWznmVerBvr1NVersion_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerBvr1NVersion
 ******************************************************************************/

PnlWznmVerBvr1NVersion::PnlWznmVerBvr1NVersion(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERBVR1NVERSION, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerBvr1NVersion(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerBvr1NVersion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerBvr1NVersion::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVerBvr1NVersion::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerBvr1NVersion::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERBVR1NVERSIONDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERBVR1NVERSIONDO) {
...
void PnlWznmVerBvr1NVersion::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerBvr1NVersion::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerBvr1NVersion::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerBvr1NVersion::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerBvr1NVersion* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerBvr1NVersion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerBvr1NVersion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerBvr1NVersion::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recVer = WznmMVersion();

			if (recSelNew) {
				if (dbswznm->tblwznmmversion->loadRecByRef(recSelNew->ref, &_recVer)) {
					recVer = *_recVer;
					delete _recVer;
				};
			};
...
void PnlWznmVerBvr1NVersion::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFPRJ) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recVer.ref, jrefNew);
		};
		if (jrefNew == 0) {
...
void PnlWznmVerBvr1NVersion::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerBvr1NVersion::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerBvr1NVersion::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
