/**
	...
	* job handler for job PnlWznmVer1NCall (implementation)
...
#include "PnlWznmVer1NCall_blks.cpp"
#include "PnlWznmVer1NCall_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NCall
 ******************************************************************************/

PnlWznmVer1NCall::PnlWznmVer1NCall(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NCALL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NCall(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NCall::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NCall::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NCall::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCALLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NCALLDO) {
...
void PnlWznmVer1NCall::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NCall::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NCall::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NCall::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NCall::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NCall::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NCall::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCal = WznmMCall();

			if (recSelNew) {
				if (dbswznm->tblwznmmcall->loadRecByRef(recSelNew->ref, &_recCal)) {
					recCal = *_recCal;
...
void PnlWznmVer1NCall::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAL, jref)) if (refVer != 0) {
			sref = "CrdWznmCal";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recCal.ref, jrefNew);
		};
...
void PnlWznmVer1NCall::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAL, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmCal";
...
void PnlWznmVer1NCall::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NCall::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NCall::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NCall::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
