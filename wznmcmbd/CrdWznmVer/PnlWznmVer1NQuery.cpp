/**
	...
	* job handler for job PnlWznmVer1NQuery (implementation)
...
#include "PnlWznmVer1NQuery_blks.cpp"
#include "PnlWznmVer1NQuery_evals.cpp"
...
/******************************************************************************
 class PnlWznmVer1NQuery
 ******************************************************************************/

PnlWznmVer1NQuery::PnlWznmVer1NQuery(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVER1NQUERY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVer1NQuery(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVer1NQuery::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVer1NQuery::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVer1NQuery::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVer1NQuery::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NQUERYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVER1NQUERYDO) {
...
void PnlWznmVer1NQuery::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVer1NQuery::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NQuery::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVer1NQuery::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVer1NQuery* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVer1NQuery::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVer1NQuery::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVer1NQuery::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recQry = WznmMQuery();

			if (recSelNew) {
				if (dbswznm->tblwznmmquery->loadRecByRef(recSelNew->ref, &_recQry)) {
					recQry = *_recQry;
					delete _recQry;
				};
			};
...
void PnlWznmVer1NQuery::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQRY, jref)) if (refVer != 0) {
			sref = "CrdWznmQry";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recQry.ref, jrefNew);
		};
...
void PnlWznmVer1NQuery::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQRY, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmQry";
...
void PnlWznmVer1NQuery::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVer1NQuery::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVer1NQuery::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVer1NQuery::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
