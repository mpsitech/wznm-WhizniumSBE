/**
	...
	* job handler for job PnlWznmVerVer1NVisual (implementation)
...
#include "PnlWznmVerVer1NVisual_blks.cpp"
#include "PnlWznmVerVer1NVisual_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerVer1NVisual
 ******************************************************************************/

PnlWznmVerVer1NVisual::PnlWznmVerVer1NVisual(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERVER1NVISUAL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerVer1NVisual(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerVer1NVisual::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerVer1NVisual::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVerVer1NVisual::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NVisual::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NVISUALDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NVISUALDO) {
...
void PnlWznmVerVer1NVisual::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NVisual::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NVisual::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerVer1NVisual::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerVer1NVisual* recSelNew = NULL;

	WznmMVisual* _recVis = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerVer1NVisual::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerVer1NVisual::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerVer1NVisual::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recVis = WznmMVisual();

			if (recSelNew) {
				if (dbswznm->tblwznmmvisual->loadRecByRef(recSelNew->ref, &_recVis)) {
					recVis = *_recVis;
					delete _recVis;
				};
			};
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVIS, jref)) {
			sref = "CrdWznmVis";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recVis.ref, jrefNew);
		};
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVIS, jref) & VecWznmWAccess::EDIT) != 0) {
			sref = "CrdWznmVis";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, -1, jrefNew);
		};
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NVisual::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerVer1NVisual::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerVer1NVisual::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
