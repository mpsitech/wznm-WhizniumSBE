/**
	...
	* job handler for job PnlWznmVerVer1NModule (implementation)
...
#include "PnlWznmVerVer1NModule_blks.cpp"
#include "PnlWznmVerVer1NModule_evals.cpp"
...
/******************************************************************************
 class PnlWznmVerVer1NModule
 ******************************************************************************/

PnlWznmVerVer1NModule::PnlWznmVerVer1NModule(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVERVER1NMODULE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVerVer1NModule(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVerVer1NModule::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVerVer1NModule::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVerVer1NModule::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NModule::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NMODULEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVERVER1NMODULEDO) {
...
void PnlWznmVerVer1NModule::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVerVer1NModule::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NModule::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVerVer1NModule::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVerVer1NModule* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVerVer1NModule::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVerVer1NModule::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVerVer1NModule::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMdl = WznmMModule();

			if (recSelNew) {
				if (dbswznm->tblwznmmmodule->loadRecByRef(recSelNew->ref, &_recMdl)) {
					recMdl = *_recMdl;
					delete _recMdl;
				};
			};
...
void PnlWznmVerVer1NModule::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NModule::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NModule::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCMDL, jref)) if (refVer != 0) {
			sref = "CrdWznmMdl";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recMdl.ref, jrefNew);
		};
...
void PnlWznmVerVer1NModule::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCMDL, jref) & VecWznmWAccess::EDIT) != 0) if (refVer != 0) {
			sref = "CrdWznmMdl";
...
void PnlWznmVerVer1NModule::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVerVer1NModule::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVerVer1NModule::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVerVer1NModule::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
