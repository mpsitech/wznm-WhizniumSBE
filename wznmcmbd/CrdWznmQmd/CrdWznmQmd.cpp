/**
	* \file CrdWznmQmd.cpp
	* job handler for job CrdWznmQmd (implementation)
...
CrdWznmQmd::CrdWznmQmd(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMQMD, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFQMD, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmQmdRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmQmdHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmQmdList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmQmd::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmQmd::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmQmd/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdQmd = StubWznm::getStubQmdStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFQMD, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmQmd::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmQmd", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFQMD, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFQMD, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFQMD, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmQmd::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmQmd::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQMDDO) {
...
void CrdWznmQmd::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmQmd::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMQMD);
};

void CrdWznmQmd::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmQmd::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmQmd::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmQmd::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmQmd::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFQMD) {
...
void CrdWznmQmd::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmQmd::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmQmd::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
