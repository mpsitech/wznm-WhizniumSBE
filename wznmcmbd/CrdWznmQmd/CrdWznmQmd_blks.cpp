/**
	* \file CrdWznmQmd_blks.cpp
	* job handler for job CrdWznmQmd (implementation of blocks)
...
void CrdWznmQmd::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdWznmQmd::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmQmd";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	me["numFSge"] = numFSge;
	me["MrlAppHlp"] = MrlAppHlp;
	me["MtxCrdQmd"] = MtxCrdQmd;
};

void CrdWznmQmd::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWznmQmd";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdQmd", MtxCrdQmd);
...
void CrdWznmQmd::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
	...
	if (difftag.length() == 0) difftag = "StatAppWznmQmd";
...
void CrdWznmQmd::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWznmVReqitmode
	...
	else itemtag = "StatitemAppWznmQmd";
...
void CrdWznmQmd::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmQmd";
...
void CrdWznmQmd::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWznmQmd";
...
void CrdWznmQmd::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmQmd";
...
CrdWznmQmd::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMQMDDO)
		{
	ixVDo = 0;
};
...
void CrdWznmQmd::DpchAppDo::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmQmdDo"];}();
...
void CrdWznmQmd::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmQmdDo");
	else
...
CrdWznmQmd::DpchEngData::DpchEngData(
			...
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMQMDDATA, jref)
		{
...
void CrdWznmQmd::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void CrdWznmQmd::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmQmdData"] = Json::Value(Json::objectValue);
...
void CrdWznmQmd::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmQmdData");
