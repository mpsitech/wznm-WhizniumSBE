/**
	* \file CrdWznmOpk.cpp
	* job handler for job CrdWznmOpk (implementation)
...
CrdWznmOpk::CrdWznmOpk(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMOPK, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFOPK, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmOpkList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmOpkHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmOpkRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmOpk::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmOpk::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmOpk/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdOpk = StubWznm::getStubOpkStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFOPK, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmOpk::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmOpk", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFOPK, jref, ref);
	...
	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFOPK, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFOPK, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmOpk::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmOpk::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKDO) {
...
void CrdWznmOpk::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmOpk::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMOPK);
};

void CrdWznmOpk::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmOpk::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmOpk::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFOPK) {
...
bool CrdWznmOpk::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmOpk::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmOpk::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmOpk::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmOpk::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
