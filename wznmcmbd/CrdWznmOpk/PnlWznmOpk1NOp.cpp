/**
	...
	* job handler for job PnlWznmOpk1NOp (implementation)
...
#include "PnlWznmOpk1NOp_blks.cpp"
#include "PnlWznmOpk1NOp_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpk1NOp
 ******************************************************************************/

PnlWznmOpk1NOp::PnlWznmOpk1NOp(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPK1NOP, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpk1NOp(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpk1NOp::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpk1NOp::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmOpk1NOp::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPK1NOPDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPK1NOPDO) {
...
void PnlWznmOpk1NOp::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpk1NOp::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpk1NOp::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpk1NOp::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpk1NOp::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpk1NOp::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpk1NOp::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOpx = WznmMOp();

			if (recSelNew) {
				if (dbswznm->tblwznmmop->loadRecByRef(recSelNew->ref, &_recOpx)) {
					recOpx = *_recOpx;
					delete _recOpx;
				};
			};
...
void PnlWznmOpk1NOp::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPX, jref)) if (refOpk != 0) {
			sref = "CrdWznmOpx";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFOPK, refOpk, sref, recOpx.ref, jrefNew);
		};
...
void PnlWznmOpk1NOp::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPX, jref) & VecWznmWAccess::EDIT) != 0) if (refOpk != 0) {
			sref = "CrdWznmOpx";
...
void PnlWznmOpk1NOp::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpk1NOp::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpk1NOp::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpk1NOp::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
