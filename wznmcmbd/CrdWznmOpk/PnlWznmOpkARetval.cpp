/**
	...
	* job handler for job PnlWznmOpkARetval (implementation)
...
#include "PnlWznmOpkARetval_blks.cpp"
#include "PnlWznmOpkARetval_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpkARetval
 ******************************************************************************/

PnlWznmOpkARetval::PnlWznmOpkARetval(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPKARETVAL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpkARetval(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpkARetval::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpkARetval::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmOpkARetval::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmOpkARetval::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKARETVALDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKARETVALDO) {
...
void PnlWznmOpkARetval::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpkARetval::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkARetval::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpkARetval::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQOpkARetval* recSelNew = NULL;

	WznmAMOppackRetval* _recOpkAret = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpkARetval::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpkARetval::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpkARetval::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOpkAret = WznmAMOppackRetval();

			if (recSelNew) {
				if (dbswznm->tblwznmamoppackretval->loadRecByRef(recSelNew->ref, &_recOpkAret)) {
					recOpkAret = *_recOpkAret;
					delete _recOpkAret;
				};
			};
...
void PnlWznmOpkARetval::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkARetval::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkARetval::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkARetval::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpkARetval::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpkARetval::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
