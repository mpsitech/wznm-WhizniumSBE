/**
	...
	* job handler for job PnlWznmOpkMNLibrary (implementation)
...
#include "PnlWznmOpkMNLibrary_blks.cpp"
#include "PnlWznmOpkMNLibrary_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpkMNLibrary
 ******************************************************************************/

PnlWznmOpkMNLibrary::PnlWznmOpkMNLibrary(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPKMNLIBRARY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpkMNLibrary(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpkMNLibrary::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpkMNLibrary::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmOpkMNLibrary::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFOPK) {
		recLibRopk = WznmRMLibraryMOppack();
...
void PnlWznmOpkMNLibrary::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKMNLIBRARYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKMNLIBRARYDO) {
...
void PnlWznmOpkMNLibrary::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpkMNLibrary::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNLibrary::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpkMNLibrary::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQOpkMNLibrary* recSelNew = NULL;

	WznmRMLibraryMOppack* _recLibRopk = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpkMNLibrary::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpkMNLibrary::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpkMNLibrary::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmlibrarymoppack->loadRecByRef(recSelNew->ref, &_recLibRopk)) {
					recLibRopk = *_recLibRopk;
					delete _recLibRopk;
				};
				if (dbswznm->tblwznmmlibrary->loadRecByRef(recSelNew->mref, &_recLib)) {
					recLib = *_recLib;
					delete _recLib;
				};
			};
...
void PnlWznmOpkMNLibrary::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCLIB, jref)) {
			sref = "CrdWznmLib";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recLib.ref, jrefNew);
		};
...
void PnlWznmOpkMNLibrary::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNLibrary::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNLibrary::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpkMNLibrary::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpkMNLibrary::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
