/**
	...
	* job handler for job PnlWznmOpkMNComponent (implementation)
...
#include "PnlWznmOpkMNComponent_blks.cpp"
#include "PnlWznmOpkMNComponent_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpkMNComponent
 ******************************************************************************/

PnlWznmOpkMNComponent::PnlWznmOpkMNComponent(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPKMNCOMPONENT, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpkMNComponent(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpkMNComponent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpkMNComponent::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmOpkMNComponent::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmOpkMNComponent::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKMNCOMPONENTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPKMNCOMPONENTDO) {
			DpchAppDo* dpchappdo = (DpchAppDo*) (req->dpchapp);

			if (dpchappdo->ixVDo != 0) {
				if (dpchappdo->ixVDo == VecVDo::BUTADDCLICK) {
...
void PnlWznmOpkMNComponent::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpkMNComponent::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNComponent::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpkMNComponent::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQOpkMNComponent* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpkMNComponent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpkMNComponent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpkMNComponent::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmcomponent->loadRecByRef(recSelNew->mref, &_recCmp)) {
					recCmp = *_recCmp;
...
void PnlWznmOpkMNComponent::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNComponent::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpkMNComponent::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpkMNComponent::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpkMNComponent::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
