...
#include "QryWznmOpkMNComponent_blks.cpp"
...
/******************************************************************************
 class QryWznmOpkMNComponent
 ******************************************************************************/

QryWznmOpkMNComponent::QryWznmOpkMNComponent(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::QRYWZNMOPKMNCOMPONENT, jrefSup, ixWznmVLocale)
		{
...
void QryWznmOpkMNComponent::refreshJnum() {
	ubigint preRefSel = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSEL, jref);

	stgiac.jnum = getJnumByRef(preRefSel);
};

void QryWznmOpkMNComponent::rerun(
			DbsWznm* dbswznm
			, const bool call
		) {
	string sqlstr;
	...
	xchg->removeClstns(VecWznmVCall::CALLWZNMCMPROPKMOD_OPKEQ, jref);
	...
	sqlstr = "UPDATE TblWznmQOpkMNComponent SET jnum = qref WHERE jref = " + to_string(jref);
	dbswznm->executeQuery(sqlstr);
	...
	xchg->addRefClstn(VecWznmVCall::CALLWZNMCMPROPKMOD_OPKEQ, jref, Clstn::VecVJobmask::ALL, 0, false, preRefOpk);
};

void QryWznmOpkMNComponent::fetch(
			DbsWznm* dbswznm
		) {
	string sqlstr;
	...
	dbswznm->tblwznmqopkmncomponent->loadRstByJref(jref, false, rst);
	...
	if (stmgr) {
		...
		stmgr->commit();
		stmgr->unlockAccess("QryWznmOpkMNComponent", "fetch");
	};

	refreshJnum();
};

uint QryWznmOpkMNComponent::getJnumByRef(
			const ubigint ref
		) {
	uint retval = 0;
...
ubigint QryWznmOpkMNComponent::getRefByJnum(
			const uint jnum
		) {
	uint ref = 0;

	WznmQOpkMNComponent* rec = getRecByJnum(jnum);
	if (rec) ref = rec->ref;
	return ref;
};

WznmQOpkMNComponent* QryWznmOpkMNComponent::getRecByJnum(
			const uint jnum
		) {
	WznmQOpkMNComponent* rec = NULL;
...
void QryWznmOpkMNComponent::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
...
bool QryWznmOpkMNComponent::handleRerun(
			DbsWznm* dbswznm
		) {
	bool retval = false;
	string input;
...
bool QryWznmOpkMNComponent::handleShow(
			DbsWznm* dbswznm
		) {
	bool retval = false;
	WznmQOpkMNComponent* rec = NULL;
...
void QryWznmOpkMNComponent::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
