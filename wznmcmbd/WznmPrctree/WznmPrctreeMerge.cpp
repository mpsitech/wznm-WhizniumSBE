/**
	* \file WznmPrctreeMerge.cpp
	* Wznm operation processor - merge extract file tree into template file tree (implementation)
...
DpchRetWznmPrctreeMerge* WznmPrctreeMerge::run(
			XchgWznm* xchg
			, DbsWznm* dbswznm
	...
	return(new DpchRetWznmPrctreeMerge("", "", ixOpVOpres, 100, logfile));
};

// IP cust --- IBEGIN
bool WznmPrctreeMerge::scanFolder(
...
void WznmPrctreeMerge::invscanFolder(
			const string& tmppath
	...
	if (keep) {
		...
		Wznm::run("cp -rp " + tmppath + "/" + extroot + extsub + "/* " + tmppath + "/" + tplfolder + extsub + "/");
		...
		for (unsigned int i = 0; i < files.size(); i++) {
			...
			if (access(s.c_str(), R_OK) == -1) {
				...
				if (ips.size() > 0) {
					if (ips[0]->type == Iptype::KEEP) Wznm::run("cp -p " + tmppath + "/" + extroot + extsub + "/" + files[i] + " " + s);
...
bool WznmPrctreeMerge::scanFile(
			const string& tmppath
	...
	if (hasextfi) {
		...
		if (success) {
			...
			if (extkeep) {
				...
				Wznm::run("cp -p " + tmppath + "/" + extfile + " " + tmppath + "/" + tplfile);
				...
				if (success) {
					...
					Wznm::run("mv " + tmppath + "/" + tmpfile + " " + tmppath + "/" + tplfile);
				};
			};
		};
...
void WznmPrctreeMerge::writeTmpfile(
			const string& tmppath
			, fstream& tmpfi
			, const string& tplfile
			, const string& extfile
...
void WznmPrctreeMerge::writeTmpfile_line(
			fstream& tmpfi
			, const string& line
			, bool& lastvoid
			, const bool skipmultvoid
