/**
	* \file WznmPrctreeExtract.cpp
	* Wznm operation processor - extract from file tree into extract file tree (implementation)
...
DpchRetWznmPrctreeExtract* WznmPrctreeExtract::run(
			XchgWznm* xchg
			, DbsWznm* dbswznm
	...
	return(new DpchRetWznmPrctreeExtract("", "", ixOpVOpres, 100, logfile));
};

// IP cust --- IBEGIN
bool WznmPrctreeExtract::scanFolder(
	...
	if ((folders.size() > 0) || (files.size() > 0)) {
		...
		if (keep) {
			Wznm::run("cp -rp " + tmppath + "/" + infolder + "/* " + s + "/");
...
bool WznmPrctreeExtract::scanFile(
			const string& tmppath
			, const string& infile
	...
	if (success) {
		...
		if (hasIpKeep) {
			// copy entire file
			Wznm::run("cp -p " + tmppath + "/" + infile + " " + tmppath + "/" + extfile);
...
void WznmPrctreeExtract::writeExtfile(
			const string& tmppath
			, const string& extfile
			, const vector<Ip*>& ips
		) {
