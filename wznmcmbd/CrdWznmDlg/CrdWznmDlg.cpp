/**
	* \file CrdWznmDlg.cpp
	* job handler for job CrdWznmDlg (implementation)
...
CrdWznmDlg::CrdWznmDlg(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMDLG, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFDLG, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmDlgList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmDlgHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmDlgRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmDlg::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmDlg::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmDlg/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdDlg = StubWznm::getStubDlgStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFDLG, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmDlg::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmDlg", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFDLG, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFDLG, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFDLG, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmDlg::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmDlg::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMDLGDO) {
...
void CrdWznmDlg::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmDlg::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMDLG);
};

void CrdWznmDlg::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmDlg::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmDlg::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFDLG) {
...
bool CrdWznmDlg::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmDlg::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmDlg::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmDlg::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmDlg::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
