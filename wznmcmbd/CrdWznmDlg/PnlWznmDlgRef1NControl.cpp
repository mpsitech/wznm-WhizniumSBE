/**
	* \file PnlWznmDlgRef1NControl.cpp
	* job handler for job PnlWznmDlgRef1NControl (implementation)
...
#include "PnlWznmDlgRef1NControl_blks.cpp"
#include "PnlWznmDlgRef1NControl_evals.cpp"
...
/******************************************************************************
 class PnlWznmDlgRef1NControl
 ******************************************************************************/

PnlWznmDlgRef1NControl::PnlWznmDlgRef1NControl(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMDLGREF1NCONTROL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmDlgRef1NControl(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmDlgRef1NControl::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmDlgRef1NControl::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmDlgRef1NControl::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmDlgRef1NControl::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMDLGREF1NCONTROLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMDLGREF1NCONTROLDO) {
...
void PnlWznmDlgRef1NControl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmDlgRef1NControl::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmDlgRef1NControl::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmDlgRef1NControl::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQDlgRef1NControl* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmDlgRef1NControl::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmDlgRef1NControl::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmDlgRef1NControl::StgIac::JNUM)) {
...
void PnlWznmDlgRef1NControl::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmDlgRef1NControl::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmDlgRef1NControl::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmDlgRef1NControl::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
