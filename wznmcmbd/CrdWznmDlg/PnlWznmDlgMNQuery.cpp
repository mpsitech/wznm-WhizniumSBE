/**
	...
	* job handler for job PnlWznmDlgMNQuery (implementation)
...
#include "PnlWznmDlgMNQuery_blks.cpp"
#include "PnlWznmDlgMNQuery_evals.cpp"
...
/******************************************************************************
 class PnlWznmDlgMNQuery
 ******************************************************************************/

PnlWznmDlgMNQuery::PnlWznmDlgMNQuery(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMDLGMNQUERY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmDlgMNQuery(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmDlgMNQuery::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmDlgMNQuery::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmDlgMNQuery::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmDlgMNQuery::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMDLGMNQUERYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMDLGMNQUERYDO) {
...
void PnlWznmDlgMNQuery::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmDlgMNQuery::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmDlgMNQuery::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmDlgMNQuery::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQDlgMNQuery* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmDlgMNQuery::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmDlgMNQuery::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmDlgMNQuery::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmquery->loadRecByRef(recSelNew->mref, &_recQry)) {
					recQry = *_recQry;
					delete _recQry;
				};
			};
...
void PnlWznmDlgMNQuery::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQRY, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmQry";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recQry.ref, jrefNew);
		};
...
void PnlWznmDlgMNQuery::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmDlgMNQuery::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmDlgMNQuery::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmDlgMNQuery::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmDlgMNQuery::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
