/**
	* \file PnlWznmDlgList_blks.cpp
	* job handler for job PnlWznmDlgList (implementation of blocks)
...
/******************************************************************************
 class PnlWznmDlgList::VecVDo
 ******************************************************************************/

uint PnlWznmDlgList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWznmDlgList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWznmDlgList::ContIac
 ******************************************************************************/

PnlWznmDlgList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
			, numFTos(numFTos)
...
bool PnlWznmDlgList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWznmDlgList"];}();
...
bool PnlWznmDlgList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWznmDlgList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacWznmDlgList";
...
void PnlWznmDlgList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWznmDlgList";
...
void PnlWznmDlgList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemIacWznmDlgList";
...
set<uint> PnlWznmDlgList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmDlgList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmDlgList::ContInf
 ******************************************************************************/

PnlWznmDlgList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWznmDlgList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmDlgList";
...
void PnlWznmDlgList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWznmDlgList";
...
set<uint> PnlWznmDlgList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmDlgList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmDlgList::StatShr
 ******************************************************************************/

PnlWznmDlgList::StatShr::StatShr(
...
void PnlWznmDlgList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmDlgList";
...
void PnlWznmDlgList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWznmDlgList";
...
set<uint> PnlWznmDlgList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmDlgList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmDlgList::StgIac
 ******************************************************************************/

PnlWznmDlgList::StgIac::StgIac(
			const uint TcoSrfWidth
			, const uint TcoTypWidth
			, const uint TcoCarWidth
			...
			, TcoCarWidth(TcoCarWidth)
			, TcoRetWidth(TcoRetWidth)
			, TcoReuWidth(TcoReuWidth)
		{
	mask = {TCOSRFWIDTH, TCOTYPWIDTH, TCOCARWIDTH, TCORETWIDTH, TCOREUWIDTH};
};

bool PnlWznmDlgList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWznmDlgList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoCarWidth")) {TcoCarWidth = me["TcoCarWidth"].asUInt(); add(TCOCARWIDTH);};
...
bool PnlWznmDlgList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWznmDlgList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWznmDlgList";
	...
	if (basefound) {
		...
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoCarWidth", TcoCarWidth)) add(TCOCARWIDTH);
...
set<uint> PnlWznmDlgList::StgIac::comm(
	...
	if (TcoCarWidth == comp->TcoCarWidth) insert(items, TCOCARWIDTH);
...
set<uint> PnlWznmDlgList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOTYPWIDTH, TCOCARWIDTH, TCORETWIDTH, TCOREUWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWznmDlgList::Tag
 ******************************************************************************/

void PnlWznmDlgList::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmDlgList";
...
void PnlWznmDlgList::Tag::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWznmDlgList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoCar", "Card");
...
/******************************************************************************
 class PnlWznmDlgList::DpchAppData
 ******************************************************************************/

PnlWznmDlgList::DpchAppData::DpchAppData() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMDLGLISTDATA)
		{
};

string PnlWznmDlgList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmDlgList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmDlgListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWznmDlgList::StgIac();
	};
};

void PnlWznmDlgList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmDlgListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmDlgList::DpchAppDo
 ******************************************************************************/

PnlWznmDlgList::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMDLGLISTDO)
		{
	ixVDo = 0;
};

string PnlWznmDlgList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmDlgList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmDlgListDo"];}();
...
void PnlWznmDlgList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmDlgListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmDlgList::DpchEngData
 ******************************************************************************/

PnlWznmDlgList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryWznmDlgList::StatShr* statshrqry
			, QryWznmDlgList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMDLGLISTDATA, jref)
		{
...
string PnlWznmDlgList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmDlgList::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void PnlWznmDlgList::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmDlgListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWznmDlgList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWznmDlgList::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmDlgListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		...
		if (has(STATAPPQRY)) QryWznmDlgList::StatApp::writeXML(wr);
