/**
	...
	* job handler for job PnlWznmMty1NMachine (implementation)
...
#include "PnlWznmMty1NMachine_blks.cpp"
#include "PnlWznmMty1NMachine_evals.cpp"
...
/******************************************************************************
 class PnlWznmMty1NMachine
 ******************************************************************************/

PnlWznmMty1NMachine::PnlWznmMty1NMachine(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMTY1NMACHINE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMty1NMachine(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMty1NMachine::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMty1NMachine::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmMty1NMachine::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMty1NMachine::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTY1NMACHINEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTY1NMACHINEDO) {
...
void PnlWznmMty1NMachine::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMty1NMachine::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMty1NMachine::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMty1NMachine::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMty1NMachine* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMty1NMachine::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMty1NMachine::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMty1NMachine::StgIac::JNUM)) {
...
void PnlWznmMty1NMachine::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmMty1NMachine::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMty1NMachine::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMty1NMachine::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
