/**
	...
	* job handler for job PnlWznmErrRec (implementation)
...
#include "PnlWznmErrRec_blks.cpp"
#include "PnlWznmErrRec_evals.cpp"
...
PnlWznmErrRec::PnlWznmErrRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMERRREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmErrRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmErrRec::refresh(
	...
	// statshr
	if (recErr.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmErrDetail(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmErrRec::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFERR) {
		...
		if (dbswznm->tblwznmmerror->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFERR, jref), &_recErr)) {
			recErr = *_recErr;
			delete _recErr;
...
void PnlWznmErrRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMERRRECDO) {
...
void PnlWznmErrRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmErrRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmErrRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
bool PnlWznmErrRec::handleCallWznmErrUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool PnlWznmErrRec::handleCallWznmErr_verEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
