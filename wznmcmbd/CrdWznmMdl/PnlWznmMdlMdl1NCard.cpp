/**
	...
	* job handler for job PnlWznmMdlMdl1NCard (implementation)
...
#include "PnlWznmMdlMdl1NCard_blks.cpp"
#include "PnlWznmMdlMdl1NCard_evals.cpp"
...
/******************************************************************************
 class PnlWznmMdlMdl1NCard
 ******************************************************************************/

PnlWznmMdlMdl1NCard::PnlWznmMdlMdl1NCard(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMDLMDL1NCARD, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMdlMdl1NCard(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMdlMdl1NCard::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMdlMdl1NCard::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmMdlMdl1NCard::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMdlMdl1NCard::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMDLMDL1NCARDDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMDLMDL1NCARDDO) {
...
void PnlWznmMdlMdl1NCard::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMdlMdl1NCard::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMdlMdl1NCard::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMdlMdl1NCard::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMdlMdl1NCard* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMdlMdl1NCard::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMdlMdl1NCard::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMdlMdl1NCard::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCar = WznmMCard();

			if (recSelNew) {
				if (dbswznm->tblwznmmcard->loadRecByRef(recSelNew->ref, &_recCar)) {
					recCar = *_recCar;
					delete _recCar;
				};
			};
...
void PnlWznmMdlMdl1NCard::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAR, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmCar";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recCar.ref, jrefNew);
		};
...
void PnlWznmMdlMdl1NCard::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAR, jref) & VecWznmWAccess::EDIT) != 0) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmCar";
...
void PnlWznmMdlMdl1NCard::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMdlMdl1NCard::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMdlMdl1NCard::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMdlMdl1NCard::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
