/**
	* \file CrdWznmMdl.cpp
	* job handler for job CrdWznmMdl (implementation)
...
CrdWznmMdl::CrdWznmMdl(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMMDL, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFMDL, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmMdlRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmMdlHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmMdlList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmMdl::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmMdl::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmMdl/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdMdl = StubWznm::getStubMdlStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFMDL, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmMdl::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmMdl", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFMDL, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFMDL, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFMDL, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmMdl::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmMdl::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMDLDO) {
...
void CrdWznmMdl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmMdl::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMMDL);
};

void CrdWznmMdl::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmMdl::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmMdl::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFMDL) {
...
bool CrdWznmMdl::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmMdl::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmMdl::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmMdl::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmMdl::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
