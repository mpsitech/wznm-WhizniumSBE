...
void PnlWznmPrj1NVersion::handleDpchAppDoButViewClick(
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref)) if (refPrj != 0) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFPRJ, refPrj, sref, recVer.ref, jrefNew);
		};
...
void PnlWznmPrj1NVersion::handleDpchAppDoButNewClick(
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref) & VecWznmWAccess::EDIT) != 0) if (refPrj != 0) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFPRJ, refPrj, sref, -1, jrefNew);
		};
