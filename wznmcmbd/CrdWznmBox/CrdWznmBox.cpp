/**
	* \file CrdWznmBox.cpp
	* job handler for job CrdWznmBox (implementation)
...
CrdWznmBox::CrdWznmBox(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMBOX, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFBOX, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmBoxList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmBoxHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmBoxRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmBox::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmBox::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmBox/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdBox = StubWznm::getStubBoxStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFBOX, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmBox::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmBox", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFBOX, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFBOX, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFBOX, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmBox::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmBox::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBOXDO) {
...
void CrdWznmBox::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmBox::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMBOX);
};

void CrdWznmBox::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmBox::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmBox::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFBOX) {
...
bool CrdWznmBox::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmBox::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmBox::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmBox::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmBox::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
