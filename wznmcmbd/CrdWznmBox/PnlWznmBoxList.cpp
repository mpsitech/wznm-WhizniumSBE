/**
	* \file PnlWznmBoxList.cpp
	* job handler for job PnlWznmBoxList (implementation)
...
#include "PnlWznmBoxList.h"

#include "PnlWznmBoxList_blks.cpp"
#include "PnlWznmBoxList_evals.cpp"
...
/******************************************************************************
 class PnlWznmBoxList
 ******************************************************************************/

PnlWznmBoxList::PnlWznmBoxList(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMBOXLIST, jrefSup, ixWznmVLocale)
		{
	...
	QryWznmBoxList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSEL, jref, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFBOX, jref));

	qry = new QryWznmBoxList(xchg, dbswznm, jref, ixWznmVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWznmVExpstate = ((xchg->getRefPreset(VecWznmVPreset::PREWZNMREFBOX, jref)) ? VecWznmVExpstate::MIND : VecWznmVExpstate::REGD);
...
DpchEngWznm* PnlWznmBoxList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmBoxList::refresh(
	...
	if ((ixPre != 0) && (ixPre != VecWznmVPreset::VOID)) {
		...
		if (ixPre == VecWznmVPreset::PREWZNMREFVIS) continf.TxtPre = StubWznm::getStubVisStd(dbswznm, xchg->getRefPreset(ixPre, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
		else if (ixPre == VecWznmVPreset::PREWZNMREFSHT) continf.TxtPre = StubWznm::getStubShtStd(dbswznm, xchg->getRefPreset(ixPre, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void PnlWznmBoxList::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmBoxList::regularize(
			DbsWznm* dbswznm
			, const bool notif
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBOXLISTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBOXLISTDO) {
...
void PnlWznmBoxList::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmBoxList::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWznmBoxList::VecVOrd::UNV) && (_contiac->numFTos <= QryWznmBoxList::VecVOrd::SHT)) {
			muteRefresh = true;
...
void PnlWznmBoxList::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmBoxList::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmBoxList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmBoxList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWznmBoxList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFBOX, refSelNew);
		};
	};
...
void PnlWznmBoxList::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDoButFilterClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxList::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmBoxList::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmBoxList::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
