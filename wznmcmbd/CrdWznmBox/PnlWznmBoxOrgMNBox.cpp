/**
	* \file PnlWznmBoxOrgMNBox.cpp
	* job handler for job PnlWznmBoxOrgMNBox (implementation)
...
#include "PnlWznmBoxOrgMNBox_blks.cpp"
#include "PnlWznmBoxOrgMNBox_evals.cpp"
...
/******************************************************************************
 class PnlWznmBoxOrgMNBox
 ******************************************************************************/

PnlWznmBoxOrgMNBox::PnlWznmBoxOrgMNBox(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMBOXORGMNBOX, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmBoxOrgMNBox(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmBoxOrgMNBox::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmBoxOrgMNBox::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmBoxOrgMNBox::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmBoxOrgMNBox::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBOXORGMNBOXDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBOXORGMNBOXDO) {
...
void PnlWznmBoxOrgMNBox::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmBoxOrgMNBox::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxOrgMNBox::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmBoxOrgMNBox::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQBoxOrgMNBox* recSelNew = NULL;

	WznmRMBoxMBox* _recBoxRbox = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmBoxOrgMNBox::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmBoxOrgMNBox::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmBoxOrgMNBox::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmboxmbox->loadRecByRef(recSelNew->ref, &_recBoxRbox)) {
					recBoxRbox = *_recBoxRbox;
					delete _recBoxRbox;
				};
				if (dbswznm->tblwznmmbox->loadRecByRef(recSelNew->mref, &_recBox)) {
					recBox = *_recBox;
					delete _recBox;
				};
			};
...
void PnlWznmBoxOrgMNBox::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCBOX, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFSHT) {
			sref = "CrdWznmBox";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recBox.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCBOX, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVIS) {
				sref = "CrdWznmBox";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recBox.ref, jrefNew);
			};
		};
...
void PnlWznmBoxOrgMNBox::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxOrgMNBox::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBoxOrgMNBox::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmBoxOrgMNBox::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmBoxOrgMNBox::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
