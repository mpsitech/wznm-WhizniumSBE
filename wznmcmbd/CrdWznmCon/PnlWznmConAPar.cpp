/**
	...
	* job handler for job PnlWznmConAPar (implementation)
...
#include "PnlWznmConAPar_blks.cpp"
#include "PnlWznmConAPar_evals.cpp"
...
/******************************************************************************
 class PnlWznmConAPar
 ******************************************************************************/

PnlWznmConAPar::PnlWznmConAPar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCONAPAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmConAPar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmConAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmConAPar::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmConAPar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONAPARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONAPARDO) {
...
void PnlWznmConAPar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmConAPar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConAPar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmConAPar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQConAPar* recSelNew = NULL;

	WznmAMControlPar* _recConApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmConAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmConAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmConAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recConApar = WznmAMControlPar();

			if (recSelNew) {
				if (dbswznm->tblwznmamcontrolpar->loadRecByRef(recSelNew->ref, &_recConApar)) {
					recConApar = *_recConApar;
					delete _recConApar;
				};
			};
...
void PnlWznmConAPar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConAPar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConAPar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConAPar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmConAPar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmConAPar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
