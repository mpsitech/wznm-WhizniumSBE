/**
	...
	* job handler for job PnlWznmConList (implementation)
...
#include "PnlWznmConList_blks.cpp"
#include "PnlWznmConList_evals.cpp"
...
/******************************************************************************
 class PnlWznmConList
 ******************************************************************************/

PnlWznmConList::PnlWznmConList(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCONLIST, jrefSup, ixWznmVLocale)
		{
	...
	QryWznmConList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSEL, jref, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCON, jref));
	xchg->addIxPreset(VecWznmVPreset::PREWZNMIXORD, jref, QryWznmConList::VecVOrd::SRF);

	qry = new QryWznmConList(xchg, dbswznm, jref, ixWznmVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWznmVExpstate = ((xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCON, jref)) ? VecWznmVExpstate::MIND : VecWznmVExpstate::REGD);
...
DpchEngWznm* PnlWznmConList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmConList::refresh(
	...
	if ((ixPre != 0) && (ixPre != VecWznmVPreset::VOID)) {
		...
		else if (ixPre == VecWznmVPreset::PREWZNMREFCAR) continf.TxtPre = StubWznm::getStubCarStd(dbswznm, xchg->getRefPreset(ixPre, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
		else if (ixPre == VecWznmVPreset::PREWZNMREFDLG) continf.TxtPre = StubWznm::getStubDlgStd(dbswznm, xchg->getRefPreset(ixPre, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
		else if (ixPre == VecWznmVPreset::PREWZNMREFPNL) continf.TxtPre = StubWznm::getStubPnlStd(dbswznm, xchg->getRefPreset(ixPre, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void PnlWznmConList::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmConList::regularize(
			DbsWznm* dbswznm
			, const bool notif
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONLISTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONLISTDO) {
...
void PnlWznmConList::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmConList::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWznmConList::VecVOrd::SUP) && (_contiac->numFTos <= QryWznmConList::VecVOrd::SRF)) {
			muteRefresh = true;
...
void PnlWznmConList::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmConList::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmConList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmConList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWznmConList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCON, refSelNew);
		};
	};
...
void PnlWznmConList::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDoButFilterClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConList::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmConList::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmConList::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
