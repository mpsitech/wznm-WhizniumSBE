/**
	* \file CrdWznmCon.cpp
	* job handler for job CrdWznmCon (implementation)
...
CrdWznmCon::CrdWznmCon(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMCON, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCON, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmConRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmConHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmConList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmCon::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmCon::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmCon/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdCon = StubWznm::getStubConStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCON, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmCon::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmCon", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCON, jref, ref);

	if (dbswznm->tblwznmmcontrol->loadRecByRef(ref, &rec)) {
		xchg->addRefPreset(VecWznmVPreset::PREWZNMREFFED, jref, rec->refWznmMFeed);

		delete rec;
	};

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCON, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCON, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmCon::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmCon::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONDO) {
...
void CrdWznmCon::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmCon::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMCON);
};

void CrdWznmCon::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmCon::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmCon::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFCON) {
...
bool CrdWznmCon::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmCon::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmCon::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmCon::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmCon::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
