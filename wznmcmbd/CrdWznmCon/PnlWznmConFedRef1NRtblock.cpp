/**
	...
	* job handler for job PnlWznmConFedRef1NRtblock (implementation)
...
#include "PnlWznmConFedRef1NRtblock_blks.cpp"
...
/******************************************************************************
 class PnlWznmConFedRef1NRtblock
 ******************************************************************************/

PnlWznmConFedRef1NRtblock::PnlWznmConFedRef1NRtblock(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCONFEDREF1NRTBLOCK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmConFedRef1NRtblock(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmConFedRef1NRtblock::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmConFedRef1NRtblock::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmConFedRef1NRtblock::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmConFedRef1NRtblock::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONFEDREF1NRTBLOCKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCONFEDREF1NRTBLOCKDO) {
...
void PnlWznmConFedRef1NRtblock::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmConFedRef1NRtblock::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmConFedRef1NRtblock::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmConFedRef1NRtblock::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQConFedRef1NRtblock* recSelNew = NULL;

	WznmMRtblock* _recRtb = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmConFedRef1NRtblock::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmConFedRef1NRtblock::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmConFedRef1NRtblock::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recRtb = WznmMRtblock();

			if (recSelNew) {
				if (dbswznm->tblwznmmrtblock->loadRecByRef(recSelNew->ref, &_recRtb)) {
					recRtb = *_recRtb;
					delete _recRtb;
				};
			};
...
void PnlWznmConFedRef1NRtblock::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmConFedRef1NRtblock::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmConFedRef1NRtblock::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
