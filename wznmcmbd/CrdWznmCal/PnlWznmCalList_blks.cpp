/**
	* \file PnlWznmCalList_blks.cpp
	* job handler for job PnlWznmCalList (implementation of blocks)
...
/******************************************************************************
 class PnlWznmCalList::VecVDo
 ******************************************************************************/

uint PnlWznmCalList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWznmCalList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWznmCalList::ContIac
 ******************************************************************************/

PnlWznmCalList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
			, numFTos(numFTos)
...
bool PnlWznmCalList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWznmCalList"];}();
...
bool PnlWznmCalList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWznmCalList");
	else
...
void PnlWznmCalList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWznmCalList";
...
void PnlWznmCalList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWznmCalList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmCalList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmCalList::ContInf
 ******************************************************************************/

PnlWznmCalList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWznmCalList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmCalList";
...
void PnlWznmCalList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWznmCalList";
...
set<uint> PnlWznmCalList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmCalList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmCalList::StatShr
 ******************************************************************************/

PnlWznmCalList::StatShr::StatShr(
...
void PnlWznmCalList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmCalList";
...
void PnlWznmCalList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWznmCalList";
...
set<uint> PnlWznmCalList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmCalList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmCalList::StgIac
 ******************************************************************************/

PnlWznmCalList::StgIac::StgIac(
			const uint TcoSrfWidth
			, const uint TcoTypWidth
			, const uint TcoVerWidth
			, const uint TcoRetWidth
			, const uint TcoReuWidth
			, const uint TcoIatWidth
			, const uint TcoRatWidth
		) :
			Block()
			...
			, TcoIatWidth(TcoIatWidth)
			, TcoRatWidth(TcoRatWidth)
		{
	mask = {TCOSRFWIDTH, TCOTYPWIDTH, TCOVERWIDTH, TCORETWIDTH, TCOREUWIDTH, TCOIATWIDTH, TCORATWIDTH};
};

bool PnlWznmCalList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWznmCalList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoIatWidth")) {TcoIatWidth = me["TcoIatWidth"].asUInt(); add(TCOIATWIDTH);};
		if (me.isMember("TcoRatWidth")) {TcoRatWidth = me["TcoRatWidth"].asUInt(); add(TCORATWIDTH);};
	};

	return basefound;
};

bool PnlWznmCalList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWznmCalList");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoIatWidth", TcoIatWidth)) add(TCOIATWIDTH);
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoRatWidth", TcoRatWidth)) add(TCORATWIDTH);
	};

	return basefound;
};

void PnlWznmCalList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWznmCalList";
	...
	me["TcoIatWidth"] = (Json::Value::UInt) TcoIatWidth;
	me["TcoRatWidth"] = (Json::Value::UInt) TcoRatWidth;
};

void PnlWznmCalList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		...
		writeUintAttr(wr, itemtag, "sref", "TcoIatWidth", TcoIatWidth);
		writeUintAttr(wr, itemtag, "sref", "TcoRatWidth", TcoRatWidth);
	xmlTextWriterEndElement(wr);
};

set<uint> PnlWznmCalList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
	...
	if (TcoIatWidth == comp->TcoIatWidth) insert(items, TCOIATWIDTH);
	if (TcoRatWidth == comp->TcoRatWidth) insert(items, TCORATWIDTH);

	return(items);
};

set<uint> PnlWznmCalList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOTYPWIDTH, TCOVERWIDTH, TCORETWIDTH, TCOREUWIDTH, TCOIATWIDTH, TCORATWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWznmCalList::Tag
 ******************************************************************************/

void PnlWznmCalList::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmCalList";
	...
	if (ixWznmVLocale == VecWznmVLocale::ENUS) {
		...
		me["TcoRet"] = "Ref. table";
		me["TcoReu"] = "Reference";
		me["TcoIat"] = "Invocation argument types";
		me["TcoRat"] = "Return argument types";
	};
...
void PnlWznmCalList::Tag::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWznmCalList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Calls");
			...
			writeStringAttr(wr, itemtag, "sref", "TcoIat", "Invocation argument types");
			writeStringAttr(wr, itemtag, "sref", "TcoRat", "Return argument types");
		};
...
/******************************************************************************
 class PnlWznmCalList::DpchAppData
 ******************************************************************************/

PnlWznmCalList::DpchAppData::DpchAppData() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMCALLISTDATA)
		{
};

string PnlWznmCalList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmCalList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmCalListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWznmCalList::StgIac();
	};
};

void PnlWznmCalList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmCalListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmCalList::DpchAppDo
 ******************************************************************************/

PnlWznmCalList::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMCALLISTDO)
		{
	ixVDo = 0;
};

string PnlWznmCalList::DpchAppDo::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmCalList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmCalListDo"];}();
...
void PnlWznmCalList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmCalListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmCalList::DpchEngData
 ******************************************************************************/

PnlWznmCalList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryWznmCalList::StatShr* statshrqry
			, QryWznmCalList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMCALLISTDATA, jref)
		{
...
string PnlWznmCalList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmCalList::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void PnlWznmCalList::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmCalListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWznmCalList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWznmCalList::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmCalListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		...
		if (has(STATAPPQRY)) QryWznmCalList::StatApp::writeXML(wr);
