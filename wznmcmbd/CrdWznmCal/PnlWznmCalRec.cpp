/**
	...
	* job handler for job PnlWznmCalRec (implementation)
...
#include "PnlWznmCalRec_blks.cpp"
#include "PnlWznmCalRec_evals.cpp"
...
PnlWznmCalRec::PnlWznmCalRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCALREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmCalRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCalRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubCalStd(dbswznm, recCal.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recCal.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (pnlmnstub) {delete pnlmnstub; pnlmnstub = NULL;};
	} else {
		if (!pnldetail) pnldetail = new PnlWznmCalDetail(xchg, dbswznm, jref, ixWznmVLocale);
		if (!pnl1nsensitivity) pnl1nsensitivity = new PnlWznmCal1NSensitivity(xchg, dbswznm, jref, ixWznmVLocale);
		if (!pnlmnstub) pnlmnstub = new PnlWznmCalMNStub(xchg, dbswznm, jref, ixWznmVLocale);
	};

	statshr.jrefDetail = ((pnldetail) ? pnldetail->jref : 0);
	statshr.jref1NSensitivity = ((pnl1nsensitivity) ? pnl1nsensitivity->jref : 0);
	statshr.jrefMNStub = ((pnlmnstub) ? pnlmnstub->jref : 0);
...
void PnlWznmCalRec::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFCAL) {
		...
		if (dbswznm->tblwznmmcall->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCAL, jref), &_recCal)) {
			recCal = *_recCal;
		...
		if (recCal.ref != 0) {
			...
			if (pnlmnstub) pnlmnstub->updatePreset(dbswznm, ixWznmVPreset, jrefTrig, notif);
		};
...
void PnlWznmCalRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCALRECDO) {
...
void PnlWznmCalRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCalRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCalRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCalRec::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMCAL_RETEQ) {
...
bool PnlWznmCalRec::handleCallWznmCal_retEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWznmCalRec::handleCallWznmCal_reuEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmCalRec::handleCallWznmCal_typEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWznmCalRec::handleCallWznmCal_verEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWznmCalRec::handleCallWznmCalUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
