...
CrdWznmUtl::CrdWznmUtl(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::CRDWZNMUTL, jrefSup, ixWznmVLocale)
		{
	...
	pnlheadbar = new PnlWznmUtlHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmUtl::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmUtl::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmUtl/" + VecWznmVLocale::getSref(ixWznmVLocale);
...
void CrdWznmUtl::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMINIT) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUTLDO) {
			...
			if (dpchappdo->ixVDo != 0) {
				if (dpchappdo->ixVDo == VecVDo::CLOSE) {
					...
					handleDpchAppDoMitCrdMipClick(dbswznm, &(req->dpcheng));
				};
...
void CrdWznmUtl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmUtl::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMUTL);
};

void CrdWznmUtl::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmUtl::handleDpchAppDoMitCrdXipClick(
	...
	if (!dlgextrip) {
		dlgextrip = new DlgWznmUtlExtrip(xchg, dbswznm, jref, ixWznmVLocale);
...
void CrdWznmUtl::handleDpchAppDoMitCrdMipClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	if (!dlgmrgip) {
		dlgmrgip = new DlgWznmUtlMrgip(xchg, dbswznm, jref, ixWznmVLocale);
...
void CrdWznmUtl::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmUtl::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmUtl::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmUtl::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmUtl::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
