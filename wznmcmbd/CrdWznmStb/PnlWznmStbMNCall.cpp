/**
	...
	* job handler for job PnlWznmStbMNCall (implementation)
...
#include "PnlWznmStbMNCall_blks.cpp"
#include "PnlWznmStbMNCall_evals.cpp"
...
/******************************************************************************
 class PnlWznmStbMNCall
 ******************************************************************************/

PnlWznmStbMNCall::PnlWznmStbMNCall(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMSTBMNCALL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmStbMNCall(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmStbMNCall::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmStbMNCall::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmStbMNCall::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTBMNCALLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTBMNCALLDO) {
...
void PnlWznmStbMNCall::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmStbMNCall::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmStbMNCall::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmStbMNCall::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmStbMNCall::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmStbMNCall::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmStbMNCall::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmcall->loadRecByRef(recSelNew->mref, &_recCal)) {
					recCal = *_recCal;
...
void PnlWznmStbMNCall::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAL, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmCal";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recCal.ref, jrefNew);
		};
...
void PnlWznmStbMNCall::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmStbMNCall::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmStbMNCall::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmStbMNCall::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmStbMNCall::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
