/**
	...
	* job handler for job PnlWznmTblHk1NVector (implementation)
...
#include "PnlWznmTblHk1NVector_blks.cpp"
#include "PnlWznmTblHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblHk1NVector
 ******************************************************************************/

PnlWznmTblHk1NVector::PnlWznmTblHk1NVector(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLHK1NVECTOR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblHk1NVector(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblHk1NVector::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTblHk1NVector::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblHk1NVector::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLHK1NVECTORDO) {
...
void PnlWznmTblHk1NVector::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblHk1NVector::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblHk1NVector::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblHk1NVector::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblHk1NVector::StgIac::JNUM)) {
...
void PnlWznmTblHk1NVector::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmTblHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblHk1NVector::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblHk1NVector::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
