/**
	...
	* job handler for job PnlWznmTblATitle (implementation)
...
#include "PnlWznmTblATitle_blks.cpp"
#include "PnlWznmTblATitle_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblATitle
 ******************************************************************************/

PnlWznmTblATitle::PnlWznmTblATitle(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLATITLE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblATitle(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblATitle::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblATitle::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblATitle::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLATITLEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLATITLEDO) {
...
void PnlWznmTblATitle::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblATitle::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblATitle::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblATitle::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblATitle* recSelNew = NULL;

	WznmAMTableTitle* _recTblAtit = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblATitle::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblATitle::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblATitle::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recTblAtit = WznmAMTableTitle();

			if (recSelNew) {
				if (dbswznm->tblwznmamtabletitle->loadRecByRef(recSelNew->ref, &_recTblAtit)) {
					recTblAtit = *_recTblAtit;
					delete _recTblAtit;
				};
			};
...
void PnlWznmTblATitle::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblATitle::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblATitle::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblATitle::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblATitle::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblATitle::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
