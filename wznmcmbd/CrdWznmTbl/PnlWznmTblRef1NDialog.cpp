/**
	...
	* job handler for job PnlWznmTblRef1NDialog (implementation)
...
#include "PnlWznmTblRef1NDialog_blks.cpp"
#include "PnlWznmTblRef1NDialog_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblRef1NDialog
 ******************************************************************************/

PnlWznmTblRef1NDialog::PnlWznmTblRef1NDialog(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLREF1NDIALOG, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblRef1NDialog(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblRef1NDialog::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblRef1NDialog::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTblRef1NDialog::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblRef1NDialog::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLREF1NDIALOGDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLREF1NDIALOGDO) {
...
void PnlWznmTblRef1NDialog::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblRef1NDialog::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblRef1NDialog::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblRef1NDialog::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblRef1NDialog* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblRef1NDialog::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblRef1NDialog::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblRef1NDialog::StgIac::JNUM)) {
...
void PnlWznmTblRef1NDialog::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmTblRef1NDialog::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblRef1NDialog::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblRef1NDialog::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
