/**
	...
	* job handler for job PnlWznmTbl1NSubset (implementation)
...
#include "PnlWznmTbl1NSubset_blks.cpp"
#include "PnlWznmTbl1NSubset_evals.cpp"
...
/******************************************************************************
 class PnlWznmTbl1NSubset
 ******************************************************************************/

PnlWznmTbl1NSubset::PnlWznmTbl1NSubset(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBL1NSUBSET, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTbl1NSubset(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTbl1NSubset::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTbl1NSubset::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmTbl1NSubset::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTbl1NSubset::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBL1NSUBSETDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBL1NSUBSETDO) {
...
void PnlWznmTbl1NSubset::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTbl1NSubset::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTbl1NSubset::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTbl1NSubset::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTbl1NSubset* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTbl1NSubset::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTbl1NSubset::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTbl1NSubset::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSbs = WznmMSubset();

			if (recSelNew) {
				if (dbswznm->tblwznmmsubset->loadRecByRef(recSelNew->ref, &_recSbs)) {
					recSbs = *_recSbs;
					delete _recSbs;
				};
			};
...
void PnlWznmTbl1NSubset::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSBS, jref)) if (refTbl != 0) {
			sref = "CrdWznmSbs";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFTBL, refTbl, sref, recSbs.ref, jrefNew);
		};
...
void PnlWznmTbl1NSubset::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSBS, jref) & VecWznmWAccess::EDIT) != 0) if (refTbl != 0) {
			sref = "CrdWznmSbs";
...
void PnlWznmTbl1NSubset::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTbl1NSubset::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTbl1NSubset::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTbl1NSubset::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
