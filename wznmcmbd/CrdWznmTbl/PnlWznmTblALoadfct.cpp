/**
	...
	* job handler for job PnlWznmTblALoadfct (implementation)
...
#include "PnlWznmTblALoadfct_blks.cpp"
#include "PnlWznmTblALoadfct_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblALoadfct
 ******************************************************************************/

PnlWznmTblALoadfct::PnlWznmTblALoadfct(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLALOADFCT, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblALoadfct(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblALoadfct::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblALoadfct::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmTblALoadfct::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblALoadfct::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLALOADFCTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLALOADFCTDO) {
...
void PnlWznmTblALoadfct::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblALoadfct::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblALoadfct::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblALoadfct::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblALoadfct* recSelNew = NULL;

	WznmAMTableLoadfct* _recTblAlfc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblALoadfct::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblALoadfct::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblALoadfct::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recTblAlfc = WznmAMTableLoadfct();

			if (recSelNew) {
				if (dbswznm->tblwznmamtableloadfct->loadRecByRef(recSelNew->ref, &_recTblAlfc)) {
					recTblAlfc = *_recTblAlfc;
					delete _recTblAlfc;
				};
			};
...
void PnlWznmTblALoadfct::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblALoadfct::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblALoadfct::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblALoadfct::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblALoadfct::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblALoadfct::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
