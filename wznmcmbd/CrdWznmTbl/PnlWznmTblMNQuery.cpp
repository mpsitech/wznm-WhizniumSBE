/**
	...
	* job handler for job PnlWznmTblMNQuery (implementation)
...
#include "PnlWznmTblMNQuery_blks.cpp"
#include "PnlWznmTblMNQuery_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblMNQuery
 ******************************************************************************/

PnlWznmTblMNQuery::PnlWznmTblMNQuery(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLMNQUERY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblMNQuery(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblMNQuery::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblMNQuery::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTblMNQuery::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblMNQuery::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLMNQUERYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLMNQUERYDO) {
...
void PnlWznmTblMNQuery::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblMNQuery::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNQuery::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblMNQuery::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblMNQuery* recSelNew = NULL;

	WznmRMQueryMTable* _recQryRtbl = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblMNQuery::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblMNQuery::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblMNQuery::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmquerymtable->loadRecByRef(recSelNew->ref, &_recQryRtbl)) {
					recQryRtbl = *_recQryRtbl;
					delete _recQryRtbl;
				};
...
void PnlWznmTblMNQuery::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmTblMNQuery::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNQuery::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNQuery::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblMNQuery::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblMNQuery::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
