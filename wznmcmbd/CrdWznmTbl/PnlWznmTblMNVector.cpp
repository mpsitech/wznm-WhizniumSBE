/**
	...
	* job handler for job PnlWznmTblMNVector (implementation)
...
#include "PnlWznmTblMNVector_blks.cpp"
#include "PnlWznmTblMNVector_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblMNVector
 ******************************************************************************/

PnlWznmTblMNVector::PnlWznmTblMNVector(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLMNVECTOR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblMNVector(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblMNVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblMNVector::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmTblMNVector::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblMNVector::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLMNVECTORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLMNVECTORDO) {
...
void PnlWznmTblMNVector::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblMNVector::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNVector::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblMNVector::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblMNVector* recSelNew = NULL;

	WznmRMTableMVector* _recTblRvec = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblMNVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblMNVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblMNVector::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmtablemvector->loadRecByRef(recSelNew->ref, &_recTblRvec)) {
					recTblRvec = *_recTblRvec;
					delete _recTblRvec;
				};
				if (dbswznm->tblwznmmvector->loadRecByRef(recSelNew->mref, &_recVec)) {
					recVec = *_recVec;
...
void PnlWznmTblMNVector::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNVector::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblMNVector::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblMNVector::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblMNVector::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
