/**
	...
	* job handler for job PnlWznmTblSrc1NFeed (implementation)
...
#include "PnlWznmTblSrc1NFeed_blks.cpp"
#include "PnlWznmTblSrc1NFeed_evals.cpp"
...
/******************************************************************************
 class PnlWznmTblSrc1NFeed
 ******************************************************************************/

PnlWznmTblSrc1NFeed::PnlWznmTblSrc1NFeed(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTBLSRC1NFEED, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTblSrc1NFeed(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTblSrc1NFeed::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTblSrc1NFeed::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTblSrc1NFeed::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTblSrc1NFeed::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLSRC1NFEEDDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTBLSRC1NFEEDDO) {
...
void PnlWznmTblSrc1NFeed::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTblSrc1NFeed::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTblSrc1NFeed::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTblSrc1NFeed::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTblSrc1NFeed* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTblSrc1NFeed::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTblSrc1NFeed::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTblSrc1NFeed::StgIac::JNUM)) {
...
void PnlWznmTblSrc1NFeed::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmTblSrc1NFeed::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTblSrc1NFeed::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTblSrc1NFeed::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
