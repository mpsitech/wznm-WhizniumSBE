/**
	...
	* job handler for job PnlWznmPnlList (implementation)
...
#include "PnlWznmPnlList_blks.cpp"
#include "PnlWznmPnlList_evals.cpp"
...
/******************************************************************************
 class PnlWznmPnlList
 ******************************************************************************/

PnlWznmPnlList::PnlWznmPnlList(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMPNLLIST, jrefSup, ixWznmVLocale)
		{
	...
	QryWznmPnlList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSEL, jref, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFPNL, jref));
	xchg->addIxPreset(VecWznmVPreset::PREWZNMIXORD, jref, QryWznmPnlList::VecVOrd::SRF);

	qry = new QryWznmPnlList(xchg, dbswznm, jref, ixWznmVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixWznmVExpstate = ((xchg->getRefPreset(VecWznmVPreset::PREWZNMREFPNL, jref)) ? VecWznmVExpstate::MIND : VecWznmVExpstate::REGD);
...
DpchEngWznm* PnlWznmPnlList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmPnlList::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmPnlList::regularize(
			DbsWznm* dbswznm
			, const bool notif
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPNLLISTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPNLLISTDO) {
...
void PnlWznmPnlList::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmPnlList::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryWznmPnlList::VecVOrd::CAR) && (_contiac->numFTos <= QryWznmPnlList::VecVOrd::SRF)) {
			muteRefresh = true;
...
void PnlWznmPnlList::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmPnlList::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmPnlList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmPnlList::StgIac::JNUM) || (diffitems.size() > 1)) {
			muteRefresh = true;
		...
		if (has(diffitems, QryWznmPnlList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFPNL, refSelNew);
		};
	};
...
void PnlWznmPnlList::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDoButFilterClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPnlList::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmPnlList::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmPnlList::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
