/**
	* \file CrdWznmPnl.cpp
	* job handler for job CrdWznmPnl (implementation)
...
CrdWznmPnl::CrdWznmPnl(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMPNL, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFPNL, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmPnlRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmPnlHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmPnlList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmPnl::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmPnl::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmPnl/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdPnl = StubWznm::getStubPnlStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFPNL, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmPnl::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmPnl", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFPNL, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFPNL, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFPNL, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmPnl::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmPnl::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPNLDO) {
...
void CrdWznmPnl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmPnl::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMPNL);
};

void CrdWznmPnl::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmPnl::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmPnl::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFPNL) {
...
bool CrdWznmPnl::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmPnl::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmPnl::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmPnl::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmPnl::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
