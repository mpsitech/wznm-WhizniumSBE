/**
	...
	* job handler for job PnlWznmOpxAInvarg (implementation)
...
#include "PnlWznmOpxAInvarg_blks.cpp"
#include "PnlWznmOpxAInvarg_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpxAInvarg
 ******************************************************************************/

PnlWznmOpxAInvarg::PnlWznmOpxAInvarg(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPXAINVARG, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpxAInvarg(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpxAInvarg::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpxAInvarg::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmOpxAInvarg::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmOpxAInvarg::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPXAINVARGDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPXAINVARGDO) {
...
void PnlWznmOpxAInvarg::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpxAInvarg::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxAInvarg::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpxAInvarg::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQOpxAInvarg* recSelNew = NULL;

	WznmAMOpInvarg* _recOpxAinv = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpxAInvarg::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpxAInvarg::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpxAInvarg::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOpxAinv = WznmAMOpInvarg();

			if (recSelNew) {
				if (dbswznm->tblwznmamopinvarg->loadRecByRef(recSelNew->ref, &_recOpxAinv)) {
					recOpxAinv = *_recOpxAinv;
					delete _recOpxAinv;
				};
			};
...
void PnlWznmOpxAInvarg::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxAInvarg::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxAInvarg::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxAInvarg::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpxAInvarg::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpxAInvarg::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
