/**
	...
	* job handler for job PnlWznmOpxARetval (implementation)
...
#include "PnlWznmOpxARetval_blks.cpp"
#include "PnlWznmOpxARetval_evals.cpp"
...
/******************************************************************************
 class PnlWznmOpxARetval
 ******************************************************************************/

PnlWznmOpxARetval::PnlWznmOpxARetval(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMOPXARETVAL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmOpxARetval(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmOpxARetval::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmOpxARetval::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmOpxARetval::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmOpxARetval::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPXARETVALDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMOPXARETVALDO) {
...
void PnlWznmOpxARetval::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmOpxARetval::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxARetval::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmOpxARetval::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQOpxARetval* recSelNew = NULL;

	WznmAMOpRetval* _recOpxAret = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmOpxARetval::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmOpxARetval::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmOpxARetval::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recOpxAret = WznmAMOpRetval();

			if (recSelNew) {
				if (dbswznm->tblwznmamopretval->loadRecByRef(recSelNew->ref, &_recOpxAret)) {
					recOpxAret = *_recOpxAret;
					delete _recOpxAret;
				};
			};
...
void PnlWznmOpxARetval::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxARetval::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxARetval::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmOpxARetval::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmOpxARetval::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmOpxARetval::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
