/**
	* \file CrdWznmCar.cpp
	* job handler for job CrdWznmCar (implementation)
...
CrdWznmCar::CrdWznmCar(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMCAR, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCAR, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmCarRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmCarHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmCarList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmCar::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmCar::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmCar/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdCar = StubWznm::getStubCarStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCAR, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmCar::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmCar", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCAR, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCAR, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCAR, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmCar::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmCar::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCARDO) {
...
void CrdWznmCar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmCar::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMCAR);
};

void CrdWznmCar::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmCar::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmCar::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmCar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmCar::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFCAR) {
...
void CrdWznmCar::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmCar::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmCar::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
