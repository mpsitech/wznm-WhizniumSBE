/**
	...
	* job handler for job PnlWznmCarHk1NControl (implementation)
...
#include "PnlWznmCarHk1NControl_blks.cpp"
#include "PnlWznmCarHk1NControl_evals.cpp"
...
/******************************************************************************
 class PnlWznmCarHk1NControl
 ******************************************************************************/

PnlWznmCarHk1NControl::PnlWznmCarHk1NControl(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCARHK1NCONTROL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCarHk1NControl(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCarHk1NControl::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCarHk1NControl::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmCarHk1NControl::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmCarHk1NControl::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCARHK1NCONTROLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCARHK1NCONTROLDO) {
...
void PnlWznmCarHk1NControl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCarHk1NControl::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCarHk1NControl::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCarHk1NControl::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCarHk1NControl* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCarHk1NControl::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCarHk1NControl::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCarHk1NControl::StgIac::JNUM)) {
...
void PnlWznmCarHk1NControl::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCarHk1NControl::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCarHk1NControl::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refCar = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCAR, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCON, jref)) if (refCar != 0) {
			sref = "CrdWznmCon";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFCAR, refCar, sref, recCon.ref, jrefNew);
		};
...
void PnlWznmCarHk1NControl::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCON, jref) & VecWznmWAccess::EDIT) != 0) if (refCar != 0) {
			sref = "CrdWznmCon";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFCAR, refCar, sref, -1, jrefNew);
		};
...
void PnlWznmCarHk1NControl::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCarHk1NControl::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCarHk1NControl::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCarHk1NControl::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
