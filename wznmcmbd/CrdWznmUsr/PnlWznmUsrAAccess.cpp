/**
	...
	* job handler for job PnlWznmUsrAAccess (implementation)
...
#include "PnlWznmUsrAAccess_blks.cpp"
#include "PnlWznmUsrAAccess_evals.cpp"
...
/******************************************************************************
 class PnlWznmUsrAAccess
 ******************************************************************************/

PnlWznmUsrAAccess::PnlWznmUsrAAccess(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMUSRAACCESS, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmUsrAAccess(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmUsrAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmUsrAAccess::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmUsrAAccess::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmUsrAAccess::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSRAACCESSDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSRAACCESSDO) {
...
void PnlWznmUsrAAccess::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmUsrAAccess::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrAAccess::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmUsrAAccess::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQUsrAAccess* recSelNew = NULL;

	WznmAMUserAccess* _recUsrAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmUsrAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmUsrAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmUsrAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsrAacc = WznmAMUserAccess();

			if (recSelNew) {
				if (dbswznm->tblwznmamuseraccess->loadRecByRef(recSelNew->ref, &_recUsrAacc)) {
...
void PnlWznmUsrAAccess::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrAAccess::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrAAccess::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrAAccess::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmUsrAAccess::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmUsrAAccess::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
