/**
	...
	* job handler for job PnlWznmUsrMNUsergroup (implementation)
...
#include "PnlWznmUsrMNUsergroup_blks.cpp"
#include "PnlWznmUsrMNUsergroup_evals.cpp"
...
/******************************************************************************
 class PnlWznmUsrMNUsergroup
 ******************************************************************************/

PnlWznmUsrMNUsergroup::PnlWznmUsrMNUsergroup(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMUSRMNUSERGROUP, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmUsrMNUsergroup(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmUsrMNUsergroup::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmUsrMNUsergroup::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmUsrMNUsergroup::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFUSR) {
		recUsrRusg = WznmRMUserMUsergroup();

		recUsg = WznmMUsergroup();
...
void PnlWznmUsrMNUsergroup::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSRMNUSERGROUPDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSRMNUSERGROUPDO) {
...
void PnlWznmUsrMNUsergroup::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmUsrMNUsergroup::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrMNUsergroup::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmUsrMNUsergroup::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQUsrMNUsergroup* recSelNew = NULL;

	WznmRMUserMUsergroup* _recUsrRusg = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmUsrMNUsergroup::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmUsrMNUsergroup::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmUsrMNUsergroup::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmusermusergroup->loadRecByRef(recSelNew->ref, &_recUsrRusg)) {
					recUsrRusg = *_recUsrRusg;
					delete _recUsrRusg;
				};
				if (dbswznm->tblwznmmusergroup->loadRecByRef(recSelNew->mref, &_recUsg)) {
					recUsg = *_recUsg;
					delete _recUsg;
				};
			};
...
void PnlWznmUsrMNUsergroup::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCUSG, jref)) {
			sref = "CrdWznmUsg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recUsg.ref, jrefNew);
		};
...
void PnlWznmUsrMNUsergroup::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrMNUsergroup::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsrMNUsergroup::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmUsrMNUsergroup::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmUsrMNUsergroup::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
