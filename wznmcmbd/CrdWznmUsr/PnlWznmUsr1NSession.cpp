/**
	...
	* job handler for job PnlWznmUsr1NSession (implementation)
...
#include "PnlWznmUsr1NSession_blks.cpp"
...
/******************************************************************************
 class PnlWznmUsr1NSession
 ******************************************************************************/

PnlWznmUsr1NSession::PnlWznmUsr1NSession(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMUSR1NSESSION, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmUsr1NSession(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmUsr1NSession::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmUsr1NSession::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmUsr1NSession::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmUsr1NSession::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSR1NSESSIONDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSR1NSESSIONDO) {
...
void PnlWznmUsr1NSession::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmUsr1NSession::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsr1NSession::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmUsr1NSession::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQUsr1NSession* recSelNew = NULL;

	WznmMSession* _recSes = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmUsr1NSession::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmUsr1NSession::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmUsr1NSession::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSes = WznmMSession();

			if (recSelNew) {
				if (dbswznm->tblwznmmsession->loadRecByRef(recSelNew->ref, &_recSes)) {
					recSes = *_recSes;
					delete _recSes;
				};
			};
...
void PnlWznmUsr1NSession::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmUsr1NSession::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmUsr1NSession::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
