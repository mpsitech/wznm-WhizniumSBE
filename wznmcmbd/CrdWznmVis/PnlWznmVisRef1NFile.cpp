/**
	* \file PnlWznmVisRef1NFile.cpp
	* job handler for job PnlWznmVisRef1NFile (implementation)
...
#include "PnlWznmVisRef1NFile_blks.cpp"
#include "PnlWznmVisRef1NFile_evals.cpp"
...
/******************************************************************************
 class PnlWznmVisRef1NFile
 ******************************************************************************/

PnlWznmVisRef1NFile::PnlWznmVisRef1NFile(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVISREF1NFILE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVisRef1NFile(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVisRef1NFile::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVisRef1NFile::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVisRef1NFile::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFVIS) {
		recFil = WznmMFile();
...
void PnlWznmVisRef1NFile::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVISREF1NFILEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVISREF1NFILEDO) {
...
void PnlWznmVisRef1NFile::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVisRef1NFile::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisRef1NFile::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVisRef1NFile::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVisRef1NFile* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVisRef1NFile::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVisRef1NFile::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVisRef1NFile::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recFil = WznmMFile();

			if (recSelNew) {
				if (dbswznm->tblwznmmfile->loadRecByRef(recSelNew->ref, &_recFil)) {
					recFil = *_recFil;
					delete _recFil;
				};
			};
...
void PnlWznmVisRef1NFile::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCFIL, jref)) {
			sref = "CrdWznmFil";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recFil.ref, jrefNew);
		};
...
void PnlWznmVisRef1NFile::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCFIL, jref) & VecWznmWAccess::EDIT) != 0) {
			sref = "CrdWznmFil";
...
void PnlWznmVisRef1NFile::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisRef1NFile::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVisRef1NFile::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVisRef1NFile::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
