/**
	...
	* job handler for job PnlWznmVisVis1NSheet (implementation)
...
#include "PnlWznmVisVis1NSheet_blks.cpp"
#include "PnlWznmVisVis1NSheet_evals.cpp"
...
/******************************************************************************
 class PnlWznmVisVis1NSheet
 ******************************************************************************/

PnlWznmVisVis1NSheet::PnlWznmVisVis1NSheet(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVISVIS1NSHEET, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVisVis1NSheet(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVisVis1NSheet::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVisVis1NSheet::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmVisVis1NSheet::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVisVis1NSheet::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVISVIS1NSHEETDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVISVIS1NSHEETDO) {
...
void PnlWznmVisVis1NSheet::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVisVis1NSheet::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisVis1NSheet::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVisVis1NSheet::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVisVis1NSheet* recSelNew = NULL;

	WznmMSheet* _recSht = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVisVis1NSheet::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVisVis1NSheet::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVisVis1NSheet::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSht = WznmMSheet();

			if (recSelNew) {
				if (dbswznm->tblwznmmsheet->loadRecByRef(recSelNew->ref, &_recSht)) {
					recSht = *_recSht;
					delete _recSht;
				};
			};
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refVis = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFVIS, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSHT, jref)) if (refVis != 0) {
			sref = "CrdWznmSht";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVIS, refVis, sref, recSht.ref, jrefNew);
		};
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSHT, jref) & VecWznmWAccess::EDIT) != 0) if (refVis != 0) {
			sref = "CrdWznmSht";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVIS, refVis, sref, -1, jrefNew);
		};
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVisVis1NSheet::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVisVis1NSheet::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVisVis1NSheet::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
