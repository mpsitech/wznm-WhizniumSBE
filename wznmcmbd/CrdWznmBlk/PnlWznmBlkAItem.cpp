/**
	...
	* job handler for job PnlWznmBlkAItem (implementation)
...
#include "PnlWznmBlkAItem_blks.cpp"
#include "PnlWznmBlkAItem_evals.cpp"
...
/******************************************************************************
 class PnlWznmBlkAItem
 ******************************************************************************/

PnlWznmBlkAItem::PnlWznmBlkAItem(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMBLKAITEM, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmBlkAItem(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmBlkAItem::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmBlkAItem::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmBlkAItem::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBLKAITEMDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBLKAITEMDO) {
...
void PnlWznmBlkAItem::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmBlkAItem::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmBlkAItem::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQBlkAItem* recSelNew = NULL;

	WznmAMBlockItem* _recBlkAitm = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmBlkAItem::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmBlkAItem::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmBlkAItem::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recBlkAitm = WznmAMBlockItem();

			if (recSelNew) {
				if (dbswznm->tblwznmamblockitem->loadRecByRef(recSelNew->ref, &_recBlkAitm)) {
					recBlkAitm = *_recBlkAitm;
					delete _recBlkAitm;
				};
			};
...
void PnlWznmBlkAItem::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkAItem::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmBlkAItem::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmBlkAItem::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
