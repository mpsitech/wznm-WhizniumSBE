/**
	* \file CrdWznmBlk.cpp
	* job handler for job CrdWznmBlk (implementation)
...
CrdWznmBlk::CrdWznmBlk(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMBLK, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFBLK, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmBlkList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmBlkHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmBlkRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmBlk::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmBlk::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmBlk/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdBlk = StubWznm::getStubBlkStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFBLK, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmBlk::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmBlk", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFBLK, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFBLK, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFBLK, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmBlk::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmBlk::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBLKDO) {
...
void CrdWznmBlk::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmBlk::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMBLK);
};

void CrdWznmBlk::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmBlk::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmBlk::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFBLK) {
...
bool CrdWznmBlk::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmBlk::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmBlk::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmBlk::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmBlk::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
