/**
	...
	* job handler for job PnlWznmBlkRef1NRtblock (implementation)
...
#include "PnlWznmBlkRef1NRtblock_blks.cpp"
...
/******************************************************************************
 class PnlWznmBlkRef1NRtblock
 ******************************************************************************/

PnlWznmBlkRef1NRtblock::PnlWznmBlkRef1NRtblock(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMBLKREF1NRTBLOCK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmBlkRef1NRtblock(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmBlkRef1NRtblock::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmBlkRef1NRtblock::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmBlkRef1NRtblock::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmBlkRef1NRtblock::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBLKREF1NRTBLOCKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMBLKREF1NRTBLOCKDO) {
...
void PnlWznmBlkRef1NRtblock::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmBlkRef1NRtblock::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmBlkRef1NRtblock::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmBlkRef1NRtblock::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQBlkRef1NRtblock* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmBlkRef1NRtblock::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmBlkRef1NRtblock::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmBlkRef1NRtblock::StgIac::JNUM)) {
...
void PnlWznmBlkRef1NRtblock::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmBlkRef1NRtblock::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmBlkRef1NRtblock::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
