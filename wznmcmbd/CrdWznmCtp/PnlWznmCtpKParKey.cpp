/**
	* \file PnlWznmCtpKParKey.cpp
	* job handler for job PnlWznmCtpKParKey (implementation)
...
#include "PnlWznmCtpKParKey_blks.cpp"
#include "PnlWznmCtpKParKey_evals.cpp"
...
/******************************************************************************
 class PnlWznmCtpKParKey
 ******************************************************************************/

PnlWznmCtpKParKey::PnlWznmCtpKParKey(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCTPKPARKEY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCtpKParKey(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCtpKParKey::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCtpKParKey::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmCtpKParKey::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmCtpKParKey::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTPKPARKEYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTPKPARKEYDO) {
...
void PnlWznmCtpKParKey::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCtpKParKey::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCtpKParKey::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCtpKParKey* recSelNew = NULL;

	WznmAVKeylistKey* _recKlsAkey = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCtpKParKey::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCtpKParKey::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCtpKParKey::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recKlsAkey = WznmAVKeylistKey();

			if (recSelNew) {
				if (dbswznm->tblwznmavkeylistkey->loadRecByRef(recSelNew->ref, &_recKlsAkey)) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpKParKey::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCtpKParKey::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCtpKParKey::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
