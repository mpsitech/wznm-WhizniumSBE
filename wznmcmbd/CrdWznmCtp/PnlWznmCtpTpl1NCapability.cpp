/**
	...
	* job handler for job PnlWznmCtpTpl1NCapability (implementation)
...
#include "PnlWznmCtpTpl1NCapability_blks.cpp"
#include "PnlWznmCtpTpl1NCapability_evals.cpp"
...
/******************************************************************************
 class PnlWznmCtpTpl1NCapability
 ******************************************************************************/

PnlWznmCtpTpl1NCapability::PnlWznmCtpTpl1NCapability(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCTPTPL1NCAPABILITY, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCtpTpl1NCapability(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCtpTpl1NCapability::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCtpTpl1NCapability::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmCtpTpl1NCapability::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmCtpTpl1NCapability::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTPTPL1NCAPABILITYDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTPTPL1NCAPABILITYDO) {
...
void PnlWznmCtpTpl1NCapability::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCtpTpl1NCapability::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtpTpl1NCapability::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCtpTpl1NCapability::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCtpTpl1NCapability* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCtpTpl1NCapability::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCtpTpl1NCapability::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCtpTpl1NCapability::StgIac::JNUM)) {
...
void PnlWznmCtpTpl1NCapability::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCAP, jref)) if (refVer != 0) {
			sref = "CrdWznmCap";
...
void PnlWznmCtpTpl1NCapability::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCtpTpl1NCapability::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCtpTpl1NCapability::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
