/**
	...
	* job handler for job PnlWznmCtp1NTag (implementation)
...
#include "PnlWznmCtp1NTag_blks.cpp"
#include "PnlWznmCtp1NTag_evals.cpp"
...
/******************************************************************************
 class PnlWznmCtp1NTag
 ******************************************************************************/

PnlWznmCtp1NTag::PnlWznmCtp1NTag(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCTP1NTAG, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCtp1NTag(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCtp1NTag::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCtp1NTag::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmCtp1NTag::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTP1NTAGDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCTP1NTAGDO) {
...
void PnlWznmCtp1NTag::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCtp1NTag::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCtp1NTag::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCtp1NTag::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCtp1NTag::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCtp1NTag::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCtp1NTag::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recTag = WznmMTag();

			if (recSelNew) {
				if (dbswznm->tblwznmmtag->loadRecByRef(recSelNew->ref, &_recTag)) {
					recTag = *_recTag;
					delete _recTag;
				};
			};
...
void PnlWznmCtp1NTag::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmCtp1NTag::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCtp1NTag::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
