/**
	* \file PnlWznmIelList_blks.cpp
	* job handler for job PnlWznmIelList (implementation of blocks)
...
/******************************************************************************
 class PnlWznmIelList::VecVDo
 ******************************************************************************/

uint PnlWznmIelList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlWznmIelList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlWznmIelList::ContIac
 ******************************************************************************/

PnlWznmIelList::ContIac::ContIac(
			const uint numFTos
		) :
			Block()
			, numFTos(numFTos)
...
bool PnlWznmIelList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWznmIelList"];}();
...
bool PnlWznmIelList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacWznmIelList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacWznmIelList";
...
void PnlWznmIelList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContIacWznmIelList";
...
void PnlWznmIelList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemIacWznmIelList";
...
set<uint> PnlWznmIelList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmIelList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmIelList::ContInf
 ******************************************************************************/

PnlWznmIelList::ContInf::ContInf(
			const string& TxtFor
			, const string& TxtPre
...
void PnlWznmIelList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmIelList";
...
void PnlWznmIelList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWznmIelList";
...
set<uint> PnlWznmIelList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmIelList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmIelList::StatShr
 ******************************************************************************/

PnlWznmIelList::StatShr::StatShr(
...
void PnlWznmIelList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmIelList";
...
void PnlWznmIelList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWznmIelList";
...
set<uint> PnlWznmIelList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlWznmIelList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlWznmIelList::StgIac
 ******************************************************************************/

PnlWznmIelList::StgIac::StgIac(
			const uint TcoSrfWidth
			, const uint TcoShoWidth
			, const uint TcoTypWidth
			, const uint TcoImeWidth
...
bool PnlWznmIelList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWznmIelList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoImeWidth")) {TcoImeWidth = me["TcoImeWidth"].asUInt(); add(TCOIMEWIDTH);};
		if (me.isMember("TcoTcoWidth")) {TcoTcoWidth = me["TcoTcoWidth"].asUInt(); add(TCOTCOWIDTH);};
		if (me.isMember("TcoCtyWidth")) {TcoCtyWidth = me["TcoCtyWidth"].asUInt(); add(TCOCTYWIDTH);};
	};

	return basefound;
};

bool PnlWznmIelList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacWznmIelList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacWznmIelList";
	...
	if (basefound) {
		...
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Si", "sref", "TcoImeWidth", TcoImeWidth)) add(TCOIMEWIDTH);
...
void PnlWznmIelList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StgIacWznmIelList";
	...
	me["TcoImeWidth"] = (Json::Value::UInt) TcoImeWidth;
	me["TcoTcoWidth"] = (Json::Value::UInt) TcoTcoWidth;
	me["TcoCtyWidth"] = (Json::Value::UInt) TcoCtyWidth;
};

void PnlWznmIelList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
...
set<uint> PnlWznmIelList::StgIac::comm(
	...
	if (TcoImeWidth == comp->TcoImeWidth) insert(items, TCOIMEWIDTH);
...
set<uint> PnlWznmIelList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {TCOSRFWIDTH, TCOSHOWIDTH, TCOTYPWIDTH, TCOIMEWIDTH, TCOTCOWIDTH, TCOCTYWIDTH};
	for (auto it = commitems.begin(); it != commitems.end(); it++) diffitems.erase(*it);
...
/******************************************************************************
 class PnlWznmIelList::Tag
 ******************************************************************************/

void PnlWznmIelList::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmIelList";
...
void PnlWznmIelList::Tag::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWznmIelList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoIme", "Imp./exp.");
			writeStringAttr(wr, itemtag, "sref", "TcoTco", "Table col.");
			writeStringAttr(wr, itemtag, "sref", "TcoCty", "Conv. type");
		};
...
/******************************************************************************
 class PnlWznmIelList::DpchAppData
 ******************************************************************************/

PnlWznmIelList::DpchAppData::DpchAppData() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMIELLISTDATA)
		{
};

string PnlWznmIelList::DpchAppData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmIelList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmIelListData"];}();
	...
	if (basefound) {
		...
		contiac = ContIac();
		stgiac = StgIac();
		stgiacqry = QryWznmIelList::StgIac();
	};
};

void PnlWznmIelList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmIelListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmIelList::DpchAppDo
 ******************************************************************************/

PnlWznmIelList::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMIELLISTDO)
		{
	ixVDo = 0;
};
...
void PnlWznmIelList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmIelListDo"];}();
...
void PnlWznmIelList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmIelListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
/******************************************************************************
 class PnlWznmIelList::DpchEngData
 ******************************************************************************/

PnlWznmIelList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryWznmIelList::StatShr* statshrqry
			, QryWznmIelList::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMIELLISTDATA, jref)
		{
...
string PnlWznmIelList::DpchEngData::getSrefsMask() {
	vector<string> ss;
	string srefs;
...
void PnlWznmIelList::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void PnlWznmIelList::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmIelListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWznmIelList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWznmIelList::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmIelListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		...
		if (has(STATAPPQRY)) QryWznmIelList::StatApp::writeXML(wr);
