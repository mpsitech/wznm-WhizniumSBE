/**
	* \file CrdWznmIel.cpp
	* job handler for job CrdWznmIel (implementation)
...
CrdWznmIel::CrdWznmIel(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMIEL, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFIEL, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmIelList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmIelHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmIelRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmIel::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmIel::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmIel/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdIel = StubWznm::getStubIelStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFIEL, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmIel::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmIel", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFIEL, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIEL, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIEL, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmIel::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmIel::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIELDO) {
...
void CrdWznmIel::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmIel::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMIEL);
};

void CrdWznmIel::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmIel::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmIel::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFIEL) {
...
bool CrdWznmIel::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmIel::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmIel::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmIel::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmIel::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
