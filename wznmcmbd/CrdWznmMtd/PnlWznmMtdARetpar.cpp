/**
	...
	* job handler for job PnlWznmMtdARetpar (implementation)
...
#include "PnlWznmMtdARetpar_blks.cpp"
#include "PnlWznmMtdARetpar_evals.cpp"
...
/******************************************************************************
 class PnlWznmMtdARetpar
 ******************************************************************************/

PnlWznmMtdARetpar::PnlWznmMtdARetpar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMTDARETPAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMtdARetpar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMtdARetpar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMtdARetpar::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmMtdARetpar::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMtdARetpar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTDARETPARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTDARETPARDO) {
...
void PnlWznmMtdARetpar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMtdARetpar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMtdARetpar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMtdARetpar* recSelNew = NULL;

	WznmAMMethodRetpar* _recMtdArpa = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMtdARetpar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMtdARetpar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMtdARetpar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMtdArpa = WznmAMMethodRetpar();

			if (recSelNew) {
				if (dbswznm->tblwznmammethodretpar->loadRecByRef(recSelNew->ref, &_recMtdArpa)) {
					recMtdArpa = *_recMtdArpa;
					delete _recMtdArpa;
				};
			};
...
void PnlWznmMtdARetpar::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdARetpar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMtdARetpar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMtdARetpar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
