...
CrdWznmMtd::CrdWznmMtd(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMMTD, jrefSup, ixWznmVLocale)
		{
	...
	pnlrec = new PnlWznmMtdRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmMtd::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmMtd::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmMtd/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdMtd = StubWznm::getStubMtdStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFMTD, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmMtd::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmMtd", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFMTD, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFMTD, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFMTD, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmMtd::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmMtd::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTDDO) {
...
void CrdWznmMtd::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmMtd::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMMTD);
};

void CrdWznmMtd::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmMtd::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmMtd::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFMTD) {
...
bool CrdWznmMtd::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmMtd::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmMtd::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmMtd::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmMtd::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
