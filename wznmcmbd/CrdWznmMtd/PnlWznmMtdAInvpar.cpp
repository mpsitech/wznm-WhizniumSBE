/**
	...
	* job handler for job PnlWznmMtdAInvpar (implementation)
...
#include "PnlWznmMtdAInvpar_blks.cpp"
#include "PnlWznmMtdAInvpar_evals.cpp"
...
/******************************************************************************
 class PnlWznmMtdAInvpar
 ******************************************************************************/

PnlWznmMtdAInvpar::PnlWznmMtdAInvpar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMTDAINVPAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMtdAInvpar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMtdAInvpar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMtdAInvpar::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmMtdAInvpar::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMtdAInvpar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTDAINVPARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMTDAINVPARDO) {
...
void PnlWznmMtdAInvpar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMtdAInvpar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMtdAInvpar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMtdAInvpar* recSelNew = NULL;

	WznmAMMethodInvpar* _recMtdAipa = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMtdAInvpar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMtdAInvpar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMtdAInvpar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMtdAipa = WznmAMMethodInvpar();

			if (recSelNew) {
				if (dbswznm->tblwznmammethodinvpar->loadRecByRef(recSelNew->ref, &_recMtdAipa)) {
					recMtdAipa = *_recMtdAipa;
					delete _recMtdAipa;
				};
			};
...
void PnlWznmMtdAInvpar::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMtdAInvpar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMtdAInvpar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMtdAInvpar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
