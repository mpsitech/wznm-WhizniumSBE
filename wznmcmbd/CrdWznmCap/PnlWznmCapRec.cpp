/**
	...
	* job handler for job PnlWznmCapRec (implementation)
...
#include "PnlWznmCapRec_blks.cpp"
#include "PnlWznmCapRec_evals.cpp"
...
PnlWznmCapRec::PnlWznmCapRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCAPREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmCapRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCapRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubCapStd(dbswznm, recCpb.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmCapDetail(xchg, dbswznm, jref, ixWznmVLocale);
		if (!pnlapar) pnlapar = new PnlWznmCapAPar(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmCapRec::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFCAP) {
		...
		if (dbswznm->tblwznmmcapability->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCAP, jref), &_recCpb)) {
			recCpb = *_recCpb;
...
void PnlWznmCapRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCAPRECDO) {
...
void PnlWznmCapRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCapRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
bool PnlWznmCapRec::handleCallWznmCpbUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool PnlWznmCapRec::handleCallWznmCpb_tplEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmCapRec::handleCallWznmCpb_verEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWznmCapRec::handleCallWznmCpb_inSbs(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
