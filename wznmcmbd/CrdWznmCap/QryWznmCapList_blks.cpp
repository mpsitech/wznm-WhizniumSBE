/**
	* \file QryWznmCapList_blks.cpp
	* job handler for job QryWznmCapList (implementation of blocks)
...
void QryWznmCapList::VecVOrd::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void QryWznmCapList::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint firstcol
	...
	if (difftag.length() == 0) difftag = "StatAppQryWznmCapList";
...
void QryWznmCapList::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint firstcol
	...
	else itemtag = "StatitemAppQryWznmCapList";
...
void QryWznmCapList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrQryWznmCapList";
...
void QryWznmCapList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrQryWznmCapList";
...
bool QryWznmCapList::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacQryWznmCapList"];}();
...
void QryWznmCapList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
