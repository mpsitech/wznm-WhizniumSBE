/**
	* \file CrdWznmCap.cpp
	* job handler for job CrdWznmCap (implementation)
...
CrdWznmCap::CrdWznmCap(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMCAP, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCAP, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmCapList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmCapHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmCapRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmCap::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmCap::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmCap/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdCap = StubWznm::getStubCapStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCAP, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmCap::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmCap", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCPB, jref, ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCAP, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCAP, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCAP, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmCap::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmCap::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCAPDO) {
...
void CrdWznmCap::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmCap::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMCAP);
};

void CrdWznmCap::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmCap::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmCap::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFCAP) {
...
bool CrdWznmCap::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmCap::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmCap::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmCap::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmCap::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
