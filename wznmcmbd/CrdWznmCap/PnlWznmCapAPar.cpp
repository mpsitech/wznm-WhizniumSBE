/**
	...
	* job handler for job PnlWznmCapAPar (implementation)
...
#include "PnlWznmCapAPar_blks.cpp"
#include "PnlWznmCapAPar_evals.cpp"
...
/******************************************************************************
 class PnlWznmCapAPar
 ******************************************************************************/

PnlWznmCapAPar::PnlWznmCapAPar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCAPAPAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCapAPar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCapAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCapAPar::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFCAP) {
		recCpbApar = WznmAMCapabilityPar();
...
void PnlWznmCapAPar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCAPAPARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCAPAPARDO) {
...
void PnlWznmCapAPar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCapAPar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCapAPar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCapAPar* recSelNew = NULL;

	WznmAMCapabilityPar* _recCpbApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCapAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCapAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCapAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recCpbApar = WznmAMCapabilityPar();

			if (recSelNew) {
				if (dbswznm->tblwznmamcapabilitypar->loadRecByRef(recSelNew->ref, &_recCpbApar)) {
					recCpbApar = *_recCpbApar;
					delete _recCpbApar;
				};
			};
...
void PnlWznmCapAPar::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCapAPar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCapAPar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCapAPar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
