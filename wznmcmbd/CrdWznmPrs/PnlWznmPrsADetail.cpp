/**
	...
	* job handler for job PnlWznmPrsADetail (implementation)
...
#include "PnlWznmPrsADetail_blks.cpp"
#include "PnlWznmPrsADetail_evals.cpp"
...
/******************************************************************************
 class PnlWznmPrsADetail
 ******************************************************************************/

PnlWznmPrsADetail::PnlWznmPrsADetail(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMPRSADETAIL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmPrsADetail(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmPrsADetail::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmPrsADetail::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmPrsADetail::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmPrsADetail::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPRSADETAILDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPRSADETAILDO) {
...
void PnlWznmPrsADetail::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmPrsADetail::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPrsADetail::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmPrsADetail::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQPrsADetail* recSelNew = NULL;

	WznmAMPersonDetail* _recPrsAdet = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmPrsADetail::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmPrsADetail::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmPrsADetail::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recPrsAdet = WznmAMPersonDetail();

			if (recSelNew) {
				if (dbswznm->tblwznmampersondetail->loadRecByRef(recSelNew->ref, &_recPrsAdet)) {
...
void PnlWznmPrsADetail::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPrsADetail::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPrsADetail::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmPrsADetail::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmPrsADetail::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmPrsADetail::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
