/**
	...
	* job handler for job PnlWznmRlsRec (implementation)
...
#include "PnlWznmRlsRec_blks.cpp"
#include "PnlWznmRlsRec_evals.cpp"
...
PnlWznmRlsRec::PnlWznmRlsRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMRLSREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmRlsRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmRlsRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubRlsStd(dbswznm, recRls.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recRls.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmRlsDetail(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmRlsRec::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFRLS) {
		...
		if (dbswznm->tblwznmmrelease->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFRLS, jref), &_recRls)) {
			recRls = *_recRls;
			delete _recRls;
...
void PnlWznmRlsRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMRLSRECDO) {
...
void PnlWznmRlsRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmRlsRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmRlsRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmRlsRec::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMRLS_CMPEQ) {
...
bool PnlWznmRlsRec::handleCallWznmRls_cmpEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmRlsRec::handleCallWznmRls_mchEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWznmRlsRec::handleCallWznmRlsUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
