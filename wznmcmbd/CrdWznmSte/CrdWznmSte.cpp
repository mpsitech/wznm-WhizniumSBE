/**
	* \file CrdWznmSte.cpp
	* job handler for job CrdWznmSte (implementation)
...
CrdWznmSte::CrdWznmSte(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMSTE, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFSTE, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmSteRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmSteHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmSteList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmSte::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmSte::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmSte/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdSte = StubWznm::getStubSteStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSTE, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmSte::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmSte", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSTE, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSTE, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSTE, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmSte::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmSte::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTEDO) {
...
void CrdWznmSte::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmSte::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMSTE);
};

void CrdWznmSte::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmSte::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmSte::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFSTE) {
...
bool CrdWznmSte::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmSte::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmSte::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmSte::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmSte::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
