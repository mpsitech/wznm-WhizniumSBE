/**
	...
	* job handler for job PnlWznmSteAAction (implementation)
...
#include "PnlWznmSteAAction_blks.cpp"
#include "PnlWznmSteAAction_evals.cpp"
...
/******************************************************************************
 class PnlWznmSteAAction
 ******************************************************************************/

PnlWznmSteAAction::PnlWznmSteAAction(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMSTEAACTION, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmSteAAction(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmSteAAction::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmSteAAction::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmSteAAction::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmSteAAction::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTEAACTIONDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTEAACTIONDO) {
...
void PnlWznmSteAAction::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmSteAAction::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteAAction::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmSteAAction::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQSteAAction* recSelNew = NULL;

	WznmAMStateAction* _recSteAact = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmSteAAction::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmSteAAction::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmSteAAction::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSteAact = WznmAMStateAction();

			if (recSelNew) {
				if (dbswznm->tblwznmamstateaction->loadRecByRef(recSelNew->ref, &_recSteAact)) {
					recSteAact = *_recSteAact;
					delete _recSteAact;
				};
			};
...
void PnlWznmSteAAction::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteAAction::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteAAction::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteAAction::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmSteAAction::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmSteAAction::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
