...
#include "PnlWznmSteRec_blks.cpp"
#include "PnlWznmSteRec_evals.cpp"
...
PnlWznmSteRec::PnlWznmSteRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMSTEREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmSteRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmSteRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubSteStd(dbswznm, recSte.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recSte.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
...
void PnlWznmSteRec::updatePreset(
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFSTE) {
		...
		if (dbswznm->tblwznmmstate->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSTE, jref), &_recSte)) {
			recSte = *_recSte;
			delete _recSte;
...
void PnlWznmSteRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSTERECDO) {
...
void PnlWznmSteRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmSteRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSteRec::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMSTE_SEQEQ) {
...
bool PnlWznmSteRec::handleCallWznmSte_seqEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmSteRec::handleCallWznmSteUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
