/**
	...
	* job handler for job PnlWznmAppApp1NSequence (implementation)
...
#include "PnlWznmAppApp1NSequence_blks.cpp"
#include "PnlWznmAppApp1NSequence_evals.cpp"
...
/******************************************************************************
 class PnlWznmAppApp1NSequence
 ******************************************************************************/

PnlWznmAppApp1NSequence::PnlWznmAppApp1NSequence(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMAPPAPP1NSEQUENCE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmAppApp1NSequence(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmAppApp1NSequence::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmAppApp1NSequence::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmAppApp1NSequence::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmAppApp1NSequence::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPPAPP1NSEQUENCEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPPAPP1NSEQUENCEDO) {
...
void PnlWznmAppApp1NSequence::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmAppApp1NSequence::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmAppApp1NSequence::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmAppApp1NSequence::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQAppApp1NSequence* recSelNew = NULL;

	WznmMSequence* _recSeq = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmAppApp1NSequence::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmAppApp1NSequence::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmAppApp1NSequence::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSeq = WznmMSequence();

			if (recSelNew) {
				if (dbswznm->tblwznmmsequence->loadRecByRef(recSelNew->ref, &_recSeq)) {
					recSeq = *_recSeq;
					delete _recSeq;
				};
			};
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refApp = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFAPP, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSEQ, jref)) if (refApp != 0) {
			sref = "CrdWznmSeq";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFAPP, refApp, sref, recSeq.ref, jrefNew);
		};
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSEQ, jref) & VecWznmWAccess::EDIT) != 0) if (refApp != 0) {
			sref = "CrdWznmSeq";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFAPP, refApp, sref, -1, jrefNew);
		};
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmAppApp1NSequence::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmAppApp1NSequence::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmAppApp1NSequence::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
