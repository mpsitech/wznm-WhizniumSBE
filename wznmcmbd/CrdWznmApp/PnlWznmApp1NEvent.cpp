/**
	...
	* job handler for job PnlWznmApp1NEvent (implementation)
...
#include "PnlWznmApp1NEvent_blks.cpp"
#include "PnlWznmApp1NEvent_evals.cpp"
...
/******************************************************************************
 class PnlWznmApp1NEvent
 ******************************************************************************/

PnlWznmApp1NEvent::PnlWznmApp1NEvent(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMAPP1NEVENT, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmApp1NEvent(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmApp1NEvent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmApp1NEvent::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmApp1NEvent::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmApp1NEvent::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPP1NEVENTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPP1NEVENTDO) {
...
void PnlWznmApp1NEvent::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmApp1NEvent::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmApp1NEvent::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmApp1NEvent::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQApp1NEvent* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmApp1NEvent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmApp1NEvent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmApp1NEvent::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recEvt = WznmMEvent();

			if (recSelNew) {
				if (dbswznm->tblwznmmevent->loadRecByRef(recSelNew->ref, &_recEvt)) {
					recEvt = *_recEvt;
					delete _recEvt;
				};
			};
...
void PnlWznmApp1NEvent::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCEVT, jref)) if (refApp != 0) {
			sref = "CrdWznmEvt";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFAPP, refApp, sref, recEvt.ref, jrefNew);
		};
...
void PnlWznmApp1NEvent::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCEVT, jref) & VecWznmWAccess::EDIT) != 0) if (refApp != 0) {
			sref = "CrdWznmEvt";
...
void PnlWznmApp1NEvent::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmApp1NEvent::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmApp1NEvent::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmApp1NEvent::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
