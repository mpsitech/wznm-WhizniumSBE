...
#include "PnlWznmAppDetail_blks.cpp"
#include "PnlWznmAppDetail_evals.cpp"
...
/******************************************************************************
 class PnlWznmAppDetail
 ******************************************************************************/

PnlWznmAppDetail::PnlWznmAppDetail(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMAPPDETAIL, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmAppDetail::getNewDpchEng(
			set<uint> items
		) {
	...
	if (items.empty()) {
		...
		dpcheng = new DpchEngData(jref, &contiac, &continf, &feedFPupTrg, &statshr, items);
	};

	return dpcheng;
};

void PnlWznmAppDetail::refreshRecApp(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
	...
	if (dbswznm->tblwznmmapp->loadRecByRef(recApp.ref, &_recApp)) {
		recApp = *_recApp;
		delete _recApp;
	...
	statshr.PupTrgActive = evalPupTrgActive(dbswznm);
...
void PnlWznmAppDetail::updatePreset(
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFAPP) {
		recApp.ref = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFAPP, jref);
		refreshRecApp(dbswznm, moditems);
...
void PnlWznmAppDetail::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPPDETAILDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMAPPDETAILDO) {
...
void PnlWznmAppDetail::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmAppDetail::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
	...
	if (hasAny(diffitems, {ContIac::TXFSHO, ContIac::TXFTIT, ContIac::NUMFPUPTRG, ContIac::TXFCMT})) {
...
void PnlWznmAppDetail::handleDpchAppDoButSaveClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmAppDetail::handleDpchAppDoButVerViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButVerViewAvail && statshr.ButVerViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref)) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recApp.verRefWznmMVersion, jrefNew);
		};
...
void PnlWznmAppDetail::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMAPP_VEREQ) {
...
bool PnlWznmAppDetail::handleCallWznmApp_verEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmAppDetail::handleCallWznmAppUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
