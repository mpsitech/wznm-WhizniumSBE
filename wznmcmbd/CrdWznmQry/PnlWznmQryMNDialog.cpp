/**
	...
	* job handler for job PnlWznmQryMNDialog (implementation)
...
#include "PnlWznmQryMNDialog_blks.cpp"
#include "PnlWznmQryMNDialog_evals.cpp"
...
/******************************************************************************
 class PnlWznmQryMNDialog
 ******************************************************************************/

PnlWznmQryMNDialog::PnlWznmQryMNDialog(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMQRYMNDIALOG, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmQryMNDialog(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmQryMNDialog::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmQryMNDialog::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmQryMNDialog::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmQryMNDialog::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYMNDIALOGDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYMNDIALOGDO) {
...
void PnlWznmQryMNDialog::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmQryMNDialog::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNDialog::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmQryMNDialog::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQQryMNDialog* recSelNew = NULL;

	WznmRMDialogMQuery* _recDlgRqry = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmQryMNDialog::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmQryMNDialog::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmQryMNDialog::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmdialogmquery->loadRecByRef(recSelNew->ref, &_recDlgRqry)) {
					recDlgRqry = *_recDlgRqry;
					delete _recDlgRqry;
				};
				if (dbswznm->tblwznmmdialog->loadRecByRef(recSelNew->mref, &_recDlg)) {
					recDlg = *_recDlg;
					delete _recDlg;
				};
			};
...
void PnlWznmQryMNDialog::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCDLG, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmDlg";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recDlg.ref, jrefNew);
		};
...
void PnlWznmQryMNDialog::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNDialog::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNDialog::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmQryMNDialog::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmQryMNDialog::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
