/**
	...
	* job handler for job PnlWznmQryQry1NQuerycol (implementation)
...
#include "PnlWznmQryQry1NQuerycol_blks.cpp"
#include "PnlWznmQryQry1NQuerycol_evals.cpp"
...
/******************************************************************************
 class PnlWznmQryQry1NQuerycol
 ******************************************************************************/

PnlWznmQryQry1NQuerycol::PnlWznmQryQry1NQuerycol(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMQRYQRY1NQUERYCOL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmQryQry1NQuerycol(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmQryQry1NQuerycol::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmQryQry1NQuerycol::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmQryQry1NQuerycol::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmQryQry1NQuerycol::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYQRY1NQUERYCOLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYQRY1NQUERYCOLDO) {
...
void PnlWznmQryQry1NQuerycol::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmQryQry1NQuerycol::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQQryQry1NQuerycol* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmQryQry1NQuerycol::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmQryQry1NQuerycol::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmQryQry1NQuerycol::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recQco = WznmMQuerycol();

			if (recSelNew) {
				if (dbswznm->tblwznmmquerycol->loadRecByRef(recSelNew->ref, &_recQco)) {
					recQco = *_recQco;
					delete _recQco;
				};
			};
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refQry = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFQRY, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQCO, jref)) if (refQry != 0) {
			sref = "CrdWznmQco";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFQRY, refQry, sref, recQco.ref, jrefNew);
		};
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQCO, jref) & VecWznmWAccess::EDIT) != 0) if (refQry != 0) {
			sref = "CrdWznmQco";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFQRY, refQry, sref, -1, jrefNew);
		};
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryQry1NQuerycol::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmQryQry1NQuerycol::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmQryQry1NQuerycol::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
