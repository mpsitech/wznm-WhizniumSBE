/**
	...
	* job handler for job PnlWznmQryMNPanel (implementation)
...
#include "PnlWznmQryMNPanel_blks.cpp"
#include "PnlWznmQryMNPanel_evals.cpp"
...
/******************************************************************************
 class PnlWznmQryMNPanel
 ******************************************************************************/

PnlWznmQryMNPanel::PnlWznmQryMNPanel(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMQRYMNPANEL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmQryMNPanel(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmQryMNPanel::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmQryMNPanel::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmQryMNPanel::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmQryMNPanel::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYMNPANELDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQRYMNPANELDO) {
...
void PnlWznmQryMNPanel::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmQryMNPanel::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNPanel::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmQryMNPanel::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQQryMNPanel* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmQryMNPanel::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmQryMNPanel::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmQryMNPanel::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmpanel->loadRecByRef(recSelNew->mref, &_recPnl)) {
					recPnl = *_recPnl;
					delete _recPnl;
				};
			};
...
void PnlWznmQryMNPanel::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmQryMNPanel::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNPanel::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmQryMNPanel::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmQryMNPanel::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmQryMNPanel::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
