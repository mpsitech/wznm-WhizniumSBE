/**
	* \file CrdWznmIme.cpp
	* job handler for job CrdWznmIme (implementation)
...
CrdWznmIme::CrdWznmIme(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMIME, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFIME, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmImeList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmImeRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmImeHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmIme::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmIme::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmIme/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdIme = StubWznm::getStubImeStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFIME, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmIme::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmIme", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFIME, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIME, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIME, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmIme::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmIme::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIMEDO) {
...
void CrdWznmIme::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmIme::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMIME);
};

void CrdWznmIme::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmIme::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmIme::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFIME) {
...
bool CrdWznmIme::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmIme::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmIme::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmIme::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmIme::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
