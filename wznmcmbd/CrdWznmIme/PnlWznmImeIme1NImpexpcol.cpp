/**
	...
	* job handler for job PnlWznmImeIme1NImpexpcol (implementation)
...
#include "PnlWznmImeIme1NImpexpcol_blks.cpp"
#include "PnlWznmImeIme1NImpexpcol_evals.cpp"
...
/******************************************************************************
 class PnlWznmImeIme1NImpexpcol
 ******************************************************************************/

PnlWznmImeIme1NImpexpcol::PnlWznmImeIme1NImpexpcol(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMIMEIME1NIMPEXPCOL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmImeIme1NImpexpcol(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmImeIme1NImpexpcol::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmImeIme1NImpexpcol::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmImeIme1NImpexpcol::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmImeIme1NImpexpcol::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIMEIME1NIMPEXPCOLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIMEIME1NIMPEXPCOLDO) {
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmImeIme1NImpexpcol::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQImeIme1NImpexpcol* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmImeIme1NImpexpcol::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmImeIme1NImpexpcol::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmImeIme1NImpexpcol::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recIel = WznmMImpexpcol();

			if (recSelNew) {
				if (dbswznm->tblwznmmimpexpcol->loadRecByRef(recSelNew->ref, &_recIel)) {
					recIel = *_recIel;
					delete _recIel;
				};
			};
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refIme = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFIME, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCIEL, jref)) if (refIme != 0) {
			sref = "CrdWznmIel";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFIME, refIme, sref, recIel.ref, jrefNew);
		};
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCIEL, jref) & VecWznmWAccess::EDIT) != 0) if (refIme != 0) {
			sref = "CrdWznmIel";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFIME, refIme, sref, -1, jrefNew);
		};
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmImeIme1NImpexpcol::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmImeIme1NImpexpcol::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmImeIme1NImpexpcol::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
