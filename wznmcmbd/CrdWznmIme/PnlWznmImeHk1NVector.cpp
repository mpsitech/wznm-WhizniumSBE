/**
	...
	* job handler for job PnlWznmImeHk1NVector (implementation)
...
#include "PnlWznmImeHk1NVector_blks.cpp"
#include "PnlWznmImeHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWznmImeHk1NVector
 ******************************************************************************/

PnlWznmImeHk1NVector::PnlWznmImeHk1NVector(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMIMEHK1NVECTOR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmImeHk1NVector(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmImeHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmImeHk1NVector::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmImeHk1NVector::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmImeHk1NVector::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIMEHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIMEHK1NVECTORDO) {
...
void PnlWznmImeHk1NVector::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmImeHk1NVector::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmImeHk1NVector::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmImeHk1NVector::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQImeHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmImeHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmImeHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmImeHk1NVector::StgIac::JNUM)) {
...
void PnlWznmImeHk1NVector::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmImeHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmImeHk1NVector::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmImeHk1NVector::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
