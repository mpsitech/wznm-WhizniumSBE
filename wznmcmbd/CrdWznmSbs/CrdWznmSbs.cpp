/**
	* \file CrdWznmSbs.cpp
	* job handler for job CrdWznmSbs (implementation)
...
CrdWznmSbs::CrdWznmSbs(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMSBS, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFSBS, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmSbsList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmSbsHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmSbsRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmSbs::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmSbs::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmSbs/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdSbs = StubWznm::getStubSbsStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSBS, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmSbs::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmSbs", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSBS, jref, ref);
	...
	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSBS, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSBS, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmSbs::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmSbs::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSBSDO) {
...
void CrdWznmSbs::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmSbs::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMSBS);
};

void CrdWznmSbs::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmSbs::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmSbs::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFSBS) {
...
bool CrdWznmSbs::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmSbs::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmSbs::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmSbs::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmSbs::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
