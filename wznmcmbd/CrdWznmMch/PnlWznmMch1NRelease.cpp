/**
	...
	* job handler for job PnlWznmMch1NRelease (implementation)
...
#include "PnlWznmMch1NRelease_blks.cpp"
#include "PnlWznmMch1NRelease_evals.cpp"
...
/******************************************************************************
 class PnlWznmMch1NRelease
 ******************************************************************************/

PnlWznmMch1NRelease::PnlWznmMch1NRelease(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMCH1NRELEASE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMch1NRelease(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMch1NRelease::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMch1NRelease::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmMch1NRelease::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMch1NRelease::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCH1NRELEASEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCH1NRELEASEDO) {
...
void PnlWznmMch1NRelease::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMch1NRelease::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMch1NRelease::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMch1NRelease::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMch1NRelease* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMch1NRelease::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMch1NRelease::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMch1NRelease::StgIac::JNUM)) {
...
void PnlWznmMch1NRelease::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCRLS, jref)) if (refVer != 0) {
			sref = "CrdWznmRls";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recRls.ref, jrefNew);
		};
...
void PnlWznmMch1NRelease::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMch1NRelease::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMch1NRelease::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
