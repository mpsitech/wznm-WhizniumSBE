/**
	...
	* job handler for job PnlWznmMchSup1NMachine (implementation)
...
#include "PnlWznmMchSup1NMachine_blks.cpp"
#include "PnlWznmMchSup1NMachine_evals.cpp"
...
/******************************************************************************
 class PnlWznmMchSup1NMachine
 ******************************************************************************/

PnlWznmMchSup1NMachine::PnlWznmMchSup1NMachine(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMCHSUP1NMACHINE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMchSup1NMachine(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMchSup1NMachine::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMchSup1NMachine::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmMchSup1NMachine::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMchSup1NMachine::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCHSUP1NMACHINEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCHSUP1NMACHINEDO) {
...
void PnlWznmMchSup1NMachine::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMchSup1NMachine::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMchSup1NMachine::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMchSup1NMachine::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMchSup1NMachine* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMchSup1NMachine::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMchSup1NMachine::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMchSup1NMachine::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMch = WznmMMachine();

			if (recSelNew) {
				if (dbswznm->tblwznmmmachine->loadRecByRef(recSelNew->ref, &_recMch)) {
					recMch = *_recMch;
					delete _recMch;
				};
			};
...
void PnlWznmMchSup1NMachine::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCMCH, jref)) {
			sref = "CrdWznmMch";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recMch.ref, jrefNew);
		};
...
void PnlWznmMchSup1NMachine::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMchSup1NMachine::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMchSup1NMachine::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
