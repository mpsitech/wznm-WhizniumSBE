/**
	...
	* job handler for job PnlWznmMchAPar (implementation)
...
#include "PnlWznmMchAPar_blks.cpp"
#include "PnlWznmMchAPar_evals.cpp"
...
/******************************************************************************
 class PnlWznmMchAPar
 ******************************************************************************/

PnlWznmMchAPar::PnlWznmMchAPar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMMCHAPAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmMchAPar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmMchAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmMchAPar::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmMchAPar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCHAPARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMMCHAPARDO) {
...
void PnlWznmMchAPar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmMchAPar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMchAPar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmMchAPar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQMchAPar* recSelNew = NULL;

	WznmAMMachinePar* _recMchApar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmMchAPar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmMchAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmMchAPar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recMchApar = WznmAMMachinePar();

			if (recSelNew) {
				if (dbswznm->tblwznmammachinepar->loadRecByRef(recSelNew->ref, &_recMchApar)) {
...
void PnlWznmMchAPar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMchAPar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMchAPar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmMchAPar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmMchAPar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmMchAPar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
