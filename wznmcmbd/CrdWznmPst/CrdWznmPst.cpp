/**
	* \file CrdWznmPst.cpp
	* job handler for job CrdWznmPst (implementation)
...
CrdWznmPst::CrdWznmPst(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMPST, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFPST, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmPstList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmPstHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmPstRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmPst::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmPst::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmPst/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdPst = StubWznm::getStubPstStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFPST, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmPst::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmPst", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFPST, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFPST, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFPST, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmPst::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmPst::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMPSTDO) {
...
void CrdWznmPst::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmPst::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMPST);
};

void CrdWznmPst::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmPst::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmPst::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFPST) {
...
bool CrdWznmPst::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmPst::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmPst::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmPst::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmPst::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
