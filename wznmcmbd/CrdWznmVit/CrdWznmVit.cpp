/**
	* \file CrdWznmVit.cpp
	* job handler for job CrdWznmVit (implementation)
...
CrdWznmVit::CrdWznmVit(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMVIT, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFVIT, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmVitRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmVitHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmVitList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmVit::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmVit::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmVit/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdVit = StubWznm::getStubVitStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFVIT, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmVit::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmVit", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFVIT, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFVIT, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFVIT, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmVit::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmVit::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVITDO) {
...
void CrdWznmVit::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmVit::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMVIT);
};

void CrdWznmVit::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmVit::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmVit::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFVIT) {
...
bool CrdWznmVit::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmVit::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmVit::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmVit::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmVit::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
