/**
	* \file CrdWznmVec.cpp
	* job handler for job CrdWznmVec (implementation)
...
CrdWznmVec::CrdWznmVec(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMVEC, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFVEC, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmVecRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmVecHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmVecList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmVec::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmVec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmVec/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdVec = StubWznm::getStubVecStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFVEC, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmVec::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmVec", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFVEC, jref, ref);
	...
	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFVEC, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFVEC, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmVec::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmVec::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVECDO) {
...
void CrdWznmVec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmVec::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMVEC);
};

void CrdWznmVec::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmVec::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmVec::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFVEC) {
...
bool CrdWznmVec::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmVec::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmVec::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmVec::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmVec::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
