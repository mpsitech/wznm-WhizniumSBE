/**
	...
	* job handler for job PnlWznmVecVec1NVectoritem (implementation)
...
#include "PnlWznmVecVec1NVectoritem_blks.cpp"
#include "PnlWznmVecVec1NVectoritem_evals.cpp"
...
/******************************************************************************
 class PnlWznmVecVec1NVectoritem
 ******************************************************************************/

PnlWznmVecVec1NVectoritem::PnlWznmVecVec1NVectoritem(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVECVEC1NVECTORITEM, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVecVec1NVectoritem(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVecVec1NVectoritem::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVecVec1NVectoritem::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVecVec1NVectoritem::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVecVec1NVectoritem::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVECVEC1NVECTORITEMDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVECVEC1NVECTORITEMDO) {
...
void PnlWznmVecVec1NVectoritem::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVecVec1NVectoritem::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVecVec1NVectoritem* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVecVec1NVectoritem::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVecVec1NVectoritem::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVecVec1NVectoritem::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recVit = WznmMVectoritem();

			if (recSelNew) {
				if (dbswznm->tblwznmmvectoritem->loadRecByRef(recSelNew->ref, &_recVit)) {
					recVit = *_recVit;
					delete _recVit;
				};
			};
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refVec = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFVEC, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVIT, jref)) if (refVec != 0) {
			sref = "CrdWznmVit";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVEC, refVec, sref, recVit.ref, jrefNew);
		};
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVIT, jref) & VecWznmWAccess::EDIT) != 0) if (refVec != 0) {
			sref = "CrdWznmVit";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVEC, refVec, sref, -1, jrefNew);
		};
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecVec1NVectoritem::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVecVec1NVectoritem::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVecVec1NVectoritem::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
