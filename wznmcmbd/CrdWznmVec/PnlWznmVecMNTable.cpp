/**
	* \file PnlWznmVecMNTable.cpp
	* job handler for job PnlWznmVecMNTable (implementation)
...
#include "PnlWznmVecMNTable_blks.cpp"
#include "PnlWznmVecMNTable_evals.cpp"
...
/******************************************************************************
 class PnlWznmVecMNTable
 ******************************************************************************/

PnlWznmVecMNTable::PnlWznmVecMNTable(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMVECMNTABLE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmVecMNTable(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmVecMNTable::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmVecMNTable::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmVecMNTable::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmVecMNTable::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVECMNTABLEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMVECMNTABLEDO) {
...
void PnlWznmVecMNTable::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmVecMNTable::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecMNTable::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmVecMNTable::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQVecMNTable* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmVecMNTable::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmVecMNTable::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmVecMNTable::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmtable->loadRecByRef(recSelNew->mref, &_recTbl)) {
					recTbl = *_recTbl;
...
void PnlWznmVecMNTable::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecMNTable::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmVecMNTable::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmVecMNTable::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmVecMNTable::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
