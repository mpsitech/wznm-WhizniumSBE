/**
	* \file WznmPrcfileIexconv.cpp
	* Wznm operation processor - convert text import input file into output file (implementation)
...
using namespace WznmPrcfile;

/******************************************************************************
 namespace WznmPrcfileIexconv
 ******************************************************************************/

DpchRetWznm* WznmPrcfileIexconv::run(
			XchgWznm* xchg
			, DbsWznm* dbswznm
...
// IP cust --- IBEGIN
void WznmPrcfileIexconv::runIconv(
			const string& infile
			, const string& outfile
		) {
	Wznm::run("iconv -f MACINTOSH -t ISO-8859-1 " +  infile + " > " + outfile);
};

void WznmPrcfileIexconv::convert(
			ifstream& txtfile
			, ofstream& iexfile
		) {
	char* buf = NULL;
...
string WznmPrcfileIexconv::unicodeToHexcode(
...
void WznmPrcfileIexconv::strISO8859(
			string& s
		) {
...
void WznmPrcfileIexconv::strRequote(
			string& s
		) {
	size_t ptr;
