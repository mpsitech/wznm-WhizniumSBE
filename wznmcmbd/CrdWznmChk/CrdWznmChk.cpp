/**
	* \file CrdWznmChk.cpp
	* job handler for job CrdWznmChk (implementation)
...
CrdWznmChk::CrdWznmChk(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMCHK, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCHK, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmChkList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmChkHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmChkRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmChk::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmChk::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmChk/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdChk = StubWznm::getStubChkStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCHK, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmChk::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmChk", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCHK, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCHK, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCHK, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmChk::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmChk::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCHKDO) {
...
void CrdWznmChk::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmChk::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMCHK);
};

void CrdWznmChk::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmChk::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmChk::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFCHK) {
...
bool CrdWznmChk::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmChk::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmChk::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmChk::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmChk::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
