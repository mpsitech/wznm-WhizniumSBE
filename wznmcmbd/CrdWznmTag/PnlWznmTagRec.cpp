...
#include "PnlWznmTagRec_blks.cpp"
#include "PnlWznmTagRec_evals.cpp"
...
PnlWznmTagRec::PnlWznmTagRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTAGREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmTagRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTagRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubTagStd(dbswznm, recTag.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recTag.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmTagDetail(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmTagRec::updatePreset(
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFTAG) {
		...
		if (dbswznm->tblwznmmtag->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFTAG, jref), &_recTag)) {
			recTag = *_recTag;
			delete _recTag;
...
void PnlWznmTagRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTAGRECDO) {
...
void PnlWznmTagRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTagRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTagRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTagRec::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMTAG_CPBEQ) {
...
bool PnlWznmTagRec::handleCallWznmTag_cpbEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmTagRec::handleCallWznmTagUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
