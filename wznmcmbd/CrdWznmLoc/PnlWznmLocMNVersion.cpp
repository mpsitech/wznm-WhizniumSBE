/**
	...
	* job handler for job PnlWznmLocMNVersion (implementation)
...
#include "PnlWznmLocMNVersion_blks.cpp"
#include "PnlWznmLocMNVersion_evals.cpp"
...
/******************************************************************************
 class PnlWznmLocMNVersion
 ******************************************************************************/

PnlWznmLocMNVersion::PnlWznmLocMNVersion(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMLOCMNVERSION, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmLocMNVersion(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmLocMNVersion::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmLocMNVersion::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmLocMNVersion::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFLOC) {
		recLocRver = WznmRMLocaleMVersion();
...
void PnlWznmLocMNVersion::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLOCMNVERSIONDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLOCMNVERSIONDO) {
...
void PnlWznmLocMNVersion::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmLocMNVersion::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLocMNVersion::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmLocMNVersion::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQLocMNVersion* recSelNew = NULL;

	WznmRMLocaleMVersion* _recLocRver = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmLocMNVersion::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmLocMNVersion::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmLocMNVersion::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmlocalemversion->loadRecByRef(recSelNew->ref, &_recLocRver)) {
					recLocRver = *_recLocRver;
					delete _recLocRver;
				};
				if (dbswznm->tblwznmmversion->loadRecByRef(recSelNew->mref, &_recVer)) {
					recVer = *_recVer;
					delete _recVer;
				};
			};
...
void PnlWznmLocMNVersion::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref)) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recVer.ref, jrefNew);
		};
...
void PnlWznmLocMNVersion::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLocMNVersion::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLocMNVersion::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmLocMNVersion::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmLocMNVersion::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
