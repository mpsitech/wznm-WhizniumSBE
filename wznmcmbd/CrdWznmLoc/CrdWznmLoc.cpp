/**
	* \file CrdWznmLoc.cpp
	* job handler for job CrdWznmLoc (implementation)
...
CrdWznmLoc::CrdWznmLoc(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMLOC, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFLOC, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmLocList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmLocHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmLocRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmLoc::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmLoc::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmLoc/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdLoc = StubWznm::getStubLocStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFLOC, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmLoc::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, VecWznmVPreset::VOID, 0, "CrdWznmLoc", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFLOC, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFLOC, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFLOC, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmLoc::updatePreset(
			DbsWznm* dbswznm
...
void CrdWznmLoc::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLOCDO) {
...
void CrdWznmLoc::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmLoc::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMLOC);
};

void CrdWznmLoc::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmLoc::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmLoc::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFLOC) {
...
bool CrdWznmLoc::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmLoc::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmLoc::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmLoc::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmLoc::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
