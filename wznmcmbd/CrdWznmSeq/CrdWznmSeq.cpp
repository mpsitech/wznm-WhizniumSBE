/**
	* \file CrdWznmSeq.cpp
	* job handler for job CrdWznmSeq (implementation)
...
CrdWznmSeq::CrdWznmSeq(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMSEQ, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFSEQ, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmSeqList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmSeqHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmSeqRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmSeq::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmSeq::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmSeq/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdSeq = StubWznm::getStubSeqStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSEQ, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmSeq::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmSeq", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSEQ, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSEQ, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSEQ, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmSeq::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmSeq::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSEQDO) {
...
void CrdWznmSeq::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmSeq::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMSEQ);
};

void CrdWznmSeq::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmSeq::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmSeq::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFSEQ) {
...
bool CrdWznmSeq::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmSeq::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmSeq::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmSeq::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmSeq::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
