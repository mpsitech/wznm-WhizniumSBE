/**
	...
	* job handler for job PnlWznmSeqSeq1NState (implementation)
...
#include "PnlWznmSeqSeq1NState_blks.cpp"
#include "PnlWznmSeqSeq1NState_evals.cpp"
...
/******************************************************************************
 class PnlWznmSeqSeq1NState
 ******************************************************************************/

PnlWznmSeqSeq1NState::PnlWznmSeqSeq1NState(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMSEQSEQ1NSTATE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmSeqSeq1NState(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmSeqSeq1NState::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmSeqSeq1NState::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmSeqSeq1NState::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmSeqSeq1NState::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSEQSEQ1NSTATEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSEQSEQ1NSTATEDO) {
...
void PnlWznmSeqSeq1NState::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmSeqSeq1NState::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSeqSeq1NState::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmSeqSeq1NState::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQSeqSeq1NState* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmSeqSeq1NState::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmSeqSeq1NState::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmSeqSeq1NState::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSte = WznmMState();

			if (recSelNew) {
				if (dbswznm->tblwznmmstate->loadRecByRef(recSelNew->ref, &_recSte)) {
					recSte = *_recSte;
					delete _recSte;
				};
			};
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSTE, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFAPP) {
			sref = "CrdWznmSte";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recSte.ref, jrefNew);
		};
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSTE, jref) & VecWznmWAccess::EDIT) != 0) if (ixPre == VecWznmVPreset::PREWZNMREFAPP) {
			sref = "CrdWznmSte";
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmSeqSeq1NState::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmSeqSeq1NState::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmSeqSeq1NState::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
