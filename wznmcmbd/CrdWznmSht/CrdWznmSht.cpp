/**
	* \file CrdWznmSht.cpp
	* job handler for job CrdWznmSht (implementation)
...
CrdWznmSht::CrdWznmSht(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMSHT, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFSHT, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmShtList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmShtHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmShtRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmSht::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmSht::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmSht/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdSht = StubWznm::getStubShtStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFSHT, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmSht::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmSht", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFSHT, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSHT, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFSHT, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmSht::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmSht::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMSHTDO) {
...
void CrdWznmSht::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmSht::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMSHT);
};

void CrdWznmSht::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmSht::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmSht::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFSHT) {
...
bool CrdWznmSht::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmSht::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmSht::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmSht::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmSht::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
