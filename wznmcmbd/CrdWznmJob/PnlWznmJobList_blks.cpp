/**
	* \file PnlWznmJobList_blks.cpp
	* job handler for job PnlWznmJobList (implementation of blocks)
...
bool PnlWznmJobList::ContIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["ContIacWznmJobList"];}();
...
void PnlWznmJobList::ContIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWznmJobList::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmJobList";
...
void PnlWznmJobList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfWznmJobList";
...
void PnlWznmJobList::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmJobList";
...
void PnlWznmJobList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "StatitemShrWznmJobList";
...
bool PnlWznmJobList::StgIac::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWznmJobList"];}();
	...
	if (basefound) {
		...
		if (me.isMember("TcoGblWidth")) {TcoGblWidth = me["TcoGblWidth"].asUInt(); add(TCOGBLWIDTH);};
		if (me.isMember("TcoClsWidth")) {TcoClsWidth = me["TcoClsWidth"].asUInt(); add(TCOCLSWIDTH);};
...
void PnlWznmJobList::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	...
	me["TcoGblWidth"] = TcoGblWidth;
	me["TcoClsWidth"] = TcoClsWidth;
...
void PnlWznmJobList::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmJobList";
...
void PnlWznmJobList::Tag::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWznmJobList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoGbl", "Global");
			writeStringAttr(wr, itemtag, "sref", "TcoCls", "Client/server");
...
PnlWznmJobList::DpchAppData::DpchAppData() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMJOBLISTDATA)
		{
};
...
void PnlWznmJobList::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmJobListData"];}();
...
void PnlWznmJobList::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmJobListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmJobList::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMJOBLISTDO)
		{
	ixVDo = 0;
};
...
void PnlWznmJobList::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmJobListDo"];}();
...
void PnlWznmJobList::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmJobListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmJobList::DpchEngData::DpchEngData(
			...
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMJOBLISTDATA, jref)
		{
...
void PnlWznmJobList::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void PnlWznmJobList::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmJobListData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWznmJobList::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWznmJobList::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmJobListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		...
		if (has(STATAPPQRY)) QryWznmJobList::StatApp::writeXML(wr);
