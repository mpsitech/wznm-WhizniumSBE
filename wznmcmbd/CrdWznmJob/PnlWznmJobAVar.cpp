/**
	...
	* job handler for job PnlWznmJobAVar (implementation)
...
#include "PnlWznmJobAVar_blks.cpp"
#include "PnlWznmJobAVar_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobAVar
 ******************************************************************************/

PnlWznmJobAVar::PnlWznmJobAVar(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBAVAR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobAVar(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobAVar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobAVar::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobAVar::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBAVARDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBAVARDO) {
...
void PnlWznmJobAVar::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobAVar::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobAVar::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobAVar* recSelNew = NULL;

	WznmAMJobVar* _recJobAvar = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobAVar::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobAVar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobAVar::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recJobAvar = WznmAMJobVar();

			if (recSelNew) {
				if (dbswznm->tblwznmamjobvar->loadRecByRef(recSelNew->ref, &_recJobAvar)) {
					recJobAvar = *_recJobAvar;
					delete _recJobAvar;
				};
			};
...
void PnlWznmJobAVar::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobAVar::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobAVar::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobAVar::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
