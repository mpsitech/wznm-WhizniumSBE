/**
	...
	* job handler for job PnlWznmJobMNOppack (implementation)
...
#include "PnlWznmJobMNOppack_blks.cpp"
#include "PnlWznmJobMNOppack_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobMNOppack
 ******************************************************************************/

PnlWznmJobMNOppack::PnlWznmJobMNOppack(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBMNOPPACK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobMNOppack(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobMNOppack::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobMNOppack::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmJobMNOppack::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobMNOppack::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBMNOPPACKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBMNOPPACKDO) {
...
void PnlWznmJobMNOppack::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobMNOppack::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOppack::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobMNOppack::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobMNOppack* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobMNOppack::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobMNOppack::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobMNOppack::StgIac::JNUM)) {
...
void PnlWznmJobMNOppack::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmJobMNOppack::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOppack::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOppack::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobMNOppack::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobMNOppack::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
