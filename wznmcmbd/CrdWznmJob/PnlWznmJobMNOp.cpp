/**
	...
	* job handler for job PnlWznmJobMNOp (implementation)
...
#include "PnlWznmJobMNOp_blks.cpp"
#include "PnlWznmJobMNOp_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobMNOp
 ******************************************************************************/

PnlWznmJobMNOp::PnlWznmJobMNOp(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBMNOP, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobMNOp(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobMNOp::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobMNOp::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobMNOp::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBMNOPDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBMNOPDO) {
...
void PnlWznmJobMNOp::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobMNOp::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOp::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobMNOp::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobMNOp* recSelNew = NULL;

	WznmRMJobMOp* _recJobRopx = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobMNOp::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobMNOp::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobMNOp::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmjobmop->loadRecByRef(recSelNew->ref, &_recJobRopx)) {
					recJobRopx = *_recJobRopx;
					delete _recJobRopx;
				};
				if (dbswznm->tblwznmmop->loadRecByRef(recSelNew->mref, &_recOpx)) {
					recOpx = *_recOpx;
					delete _recOpx;
				};
			};
...
void PnlWznmJobMNOp::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPX, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmOpx";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recOpx.ref, jrefNew);
		};
...
void PnlWznmJobMNOp::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOp::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobMNOp::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobMNOp::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobMNOp::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
