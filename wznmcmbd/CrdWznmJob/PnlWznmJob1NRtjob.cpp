/**
	...
	* job handler for job PnlWznmJob1NRtjob (implementation)
...
#include "PnlWznmJob1NRtjob_blks.cpp"
#include "PnlWznmJob1NRtjob_evals.cpp"
...
/******************************************************************************
 class PnlWznmJob1NRtjob
 ******************************************************************************/

PnlWznmJob1NRtjob::PnlWznmJob1NRtjob(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOB1NRTJOB, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJob1NRtjob(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJob1NRtjob::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJob1NRtjob::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmJob1NRtjob::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJob1NRtjob::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOB1NRTJOBDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOB1NRTJOBDO) {
...
void PnlWznmJob1NRtjob::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJob1NRtjob::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJob1NRtjob::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJob1NRtjob::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJob1NRtjob* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJob1NRtjob::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJob1NRtjob::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJob1NRtjob::StgIac::JNUM)) {
...
void PnlWznmJob1NRtjob::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmJob1NRtjob::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJob1NRtjob::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJob1NRtjob::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
