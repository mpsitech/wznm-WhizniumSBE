/**
	...
	* job handler for job PnlWznmJobHk1NVector (implementation)
...
#include "PnlWznmJobHk1NVector_blks.cpp"
#include "PnlWznmJobHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobHk1NVector
 ******************************************************************************/

PnlWznmJobHk1NVector::PnlWznmJobHk1NVector(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBHK1NVECTOR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobHk1NVector(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobHk1NVector::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmJobHk1NVector::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobHk1NVector::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBHK1NVECTORDO) {
...
void PnlWznmJobHk1NVector::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobHk1NVector::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobHk1NVector::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobHk1NVector::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobHk1NVector::StgIac::JNUM)) {
...
void PnlWznmJobHk1NVector::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVEC, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmVec";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recVec.ref, jrefNew);
		};
...
void PnlWznmJobHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobHk1NVector::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobHk1NVector::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
