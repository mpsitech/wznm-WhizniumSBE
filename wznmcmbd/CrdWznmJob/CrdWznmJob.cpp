/**
	* \file CrdWznmJob.cpp
	* job handler for job CrdWznmJob (implementation)
...
CrdWznmJob::CrdWznmJob(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMJOB, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFJOB, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmJobList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmJobHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmJobRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmJob::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmJob::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmJob/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdJob = StubWznm::getStubJobStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFJOB, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmJob::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmJob", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFJOB, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFJOB, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFJOB, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmJob::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmJob::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBDO) {
...
void CrdWznmJob::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmJob::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMJOB);
};

void CrdWznmJob::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmJob::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmJob::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFJOB) {
...
bool CrdWznmJob::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmJob::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmJob::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmJob::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmJob::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
