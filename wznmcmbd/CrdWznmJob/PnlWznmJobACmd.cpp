/**
	...
	* job handler for job PnlWznmJobACmd (implementation)
...
#include "PnlWznmJobACmd_blks.cpp"
#include "PnlWznmJobACmd_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobACmd
 ******************************************************************************/

PnlWznmJobACmd::PnlWznmJobACmd(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBACMD, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobACmd(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobACmd::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobACmd::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobACmd::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBACMDDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBACMDDO) {
...
void PnlWznmJobACmd::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobACmd::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobACmd::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobACmd::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobACmd* recSelNew = NULL;

	WznmAMJobCmd* _recJobAcmd = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobACmd::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobACmd::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobACmd::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recJobAcmd = WznmAMJobCmd();

			if (recSelNew) {
				if (dbswznm->tblwznmamjobcmd->loadRecByRef(recSelNew->ref, &_recJobAcmd)) {
					recJobAcmd = *_recJobAcmd;
					delete _recJobAcmd;
				};
			};
...
void PnlWznmJobACmd::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobACmd::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobACmd::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobACmd::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobACmd::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobACmd::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
