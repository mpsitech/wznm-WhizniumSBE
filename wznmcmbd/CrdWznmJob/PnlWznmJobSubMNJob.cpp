/**
	...
	* job handler for job PnlWznmJobSubMNJob (implementation)
...
#include "PnlWznmJobSubMNJob_blks.cpp"
#include "PnlWznmJobSubMNJob_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobSubMNJob
 ******************************************************************************/

PnlWznmJobSubMNJob::PnlWznmJobSubMNJob(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBSUBMNJOB, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobSubMNJob(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobSubMNJob::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobSubMNJob::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmJobSubMNJob::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobSubMNJob::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBSUBMNJOBDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBSUBMNJOBDO) {
...
void PnlWznmJobSubMNJob::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobSubMNJob::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobSubMNJob::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobSubMNJob::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobSubMNJob* recSelNew = NULL;

	WznmRMJobMJob* _recJobRjob = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobSubMNJob::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobSubMNJob::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobSubMNJob::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmjobmjob->loadRecByRef(recSelNew->ref, &_recJobRjob)) {
					recJobRjob = *_recJobRjob;
					delete _recJobRjob;
				};
...
void PnlWznmJobSubMNJob::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmJobSubMNJob::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobSubMNJob::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobSubMNJob::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobSubMNJob::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobSubMNJob::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
