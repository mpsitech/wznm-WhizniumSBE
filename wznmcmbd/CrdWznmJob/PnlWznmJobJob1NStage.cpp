/**
	...
	* job handler for job PnlWznmJobJob1NStage (implementation)
...
#include "PnlWznmJobJob1NStage_blks.cpp"
#include "PnlWznmJobJob1NStage_evals.cpp"
...
/******************************************************************************
 class PnlWznmJobJob1NStage
 ******************************************************************************/

PnlWznmJobJob1NStage::PnlWznmJobJob1NStage(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMJOBJOB1NSTAGE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmJobJob1NStage(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmJobJob1NStage::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmJobJob1NStage::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmJobJob1NStage::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmJobJob1NStage::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBJOB1NSTAGEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMJOBJOB1NSTAGEDO) {
...
void PnlWznmJobJob1NStage::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmJobJob1NStage::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobJob1NStage::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmJobJob1NStage::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQJobJob1NStage* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmJobJob1NStage::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmJobJob1NStage::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmJobJob1NStage::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recSge = WznmMStage();

			if (recSelNew) {
				if (dbswznm->tblwznmmstage->loadRecByRef(recSelNew->ref, &_recSge)) {
					recSge = *_recSge;
					delete _recSge;
				};
			};
...
void PnlWznmJobJob1NStage::handleDpchAppDoButUpClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobJob1NStage::handleDpchAppDoButDownClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobJob1NStage::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	ubigint refJob = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFJOB, jref);

	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSGE, jref)) if (refJob != 0) {
			sref = "CrdWznmSge";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFJOB, refJob, sref, recSge.ref, jrefNew);
		};
...
void PnlWznmJobJob1NStage::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCSGE, jref) & VecWznmWAccess::EDIT) != 0) if (refJob != 0) {
			sref = "CrdWznmSge";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFJOB, refJob, sref, -1, jrefNew);
		};
...
void PnlWznmJobJob1NStage::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmJobJob1NStage::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmJobJob1NStage::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmJobJob1NStage::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
