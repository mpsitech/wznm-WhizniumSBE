/**
	* \file CrdWznmIex.cpp
	* job handler for job CrdWznmIex (implementation)
...
CrdWznmIex::CrdWznmIex(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMIEX, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFIEX, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmIexList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmIexHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmIexRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmIex::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmIex::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmIex/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdIex = StubWznm::getStubIexStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFIEX, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmIex::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmIex", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFIEX, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIEX, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFIEX, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmIex::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmIex::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIEXDO) {
...
void CrdWznmIex::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmIex::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMIEX);
};

void CrdWznmIex::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmIex::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmIex::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFIEX) {
...
bool CrdWznmIex::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmIex::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmIex::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmIex::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmIex::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
