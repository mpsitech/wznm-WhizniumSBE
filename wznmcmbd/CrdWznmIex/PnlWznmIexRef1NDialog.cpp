/**
	* \file PnlWznmIexRef1NDialog.cpp
	* job handler for job PnlWznmIexRef1NDialog (implementation)
...
#include "PnlWznmIexRef1NDialog_blks.cpp"
#include "PnlWznmIexRef1NDialog_evals.cpp"
...
/******************************************************************************
 class PnlWznmIexRef1NDialog
 ******************************************************************************/

PnlWznmIexRef1NDialog::PnlWznmIexRef1NDialog(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMIEXREF1NDIALOG, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmIexRef1NDialog(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmIexRef1NDialog::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmIexRef1NDialog::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmIexRef1NDialog::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmIexRef1NDialog::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIEXREF1NDIALOGDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIEXREF1NDIALOGDO) {
...
void PnlWznmIexRef1NDialog::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmIexRef1NDialog::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmIexRef1NDialog::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmIexRef1NDialog::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQIexRef1NDialog* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmIexRef1NDialog::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmIexRef1NDialog::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmIexRef1NDialog::StgIac::JNUM)) {
...
void PnlWznmIexRef1NDialog::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmIexRef1NDialog::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmIexRef1NDialog::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmIexRef1NDialog::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
