/**
	...
	* job handler for job PnlWznmIexHk1NVector (implementation)
...
#include "PnlWznmIexHk1NVector_blks.cpp"
#include "PnlWznmIexHk1NVector_evals.cpp"
...
/******************************************************************************
 class PnlWznmIexHk1NVector
 ******************************************************************************/

PnlWznmIexHk1NVector::PnlWznmIexHk1NVector(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMIEXHK1NVECTOR, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmIexHk1NVector(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmIexHk1NVector::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmIexHk1NVector::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmIexHk1NVector::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmIexHk1NVector::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIEXHK1NVECTORDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMIEXHK1NVECTORDO) {
...
void PnlWznmIexHk1NVector::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmIexHk1NVector::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmIexHk1NVector::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmIexHk1NVector::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQIexHk1NVector* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmIexHk1NVector::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmIexHk1NVector::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmIexHk1NVector::StgIac::JNUM)) {
...
void PnlWznmIexHk1NVector::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlWznmIexHk1NVector::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmIexHk1NVector::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmIexHk1NVector::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
