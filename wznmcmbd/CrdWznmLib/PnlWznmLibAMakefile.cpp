/**
	...
	* job handler for job PnlWznmLibAMakefile (implementation)
...
#include "PnlWznmLibAMakefile_blks.cpp"
#include "PnlWznmLibAMakefile_evals.cpp"
...
/******************************************************************************
 class PnlWznmLibAMakefile
 ******************************************************************************/

PnlWznmLibAMakefile::PnlWznmLibAMakefile(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMLIBAMAKEFILE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmLibAMakefile(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmLibAMakefile::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmLibAMakefile::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmLibAMakefile::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFLIB) {
		recLibAmkf = WznmAMLibraryMakefile();
...
void PnlWznmLibAMakefile::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBAMAKEFILEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBAMAKEFILEDO) {
...
void PnlWznmLibAMakefile::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmLibAMakefile::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAMakefile::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmLibAMakefile::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQLibAMakefile* recSelNew = NULL;

	WznmAMLibraryMakefile* _recLibAmkf = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmLibAMakefile::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmLibAMakefile::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmLibAMakefile::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recLibAmkf = WznmAMLibraryMakefile();

			if (recSelNew) {
				if (dbswznm->tblwznmamlibrarymakefile->loadRecByRef(recSelNew->ref, &_recLibAmkf)) {
...
void PnlWznmLibAMakefile::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAMakefile::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAMakefile::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAMakefile::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmLibAMakefile::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmLibAMakefile::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
