/**
	* \file PnlWznmLibMNComponent.cpp
	* job handler for job PnlWznmLibMNComponent (implementation)
...
#include "PnlWznmLibMNComponent_blks.cpp"
#include "PnlWznmLibMNComponent_evals.cpp"
...
/******************************************************************************
 class PnlWznmLibMNComponent
 ******************************************************************************/

PnlWznmLibMNComponent::PnlWznmLibMNComponent(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMLIBMNCOMPONENT, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmLibMNComponent(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmLibMNComponent::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmLibMNComponent::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmLibMNComponent::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmLibMNComponent::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBMNCOMPONENTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBMNCOMPONENTDO) {
...
void PnlWznmLibMNComponent::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmLibMNComponent::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNComponent::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmLibMNComponent::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQLibMNComponent* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmLibMNComponent::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmLibMNComponent::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmLibMNComponent::StgIac::JNUM)) {
...
void PnlWznmLibMNComponent::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNComponent::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNComponent::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmLibMNComponent::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmLibMNComponent::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
