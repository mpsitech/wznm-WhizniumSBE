...
CrdWznmLib::CrdWznmLib(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMLIB, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFLIB, ref);
...
DpchEngWznm* CrdWznmLib::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmLib::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmLib/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdLib = StubWznm::getStubLibStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFLIB, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmLib::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, VecWznmVPreset::VOID, 0, "CrdWznmLib", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFLIB, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFLIB, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFLIB, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmLib::updatePreset(
			DbsWznm* dbswznm
...
void CrdWznmLib::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBDO) {
...
void CrdWznmLib::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmLib::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMLIB);
};

void CrdWznmLib::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmLib::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmLib::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFLIB) {
...
bool CrdWznmLib::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmLib::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmLib::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmLib::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmLib::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
