/**
	...
	* job handler for job PnlWznmLibAPkglist (implementation)
...
#include "PnlWznmLibAPkglist_blks.cpp"
#include "PnlWznmLibAPkglist_evals.cpp"
...
/******************************************************************************
 class PnlWznmLibAPkglist
 ******************************************************************************/

PnlWznmLibAPkglist::PnlWznmLibAPkglist(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMLIBAPKGLIST, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmLibAPkglist(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmLibAPkglist::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmLibAPkglist::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmLibAPkglist::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFLIB) {
		recLibApkl = WznmAMLibraryPkglist();
...
void PnlWznmLibAPkglist::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBAPKGLISTDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBAPKGLISTDO) {
...
void PnlWznmLibAPkglist::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmLibAPkglist::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAPkglist::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmLibAPkglist::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQLibAPkglist* recSelNew = NULL;

	WznmAMLibraryPkglist* _recLibApkl = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmLibAPkglist::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmLibAPkglist::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmLibAPkglist::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recLibApkl = WznmAMLibraryPkglist();

			if (recSelNew) {
				if (dbswznm->tblwznmamlibrarypkglist->loadRecByRef(recSelNew->ref, &_recLibApkl)) {
					recLibApkl = *_recLibApkl;
					delete _recLibApkl;
				};
			};
...
void PnlWznmLibAPkglist::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAPkglist::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAPkglist::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibAPkglist::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmLibAPkglist::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmLibAPkglist::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
