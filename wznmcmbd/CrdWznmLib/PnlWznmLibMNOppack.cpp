/**
	...
	* job handler for job PnlWznmLibMNOppack (implementation)
...
#include "PnlWznmLibMNOppack_blks.cpp"
#include "PnlWznmLibMNOppack_evals.cpp"
...
/******************************************************************************
 class PnlWznmLibMNOppack
 ******************************************************************************/

PnlWznmLibMNOppack::PnlWznmLibMNOppack(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMLIBMNOPPACK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmLibMNOppack(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmLibMNOppack::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmLibMNOppack::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmLibMNOppack::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmLibMNOppack::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBMNOPPACKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMLIBMNOPPACKDO) {
...
void PnlWznmLibMNOppack::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmLibMNOppack::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNOppack::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmLibMNOppack::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQLibMNOppack* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmLibMNOppack::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmLibMNOppack::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmLibMNOppack::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbswznm->tblwznmmoppack->loadRecByRef(recSelNew->mref, &_recOpk)) {
					recOpk = *_recOpk;
					delete _recOpk;
				};
			};
...
void PnlWznmLibMNOppack::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPK, jref)) if (refVer != 0) {
			sref = "CrdWznmOpk";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, VecWznmVPreset::PREWZNMREFVER, refVer, sref, recOpk.ref, jrefNew);
		};
...
void PnlWznmLibMNOppack::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNOppack::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmLibMNOppack::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmLibMNOppack::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmLibMNOppack::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
