/**
	* \file PnlWznmLibMNOppack_blks.cpp
	* job handler for job PnlWznmLibMNOppack (implementation of blocks)
...
void PnlWznmLibMNOppack::ContInf::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "ContInfWznmLibMNOppack";
...
void PnlWznmLibMNOppack::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	else itemtag = "ContitemInfWznmLibMNOppack";
...
void PnlWznmLibMNOppack::StatApp::writeJSON(
			Json::Value& sup
			, string difftag
			, const uint ixWznmVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppWznmLibMNOppack";
...
void PnlWznmLibMNOppack::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixWznmVExpstate
	...
	else itemtag = "StatitemAppWznmLibMNOppack";
...
void PnlWznmLibMNOppack::StatShr::writeJSON(
			Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "StatShrWznmLibMNOppack";
...
void PnlWznmLibMNOppack::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	...
	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrWznmLibMNOppack";
...
bool PnlWznmLibMNOppack::StgIac::readJSON(
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["StgIacWznmLibMNOppack"];}();
...
void PnlWznmLibMNOppack::StgIac::writeJSON(
			Json::Value& sup
			, string difftag
		) {
...
void PnlWznmLibMNOppack::Tag::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
			, string difftag
		) {
	if (difftag.length() == 0) difftag = "TagWznmLibMNOppack";

	Json::Value& me = sup[difftag] = Json::Value(Json::objectValue);

	if (ixWznmVLocale == VecWznmVLocale::ENUS) {
		me["Cpt"] = "Dependent operation packs";
		me["TcoMref"] = "Op. pack";
	};
...
void PnlWznmLibMNOppack::Tag::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	else itemtag = "TagitemWznmLibMNOppack";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixWznmVLocale == VecWznmVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Dependent operation packs");
...
PnlWznmLibMNOppack::DpchAppData::DpchAppData() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMLIBMNOPPACKDATA)
		{
};
...
void PnlWznmLibMNOppack::DpchAppData::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmLibMNOppackData"];}();
...
void PnlWznmLibMNOppack::DpchAppData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmLibMNOppackData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmLibMNOppack::DpchAppDo::DpchAppDo() :
			DpchAppWznm(VecWznmVDpch::DPCHAPPWZNMLIBMNOPPACKDO)
		{
	ixVDo = 0;
};
...
void PnlWznmLibMNOppack::DpchAppDo::readJSON(
			const Json::Value& sup
			, bool addbasetag
		) {
	clear();
	...
	const Json::Value& me = [&]{if (!addbasetag) return sup; return sup["DpchAppWznmLibMNOppackDo"];}();
...
void PnlWznmLibMNOppack::DpchAppDo::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppWznmLibMNOppackDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlWznmLibMNOppack::DpchEngData::DpchEngData(
			...
			, QryWznmLibMNOppack::StatShr* statshrqry
			, QryWznmLibMNOppack::StgIac* stgiacqry
			, const set<uint>& mask
		) :
			DpchEngWznm(VecWznmVDpch::DPCHENGWZNMLIBMNOPPACKDATA, jref)
		{
...
void PnlWznmLibMNOppack::DpchEngData::merge(
			DpchEngWznm* dpcheng
		) {
...
void PnlWznmLibMNOppack::DpchEngData::writeJSON(
			const uint ixWznmVLocale
			, Json::Value& sup
		) {
	Json::Value& me = sup["DpchEngWznmLibMNOppackData"] = Json::Value(Json::objectValue);
	...
	if (has(STATAPPQRY)) QryWznmLibMNOppack::StatApp::writeJSON(me);
	if (has(STATSHRQRY)) statshrqry.writeJSON(me);
	if (has(STGIACQRY)) stgiacqry.writeJSON(me);
};

void PnlWznmLibMNOppack::DpchEngData::writeXML(
			const uint ixWznmVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngWznmLibMNOppackData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/wznm");
		...
		if (has(STATAPPQRY)) QryWznmLibMNOppack::StatApp::writeXML(wr);
