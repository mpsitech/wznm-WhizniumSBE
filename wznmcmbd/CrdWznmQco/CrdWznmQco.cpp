/**
	* \file CrdWznmQco.cpp
	* job handler for job CrdWznmQco (implementation)
...
CrdWznmQco::CrdWznmQco(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMQCO, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFQCO, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmQcoList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmQcoHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmQcoRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmQco::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmQco::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmQco/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdQco = StubWznm::getStubQcoStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFQCO, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmQco::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmQco", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFQCO, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFQCO, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFQCO, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmQco::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmQco::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMQCODO) {
...
void CrdWznmQco::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmQco::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMQCO);
};

void CrdWznmQco::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmQco::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmQco::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFQCO) {
...
bool CrdWznmQco::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmQco::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmQco::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmQco::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmQco::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
