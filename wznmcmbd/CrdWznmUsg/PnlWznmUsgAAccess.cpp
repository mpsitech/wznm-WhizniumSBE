/**
	* \file PnlWznmUsgAAccess.cpp
	* job handler for job PnlWznmUsgAAccess (implementation)
...
#include "PnlWznmUsgAAccess_blks.cpp"
#include "PnlWznmUsgAAccess_evals.cpp"
...
/******************************************************************************
 class PnlWznmUsgAAccess
 ******************************************************************************/

PnlWznmUsgAAccess::PnlWznmUsgAAccess(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMUSGAACCESS, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmUsgAAccess(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmUsgAAccess::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmUsgAAccess::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmUsgAAccess::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFUSG) {
		recUsgAacc = WznmAMUsergroupAccess();
...
void PnlWznmUsgAAccess::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSGAACCESSDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSGAACCESSDO) {
...
void PnlWznmUsgAAccess::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmUsgAAccess::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsgAAccess::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmUsgAAccess::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQUsgAAccess* recSelNew = NULL;

	WznmAMUsergroupAccess* _recUsgAacc = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmUsgAAccess::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmUsgAAccess::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmUsgAAccess::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recUsgAacc = WznmAMUsergroupAccess();

			if (recSelNew) {
				if (dbswznm->tblwznmamusergroupaccess->loadRecByRef(recSelNew->ref, &_recUsgAacc)) {
...
void PnlWznmUsgAAccess::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsgAAccess::handleDpchAppDoButDuplicateClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsgAAccess::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsgAAccess::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmUsgAAccess::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmUsgAAccess::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
