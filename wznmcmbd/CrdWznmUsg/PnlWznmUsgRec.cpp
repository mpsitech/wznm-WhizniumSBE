...
#include "PnlWznmUsgRec_blks.cpp"
#include "PnlWznmUsgRec_evals.cpp"
...
PnlWznmUsgRec::PnlWznmUsgRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMUSGREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmUsgRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmUsgRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubUsgStd(dbswznm, recUsg.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recUsg.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmUsgDetail(xchg, dbswznm, jref, ixWznmVLocale);
		if (!pnlaaccess) pnlaaccess = new PnlWznmUsgAAccess(xchg, dbswznm, jref, ixWznmVLocale);
		if (!pnlmnuser) pnlmnuser = new PnlWznmUsgMNUser(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmUsgRec::updatePreset(
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFUSG) {
		...
		if (dbswznm->tblwznmmusergroup->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFUSG, jref), &_recUsg)) {
			recUsg = *_recUsg;
			delete _recUsg;
		...
		if (recUsg.ref != 0) {
			...
			if (pnlmnuser) pnlmnuser->updatePreset(dbswznm, ixWznmVPreset, jrefTrig, notif);
		};
...
void PnlWznmUsgRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMUSGRECDO) {
...
void PnlWznmUsgRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmUsgRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmUsgRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
bool PnlWznmUsgRec::handleCallWznmUsgUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
