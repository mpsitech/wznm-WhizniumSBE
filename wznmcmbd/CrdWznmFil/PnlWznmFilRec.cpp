...
#include "PnlWznmFilRec_blks.cpp"
#include "PnlWznmFilRec_evals.cpp"
...
PnlWznmFilRec::PnlWznmFilRec(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMFILREC, jrefSup, ixWznmVLocale)
		{
...
DpchEngWznm* PnlWznmFilRec::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmFilRec::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.TxtRef = StubWznm::getStubFilStd(dbswznm, recFil.ref, ixWznmVLocale, Stub::VecVNonetype::FULL);

	// statshr
	if (recFil.ref == 0) statshr.ixWznmVExpstate = VecWznmVExpstate::MIND;
	...
	if (statshr.ixWznmVExpstate == VecWznmVExpstate::MIND) {
		...
		if (!pnldetail) pnldetail = new PnlWznmFilDetail(xchg, dbswznm, jref, ixWznmVLocale);
	};
...
void PnlWznmFilRec::updatePreset(
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFFIL) {
		...
		if (dbswznm->tblwznmmfile->loadRecByRef(xchg->getRefPreset(VecWznmVPreset::PREWZNMREFFIL, jref), &_recFil)) {
			recFil = *_recFil;
			delete _recFil;
...
void PnlWznmFilRec::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMFILRECDO) {
...
void PnlWznmFilRec::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmFilRec::handleDpchAppDoButMinimizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmFilRec::handleDpchAppDoButRegularizeClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmFilRec::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMFIL_CLUEQ) {
...
bool PnlWznmFilRec::handleCallWznmFil_cluEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmFilRec::handleCallWznmFil_retEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWznmFilRec::handleCallWznmFil_reuEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const ubigint refInv
			, bool& boolvalRet
...
bool PnlWznmFilRec::handleCallWznmFilUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
