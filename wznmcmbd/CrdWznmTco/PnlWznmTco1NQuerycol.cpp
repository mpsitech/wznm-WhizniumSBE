/**
	...
	* job handler for job PnlWznmTco1NQuerycol (implementation)
...
#include "PnlWznmTco1NQuerycol_blks.cpp"
#include "PnlWznmTco1NQuerycol_evals.cpp"
...
/******************************************************************************
 class PnlWznmTco1NQuerycol
 ******************************************************************************/

PnlWznmTco1NQuerycol::PnlWznmTco1NQuerycol(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTCO1NQUERYCOL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTco1NQuerycol(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTco1NQuerycol::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTco1NQuerycol::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTco1NQuerycol::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTco1NQuerycol::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NQUERYCOLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NQUERYCOLDO) {
...
void PnlWznmTco1NQuerycol::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTco1NQuerycol::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTco1NQuerycol::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTco1NQuerycol::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTco1NQuerycol* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTco1NQuerycol::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTco1NQuerycol::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTco1NQuerycol::StgIac::JNUM)) {
...
void PnlWznmTco1NQuerycol::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCQCO, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmQco";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recQco.ref, jrefNew);
		};
...
void PnlWznmTco1NQuerycol::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTco1NQuerycol::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTco1NQuerycol::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
