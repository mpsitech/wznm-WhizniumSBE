/**
	...
	* job handler for job PnlWznmTco1NImpexpcol (implementation)
...
#include "PnlWznmTco1NImpexpcol_blks.cpp"
...
/******************************************************************************
 class PnlWznmTco1NImpexpcol
 ******************************************************************************/

PnlWznmTco1NImpexpcol::PnlWznmTco1NImpexpcol(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTCO1NIMPEXPCOL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTco1NImpexpcol(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTco1NImpexpcol::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTco1NImpexpcol::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmTco1NImpexpcol::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTco1NImpexpcol::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NIMPEXPCOLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NIMPEXPCOLDO) {
...
void PnlWznmTco1NImpexpcol::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTco1NImpexpcol::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTco1NImpexpcol::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTco1NImpexpcol::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTco1NImpexpcol* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTco1NImpexpcol::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTco1NImpexpcol::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTco1NImpexpcol::StgIac::JNUM)) {
...
void PnlWznmTco1NImpexpcol::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTco1NImpexpcol::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTco1NImpexpcol::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
