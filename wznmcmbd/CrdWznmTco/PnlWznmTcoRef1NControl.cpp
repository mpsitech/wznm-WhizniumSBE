/**
	...
	* job handler for job PnlWznmTcoRef1NControl (implementation)
...
#include "PnlWznmTcoRef1NControl_blks.cpp"
#include "PnlWznmTcoRef1NControl_evals.cpp"
...
/******************************************************************************
 class PnlWznmTcoRef1NControl
 ******************************************************************************/

PnlWznmTcoRef1NControl::PnlWznmTcoRef1NControl(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTCOREF1NCONTROL, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTcoRef1NControl(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTcoRef1NControl::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTcoRef1NControl::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmTcoRef1NControl::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTcoRef1NControl::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCOREF1NCONTROLDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCOREF1NCONTROLDO) {
...
void PnlWznmTcoRef1NControl::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTcoRef1NControl::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTcoRef1NControl::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTcoRef1NControl::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTcoRef1NControl* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTcoRef1NControl::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTcoRef1NControl::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTcoRef1NControl::StgIac::JNUM)) {
...
void PnlWznmTcoRef1NControl::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCON, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmCon";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recCon.ref, jrefNew);
		};
...
void PnlWznmTcoRef1NControl::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTcoRef1NControl::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTcoRef1NControl::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
