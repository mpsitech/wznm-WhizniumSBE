/**
	...
	* job handler for job PnlWznmTco1NCheck (implementation)
...
#include "PnlWznmTco1NCheck_blks.cpp"
#include "PnlWznmTco1NCheck_evals.cpp"
...
/******************************************************************************
 class PnlWznmTco1NCheck
 ******************************************************************************/

PnlWznmTco1NCheck::PnlWznmTco1NCheck(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMTCO1NCHECK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmTco1NCheck(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmTco1NCheck::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmTco1NCheck::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmTco1NCheck::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmTco1NCheck::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NCHECKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCO1NCHECKDO) {
...
void PnlWznmTco1NCheck::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmTco1NCheck::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTco1NCheck::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmTco1NCheck::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQTco1NCheck* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmTco1NCheck::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmTco1NCheck::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmTco1NCheck::StgIac::JNUM)) {
...
void PnlWznmTco1NCheck::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCHK, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFTBL) {
			sref = "CrdWznmChk";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recChk.ref, jrefNew);
		};
		if (jrefNew == 0) {
			if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCHK, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
				sref = "CrdWznmChk";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recChk.ref, jrefNew);
			};
		};
...
void PnlWznmTco1NCheck::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCHK, jref) & VecWznmWAccess::EDIT) != 0) if (ixPre == VecWznmVPreset::PREWZNMREFTBL) {
			sref = "CrdWznmChk";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, -1, jrefNew);
		};
		if (jrefNew == 0) {
			if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCCHK, jref) & VecWznmWAccess::EDIT) != 0) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
				sref = "CrdWznmChk";
				xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, -1, jrefNew);
			};
		};
...
void PnlWznmTco1NCheck::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmTco1NCheck::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmTco1NCheck::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmTco1NCheck::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
