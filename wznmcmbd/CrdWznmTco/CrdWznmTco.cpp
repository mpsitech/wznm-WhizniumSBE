/**
	* \file CrdWznmTco.cpp
	* job handler for job CrdWznmTco (implementation)
...
CrdWznmTco::CrdWznmTco(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMTCO, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFTCO, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnllist = new PnlWznmTcoList(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmTcoHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnlrec = new PnlWznmTcoRec(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmTco::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmTco::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmTco/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdTco = StubWznm::getStubTcoStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFTCO, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmTco::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmTco", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFTCO, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFTCO, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFTCO, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmTco::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmTco::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMTCODO) {
...
void CrdWznmTco::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmTco::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMTCO);
};

void CrdWznmTco::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmTco::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmTco::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFTCO) {
...
bool CrdWznmTco::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmTco::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmTco::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmTco::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmTco::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
