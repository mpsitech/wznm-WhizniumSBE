/**
	...
	* job handler for job PnlWznmCmpMNOppack (implementation)
...
#include "PnlWznmCmpMNOppack_blks.cpp"
#include "PnlWznmCmpMNOppack_evals.cpp"
...
/******************************************************************************
 class PnlWznmCmpMNOppack
 ******************************************************************************/

PnlWznmCmpMNOppack::PnlWznmCmpMNOppack(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCMPMNOPPACK, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCmpMNOppack(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCmpMNOppack::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCmpMNOppack::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
...
void PnlWznmCmpMNOppack::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFCMP) {
		recCmpRopk = WznmRMComponentMOppack();
...
void PnlWznmCmpMNOppack::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMPMNOPPACKDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMPMNOPPACKDO) {
...
void PnlWznmCmpMNOppack::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCmpMNOppack::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmpMNOppack::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCmpMNOppack::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCmpMNOppack* recSelNew = NULL;

	WznmRMComponentMOppack* _recCmpRopk = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCmpMNOppack::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCmpMNOppack::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCmpMNOppack::StgIac::JNUM)) {
			...
			if (recSelNew) {
				if (dbswznm->tblwznmrmcomponentmoppack->loadRecByRef(recSelNew->ref, &_recCmpRopk)) {
					recCmpRopk = *_recCmpRopk;
					delete _recCmpRopk;
				};
...
void PnlWznmCmpMNOppack::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCOPK, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmOpk";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recOpk.ref, jrefNew);
		};
...
void PnlWznmCmpMNOppack::handleDpchAppDoButAddClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmpMNOppack::handleDpchAppDoButSubClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmpMNOppack::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCmpMNOppack::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCmpMNOppack::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
