/**
	* \file CrdWznmCmp.cpp
	* job handler for job CrdWznmCmp (implementation)
...
CrdWznmCmp::CrdWznmCmp(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMCMP, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFCMP, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmCmpRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmCmpHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmCmpList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmCmp::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmCmp::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmCmp/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdCmp = StubWznm::getStubCmpStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCMP, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);

	// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdWznmCmp::changeRef(
			DbsWznm* dbswznm
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmCmp", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFCMP, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCMP, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFCMP, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmCmp::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmCmp::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMPDO) {
...
void CrdWznmCmp::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmCmp::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMCMP);
};

void CrdWznmCmp::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmCmp::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmCmp::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFCMP) {
...
bool CrdWznmCmp::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmCmp::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
void CrdWznmCmp::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmCmp::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmCmp::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
