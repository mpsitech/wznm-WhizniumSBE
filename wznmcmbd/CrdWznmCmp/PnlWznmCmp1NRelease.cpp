/**
	...
	* job handler for job PnlWznmCmp1NRelease (implementation)
...
#include "PnlWznmCmp1NRelease_blks.cpp"
#include "PnlWznmCmp1NRelease_evals.cpp"
...
/******************************************************************************
 class PnlWznmCmp1NRelease
 ******************************************************************************/

PnlWznmCmp1NRelease::PnlWznmCmp1NRelease(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCMP1NRELEASE, jrefSup, ixWznmVLocale)
		{
	...
	qry = new QryWznmCmp1NRelease(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* PnlWznmCmp1NRelease::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCmp1NRelease::refresh(
			DbsWznm* dbswznm
			, set<uint>& moditems
			, const bool unmute
		) {
...
void PnlWznmCmp1NRelease::updatePreset(
			DbsWznm* dbswznm
...
void PnlWznmCmp1NRelease::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMP1NRELEASEDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMP1NRELEASEDO) {
...
void PnlWznmCmp1NRelease::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCmp1NRelease::handleDpchAppDataStgiac(
			DbsWznm* dbswznm
			, StgIac* _stgiac
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmp1NRelease::handleDpchAppDataStgiacqry(
			DbsWznm* dbswznm
			, QryWznmCmp1NRelease::StgIac* _stgiacqry
			, DpchEngWznm** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	WznmQCmp1NRelease* recSelNew = NULL;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryWznmCmp1NRelease::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryWznmCmp1NRelease::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbswznm);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryWznmCmp1NRelease::StgIac::JNUM)) {
			if (!recSelNew) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

			recRls = WznmMRelease();

			if (recSelNew) {
				if (dbswznm->tblwznmmrelease->loadRecByRef(recSelNew->ref, &_recRls)) {
					recRls = *_recRls;
					delete _recRls;
				};
			};
...
void PnlWznmCmp1NRelease::handleDpchAppDoButViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCRLS, jref)) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmRls";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, ixPre, refPre, sref, recRls.ref, jrefNew);
		};
...
void PnlWznmCmp1NRelease::handleDpchAppDoButNewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButNewAvail) {
		if ((xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCRLS, jref) & VecWznmWAccess::EDIT) != 0) if (ixPre == VecWznmVPreset::PREWZNMREFVER) {
			sref = "CrdWznmRls";
...
void PnlWznmCmp1NRelease::handleDpchAppDoButDeleteClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmp1NRelease::handleDpchAppDoButRefreshClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	set<uint> moditems;
...
void PnlWznmCmp1NRelease::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
...
bool PnlWznmCmp1NRelease::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
