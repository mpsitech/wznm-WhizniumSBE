/**
	...
	* job handler for job PnlWznmCmpDetail (implementation)
...
#include "PnlWznmCmpDetail_blks.cpp"
#include "PnlWznmCmpDetail_evals.cpp"
...
/******************************************************************************
 class PnlWznmCmpDetail
 ******************************************************************************/

PnlWznmCmpDetail::PnlWznmCmpDetail(
			XchgWznm* xchg
			, DbsWznm* dbswznm
			, const ubigint jrefSup
			, const uint ixWznmVLocale
		) :
			JobWznm(xchg, VecWznmVJob::PNLWZNMCMPDETAIL, jrefSup, ixWznmVLocale)
		{
	jref = xchg->addJob(dbswznm, this, jrefSup);

	feedFPupTyp.tag = "FeedFPupTyp";
	VecWznmVMComponentBasetype::fillFeed(ixWznmVLocale, feedFPupTyp);
...
DpchEngWznm* PnlWznmCmpDetail::getNewDpchEng(
			set<uint> items
		) {
...
void PnlWznmCmpDetail::refreshRecCmp(
			DbsWznm* dbswznm
			, set<uint>& moditems
		) {
	...
	if (dbswznm->tblwznmmcomponent->loadRecByRef(recCmp.ref, &_recCmp)) {
		recCmp = *_recCmp;
	...
	contiac.TxfTit = recCmp.Title;
	contiac.numFPupTyp = feedFPupTyp.getNumByIx(recCmp.ixVBasetype);
	continf.TxtVer = StubWznm::getStubVerStd(dbswznm, recCmp.refWznmMVersion, ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void PnlWznmCmpDetail::updatePreset(
			DbsWznm* dbswznm
	...
	if (ixWznmVPreset == VecWznmVPreset::PREWZNMREFCMP) {
		recCmp.ref = xchg->getRefPreset(VecWznmVPreset::PREWZNMREFCMP, jref);
		refreshRecCmp(dbswznm, moditems);
...
void PnlWznmCmpDetail::handleRequest(
			DbsWznm* dbswznm
			, ReqWznm* req
		) {
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMPDETAILDATA) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMCMPDETAILDO) {
...
void PnlWznmCmpDetail::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void PnlWznmCmpDetail::handleDpchAppDataContiac(
			DbsWznm* dbswznm
			, ContIac* _contiac
			, DpchEngWznm** dpcheng
		) {
	...
	if (hasAny(diffitems, {ContIac::TXFTIT, ContIac::NUMFPUPTYP, ContIac::TXFCMT})) {
...
void PnlWznmCmpDetail::handleDpchAppDoButSaveClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void PnlWznmCmpDetail::handleDpchAppDoButVerViewClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButVerViewAvail && statshr.ButVerViewActive) {
		if (xchg->getIxPreset(VecWznmVPreset::PREWZNMIXCRDACCVER, jref)) {
			sref = "CrdWznmVer";
			xchg->triggerIxRefSrefIntvalToRefCall(dbswznm, VecWznmVCall::CALLWZNMCRDOPEN, jref, 0, 0, sref, recCmp.refWznmMVersion, jrefNew);
		};
...
void PnlWznmCmpDetail::handleCall(
			DbsWznm* dbswznm
			, Call* call
		) {
	if (call->ixVCall == VecWznmVCall::CALLWZNMCMP_VEREQ) {
...
bool PnlWznmCmpDetail::handleCallWznmCmp_verEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
...
bool PnlWznmCmpDetail::handleCallWznmCmp_typEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWznmCmpDetail::handleCallWznmCmp_inSbs(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
			, bool& boolvalRet
...
bool PnlWznmCmpDetail::handleCallWznmCmpUpd_refEq(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
