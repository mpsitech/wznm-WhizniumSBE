/**
	* \file CrdWznmRtj.cpp
	* job handler for job CrdWznmRtj (implementation)
...
CrdWznmRtj::CrdWznmRtj(
			...
			JobWznm(xchg, VecWznmVJob::CRDWZNMRTJ, jrefSup, ixWznmVLocale)
		{
	...
	if ((ref + 1) != 0) xchg->triggerIxRefCall(dbswznm, VecWznmVCall::CALLWZNMREFPRESET, jref, VecWznmVPreset::PREWZNMREFRTJ, ref);

	// initialize according to ref
	changeRef(dbswznm, jref, ((ref + 1) == 0) ? 0 : ref, false);

	pnlrec = new PnlWznmRtjRec(xchg, dbswznm, jref, ixWznmVLocale);
	pnlheadbar = new PnlWznmRtjHeadbar(xchg, dbswznm, jref, ixWznmVLocale);
	pnllist = new PnlWznmRtjList(xchg, dbswznm, jref, ixWznmVLocale);
...
DpchEngWznm* CrdWznmRtj::getNewDpchEng(
			set<uint> items
		) {
...
void CrdWznmRtj::refresh(
	...
	// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdWznmRtj/" + VecWznmVLocale::getSref(ixWznmVLocale);
	continf.MtxCrdRtj = StubWznm::getStubRtjStd(dbswznm, xchg->getRefPreset(VecWznmVPreset::PREWZNMREFRTJ, jref), ixWznmVLocale, Stub::VecVNonetype::FULL);
...
void CrdWznmRtj::changeRef(
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbswznm, VecWznmVCall::CALLWZNMLOG, jref, ixWznmVPreset, xchg->getRefPreset(ixWznmVPreset, jref), "CrdWznmRtj", ref);
	xchg->addRefPreset(VecWznmVPreset::PREWZNMREFRTJ, jref, ref);

	if (pnllist) pnllist->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFRTJ, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbswznm, VecWznmVPreset::PREWZNMREFRTJ, jrefTrig, notif);

	refresh(dbswznm, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));
};

void CrdWznmRtj::updatePreset(
			DbsWznm* dbswznm
			, const uint ixWznmVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdWznmRtj::handleRequest(
	...
	if (req->ixVBasetype == ReqWznm::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixWznmVDpch == VecWznmVDpch::DPCHAPPWZNMRTJDO) {
...
void CrdWznmRtj::handleDpchAppWznmInit(
			DbsWznm* dbswznm
...
void CrdWznmRtj::handleDpchAppDoClose(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
	muteRefresh = true;
	...
	*dpcheng = new DpchEngWznmConfirm(true, jref, "");
	xchg->triggerIxCall(dbswznm, VecWznmVCall::CALLWZNMCRDCLOSE, jref, VecWznmVCard::CRDWZNMRTJ);
};

void CrdWznmRtj::handleDpchAppDoMitAppAbtClick(
			DbsWznm* dbswznm
			, DpchEngWznm** dpcheng
		) {
...
void CrdWznmRtj::handleDpchAppWznmAlert(
			DbsWznm* dbswznm
...
bool CrdWznmRtj::handleCallWznmDlgClose(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmRtj::handleCallWznmStatChg(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdWznmRtj::handleCallWznmRefPreSet(
			DbsWznm* dbswznm
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecWznmVPreset::PREWZNMREFRTJ) {
...
void CrdWznmRtj::leaveSgeIdle(
			DbsWznm* dbswznm
		) {
	// IP leaveSgeIdle --- INSERT
};

uint CrdWznmRtj::enterSgeAlrwznmabt(
			DbsWznm* dbswznm
			, const bool reenter
		) {
...
void CrdWznmRtj::leaveSgeAlrwznmabt(
			DbsWznm* dbswznm
		) {
