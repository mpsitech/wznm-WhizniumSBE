...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmVerVer1NError.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmVerVer1NErrorData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmVerVer1NError": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmVerVer1NError"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmVerVer1NErrorData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmVerVer1NErrorData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmVerVer1NError) this.continf = dpcheng.ContInfWznmVerVer1NError;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmVerVer1NError) this.statapp = dpcheng.StatAppWznmVerVer1NError;
				if (dpcheng.StatShrWznmVerVer1NError) this.statshr = dpcheng.StatShrWznmVerVer1NError;
				if (dpcheng.StgIacWznmVerVer1NError) this.stgiac = dpcheng.StgIacWznmVerVer1NError;
				if (dpcheng.TagWznmVerVer1NError) {
					Wznm.unescapeBlock(dpcheng.TagWznmVerVer1NError);
					this.tag = dpcheng.TagWznmVerVer1NError;
				}
				if (dpcheng.ListWznmQVerVer1NError) this.rst = dpcheng.ListWznmQVerVer1NError;
				if (dpcheng.StatAppQryWznmVerVer1NError) this.statappqry = dpcheng.StatAppQryWznmVerVer1NError;
				if (dpcheng.StatShrQryWznmVerVer1NError) this.statshrqry = dpcheng.StatShrQryWznmVerVer1NError;
				if (dpcheng.StgIacQryWznmVerVer1NError) this.stgiacqry = dpcheng.StgIacQryWznmVerVer1NError;
				/*
				...
				*/
				...
				if (dpcheng.ListWznmQVerVer1NError) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmVerVer1NErrorData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmVerVer1NErrorData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
