...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmVer1NCapability.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmVer1NCapabilityData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmVer1NCapability": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmVer1NCapability"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmVer1NCapabilityData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmVer1NCapabilityData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmVer1NCapability) this.continf = dpcheng.ContInfWznmVer1NCapability;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmVer1NCapability) this.statapp = dpcheng.StatAppWznmVer1NCapability;
				if (dpcheng.StatShrWznmVer1NCapability) this.statshr = dpcheng.StatShrWznmVer1NCapability;
				if (dpcheng.StgIacWznmVer1NCapability) this.stgiac = dpcheng.StgIacWznmVer1NCapability;
				if (dpcheng.TagWznmVer1NCapability) {
					Wznm.unescapeBlock(dpcheng.TagWznmVer1NCapability);
					this.tag = dpcheng.TagWznmVer1NCapability;
				}
				if (dpcheng.ListWznmQVer1NCapability) this.rst = dpcheng.ListWznmQVer1NCapability;
				if (dpcheng.StatAppQryWznmVer1NCapability) this.statappqry = dpcheng.StatAppQryWznmVer1NCapability;
				if (dpcheng.StatShrQryWznmVer1NCapability) this.statshrqry = dpcheng.StatShrQryWznmVer1NCapability;
				if (dpcheng.StgIacQryWznmVer1NCapability) this.stgiacqry = dpcheng.StgIacQryWznmVer1NCapability;
				/*
				...
				*/
				...
				if (dpcheng.ListWznmQVer1NCapability) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmVer1NCapabilityData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmVer1NCapabilityData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
