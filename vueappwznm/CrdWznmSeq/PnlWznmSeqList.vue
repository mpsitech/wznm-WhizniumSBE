...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmSeqList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmSeqListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmSeqList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmSeqList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmSeqListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmSeqListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmSeqList) this.contiac = dpcheng.ContIacWznmSeqList;
				if (dpcheng.ContInfWznmSeqList) this.continf = dpcheng.ContInfWznmSeqList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmSeqList) this.statshr = dpcheng.StatShrWznmSeqList;
				if (dpcheng.StgIacWznmSeqList) this.stgiac = dpcheng.StgIacWznmSeqList;
				if (dpcheng.TagWznmSeqList) {
					Wznm.unescapeBlock(dpcheng.TagWznmSeqList);
					this.tag = dpcheng.TagWznmSeqList;
				}
				if (dpcheng.ListWznmQSeqList) this.rst = dpcheng.ListWznmQSeqList;
				if (dpcheng.StatAppQryWznmSeqList) this.statappqry = dpcheng.StatAppQryWznmSeqList;
				if (dpcheng.StatShrQryWznmSeqList) this.statshrqry = dpcheng.StatShrQryWznmSeqList;
				if (dpcheng.StgIacQryWznmSeqList) this.stgiacqry = dpcheng.StgIacQryWznmSeqList;
				if (dpcheng.ContInfWznmSeqList) {
				...
				*/
				...
				if (dpcheng.TagWznmSeqList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoApp", text: this.tag.TcoApp});
					/*
					*/
				...
				if (dpcheng.ListWznmQSeqList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"], TcoApp: rec["app"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmSeqListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmSeqListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
