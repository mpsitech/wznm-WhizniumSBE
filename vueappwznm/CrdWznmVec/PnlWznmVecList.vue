...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmVecList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmVecListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmVecList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmVecList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmVecListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmVecListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmVecList) this.contiac = dpcheng.ContIacWznmVecList;
				if (dpcheng.ContInfWznmVecList) this.continf = dpcheng.ContInfWznmVecList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmVecList) this.statshr = dpcheng.StatShrWznmVecList;
				if (dpcheng.StgIacWznmVecList) this.stgiac = dpcheng.StgIacWznmVecList;
				if (dpcheng.TagWznmVecList) {
					Wznm.unescapeBlock(dpcheng.TagWznmVecList);
					this.tag = dpcheng.TagWznmVecList;
				}
				if (dpcheng.ListWznmQVecList) this.rst = dpcheng.ListWznmQVecList;
				if (dpcheng.StatAppQryWznmVecList) this.statappqry = dpcheng.StatAppQryWznmVecList;
				if (dpcheng.StatShrQryWznmVecList) this.statshrqry = dpcheng.StatShrQryWznmVecList;
				if (dpcheng.StgIacQryWznmVecList) this.stgiacqry = dpcheng.StgIacQryWznmVecList;
				if (dpcheng.ContInfWznmVecList) {
				...
				*/
				...
				if (dpcheng.TagWznmVecList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoVer", text: this.tag.TcoVer}, {value: "TcoHkt", text: this.tag.TcoHkt}, {value: "TcoHku", text: this.tag.TcoHku}, {value: "TcoTgr", text: this.tag.TcoTgr});
					/*
					*/
				...
				if (dpcheng.ListWznmQVecList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoVer: rec["ver"], TcoHkt: rec["hkt2"], TcoHku: rec["hku"], TcoTgr: rec["tgr2"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmVecListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmVecListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
