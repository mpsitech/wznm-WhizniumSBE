...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmRtjList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmRtjListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmRtjList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmRtjList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmRtjListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmRtjListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmRtjList) this.contiac = dpcheng.ContIacWznmRtjList;
				if (dpcheng.ContInfWznmRtjList) this.continf = dpcheng.ContInfWznmRtjList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmRtjList) this.statshr = dpcheng.StatShrWznmRtjList;
				if (dpcheng.StgIacWznmRtjList) this.stgiac = dpcheng.StgIacWznmRtjList;
				if (dpcheng.TagWznmRtjList) {
					Wznm.unescapeBlock(dpcheng.TagWznmRtjList);
					this.tag = dpcheng.TagWznmRtjList;
				}
				if (dpcheng.ListWznmQRtjList) this.rst = dpcheng.ListWznmQRtjList;
				if (dpcheng.StatAppQryWznmRtjList) this.statappqry = dpcheng.StatAppQryWznmRtjList;
				if (dpcheng.StatShrQryWznmRtjList) this.statshrqry = dpcheng.StatShrQryWznmRtjList;
				if (dpcheng.StgIacQryWznmRtjList) this.stgiacqry = dpcheng.StgIacQryWznmRtjList;
				if (dpcheng.ContInfWznmRtjList) {
				...
				*/
				...
				if (dpcheng.TagWznmRtjList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoApp", text: this.tag.TcoApp}, {value: "TcoSup", text: this.tag.TcoSup}, {value: "TcoJob", text: this.tag.TcoJob});
					/*
					*/
				...
				if (dpcheng.ListWznmQRtjList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoApp: rec["app"], TcoSup: rec["sup"], TcoJob: rec["job"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmRtjListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmRtjListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
