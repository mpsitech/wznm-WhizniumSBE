...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmSgeList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmSgeListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmSgeList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmSgeList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmSgeListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmSgeListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmSgeList) this.contiac = dpcheng.ContIacWznmSgeList;
				if (dpcheng.ContInfWznmSgeList) this.continf = dpcheng.ContInfWznmSgeList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmSgeList) this.statshr = dpcheng.StatShrWznmSgeList;
				if (dpcheng.StgIacWznmSgeList) this.stgiac = dpcheng.StgIacWznmSgeList;
				if (dpcheng.TagWznmSgeList) {
					Wznm.unescapeBlock(dpcheng.TagWznmSgeList);
					this.tag = dpcheng.TagWznmSgeList;
				}
				if (dpcheng.ListWznmQSgeList) this.rst = dpcheng.ListWznmQSgeList;
				if (dpcheng.StatAppQryWznmSgeList) this.statappqry = dpcheng.StatAppQryWznmSgeList;
				if (dpcheng.StatShrQryWznmSgeList) this.statshrqry = dpcheng.StatShrQryWznmSgeList;
				if (dpcheng.StgIacQryWznmSgeList) this.stgiacqry = dpcheng.StgIacQryWznmSgeList;
				if (dpcheng.ContInfWznmSgeList) {
				...
				*/
				...
				if (dpcheng.TagWznmSgeList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoJob", text: this.tag.TcoJob});
					/*
					*/
				...
				if (dpcheng.ListWznmQSgeList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoJob: rec["job"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmSgeListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmSgeListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
