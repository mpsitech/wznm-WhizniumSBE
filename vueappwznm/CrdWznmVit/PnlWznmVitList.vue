...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmVitList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmVitListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmVitList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmVitList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmVitListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmVitListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmVitList) this.contiac = dpcheng.ContIacWznmVitList;
				if (dpcheng.ContInfWznmVitList) this.continf = dpcheng.ContInfWznmVitList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmVitList) this.statshr = dpcheng.StatShrWznmVitList;
				if (dpcheng.StgIacWznmVitList) this.stgiac = dpcheng.StgIacWznmVitList;
				if (dpcheng.TagWznmVitList) {
					Wznm.unescapeBlock(dpcheng.TagWznmVitList);
					this.tag = dpcheng.TagWznmVitList;
				}
				if (dpcheng.ListWznmQVitList) this.rst = dpcheng.ListWznmQVitList;
				if (dpcheng.StatAppQryWznmVitList) this.statappqry = dpcheng.StatAppQryWznmVitList;
				if (dpcheng.StatShrQryWznmVitList) this.statshrqry = dpcheng.StatShrQryWznmVitList;
				if (dpcheng.StgIacQryWznmVitList) this.stgiacqry = dpcheng.StgIacQryWznmVitList;
				if (dpcheng.ContInfWznmVitList) {
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmVitListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmVitListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
