...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmSteList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmSteListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmSteList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmSteList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmSteListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmSteListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmSteList) this.contiac = dpcheng.ContIacWznmSteList;
				if (dpcheng.ContInfWznmSteList) this.continf = dpcheng.ContInfWznmSteList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmSteList) this.statshr = dpcheng.StatShrWznmSteList;
				if (dpcheng.StgIacWznmSteList) this.stgiac = dpcheng.StgIacWznmSteList;
				if (dpcheng.TagWznmSteList) {
					Wznm.unescapeBlock(dpcheng.TagWznmSteList);
					this.tag = dpcheng.TagWznmSteList;
				}
				if (dpcheng.ListWznmQSteList) this.rst = dpcheng.ListWznmQSteList;
				if (dpcheng.StatAppQryWznmSteList) this.statappqry = dpcheng.StatAppQryWznmSteList;
				if (dpcheng.StatShrQryWznmSteList) this.statshrqry = dpcheng.StatShrQryWznmSteList;
				if (dpcheng.StgIacQryWznmSteList) this.stgiacqry = dpcheng.StgIacQryWznmSteList;
				if (dpcheng.ContInfWznmSteList) {
				...
				*/
				...
				if (dpcheng.TagWznmSteList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoSeq", text: this.tag.TcoSeq});
					/*
					*/
				...
				if (dpcheng.ListWznmQSteList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoSeq: rec["seq"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmSteListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmSteListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
