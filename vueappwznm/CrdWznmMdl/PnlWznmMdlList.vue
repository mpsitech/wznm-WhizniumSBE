...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmMdlList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmMdlListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmMdlList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmMdlList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmMdlListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmMdlListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmMdlList) this.contiac = dpcheng.ContIacWznmMdlList;
				if (dpcheng.ContInfWznmMdlList) this.continf = dpcheng.ContInfWznmMdlList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmMdlList) this.statshr = dpcheng.StatShrWznmMdlList;
				if (dpcheng.StgIacWznmMdlList) this.stgiac = dpcheng.StgIacWznmMdlList;
				if (dpcheng.TagWznmMdlList) {
					Wznm.unescapeBlock(dpcheng.TagWznmMdlList);
					this.tag = dpcheng.TagWznmMdlList;
				}
				if (dpcheng.ListWznmQMdlList) this.rst = dpcheng.ListWznmQMdlList;
				if (dpcheng.StatAppQryWznmMdlList) this.statappqry = dpcheng.StatAppQryWznmMdlList;
				if (dpcheng.StatShrQryWznmMdlList) this.statshrqry = dpcheng.StatShrQryWznmMdlList;
				if (dpcheng.StgIacQryWznmMdlList) this.stgiacqry = dpcheng.StgIacQryWznmMdlList;
				if (dpcheng.ContInfWznmMdlList) {
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmMdlListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmMdlListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
