...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmChkList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmChkListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmChkList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmChkList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmChkListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmChkListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmChkList) this.contiac = dpcheng.ContIacWznmChkList;
				if (dpcheng.ContInfWznmChkList) this.continf = dpcheng.ContInfWznmChkList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmChkList) this.statshr = dpcheng.StatShrWznmChkList;
				if (dpcheng.StgIacWznmChkList) this.stgiac = dpcheng.StgIacWznmChkList;
				if (dpcheng.TagWznmChkList) {
					Wznm.unescapeBlock(dpcheng.TagWznmChkList);
					this.tag = dpcheng.TagWznmChkList;
				}
				if (dpcheng.ListWznmQChkList) this.rst = dpcheng.ListWznmQChkList;
				if (dpcheng.StatAppQryWznmChkList) this.statappqry = dpcheng.StatAppQryWznmChkList;
				if (dpcheng.StatShrQryWznmChkList) this.statshrqry = dpcheng.StatShrQryWznmChkList;
				if (dpcheng.StgIacQryWznmChkList) this.stgiacqry = dpcheng.StgIacQryWznmChkList;
				if (dpcheng.ContInfWznmChkList) {
				...
				*/
				...
				if (dpcheng.TagWznmChkList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTyp", text: this.tag.TcoTyp}, {value: "TcoTbl", text: this.tag.TcoTbl}, {value: "TcoTco", text: this.tag.TcoTco});
					/*
					*/
				...
				if (dpcheng.ListWznmQChkList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTyp: rec["typ2"], TcoTbl: rec["tbl"], TcoTco: rec["tco"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmChkListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmChkListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
