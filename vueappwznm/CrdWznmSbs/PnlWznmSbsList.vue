...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmSbsList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmSbsListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmSbsList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmSbsList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmSbsListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmSbsListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmSbsList) this.contiac = dpcheng.ContIacWznmSbsList;
				if (dpcheng.ContInfWznmSbsList) this.continf = dpcheng.ContInfWznmSbsList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmSbsList) this.statshr = dpcheng.StatShrWznmSbsList;
				if (dpcheng.StgIacWznmSbsList) this.stgiac = dpcheng.StgIacWznmSbsList;
				if (dpcheng.TagWznmSbsList) {
					Wznm.unescapeBlock(dpcheng.TagWznmSbsList);
					this.tag = dpcheng.TagWznmSbsList;
				}
				if (dpcheng.ListWznmQSbsList) this.rst = dpcheng.ListWznmQSbsList;
				if (dpcheng.StatAppQryWznmSbsList) this.statappqry = dpcheng.StatAppQryWznmSbsList;
				if (dpcheng.StatShrQryWznmSbsList) this.statshrqry = dpcheng.StatShrQryWznmSbsList;
				if (dpcheng.StgIacQryWznmSbsList) this.stgiacqry = dpcheng.StgIacQryWznmSbsList;
				if (dpcheng.ContInfWznmSbsList) {
				...
				*/
				...
				if (dpcheng.TagWznmSbsList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoSho", text: this.tag.TcoSho}, {value: "TcoTbl", text: this.tag.TcoTbl});
					/*
					*/
				...
				if (dpcheng.ListWznmQSbsList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoSho: rec["sho"], TcoTbl: rec["tbl"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmSbsListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmSbsListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
