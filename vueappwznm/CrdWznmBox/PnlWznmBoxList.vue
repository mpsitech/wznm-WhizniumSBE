...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmBoxList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmBoxListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmBoxList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmBoxList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmBoxListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmBoxListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmBoxList) this.contiac = dpcheng.ContIacWznmBoxList;
				if (dpcheng.ContInfWznmBoxList) this.continf = dpcheng.ContInfWznmBoxList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmBoxList) this.statshr = dpcheng.StatShrWznmBoxList;
				if (dpcheng.StgIacWznmBoxList) this.stgiac = dpcheng.StgIacWznmBoxList;
				if (dpcheng.TagWznmBoxList) {
					Wznm.unescapeBlock(dpcheng.TagWznmBoxList);
					this.tag = dpcheng.TagWznmBoxList;
				}
				if (dpcheng.ListWznmQBoxList) this.rst = dpcheng.ListWznmQBoxList;
				if (dpcheng.StatAppQryWznmBoxList) this.statappqry = dpcheng.StatAppQryWznmBoxList;
				if (dpcheng.StatShrQryWznmBoxList) this.statshrqry = dpcheng.StatShrQryWznmBoxList;
				if (dpcheng.StgIacQryWznmBoxList) this.stgiacqry = dpcheng.StgIacQryWznmBoxList;
				if (dpcheng.ContInfWznmBoxList) {
				...
				*/
				...
				if (dpcheng.TagWznmBoxList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSht", text: this.tag.TcoSht}, {value: "TcoMtb", text: this.tag.TcoMtb}, {value: "TcoUnv", text: this.tag.TcoUnv});
					/*
					*/
				...
				if (dpcheng.ListWznmQBoxList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSht: rec["sht"], TcoMtb: rec["mtb2"], TcoUnv: rec["unv"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmBoxListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmBoxListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
