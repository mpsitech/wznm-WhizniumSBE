...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmImeList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmImeListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmImeList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmImeList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmImeListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmImeListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmImeList) this.contiac = dpcheng.ContIacWznmImeList;
				if (dpcheng.ContInfWznmImeList) this.continf = dpcheng.ContInfWznmImeList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmImeList) this.statshr = dpcheng.StatShrWznmImeList;
				if (dpcheng.StgIacWznmImeList) this.stgiac = dpcheng.StgIacWznmImeList;
				if (dpcheng.TagWznmImeList) {
					Wznm.unescapeBlock(dpcheng.TagWznmImeList);
					this.tag = dpcheng.TagWznmImeList;
				}
				if (dpcheng.ListWznmQImeList) this.rst = dpcheng.ListWznmQImeList;
				if (dpcheng.StatAppQryWznmImeList) this.statappqry = dpcheng.StatAppQryWznmImeList;
				if (dpcheng.StatShrQryWznmImeList) this.statshrqry = dpcheng.StatShrQryWznmImeList;
				if (dpcheng.StgIacQryWznmImeList) this.stgiacqry = dpcheng.StgIacQryWznmImeList;
				if (dpcheng.ContInfWznmImeList) {
				...
				*/
				...
				if (dpcheng.TagWznmImeList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoIex", text: this.tag.TcoIex}, {value: "TcoSup", text: this.tag.TcoSup}, {value: "TcoTbl", text: this.tag.TcoTbl});
					/*
					*/
				...
				if (dpcheng.ListWznmQImeList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoIex: rec["iex"], TcoSup: rec["sup"], TcoTbl: rec["tbl"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmImeListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmImeListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
