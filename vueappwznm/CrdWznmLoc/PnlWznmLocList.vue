...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmLocList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmLocListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmLocList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmLocList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmLocListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmLocListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmLocList) this.contiac = dpcheng.ContIacWznmLocList;
				if (dpcheng.ContInfWznmLocList) this.continf = dpcheng.ContInfWznmLocList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmLocList) this.statshr = dpcheng.StatShrWznmLocList;
				if (dpcheng.StgIacWznmLocList) this.stgiac = dpcheng.StgIacWznmLocList;
				if (dpcheng.TagWznmLocList) {
					Wznm.unescapeBlock(dpcheng.TagWznmLocList);
					this.tag = dpcheng.TagWznmLocList;
				}
				if (dpcheng.ListWznmQLocList) this.rst = dpcheng.ListWznmQLocList;
				if (dpcheng.StatAppQryWznmLocList) this.statappqry = dpcheng.StatAppQryWznmLocList;
				if (dpcheng.StatShrQryWznmLocList) this.statshrqry = dpcheng.StatShrQryWznmLocList;
				if (dpcheng.StgIacQryWznmLocList) this.stgiacqry = dpcheng.StgIacQryWznmLocList;
				if (dpcheng.ContInfWznmLocList) {
				...
				*/
				...
				if (dpcheng.TagWznmLocList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit});
					/*
					*/
				...
				if (dpcheng.ListWznmQLocList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmLocListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmLocListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
