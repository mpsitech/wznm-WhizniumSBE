...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobACmd.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobACmdData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobACmd": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobACmd"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobACmdData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobACmdData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobACmd) this.continf = dpcheng.ContInfWznmJobACmd;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobACmd) this.statapp = dpcheng.StatAppWznmJobACmd;
				if (dpcheng.StatShrWznmJobACmd) this.statshr = dpcheng.StatShrWznmJobACmd;
				if (dpcheng.StgIacWznmJobACmd) this.stgiac = dpcheng.StgIacWznmJobACmd;
				if (dpcheng.TagWznmJobACmd) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobACmd);
					this.tag = dpcheng.TagWznmJobACmd;
				}
				if (dpcheng.ListWznmQJobACmd) this.rst = dpcheng.ListWznmQJobACmd;
				if (dpcheng.StatAppQryWznmJobACmd) this.statappqry = dpcheng.StatAppQryWznmJobACmd;
				if (dpcheng.StatShrQryWznmJobACmd) this.statshrqry = dpcheng.StatShrQryWznmJobACmd;
				if (dpcheng.StgIacQryWznmJobACmd) this.stgiacqry = dpcheng.StgIacQryWznmJobACmd;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobACmdData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobACmdData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
