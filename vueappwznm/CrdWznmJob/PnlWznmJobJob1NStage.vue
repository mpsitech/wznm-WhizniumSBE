...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobJob1NStage.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobJob1NStageData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobJob1NStage": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobJob1NStage"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobJob1NStageData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobJob1NStageData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobJob1NStage) this.continf = dpcheng.ContInfWznmJobJob1NStage;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobJob1NStage) this.statapp = dpcheng.StatAppWznmJobJob1NStage;
				if (dpcheng.StatShrWznmJobJob1NStage) this.statshr = dpcheng.StatShrWznmJobJob1NStage;
				if (dpcheng.StgIacWznmJobJob1NStage) this.stgiac = dpcheng.StgIacWznmJobJob1NStage;
				if (dpcheng.TagWznmJobJob1NStage) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobJob1NStage);
					this.tag = dpcheng.TagWznmJobJob1NStage;
				}
				if (dpcheng.ListWznmQJobJob1NStage) this.rst = dpcheng.ListWznmQJobJob1NStage;
				if (dpcheng.StatAppQryWznmJobJob1NStage) this.statappqry = dpcheng.StatAppQryWznmJobJob1NStage;
				if (dpcheng.StatShrQryWznmJobJob1NStage) this.statshrqry = dpcheng.StatShrQryWznmJobJob1NStage;
				if (dpcheng.StgIacQryWznmJobJob1NStage) this.stgiacqry = dpcheng.StgIacQryWznmJobJob1NStage;
				/*
				...
				*/
				...
				if (dpcheng.ListWznmQJobJob1NStage) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobJob1NStageData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobJob1NStageData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
