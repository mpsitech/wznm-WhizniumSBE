...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobMNOp.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobMNOpData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobMNOp": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobMNOp"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobMNOpData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobMNOpData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobMNOp) this.continf = dpcheng.ContInfWznmJobMNOp;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobMNOp) this.statapp = dpcheng.StatAppWznmJobMNOp;
				if (dpcheng.StatShrWznmJobMNOp) this.statshr = dpcheng.StatShrWznmJobMNOp;
				if (dpcheng.StgIacWznmJobMNOp) this.stgiac = dpcheng.StgIacWznmJobMNOp;
				if (dpcheng.TagWznmJobMNOp) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobMNOp);
					this.tag = dpcheng.TagWznmJobMNOp;
				}
				if (dpcheng.ListWznmQJobMNOp) this.rst = dpcheng.ListWznmQJobMNOp;
				if (dpcheng.StatAppQryWznmJobMNOp) this.statappqry = dpcheng.StatAppQryWznmJobMNOp;
				if (dpcheng.StatShrQryWznmJobMNOp) this.statshrqry = dpcheng.StatShrQryWznmJobMNOp;
				if (dpcheng.StgIacQryWznmJobMNOp) this.stgiacqry = dpcheng.StgIacQryWznmJobMNOp;
				/*
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobMNOpData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobMNOpData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
