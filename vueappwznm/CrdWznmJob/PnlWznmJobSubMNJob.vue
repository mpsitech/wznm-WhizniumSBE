...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobSubMNJob.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobSubMNJobData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobSubMNJob": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobSubMNJob"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobSubMNJobData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobSubMNJobData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobSubMNJob) this.continf = dpcheng.ContInfWznmJobSubMNJob;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobSubMNJob) this.statapp = dpcheng.StatAppWznmJobSubMNJob;
				if (dpcheng.StatShrWznmJobSubMNJob) this.statshr = dpcheng.StatShrWznmJobSubMNJob;
				if (dpcheng.StgIacWznmJobSubMNJob) this.stgiac = dpcheng.StgIacWznmJobSubMNJob;
				if (dpcheng.TagWznmJobSubMNJob) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobSubMNJob);
					this.tag = dpcheng.TagWznmJobSubMNJob;
				}
				if (dpcheng.ListWznmQJobSubMNJob) this.rst = dpcheng.ListWznmQJobSubMNJob;
				if (dpcheng.StatAppQryWznmJobSubMNJob) this.statappqry = dpcheng.StatAppQryWznmJobSubMNJob;
				if (dpcheng.StatShrQryWznmJobSubMNJob) this.statshrqry = dpcheng.StatShrQryWznmJobSubMNJob;
				if (dpcheng.StgIacQryWznmJobSubMNJob) this.stgiacqry = dpcheng.StgIacQryWznmJobSubMNJob;
				/*
				...
				*/
				...
				if (dpcheng.TagWznmJobSubMNJob) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoMref", text: this.tag.TcoMref}, {value: "TcoSho", text: this.tag.TcoSho}, {value: "TcoMlt", text: this.tag.TcoMlt}, {value: "TcoCsa", text: this.tag.TcoCsa});
					/*
					*/
				...
				if (dpcheng.ListWznmQJobSubMNJob) {
					var rows = [];
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoMref: rec["mref"], TcoSho: rec["sho"], TcoMlt: rec["mlt"], TcoCsa: rec["csa"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobSubMNJobData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobSubMNJobData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
