...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobMNOppack.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobMNOppackData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobMNOppack": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobMNOppack"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobMNOppackData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobMNOppackData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobMNOppack) this.continf = dpcheng.ContInfWznmJobMNOppack;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobMNOppack) this.statapp = dpcheng.StatAppWznmJobMNOppack;
				if (dpcheng.StatShrWznmJobMNOppack) this.statshr = dpcheng.StatShrWznmJobMNOppack;
				if (dpcheng.StgIacWznmJobMNOppack) this.stgiac = dpcheng.StgIacWznmJobMNOppack;
				if (dpcheng.TagWznmJobMNOppack) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobMNOppack);
					this.tag = dpcheng.TagWznmJobMNOppack;
				}
				if (dpcheng.ListWznmQJobMNOppack) this.rst = dpcheng.ListWznmQJobMNOppack;
				if (dpcheng.StatAppQryWznmJobMNOppack) this.statappqry = dpcheng.StatAppQryWznmJobMNOppack;
				if (dpcheng.StatShrQryWznmJobMNOppack) this.statshrqry = dpcheng.StatShrQryWznmJobMNOppack;
				if (dpcheng.StgIacQryWznmJobMNOppack) this.stgiacqry = dpcheng.StgIacQryWznmJobMNOppack;
				/*
				...
				*/
				...
				if (dpcheng.ListWznmQJobMNOppack) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobMNOppackData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobMNOppackData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
