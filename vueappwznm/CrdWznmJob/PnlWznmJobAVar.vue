...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmJobAVar.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmJobAVarData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmJobAVar": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmJobAVar"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmJobAVarData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmJobAVarData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmJobAVar) this.continf = dpcheng.ContInfWznmJobAVar;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmJobAVar) this.statapp = dpcheng.StatAppWznmJobAVar;
				if (dpcheng.StatShrWznmJobAVar) this.statshr = dpcheng.StatShrWznmJobAVar;
				if (dpcheng.StgIacWznmJobAVar) this.stgiac = dpcheng.StgIacWznmJobAVar;
				if (dpcheng.TagWznmJobAVar) {
					Wznm.unescapeBlock(dpcheng.TagWznmJobAVar);
					this.tag = dpcheng.TagWznmJobAVar;
				}
				if (dpcheng.ListWznmQJobAVar) this.rst = dpcheng.ListWznmQJobAVar;
				if (dpcheng.StatAppQryWznmJobAVar) this.statappqry = dpcheng.StatAppQryWznmJobAVar;
				if (dpcheng.StatShrQryWznmJobAVar) this.statshrqry = dpcheng.StatShrQryWznmJobAVar;
				if (dpcheng.StgIacQryWznmJobAVar) this.stgiacqry = dpcheng.StgIacQryWznmJobAVar;
				/*
				...
				*/
				...
				if (dpcheng.TagWznmJobAVar) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoVty", text: this.tag.TcoVty}, {value: "TcoVec", text: this.tag.TcoVec}, {value: "TcoLen", text: this.tag.TcoLen}, {value: "TcoShr", text: this.tag.TcoShr}, {value: "TcoCmt", text: this.tag.TcoCmt});
					/*
					*/
				...
				if (dpcheng.ListWznmQJobAVar) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoVty: rec["vty"], TcoVec: rec["vec"], TcoLen: rec["len"], TcoShr: rec["shr"], TcoCmt: rec["cmt"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmJobAVarData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmJobAVarData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
