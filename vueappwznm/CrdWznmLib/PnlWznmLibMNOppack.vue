...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmLibMNOppack.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmLibMNOppackData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmLibMNOppack": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmLibMNOppack"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmLibMNOppackData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmLibMNOppackData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContInfWznmLibMNOppack) this.continf = dpcheng.ContInfWznmLibMNOppack;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.StatAppWznmLibMNOppack) this.statapp = dpcheng.StatAppWznmLibMNOppack;
				if (dpcheng.StatShrWznmLibMNOppack) this.statshr = dpcheng.StatShrWznmLibMNOppack;
				if (dpcheng.StgIacWznmLibMNOppack) this.stgiac = dpcheng.StgIacWznmLibMNOppack;
				if (dpcheng.TagWznmLibMNOppack) {
					Wznm.unescapeBlock(dpcheng.TagWznmLibMNOppack);
					this.tag = dpcheng.TagWznmLibMNOppack;
				}
				if (dpcheng.ListWznmQLibMNOppack) this.rst = dpcheng.ListWznmQLibMNOppack;
				if (dpcheng.StatAppQryWznmLibMNOppack) this.statappqry = dpcheng.StatAppQryWznmLibMNOppack;
				if (dpcheng.StatShrQryWznmLibMNOppack) this.statshrqry = dpcheng.StatShrQryWznmLibMNOppack;
				if (dpcheng.StgIacQryWznmLibMNOppack) this.stgiacqry = dpcheng.StgIacQryWznmLibMNOppack;
				/*
				...
				*/
				...
				if (dpcheng.ListWznmQLibMNOppack) {
					var rows = [];
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmLibMNOppackData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmLibMNOppackData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
