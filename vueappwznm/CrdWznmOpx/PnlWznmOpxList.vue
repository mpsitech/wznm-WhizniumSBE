...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmOpxList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmOpxListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmOpxList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmOpxList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmOpxListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmOpxListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmOpxList) this.contiac = dpcheng.ContIacWznmOpxList;
				if (dpcheng.ContInfWznmOpxList) this.continf = dpcheng.ContInfWznmOpxList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmOpxList) this.statshr = dpcheng.StatShrWznmOpxList;
				if (dpcheng.StgIacWznmOpxList) this.stgiac = dpcheng.StgIacWznmOpxList;
				if (dpcheng.TagWznmOpxList) {
					Wznm.unescapeBlock(dpcheng.TagWznmOpxList);
					this.tag = dpcheng.TagWznmOpxList;
				}
				if (dpcheng.ListWznmQOpxList) this.rst = dpcheng.ListWznmQOpxList;
				if (dpcheng.StatAppQryWznmOpxList) this.statappqry = dpcheng.StatAppQryWznmOpxList;
				if (dpcheng.StatShrQryWznmOpxList) this.statshrqry = dpcheng.StatShrQryWznmOpxList;
				if (dpcheng.StgIacQryWznmOpxList) this.stgiacqry = dpcheng.StgIacQryWznmOpxList;
				if (dpcheng.ContInfWznmOpxList) {
				...
				*/
				...
				if (dpcheng.TagWznmOpxList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoOpk", text: this.tag.TcoOpk}, {value: "TcoShd", text: this.tag.TcoShd});
					/*
					*/
				...
				if (dpcheng.ListWznmQOpxList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoOpk: rec["opk"], TcoShd: rec["shd"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmOpxListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmOpxListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
