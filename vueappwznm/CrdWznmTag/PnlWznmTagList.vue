...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmTagList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmTagListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmTagList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmTagList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmTagListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmTagListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmTagList) this.contiac = dpcheng.ContIacWznmTagList;
				if (dpcheng.ContInfWznmTagList) this.continf = dpcheng.ContInfWznmTagList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmTagList) this.statshr = dpcheng.StatShrWznmTagList;
				if (dpcheng.StgIacWznmTagList) this.stgiac = dpcheng.StgIacWznmTagList;
				if (dpcheng.TagWznmTagList) {
					Wznm.unescapeBlock(dpcheng.TagWznmTagList);
					this.tag = dpcheng.TagWznmTagList;
				}
				if (dpcheng.ListWznmQTagList) this.rst = dpcheng.ListWznmQTagList;
				if (dpcheng.StatAppQryWznmTagList) this.statappqry = dpcheng.StatAppQryWznmTagList;
				if (dpcheng.StatShrQryWznmTagList) this.statshrqry = dpcheng.StatShrQryWznmTagList;
				if (dpcheng.StgIacQryWznmTagList) this.stgiacqry = dpcheng.StgIacQryWznmTagList;
				if (dpcheng.ContInfWznmTagList) {
				...
				*/
				...
				if (dpcheng.TagWznmTagList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoCpb", text: this.tag.TcoCpb}, {value: "TcoGrp", text: this.tag.TcoGrp});
					/*
					*/
				...
				if (dpcheng.ListWznmQTagList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoTit: rec["tit"], TcoCpb: rec["cpb"], TcoGrp: rec["grp2"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmTagListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmTagListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
