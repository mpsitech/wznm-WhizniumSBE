...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmShtList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmShtListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmShtList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmShtList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmShtListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmShtListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmShtList) this.contiac = dpcheng.ContIacWznmShtList;
				if (dpcheng.ContInfWznmShtList) this.continf = dpcheng.ContInfWznmShtList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmShtList) this.statshr = dpcheng.StatShrWznmShtList;
				if (dpcheng.StgIacWznmShtList) this.stgiac = dpcheng.StgIacWznmShtList;
				if (dpcheng.TagWznmShtList) {
					Wznm.unescapeBlock(dpcheng.TagWznmShtList);
					this.tag = dpcheng.TagWznmShtList;
				}
				if (dpcheng.ListWznmQShtList) this.rst = dpcheng.ListWznmQShtList;
				if (dpcheng.StatAppQryWznmShtList) this.statappqry = dpcheng.StatAppQryWznmShtList;
				if (dpcheng.StatShrQryWznmShtList) this.statshrqry = dpcheng.StatShrQryWznmShtList;
				if (dpcheng.StgIacQryWznmShtList) this.stgiacqry = dpcheng.StgIacQryWznmShtList;
				if (dpcheng.ContInfWznmShtList) {
				...
				*/
				...
				if (dpcheng.TagWznmShtList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoTit", text: this.tag.TcoTit}, {value: "TcoVis", text: this.tag.TcoVis});
					/*
					*/
				...
				if (dpcheng.ListWznmQShtList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoTit: rec["tit"], TcoVis: rec["vis"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmShtListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmShtListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
