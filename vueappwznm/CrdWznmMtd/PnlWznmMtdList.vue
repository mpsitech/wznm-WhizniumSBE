...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmMtdList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmMtdListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmMtdList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmMtdList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmMtdListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmMtdListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmMtdList) this.contiac = dpcheng.ContIacWznmMtdList;
				if (dpcheng.ContInfWznmMtdList) this.continf = dpcheng.ContInfWznmMtdList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmMtdList) this.statshr = dpcheng.StatShrWznmMtdList;
				if (dpcheng.StgIacWznmMtdList) this.stgiac = dpcheng.StgIacWznmMtdList;
				if (dpcheng.TagWznmMtdList) {
					Wznm.unescapeBlock(dpcheng.TagWznmMtdList);
					this.tag = dpcheng.TagWznmMtdList;
				}
				if (dpcheng.ListWznmQMtdList) this.rst = dpcheng.ListWznmQMtdList;
				if (dpcheng.StatAppQryWznmMtdList) this.statappqry = dpcheng.StatAppQryWznmMtdList;
				if (dpcheng.StatShrQryWznmMtdList) this.statshrqry = dpcheng.StatShrQryWznmMtdList;
				if (dpcheng.StgIacQryWznmMtdList) this.stgiacqry = dpcheng.StgIacQryWznmMtdList;
				if (dpcheng.ContInfWznmMtdList) {
				...
				*/
				...
				if (dpcheng.TagWznmMtdList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoJob", text: this.tag.TcoJob}, {value: "TcoExs", text: this.tag.TcoExs});
					/*
					*/
				...
				if (dpcheng.ListWznmQMtdList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoJob: rec["job"], TcoExs: rec["exs"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmMtdListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmMtdListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
