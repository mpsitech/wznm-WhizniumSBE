...
<script>
	...
	export default {
		...
		methods: {
			...
			handleLogin: function() {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngWznmConfirm;

					if (dpcheng.accepted) {
						//console.log("CrdWznmStart.handleLogin() login accepted");
						...
						//console.log("CrdWznmStart.handleLogin() login denied");

						vm.fiFSge = getFiBySref(vm.feedFSge, "dny");
					}

				}).catch(function (error) {
					console.log("CrdWznmStart.handleLogin() error: " + error);
				});
			},
			...
			initSess: function(scrJrefSess) {
				...
				dpch.post('', data).then(function (response) {
					const dpcheng = response.data.DpchEngSessWznmData;

					vm.$emit("login", {scrJrefSess: scrJrefSess, scrJrefCrdnav: dpcheng.StatShrSessWznm.scrJrefCrdnav});

				}).catch(function (error) {
					console.log("CrdWznmStart.initSess() error: " + error);
				});
			},
			...
			handlePupLocaleChange: function() {
				...
				//console.log("CrdWznmStart.handlePupLocaleChange() new locale is " + this.fiFPupLocale.sref);
			},
		...
		computed: {
			CptVersion() {
				return(Wznm.title + " v" + Wznm.version);
			},
