...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmUsrList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmUsrListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmUsrList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmUsrList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmUsrListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmUsrListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmUsrList) this.contiac = dpcheng.ContIacWznmUsrList;
				if (dpcheng.ContInfWznmUsrList) this.continf = dpcheng.ContInfWznmUsrList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmUsrList) this.statshr = dpcheng.StatShrWznmUsrList;
				if (dpcheng.StgIacWznmUsrList) this.stgiac = dpcheng.StgIacWznmUsrList;
				if (dpcheng.TagWznmUsrList) {
					Wznm.unescapeBlock(dpcheng.TagWznmUsrList);
					this.tag = dpcheng.TagWznmUsrList;
				}
				if (dpcheng.ListWznmQUsrList) this.rst = dpcheng.ListWznmQUsrList;
				if (dpcheng.StatAppQryWznmUsrList) this.statappqry = dpcheng.StatAppQryWznmUsrList;
				if (dpcheng.StatShrQryWznmUsrList) this.statshrqry = dpcheng.StatShrQryWznmUsrList;
				if (dpcheng.StgIacQryWznmUsrList) this.stgiacqry = dpcheng.StgIacQryWznmUsrList;
				if (dpcheng.ContInfWznmUsrList) {
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmUsrListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmUsrListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
