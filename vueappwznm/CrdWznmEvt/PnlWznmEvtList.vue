...
<script>
	...
	*/
	...
	export default {
		...
		mounted() {
			//console.log("PnlWznmEvtList.mounted() with scrJref = " + this.scrJref);
		...
		methods: {
			...
			*/
			...
			handleSelect: function(obj) {
				...
				const dpchapp = {
					"DpchAppWznmEvtListData": {
						"scrJref": this.scrJref,
						"StgIacQryWznmEvtList": stgiacqry
					}
				};
			...
			updateEng: function(mask) {
				...
				if (mask.indexOf("contiac") != -1) dpchapp["ContIacWznmEvtList"] = this.contiac;

				//console.log(JSON.stringify({"DpchAppWznmEvtListData": dpchapp}))

				this.$emit("request", {scrJref: this.scrJref, dpchapp: {"DpchAppWznmEvtListData": dpchapp}, then: "handleDpchAppDataDoReply"});
			},
			...
			mergeDpchEngData: function(dpcheng) {
				...
				*/
				if (dpcheng.ContIacWznmEvtList) this.contiac = dpcheng.ContIacWznmEvtList;
				if (dpcheng.ContInfWznmEvtList) this.continf = dpcheng.ContInfWznmEvtList;
				if (dpcheng.FeedFCsiQst) this.feedFCsiQst = dpcheng.FeedFCsiQst;
				if (dpcheng.FeedFTos) this.feedFTos = dpcheng.FeedFTos;
				if (dpcheng.StatShrWznmEvtList) this.statshr = dpcheng.StatShrWznmEvtList;
				if (dpcheng.StgIacWznmEvtList) this.stgiac = dpcheng.StgIacWznmEvtList;
				if (dpcheng.TagWznmEvtList) {
					Wznm.unescapeBlock(dpcheng.TagWznmEvtList);
					this.tag = dpcheng.TagWznmEvtList;
				}
				if (dpcheng.ListWznmQEvtList) this.rst = dpcheng.ListWznmQEvtList;
				if (dpcheng.StatAppQryWznmEvtList) this.statappqry = dpcheng.StatAppQryWznmEvtList;
				if (dpcheng.StatShrQryWznmEvtList) this.statshrqry = dpcheng.StatShrQryWznmEvtList;
				if (dpcheng.StgIacQryWznmEvtList) this.stgiacqry = dpcheng.StgIacQryWznmEvtList;
				if (dpcheng.ContInfWznmEvtList) {
				...
				*/
				...
				if (dpcheng.TagWznmEvtList) {
					...
					/*
					*/
					tcos.push({value: "jnum", text: "jnum"}, {value: "TcoSrf", text: this.tag.TcoSrf}, {value: "TcoApp", text: this.tag.TcoApp});
					/*
					*/
				...
				if (dpcheng.ListWznmQEvtList) {
					...
					for (var i = 0; i < this.rst.length; i++) {
						...
						/*
						*/
						rows.push({jnum: rec["jnum"], TcoSrf: rec["srf"], TcoApp: rec["app"]})
						/*
						*/
			...
			handleDpchAppDataDoReply: function(srefIxWznmVDpch, dpcheng) {
				if (srefIxWznmVDpch == "DpchEngWznmEvtListData") this.mergeDpchEngData(dpcheng);
			},
			...
			*/
			...
			handleUpdate: function(obj) {
				/*
				*/
				if (obj.srefIxWznmVDpch == "DpchEngWznmEvtListData") this.mergeDpchEngData(obj.dpcheng);
				/*
				*/
			}
		},
