/**
  * \file PnlWznmRelRef1NPanel.js
  * web client functionality for panel PnlWznmRelRef1NPanel
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmRelRef1NPanel_aside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmRelRef1NPanel_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmRelRef1NPanel_bside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmRelRef1NPanel_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmRelRef1NPanel", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelRef1NPanel", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelRef1NPanel", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelRef1NPanel", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelRef1NPanel", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelRef1NPanel", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmRelRef1NPanel", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQRelRef1NPanel
		rownode = getRownode(srcdoc, "ListWznmQRelRef1NPanel", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmRelRef1NPanel", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmRelRef1NPanel", "ListWznmQRelRef1NPanel", retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmRelRef1NPanel", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelRef1NPanel", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelRef1NPanel", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmRelRef1NPanel", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmRelRef1NPanel", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmRelRef1NPanelData", scrJref, "StgIacQryWznmRelRef1NPanel");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmRelRef1NPanel", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmRelRef1NPanel", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelRef1NPanel", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmRelRef1NPanelDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "ContInfWznmRelRef1NPanel", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StatAppWznmRelRef1NPanel", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StatShrWznmRelRef1NPanel", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StgIacWznmRelRef1NPanel", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "TagWznmRelRef1NPanel", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "ListWznmQRelRef1NPanel", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StatAppQryWznmRelRef1NPanel", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StatShrQryWznmRelRef1NPanel", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmRelRef1NPanelData", "StgIacQryWznmRelRef1NPanel", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmRelRef1NPanelData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmRelRef1NPanelData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmRelRef1NPanelData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
