...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmRelList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmRelList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmRelList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmRelList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmRelList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmRelList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmRelList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmRelList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmRelList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmRelList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmRelList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmRelList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmRelList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmRelList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmRelList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmRelList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "ncol"));

		var TcoFrtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoFrtWidth"));
		var TcoFrsWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoFrsWidth"));
		var TcoTotWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoTotWidth"));
		var TcoTosWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoTosWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoTypWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoVerWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoSupWidth"));
		var TcoSrlWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoSrlWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoTblWidth"));
		var TcoPfxWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmRelList", "TcoPfxWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmRelList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoFrt"); doc.widths.push(TcoFrtWidth); doc.fs.push("frt"); doc.tos.push(1); wtot += TcoFrtWidth;
			} else if (i == 2) {
				doc.cols.push("TcoFrs"); doc.widths.push(TcoFrsWidth); doc.fs.push("frs"); doc.tos.push(2); wtot += TcoFrsWidth;
			} else if (i == 3) {
				doc.cols.push("TcoTot"); doc.widths.push(TcoTotWidth); doc.fs.push("tot"); doc.tos.push(3); wtot += TcoTotWidth;
			} else if (i == 4) {
				doc.cols.push("TcoTos"); doc.widths.push(TcoTosWidth); doc.fs.push("tos"); doc.tos.push(4); wtot += TcoTosWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ2"); doc.tos.push(0); wtot += TcoTypWidth;
			} else if (i == 6) {
				doc.cols.push("TcoVer"); doc.widths.push(TcoVerWidth); doc.fs.push("ver"); doc.tos.push(6); wtot += TcoVerWidth;
			} else if (i == 7) {
				doc.cols.push("TcoSup"); doc.widths.push(TcoSupWidth); doc.fs.push("sup"); doc.tos.push(7); wtot += TcoSupWidth;
			} else if (i == 8) {
				doc.cols.push("TcoSrl"); doc.widths.push(TcoSrlWidth); doc.fs.push("srl2"); doc.tos.push(0); wtot += TcoSrlWidth;
			} else if (i == 9) {
				doc.cols.push("TcoTbl"); doc.widths.push(TcoTblWidth); doc.fs.push("tbl"); doc.tos.push(9); wtot += TcoTblWidth;
			} else if (i == 10) {
				doc.cols.push("TcoPfx"); doc.widths.push(TcoPfxWidth); doc.fs.push("pfx"); doc.tos.push(0); wtot += TcoPfxWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmRelList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmRelList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQRelList
		rownode = getRownode(srcdoc, "ListWznmQRelList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmRelList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmRelList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmRelList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmRelList", "ListWznmQRelList", retrieveTi(srcdoc, "TagWznmRelList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmRelList", "TxtRecord2"),
				["Frt","Frs","Tot","Tos","","Typ","Ver","Sup","","Srl","Tbl","Pfx"],
				["frt","frs","tot","tos","typ","typ2","ver","sup","srl","srl2","tbl","pfx"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmRelList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmRelList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmRelList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmRelListData", scrJref, "StgIacQryWznmRelList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmRelList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmRelList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmRelList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmRelListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmRelList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmRelList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmRelListData", scrJref, "ContIacWznmRelList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "ContIacWznmRelList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "ContInfWznmRelList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "StatShrWznmRelList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "StgIacWznmRelList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "TagWznmRelList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "ListWznmQRelList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "StatAppQryWznmRelList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "StatShrQryWznmRelList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmRelListData", "StgIacQryWznmRelList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmRelListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmRelListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmRelListData") {
				mask = mergeDpchEngData(dom);
