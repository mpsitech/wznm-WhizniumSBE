/**
  * \file PnlWznmStbMNSquawk.js
  * web client functionality for panel PnlWznmStbMNSquawk
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNSquawk", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNSquawk_side").setAttribute("height", "30");
	doc.getElementById("MNSquawk_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNSquawk_side").src = "./PnlWznmStbMNSquawk_aside.html";
	doc.getElementById("MNSquawk_cont").src = "./PnlWznmStbMNSquawk_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNSquawk", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNSquawk_side").setAttribute("height", "255");
	doc.getElementById("MNSquawk_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNSquawk_side").src = "./PnlWznmStbMNSquawk_bside.html";
	doc.getElementById("MNSquawk_cont").src = "./PnlWznmStbMNSquawk_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNSquawk_side").contentDocument;
	contdoc = doc.getElementById("MNSquawk_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNSquawk_side").contentDocument;
	contdoc = doc.getElementById("MNSquawk_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNSquawk_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNSquawk_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNSquawk_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbMNSquawk", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmStbMNSquawk", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbMNSquawk", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmStbMNSquawk", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNSquawk", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNSquawk", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNSquawk", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNSquawk", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbMNSquawk", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmStbMNSquawk", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmStbMNSquawk", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQStbMNSquawk
		rownode = getRownode(srcdoc, "ListWznmQStbMNSquawk", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmStbMNSquawk", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmStbMNSquawk", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmStbMNSquawk", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNSquawk", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSquawk_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNSquawk_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNSquawk_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNSquawk_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmStbMNSquawk", "ListWznmQStbMNSquawk", retrieveTi(srcdoc, "TagWznmStbMNSquawk", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmStbMNSquawk", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNSquawk", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNSquawk", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmStbMNSquawk", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmStbMNSquawk", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmStbMNSquawkData", scrJref, "StgIacQryWznmStbMNSquawk");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNSquawk", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmStbMNSquawk", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmStbMNSquawk", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNSquawk", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmStbMNSquawkDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "ContInfWznmStbMNSquawk", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StatAppWznmStbMNSquawk", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StatShrWznmStbMNSquawk", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StgIacWznmStbMNSquawk", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "TagWznmStbMNSquawk", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "ListWznmQStbMNSquawk", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StatAppQryWznmStbMNSquawk", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StatShrQryWznmStbMNSquawk", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmStbMNSquawkData", "StgIacQryWznmStbMNSquawk", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmStbMNSquawkData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmStbMNSquawkData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNSquawk");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmStbMNSquawkData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
