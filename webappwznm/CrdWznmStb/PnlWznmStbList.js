...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmStbList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmStbList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmStbList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmStbList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmStbList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmStbList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmStbList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmStbList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmStbList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmStbList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmStbList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmStbList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmStbList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbList", "TcoSrfWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbList", "TcoTypWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbList", "TcoTblWidth"));
		var TcoSbsWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbList", "TcoSbsWidth"));
		var TcoExaWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbList", "TcoExaWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmStbList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSbs"); doc.widths.push(TcoSbsWidth); doc.fs.push("sbs"); doc.tos.push(4); wtot += TcoSbsWidth;
			} else if (i == 5) {
				doc.cols.push("TcoExa"); doc.widths.push(TcoExaWidth); doc.fs.push("exa"); doc.tos.push(0); wtot += TcoExaWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmStbList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmStbList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQStbList
		rownode = getRownode(srcdoc, "ListWznmQStbList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmStbList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmStbList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmStbList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmStbList", "ListWznmQStbList", retrieveTi(srcdoc, "TagWznmStbList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmStbList", "TxtRecord2"),
				["Srf","","Typ","Tbl","Sbs","Exa"],
				["srf","typ","typ2","tbl","sbs","exa"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmStbList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmStbList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmStbListData", scrJref, "StgIacQryWznmStbList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmStbList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmStbListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmStbList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmStbList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmStbListData", scrJref, "ContIacWznmStbList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "ContIacWznmStbList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "ContInfWznmStbList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "StatShrWznmStbList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "StgIacWznmStbList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "TagWznmStbList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "ListWznmQStbList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "StatAppQryWznmStbList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "StatShrQryWznmStbList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmStbListData", "StgIacQryWznmStbList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmStbListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmStbListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmStbListData") {
				mask = mergeDpchEngData(dom);
