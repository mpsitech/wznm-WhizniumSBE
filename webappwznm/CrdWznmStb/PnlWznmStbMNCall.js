/**
  * \file PnlWznmStbMNCall.js
  * web client functionality for panel PnlWznmStbMNCall
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNCall", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNCall_side").setAttribute("height", "30");
	doc.getElementById("MNCall_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNCall_side").src = "./PnlWznmStbMNCall_aside.html";
	doc.getElementById("MNCall_cont").src = "./PnlWznmStbMNCall_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNCall", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNCall_side").setAttribute("height", "255");
	doc.getElementById("MNCall_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNCall_side").src = "./PnlWznmStbMNCall_bside.html";
	doc.getElementById("MNCall_cont").src = "./PnlWznmStbMNCall_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNCall_side").contentDocument;
	contdoc = doc.getElementById("MNCall_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNCall_side").contentDocument;
	contdoc = doc.getElementById("MNCall_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNCall_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNCall_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNCall_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbMNCall", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmStbMNCall", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbMNCall", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmStbMNCall", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNCall", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNCall", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNCall", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNCall", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbMNCall", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmStbMNCall", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmStbMNCall", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQStbMNCall
		rownode = getRownode(srcdoc, "ListWznmQStbMNCall", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmStbMNCall", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmStbMNCall", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmStbMNCall", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNCall", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCall_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCall_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNCall_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNCall_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmStbMNCall", "ListWznmQStbMNCall", retrieveTi(srcdoc, "TagWznmStbMNCall", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmStbMNCall", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNCall", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbMNCall", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmStbMNCall", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmStbMNCall", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmStbMNCallData", scrJref, "StgIacQryWznmStbMNCall");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbMNCall", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmStbMNCall", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmStbMNCall", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbMNCall", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmStbMNCallDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "ContInfWznmStbMNCall", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StatAppWznmStbMNCall", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StatShrWznmStbMNCall", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StgIacWznmStbMNCall", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "TagWznmStbMNCall", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "ListWznmQStbMNCall", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StatAppQryWznmStbMNCall", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StatShrQryWznmStbMNCall", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmStbMNCallData", "StgIacQryWznmStbMNCall", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmStbMNCallData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmStbMNCallData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNCall");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmStbMNCallData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
