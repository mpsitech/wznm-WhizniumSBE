/**
  * \file PnlWznmStbSupMNStub.js
  * web client functionality for panel PnlWznmStbSupMNStub
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("SupMNStub", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("SupMNStub_side").setAttribute("height", "30");
	doc.getElementById("SupMNStub_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("SupMNStub_side").src = "./PnlWznmStbSupMNStub_aside.html";
	doc.getElementById("SupMNStub_cont").src = "./PnlWznmStbSupMNStub_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("SupMNStub", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("SupMNStub_side").setAttribute("height", "255");
	doc.getElementById("SupMNStub_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("SupMNStub_side").src = "./PnlWznmStbSupMNStub_bside.html";
	doc.getElementById("SupMNStub_cont").src = "./PnlWznmStbSupMNStub_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("SupMNStub_side").contentDocument;
	contdoc = doc.getElementById("SupMNStub_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("SupMNStub_side").contentDocument;
	contdoc = doc.getElementById("SupMNStub_cont").contentDocument;
	hdrdoc = contdoc.getElementById("SupMNStub_hdr").contentDocument;
	tbldoc = contdoc.getElementById("SupMNStub_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("SupMNStub_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbSupMNStub", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmStbSupMNStub", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmStbSupMNStub", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmStbSupMNStub", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbSupMNStub", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbSupMNStub", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbSupMNStub", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbSupMNStub", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmStbSupMNStub", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmStbSupMNStub", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmStbSupMNStub", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQStbSupMNStub
		rownode = getRownode(srcdoc, "ListWznmQStbSupMNStub", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmStbSupMNStub", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmStbSupMNStub", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmStbSupMNStub", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("SupMNStub", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("SupMNStub_side").setAttribute("height", "" + (height+55));
	doc.getElementById("SupMNStub_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("SupMNStub_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("SupMNStub_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmStbSupMNStub", "ListWznmQStbSupMNStub", retrieveTi(srcdoc, "TagWznmStbSupMNStub", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmStbSupMNStub", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbSupMNStub", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmStbSupMNStub", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmStbSupMNStub", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmStbSupMNStub", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmStbSupMNStubData", scrJref, "StgIacQryWznmStbSupMNStub");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmStbSupMNStub", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmStbSupMNStub", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmStbSupMNStub", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmStbSupMNStub", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmStbSupMNStubDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "ContInfWznmStbSupMNStub", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StatAppWznmStbSupMNStub", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StatShrWznmStbSupMNStub", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StgIacWznmStbSupMNStub", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "TagWznmStbSupMNStub", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "ListWznmQStbSupMNStub", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StatAppQryWznmStbSupMNStub", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StatShrQryWznmStbSupMNStub", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmStbSupMNStubData", "StgIacQryWznmStbSupMNStub", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmStbSupMNStubData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmStbSupMNStubData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("SupMNStub");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmStbSupMNStubData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
