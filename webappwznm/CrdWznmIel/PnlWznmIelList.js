...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmIelList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmIelList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmIelList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmIelList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIelList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmIelList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmIelList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmIelList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmIelList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIelList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmIelList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIelList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmIelList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmIelList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIelList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIelList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIelList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIelList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmIelList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmIelList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoSrfWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoShoWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoTypWidth"));
		var TcoImeWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoImeWidth"));
		var TcoTcoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoTcoWidth"));
		var TcoCtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIelList", "TcoCtyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmIelList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoIme"); doc.widths.push(TcoImeWidth); doc.fs.push("ime"); doc.tos.push(3); wtot += TcoImeWidth;
			} else if (i == 5) {
				doc.cols.push("TcoTco"); doc.widths.push(TcoTcoWidth); doc.fs.push("tco"); doc.tos.push(4); wtot += TcoTcoWidth;
			} else if (i == 6) {
				doc.cols.push("TcoCty"); doc.widths.push(TcoCtyWidth); doc.fs.push("cty2"); doc.tos.push(0); wtot += TcoCtyWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmIelList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmIelList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQIelList
		rownode = getRownode(srcdoc, "ListWznmQIelList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmIelList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmIelList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmIelList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmIelList", "ListWznmQIelList", retrieveTi(srcdoc, "TagWznmIelList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmIelList", "TxtRecord2"),
				["Srf","Sho","","Typ","Ime","Tco","","Cty"],
				["srf","sho","typ","typ2","ime","tco","cty","cty2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIelList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIelList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmIelList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmIelList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmIelListData", scrJref, "StgIacQryWznmIelList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIelList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmIelList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIelList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmIelListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmIelList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmIelList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmIelListData", scrJref, "ContIacWznmIelList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "ContIacWznmIelList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "ContInfWznmIelList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "StatShrWznmIelList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "StgIacWznmIelList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "TagWznmIelList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "ListWznmQIelList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "StatAppQryWznmIelList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "StatShrQryWznmIelList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmIelListData", "StgIacQryWznmIelList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmIelListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmIelListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmIelListData") {
				mask = mergeDpchEngData(dom);
