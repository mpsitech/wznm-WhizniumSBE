/**
  * \file PnlWznmCapAPar.js
  * web client functionality for panel PnlWznmCapAPar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWznmCapAPar_aside.html";
	doc.getElementById("APar_cont").src = "./PnlWznmCapAPar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlWznmCapAPar_bside.html";
	doc.getElementById("APar_cont").src = "./PnlWznmCapAPar_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCapAPar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCapAPar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCapAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCapAPar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCapAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCapAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCapAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCapAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCapAPar", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCapAPar", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCapAPar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCapAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCapAPar
		rownode = getRownode(srcdoc, "ListWznmQCapAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCapAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCapAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCapAPar", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCapAPar", "ListWznmQCapAPar", retrieveTi(srcdoc, "TagWznmCapAPar", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCapAPar", "TxtRecord2"),
				["Key","","Val"],
				["key","key2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCapAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCapAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCapAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCapAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCapAParData", scrJref, "StgIacQryWznmCapAPar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCapAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCapAPar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCapAPar", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCapAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCapAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "ContInfWznmCapAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StatAppWznmCapAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StatShrWznmCapAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StgIacWznmCapAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "TagWznmCapAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "ListWznmQCapAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StatAppQryWznmCapAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StatShrQryWznmCapAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCapAParData", "StgIacQryWznmCapAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCapAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCapAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCapAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
