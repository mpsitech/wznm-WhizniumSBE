...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("OrgMNBox", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("OrgMNBox_side").setAttribute("height", "30");
	doc.getElementById("OrgMNBox_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("OrgMNBox_side").src = "./PnlWznmBoxOrgMNBox_aside.html";
	doc.getElementById("OrgMNBox_cont").src = "./PnlWznmBoxOrgMNBox_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("OrgMNBox", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("OrgMNBox_side").setAttribute("height", "255");
	doc.getElementById("OrgMNBox_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("OrgMNBox_side").src = "./PnlWznmBoxOrgMNBox_bside.html";
	doc.getElementById("OrgMNBox_cont").src = "./PnlWznmBoxOrgMNBox_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("OrgMNBox_side").contentDocument;
	contdoc = doc.getElementById("OrgMNBox_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("OrgMNBox_side").contentDocument;
	contdoc = doc.getElementById("OrgMNBox_cont").contentDocument;
	hdrdoc = contdoc.getElementById("OrgMNBox_hdr").contentDocument;
	tbldoc = contdoc.getElementById("OrgMNBox_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("OrgMNBox_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBoxOrgMNBox", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxOrgMNBox", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxOrgMNBox", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxOrgMNBox", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxOrgMNBox", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxOrgMNBox", "TcoMrefWidth"));
		var TcoOclWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxOrgMNBox", "TcoOclWidth"));
		var TcoDclWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxOrgMNBox", "TcoDclWidth"));
		var TcoWgtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxOrgMNBox", "TcoWgtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQBoxOrgMNBox
		rownode = getRownode(srcdoc, "ListWznmQBoxOrgMNBox", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmBoxOrgMNBox", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("OrgMNBox", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNBox_side").setAttribute("height", "" + (height+55));
	doc.getElementById("OrgMNBox_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("OrgMNBox_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("OrgMNBox_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmBoxOrgMNBox", "ListWznmQBoxOrgMNBox", retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmBoxOrgMNBox", "TxtRecord2"),
				["Mref","Ocl","Dcl","Wgt"],
				["mref","ocl","dcl","wgt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxOrgMNBox", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxOrgMNBox", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmBoxOrgMNBox", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmBoxOrgMNBox", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBoxOrgMNBoxData", scrJref, "StgIacQryWznmBoxOrgMNBox");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmBoxOrgMNBox", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmBoxOrgMNBox", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxOrgMNBox", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmBoxOrgMNBoxDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "ContInfWznmBoxOrgMNBox", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StatAppWznmBoxOrgMNBox", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StatShrWznmBoxOrgMNBox", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StgIacWznmBoxOrgMNBox", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "TagWznmBoxOrgMNBox", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "ListWznmQBoxOrgMNBox", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StatAppQryWznmBoxOrgMNBox", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StatShrQryWznmBoxOrgMNBox", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxOrgMNBoxData", "StgIacQryWznmBoxOrgMNBox", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmBoxOrgMNBoxData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmBoxOrgMNBoxData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("OrgMNBox");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmBoxOrgMNBoxData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
