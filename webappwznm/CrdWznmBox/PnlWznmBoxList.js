...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmBoxList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmBoxList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmBoxList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmBoxList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmBoxList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmBoxList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmBoxList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmBoxList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBoxList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmBoxList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBoxList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmBoxList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmBoxList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "ncol"));

		var TcoShtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxList", "TcoShtWidth"));
		var TcoMtbWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxList", "TcoMtbWidth"));
		var TcoUnvWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxList", "TcoUnvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmBoxList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoSht"); doc.widths.push(TcoShtWidth); doc.fs.push("sht"); doc.tos.push(3); wtot += TcoShtWidth;
			} else if (i == 2) {
				doc.cols.push("TcoMtb"); doc.widths.push(TcoMtbWidth); doc.fs.push("mtb2"); doc.tos.push(0); wtot += TcoMtbWidth;
			} else if (i == 3) {
				doc.cols.push("TcoUnv"); doc.widths.push(TcoUnvWidth); doc.fs.push("unv"); doc.tos.push(1); wtot += TcoUnvWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmBoxList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmBoxList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQBoxList
		rownode = getRownode(srcdoc, "ListWznmQBoxList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmBoxList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmBoxList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmBoxList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmBoxList", "ListWznmQBoxList", retrieveTi(srcdoc, "TagWznmBoxList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmBoxList", "TxtRecord2"),
				["Sht","","Mtb","Unv"],
				["sht","mtb","mtb2","unv"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmBoxList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmBoxList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBoxListData", scrJref, "StgIacQryWznmBoxList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmBoxList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmBoxListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmBoxList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmBoxList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBoxListData", scrJref, "ContIacWznmBoxList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "ContIacWznmBoxList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "ContInfWznmBoxList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "StatShrWznmBoxList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "StgIacWznmBoxList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "TagWznmBoxList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "ListWznmQBoxList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "StatAppQryWznmBoxList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "StatShrQryWznmBoxList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxListData", "StgIacQryWznmBoxList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmBoxListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmBoxListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmBoxListData") {
				mask = mergeDpchEngData(dom);
