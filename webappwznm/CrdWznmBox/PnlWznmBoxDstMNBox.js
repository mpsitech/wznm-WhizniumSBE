...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("DstMNBox", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("DstMNBox_side").setAttribute("height", "30");
	doc.getElementById("DstMNBox_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("DstMNBox_side").src = "./PnlWznmBoxDstMNBox_aside.html";
	doc.getElementById("DstMNBox_cont").src = "./PnlWznmBoxDstMNBox_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("DstMNBox", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("DstMNBox_side").setAttribute("height", "255");
	doc.getElementById("DstMNBox_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("DstMNBox_side").src = "./PnlWznmBoxDstMNBox_bside.html";
	doc.getElementById("DstMNBox_cont").src = "./PnlWznmBoxDstMNBox_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("DstMNBox_side").contentDocument;
	contdoc = doc.getElementById("DstMNBox_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("DstMNBox_side").contentDocument;
	contdoc = doc.getElementById("DstMNBox_cont").contentDocument;
	hdrdoc = contdoc.getElementById("DstMNBox_hdr").contentDocument;
	tbldoc = contdoc.getElementById("DstMNBox_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("DstMNBox_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBoxDstMNBox", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxDstMNBox", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxDstMNBox", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxDstMNBox", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxDstMNBox", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxDstMNBox", "TcoMrefWidth"));
		var TcoOclWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxDstMNBox", "TcoOclWidth"));
		var TcoDclWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxDstMNBox", "TcoDclWidth"));
		var TcoWgtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBoxDstMNBox", "TcoWgtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoOcl"); doc.widths.push(TcoOclWidth); doc.fs.push("ocl"); wtot += TcoOclWidth;
			} else if (i == 3) {
				doc.cols.push("TcoDcl"); doc.widths.push(TcoDclWidth); doc.fs.push("dcl"); wtot += TcoDclWidth;
			} else if (i == 4) {
				doc.cols.push("TcoWgt"); doc.widths.push(TcoWgtWidth); doc.fs.push("wgt"); wtot += TcoWgtWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmBoxDstMNBox", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQBoxDstMNBox
		rownode = getRownode(srcdoc, "ListWznmQBoxDstMNBox", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmBoxDstMNBox", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("DstMNBox", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("DstMNBox_side").setAttribute("height", "" + (height+55));
	doc.getElementById("DstMNBox_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("DstMNBox_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("DstMNBox_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmBoxDstMNBox", "ListWznmQBoxDstMNBox", retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmBoxDstMNBox", "TxtRecord2"),
				["Mref","Ocl","Dcl","Wgt"],
				["mref","ocl","dcl","wgt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxDstMNBox", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBoxDstMNBox", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmBoxDstMNBox", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmBoxDstMNBox", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBoxDstMNBoxData", scrJref, "StgIacQryWznmBoxDstMNBox");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmBoxDstMNBox", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmBoxDstMNBox", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBoxDstMNBox", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmBoxDstMNBoxDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "ContInfWznmBoxDstMNBox", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StatAppWznmBoxDstMNBox", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StatShrWznmBoxDstMNBox", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StgIacWznmBoxDstMNBox", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "TagWznmBoxDstMNBox", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "ListWznmQBoxDstMNBox", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StatAppQryWznmBoxDstMNBox", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StatShrQryWznmBoxDstMNBox", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmBoxDstMNBoxData", "StgIacQryWznmBoxDstMNBox", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmBoxDstMNBoxData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmBoxDstMNBoxData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("DstMNBox");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmBoxDstMNBoxData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
