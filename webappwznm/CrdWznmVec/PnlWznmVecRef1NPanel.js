/**
  * \file PnlWznmVecRef1NPanel.js
  * web client functionality for panel PnlWznmVecRef1NPanel
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NPanel", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NPanel_side").setAttribute("height", "30");
	doc.getElementById("Ref1NPanel_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmVecRef1NPanel_aside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmVecRef1NPanel_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NPanel", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NPanel_side").setAttribute("height", "255");
	doc.getElementById("Ref1NPanel_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmVecRef1NPanel_bside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmVecRef1NPanel_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NPanel_side").contentDocument;
	contdoc = doc.getElementById("Ref1NPanel_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NPanel_side").contentDocument;
	contdoc = doc.getElementById("Ref1NPanel_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NPanel_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NPanel_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NPanel_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVecRef1NPanel", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecRef1NPanel", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecRef1NPanel", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecRef1NPanel", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecRef1NPanel", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecRef1NPanel", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVecRef1NPanel", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVecRef1NPanel
		rownode = getRownode(srcdoc, "ListWznmQVecRef1NPanel", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVecRef1NPanel", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NPanel", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NPanel_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NPanel_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NPanel_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NPanel_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVecRef1NPanel", "ListWznmQVecRef1NPanel", retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVecRef1NPanel", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecRef1NPanel", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecRef1NPanel", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVecRef1NPanel", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVecRef1NPanel", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVecRef1NPanelData", scrJref, "StgIacQryWznmVecRef1NPanel");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVecRef1NPanel", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVecRef1NPanel", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecRef1NPanel", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVecRef1NPanelDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "ContInfWznmVecRef1NPanel", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StatAppWznmVecRef1NPanel", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StatShrWznmVecRef1NPanel", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StgIacWznmVecRef1NPanel", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "TagWznmVecRef1NPanel", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "ListWznmQVecRef1NPanel", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StatAppQryWznmVecRef1NPanel", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StatShrQryWznmVecRef1NPanel", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVecRef1NPanelData", "StgIacQryWznmVecRef1NPanel", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVecRef1NPanelData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVecRef1NPanelData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NPanel");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVecRef1NPanelData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
