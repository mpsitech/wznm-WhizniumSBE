/**
  * \file PnlWznmVecVec1NVectoritem.js
  * web client functionality for panel PnlWznmVecVec1NVectoritem
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Vec1NVectoritem_side").src = "./PnlWznmVecVec1NVectoritem_aside.html";
	doc.getElementById("Vec1NVectoritem_cont").src = "./PnlWznmVecVec1NVectoritem_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Vec1NVectoritem_side").src = "./PnlWznmVecVec1NVectoritem_bside.html";
	doc.getElementById("Vec1NVectoritem_cont").src = "./PnlWznmVecVec1NVectoritem_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVecVec1NVectoritem", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecVec1NVectoritem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecVec1NVectoritem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecVec1NVectoritem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecVec1NVectoritem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecVec1NVectoritem", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVecVec1NVectoritem
		rownode = getRownode(srcdoc, "ListWznmQVecVec1NVectoritem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVecVec1NVectoritem", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVecVec1NVectoritem", "ListWznmQVecVec1NVectoritem", retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVecVec1NVectoritem", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecVec1NVectoritem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecVec1NVectoritem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVecVec1NVectoritem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVecVec1NVectoritem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVecVec1NVectoritemData", scrJref, "StgIacQryWznmVecVec1NVectoritem");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVecVec1NVectoritem", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVecVec1NVectoritem", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecVec1NVectoritem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVecVec1NVectoritemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "ContInfWznmVecVec1NVectoritem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StatAppWznmVecVec1NVectoritem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StatShrWznmVecVec1NVectoritem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StgIacWznmVecVec1NVectoritem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "TagWznmVecVec1NVectoritem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "ListWznmQVecVec1NVectoritem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StatAppQryWznmVecVec1NVectoritem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StatShrQryWznmVecVec1NVectoritem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVecVec1NVectoritemData", "StgIacQryWznmVecVec1NVectoritem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVecVec1NVectoritemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVecVec1NVectoritemData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVecVec1NVectoritemData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
