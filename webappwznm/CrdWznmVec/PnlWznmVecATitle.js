/**
  * \file PnlWznmVecATitle.js
  * web client functionality for panel PnlWznmVecATitle
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("ATitle_side").src = "./PnlWznmVecATitle_aside.html";
	doc.getElementById("ATitle_cont").src = "./PnlWznmVecATitle_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("ATitle_side").src = "./PnlWznmVecATitle_bside.html";
	doc.getElementById("ATitle_cont").src = "./PnlWznmVecATitle_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecATitle", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVecATitle", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecATitle", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVecATitle", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecATitle", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecATitle", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecATitle", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecATitle", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecATitle", "TcoTypWidth"));
		var TcoLocWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecATitle", "TcoLocWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecATitle", "TcoTitWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVecATitle", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVecATitle", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVecATitle
		rownode = getRownode(srcdoc, "ListWznmQVecATitle", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVecATitle", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVecATitle", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVecATitle", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVecATitle", "ListWznmQVecATitle", retrieveTi(srcdoc, "TagWznmVecATitle", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVecATitle", "TxtRecord2"),
				["Typ","","Loc","Tit"],
				["typ","typ2","loc","tit"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecATitle", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecATitle", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVecATitle", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVecATitle", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVecATitleData", scrJref, "StgIacQryWznmVecATitle");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecATitle", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVecATitle", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVecATitle", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecATitle", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVecATitleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "ContInfWznmVecATitle", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StatAppWznmVecATitle", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StatShrWznmVecATitle", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StgIacWznmVecATitle", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "TagWznmVecATitle", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "ListWznmQVecATitle", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StatAppQryWznmVecATitle", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StatShrQryWznmVecATitle", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVecATitleData", "StgIacQryWznmVecATitle", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVecATitleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVecATitleData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVecATitleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
