/**
  * \file PnlWznmVecMNTable.js
  * web client functionality for panel PnlWznmVecMNTable
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNTable_side").src = "./PnlWznmVecMNTable_aside.html";
	doc.getElementById("MNTable_cont").src = "./PnlWznmVecMNTable_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNTable_side").src = "./PnlWznmVecMNTable_bside.html";
	doc.getElementById("MNTable_cont").src = "./PnlWznmVecMNTable_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecMNTable", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVecMNTable", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVecMNTable", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVecMNTable", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecMNTable", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecMNTable", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecMNTable", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecMNTable", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecMNTable", "TcoMrefWidth"));
		var TcoSbsWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVecMNTable", "TcoSbsWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVecMNTable", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVecMNTable", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVecMNTable
		rownode = getRownode(srcdoc, "ListWznmQVecMNTable", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVecMNTable", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVecMNTable", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVecMNTable", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVecMNTable", "ListWznmQVecMNTable", retrieveTi(srcdoc, "TagWznmVecMNTable", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVecMNTable", "TxtRecord2"),
				["Mref","Sbs"],
				["mref","sbs"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecMNTable", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVecMNTable", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVecMNTable", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVecMNTable", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVecMNTableData", scrJref, "StgIacQryWznmVecMNTable");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVecMNTable", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVecMNTable", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVecMNTable", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVecMNTable", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVecMNTableDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "ContInfWznmVecMNTable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StatAppWznmVecMNTable", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StatShrWznmVecMNTable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StgIacWznmVecMNTable", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "TagWznmVecMNTable", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "ListWznmQVecMNTable", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StatAppQryWznmVecMNTable", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StatShrQryWznmVecMNTable", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVecMNTableData", "StgIacQryWznmVecMNTable", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVecMNTableData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVecMNTableData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVecMNTableData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
