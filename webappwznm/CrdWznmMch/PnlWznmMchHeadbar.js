...
function getMenCptwidth(short) {
	return(parseInt(retrieveSi(srcdoc, "StgInfWznmMchHeadbar", "Men" + short + "Cptwidth")));
};
...
function init() {
	...
	setTextContent(contdoc, contdoc.getElementById("MenApp"), "\\u00a0" + retrieveTi(srcdoc, "TagWznmMchHeadbar", "MenApp") + "\\u00a0");
	w = getMenCptwidth("Crd"); contdoc.getElementById("colMenCrd").setAttribute("width", "" + w); wFill -= w + 1;
	setTextContent(contdoc, contdoc.getElementById("MenCrd"), "\\u00a0" + retrieveTi(srcdoc, "TagWznmMchHeadbar", "MenCrd") + "\\u00a0");
...
function handleMenAppClick() {
	getCrdwnd().toggleMenu("App", getMenLeft("App"), parseInt(retrieveSi(srcdoc, "StgInfWznmMchHeadbar", "MenAppWidth")), 42);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmMchHeadbarData", "StgInfWznmMchHeadbar", srcdoc)) mask.push("stginf");
	if (updateSrcblock(dom, "DpchEngWznmMchHeadbarData", "TagWznmMchHeadbar", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmMchHeadbarData") {
