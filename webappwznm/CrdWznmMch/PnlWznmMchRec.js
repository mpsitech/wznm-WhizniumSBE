...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmMchRec", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmMchRec", "initdone" + pnlshort, "true");
...
function checkInitdone() {
	...
	if (!initdoneDetail) {
		...
		rhsdoc.getElementById("Sup1NMachine").src = "./PnlWznmMchSup1NMachine.html?scrJref=" + scrJrefSup1NMachine;
...
function minimize() {
	...
	doc.getElementById("Rec_side").src = "./PnlWznmMchRec_aside.html";
	doc.getElementById("Rec_cont").src = "./PnlWznmMchRec_a.html";
};
...
function regularize() {
	...
	doc.getElementById("Rec_side").src = "./PnlWznmMchRec_bside.html";
	doc.getElementById("Rec_cont").src = "./PnlWznmMchRec_b.html";
};
...
function initA() {
	...
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMchRec", "Cpt"));
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMchRec", "Cpt"));

	checkInitdone();
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmMchRec", "srefIxWznmVExpstate");
...
function refreshAside() {
	...
	ButRegularizeActive = (retrieveSi(srcdoc, "StatShrWznmMchRec", "ButRegularizeActive") == "true");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWznmMchRec", "TxtRef"));
};

function refreshB() {

	setTextContent(hdrdoc, hdrdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWznmMchRec", "TxtRef"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmMchRecDo", scrJref, ctlsref + "Click");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmMchRecData", "ContInfWznmMchRec", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmMchRecData", "StatAppWznmMchRec", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmMchRecData", "StatShrWznmMchRec", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmMchRecData", "TagWznmMchRec", srcdoc)) mask.push("tag");
...
function handleDpchEng(dom, dpch) {
	...
	srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmMchRec", "srefIxWznmVExpstate");

	if (_scrJref == scrJref) {
		if (dpch == "DpchEngWznmMchRecData") {
			mask = mergeDpchEngData(dom);

			if (mask.indexOf("statshr") != -1) {
				newSrefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmMchRec", "srefIxWznmVExpstate");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmMchRecData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmMchRecData") {
				srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmMchRec", "srefIxWznmVExpstate");
