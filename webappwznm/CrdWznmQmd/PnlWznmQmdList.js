/**
  * \file PnlWznmQmdList.js
  * web client functionality for panel PnlWznmQmdList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmQmdList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQmdList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmQmdList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQmdList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQmdList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQmdList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmQmdList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmQmdList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQmdList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQmdList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQmdList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQmdList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQmdList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQmdList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQmdList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQmdList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQmdList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQmdList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQmdList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQmdList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQmdList", "TcoTypWidth"));
		var TcoQryWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQmdList", "TcoQryWidth"));
		var TcoPstWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQmdList", "TcoPstWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQmdList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQmdList", "TcoReuWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQmdList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoQry"); doc.widths.push(TcoQryWidth); doc.fs.push("qry"); doc.tos.push(5); wtot += TcoQryWidth;
			} else if (i == 3) {
				doc.cols.push("TcoPst"); doc.widths.push(TcoPstWidth); doc.fs.push("pst"); doc.tos.push(3); wtot += TcoPstWidth;
			} else if (i == 4) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmQmdList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQmdList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQmdList
		rownode = getRownode(srcdoc, "ListWznmQQmdList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQmdList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQmdList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQmdList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQmdList", "ListWznmQQmdList", retrieveTi(srcdoc, "TagWznmQmdList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQmdList", "TxtRecord2"),
				["","Typ","Qry","Pst","","Ret","Reu"],
				["typ","typ2","qry","pst","ret","ret2","reu"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQmdList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQmdList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQmdList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQmdList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQmdListData", scrJref, "StgIacQryWznmQmdList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQmdList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQmdList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQmdList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQmdListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQmdList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmQmdList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQmdListData", scrJref, "ContIacWznmQmdList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "ContIacWznmQmdList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "ContInfWznmQmdList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "StatShrWznmQmdList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "StgIacWznmQmdList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "TagWznmQmdList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "ListWznmQQmdList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "StatAppQryWznmQmdList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "StatShrQryWznmQmdList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQmdListData", "StgIacQryWznmQmdList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmQmdListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQmdListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQmdListData") {
				mask = mergeDpchEngData(dom);
