/**
  * \file PnlWznmLibMNComponent.js
  * web client functionality for panel PnlWznmLibMNComponent
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNComponent_side").src = "./PnlWznmLibMNComponent_aside.html";
	doc.getElementById("MNComponent_cont").src = "./PnlWznmLibMNComponent_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNComponent_side").src = "./PnlWznmLibMNComponent_bside.html";
	doc.getElementById("MNComponent_cont").src = "./PnlWznmLibMNComponent_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibMNComponent", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLibMNComponent", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibMNComponent", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLibMNComponent", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNComponent", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNComponent", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNComponent", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNComponent", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibMNComponent", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmLibMNComponent", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLibMNComponent", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLibMNComponent
		rownode = getRownode(srcdoc, "ListWznmQLibMNComponent", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLibMNComponent", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLibMNComponent", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLibMNComponent", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLibMNComponent", "ListWznmQLibMNComponent", retrieveTi(srcdoc, "TagWznmLibMNComponent", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLibMNComponent", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNComponent", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNComponent", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLibMNComponent", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLibMNComponent", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibMNComponentData", scrJref, "StgIacQryWznmLibMNComponent");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNComponent", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLibMNComponent", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmLibMNComponent", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNComponent", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibMNComponentDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "ContInfWznmLibMNComponent", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StatAppWznmLibMNComponent", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StatShrWznmLibMNComponent", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StgIacWznmLibMNComponent", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "TagWznmLibMNComponent", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "ListWznmQLibMNComponent", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StatAppQryWznmLibMNComponent", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StatShrQryWznmLibMNComponent", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLibMNComponentData", "StgIacQryWznmLibMNComponent", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmLibMNComponentData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibMNComponentData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLibMNComponentData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
