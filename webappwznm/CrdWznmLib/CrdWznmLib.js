/**
  * \file CrdWznmLib.js
  * web client functionality for card CrdWznmLib
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmLib", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmLib", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmLib", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmLib", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmLib", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmLibHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmLibList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmLibRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmLib", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmLib", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmLib", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmLib", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmLib", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmLib", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmLib", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdLib"), retrieveCi(srcdoc, "ContInfWznmLib", "MtxCrdLib"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfWznmLib", "MtxCrdLib") + " - WhizniumSBE v1.0.6";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmLib", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibData", "ContInfWznmLib", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmLibData", "StatAppWznmLib", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLibData", "StatShrWznmLib", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibData", "TagWznmLib", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmLib", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmLib", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmLib", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmLibData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmLib", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmLib", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmLib", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmLib", "latency"));
