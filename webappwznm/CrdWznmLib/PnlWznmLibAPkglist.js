...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("APkglist", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("APkglist_side").setAttribute("height", "30");
	doc.getElementById("APkglist_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("APkglist_side").src = "./PnlWznmLibAPkglist_aside.html";
	doc.getElementById("APkglist_cont").src = "./PnlWznmLibAPkglist_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("APkglist", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("APkglist_side").setAttribute("height", "255");
	doc.getElementById("APkglist_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("APkglist_side").src = "./PnlWznmLibAPkglist_bside.html";
	doc.getElementById("APkglist_cont").src = "./PnlWznmLibAPkglist_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("APkglist_side").contentDocument;
	contdoc = doc.getElementById("APkglist_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("APkglist_side").contentDocument;
	contdoc = doc.getElementById("APkglist_cont").contentDocument;
	hdrdoc = contdoc.getElementById("APkglist_hdr").contentDocument;
	tbldoc = contdoc.getElementById("APkglist_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("APkglist_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibAPkglist", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLibAPkglist", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibAPkglist", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLibAPkglist", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAPkglist", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAPkglist", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAPkglist", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAPkglist", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAPkglist", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAPkglist", "ntot"));
	...
	if (chgcol) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmLibAPkglist", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLibAPkglist", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLibAPkglist
		rownode = getRownode(srcdoc, "ListWznmQLibAPkglist", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLibAPkglist", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLibAPkglist", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLibAPkglist", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("APkglist", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("APkglist_side").setAttribute("height", "" + (height+55));
	doc.getElementById("APkglist_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("APkglist_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("APkglist_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAPkglist", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAPkglist", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAPkglist", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLibAPkglist", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLibAPkglist", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibAPkglistData", scrJref, "StgIacQryWznmLibAPkglist");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAPkglist", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLibAPkglist", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmLibAPkglist", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAPkglist", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibAPkglistDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "ContInfWznmLibAPkglist", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StatAppWznmLibAPkglist", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StatShrWznmLibAPkglist", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StgIacWznmLibAPkglist", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "TagWznmLibAPkglist", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "ListWznmQLibAPkglist", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StatAppQryWznmLibAPkglist", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StatShrQryWznmLibAPkglist", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLibAPkglistData", "StgIacQryWznmLibAPkglist", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmLibAPkglistData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibAPkglistData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("APkglist");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLibAPkglistData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
