/**
  * \file PnlWznmLibAMakefile.js
  * web client functionality for panel PnlWznmLibAMakefile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWznmLibAMakefile_aside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWznmLibAMakefile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("AMakefile_side").src = "./PnlWznmLibAMakefile_bside.html";
	doc.getElementById("AMakefile_cont").src = "./PnlWznmLibAMakefile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibAMakefile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLibAMakefile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibAMakefile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLibAMakefile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAMakefile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAMakefile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAMakefile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAMakefile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "ncol"));

		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibAMakefile", "TcoReuWidth"));
		var TcoTagWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibAMakefile", "TcoTagWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibAMakefile", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmLibAMakefile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLibAMakefile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLibAMakefile
		rownode = getRownode(srcdoc, "ListWznmQLibAMakefile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLibAMakefile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLibAMakefile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLibAMakefile", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLibAMakefile", "ListWznmQLibAMakefile", retrieveTi(srcdoc, "TagWznmLibAMakefile", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLibAMakefile", "TxtRecord2"),
				["","","Reu","Tag","","Val"],
				["ret","ret2","reu","tag","tag2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAMakefile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibAMakefile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLibAMakefile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLibAMakefile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibAMakefileData", scrJref, "StgIacQryWznmLibAMakefile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibAMakefile", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLibAMakefile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmLibAMakefile", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibAMakefile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibAMakefileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "ContInfWznmLibAMakefile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StatAppWznmLibAMakefile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StatShrWznmLibAMakefile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StgIacWznmLibAMakefile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "TagWznmLibAMakefile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "ListWznmQLibAMakefile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StatAppQryWznmLibAMakefile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StatShrQryWznmLibAMakefile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLibAMakefileData", "StgIacQryWznmLibAMakefile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmLibAMakefileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibAMakefileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLibAMakefileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
