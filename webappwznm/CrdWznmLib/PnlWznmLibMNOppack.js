/**
  * \file PnlWznmLibMNOppack.js
  * web client functionality for panel PnlWznmLibMNOppack
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNOppack_side").src = "./PnlWznmLibMNOppack_aside.html";
	doc.getElementById("MNOppack_cont").src = "./PnlWznmLibMNOppack_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNOppack_side").src = "./PnlWznmLibMNOppack_bside.html";
	doc.getElementById("MNOppack_cont").src = "./PnlWznmLibMNOppack_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibMNOppack", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLibMNOppack", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibMNOppack", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLibMNOppack", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNOppack", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNOppack", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNOppack", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNOppack", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibMNOppack", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmLibMNOppack", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLibMNOppack", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLibMNOppack
		rownode = getRownode(srcdoc, "ListWznmQLibMNOppack", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLibMNOppack", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLibMNOppack", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLibMNOppack", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLibMNOppack", "ListWznmQLibMNOppack", retrieveTi(srcdoc, "TagWznmLibMNOppack", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLibMNOppack", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNOppack", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibMNOppack", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLibMNOppack", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLibMNOppack", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibMNOppackData", scrJref, "StgIacQryWznmLibMNOppack");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibMNOppack", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLibMNOppack", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmLibMNOppack", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibMNOppack", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibMNOppackDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "ContInfWznmLibMNOppack", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StatAppWznmLibMNOppack", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StatShrWznmLibMNOppack", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StgIacWznmLibMNOppack", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "TagWznmLibMNOppack", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "ListWznmQLibMNOppack", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StatAppQryWznmLibMNOppack", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StatShrQryWznmLibMNOppack", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLibMNOppackData", "StgIacQryWznmLibMNOppack", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmLibMNOppackData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibMNOppackData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLibMNOppackData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
