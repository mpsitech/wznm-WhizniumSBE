/**
  * \file PnlWznmLibList.js
  * web client functionality for panel PnlWznmLibList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmLibList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmLibList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmLibList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmLibList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLibList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmLibList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLibList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLibList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibList", "TcoTitWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibList", "TcoVerWidth"));
		var TcoLtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLibList", "TcoLtyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmLibList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoLty"); doc.widths.push(TcoLtyWidth); doc.fs.push("lty2"); doc.tos.push(0); wtot += TcoLtyWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmLibList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLibList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLibList
		rownode = getRownode(srcdoc, "ListWznmQLibList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLibList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLibList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLibList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLibList", "ListWznmQLibList", retrieveTi(srcdoc, "TagWznmLibList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLibList", "TxtRecord2"),
				["Srf","Tit","Ver","","Lty"],
				["srf","tit","ver","lty","lty2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLibList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLibList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLibList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibListData", scrJref, "StgIacQryWznmLibList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLibList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLibList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLibList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLibListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmLibList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmLibList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLibListData", scrJref, "ContIacWznmLibList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "ContIacWznmLibList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "ContInfWznmLibList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "StatShrWznmLibList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "StgIacWznmLibList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "TagWznmLibList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "ListWznmQLibList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "StatAppQryWznmLibList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "StatShrQryWznmLibList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLibListData", "StgIacQryWznmLibList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmLibListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLibListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLibListData") {
				mask = mergeDpchEngData(dom);
