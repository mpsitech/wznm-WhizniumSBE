/**
  * \file CrdWznmApp.js
  * web client functionality for card CrdWznmApp
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmApp", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmApp", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmApp", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmApp", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmApp", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmAppHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmAppList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmAppRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmApp", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmApp", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmApp", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmApp", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmApp", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmApp", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmApp", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWznmApp", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrWznmApp", "MitCrdNewAvail") == "true");
	MitCrdIstAvail = (retrieveSi(srcdoc, "StatShrWznmApp", "MitCrdIstAvail") == "true");
	MitCrdIstActive = (retrieveSi(srcdoc, "StatShrWznmApp", "MitCrdIstActive") == "true");
	MitCrdWrcAvail = (retrieveSi(srcdoc, "StatShrWznmApp", "MitCrdWrcAvail") == "true");
	MitCrdWrcActive = (retrieveSi(srcdoc, "StatShrWznmApp", "MitCrdWrcActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmApp", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdApp"), retrieveCi(srcdoc, "ContInfWznmApp", "MtxCrdApp"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagWznmApp", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdIst"), retrieveTi(srcdoc, "TagWznmApp", "MitCrdIst"));
	setMitActive("MitCrdIst", MitCrdIstActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdWrc"), retrieveTi(srcdoc, "TagWznmApp", "MitCrdWrc"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWznmAppImpstr") || (sref == "DlgWznmAppWrite")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgimpstr = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefDlgimpstr");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefDlgnew");
	var scrJrefDlgwrite = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefDlgwrite");

	if (scrJrefDlgimpstr != "") {
		if (scrJrefDlg != scrJrefDlgimpstr) showDlg("DlgWznmAppImpstr", scrJrefDlgimpstr);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgWznmAppNew", scrJrefDlgnew);
	} else if (scrJrefDlgwrite != "") {
		if (scrJrefDlg != scrJrefDlgwrite) showDlg("DlgWznmAppWrite", scrJrefDlgwrite);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWznmApp", "MtxCrdApp") + " - WhizniumSBE v1.0.7";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmAppDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmAppDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmApp", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmAppData", "ContInfWznmApp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmAppData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmAppData", "StatAppWznmApp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmAppData", "StatShrWznmApp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmAppData", "TagWznmApp", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmAppData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmApp", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmAppData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmApp", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmApp", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmApp", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmApp", "latency"));
