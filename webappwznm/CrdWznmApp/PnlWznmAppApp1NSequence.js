...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("App1NSequence", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("App1NSequence_side").setAttribute("height", "30");
	doc.getElementById("App1NSequence_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("App1NSequence_side").src = "./PnlWznmAppApp1NSequence_aside.html";
	doc.getElementById("App1NSequence_cont").src = "./PnlWznmAppApp1NSequence_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("App1NSequence", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("App1NSequence_side").setAttribute("height", "255");
	doc.getElementById("App1NSequence_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("App1NSequence_side").src = "./PnlWznmAppApp1NSequence_bside.html";
	doc.getElementById("App1NSequence_cont").src = "./PnlWznmAppApp1NSequence_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("App1NSequence_side").contentDocument;
	contdoc = doc.getElementById("App1NSequence_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("App1NSequence_side").contentDocument;
	contdoc = doc.getElementById("App1NSequence_cont").contentDocument;
	hdrdoc = contdoc.getElementById("App1NSequence_hdr").contentDocument;
	tbldoc = contdoc.getElementById("App1NSequence_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("App1NSequence_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmAppApp1NSequence", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppApp1NSequence", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppApp1NSequence", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppApp1NSequence", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppApp1NSequence", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppApp1NSequence", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmAppApp1NSequence", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQAppApp1NSequence
		rownode = getRownode(srcdoc, "ListWznmQAppApp1NSequence", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmAppApp1NSequence", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("App1NSequence", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("App1NSequence_side").setAttribute("height", "" + (height+55));
	doc.getElementById("App1NSequence_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("App1NSequence_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("App1NSequence_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmAppApp1NSequence", "ListWznmQAppApp1NSequence", retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmAppApp1NSequence", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppApp1NSequence", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppApp1NSequence", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmAppApp1NSequence", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmAppApp1NSequence", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmAppApp1NSequenceData", scrJref, "StgIacQryWznmAppApp1NSequence");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmAppApp1NSequence", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmAppApp1NSequence", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppApp1NSequence", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmAppApp1NSequenceDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "ContInfWznmAppApp1NSequence", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StatAppWznmAppApp1NSequence", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StatShrWznmAppApp1NSequence", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StgIacWznmAppApp1NSequence", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "TagWznmAppApp1NSequence", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "ListWznmQAppApp1NSequence", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StatAppQryWznmAppApp1NSequence", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StatShrQryWznmAppApp1NSequence", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmAppApp1NSequenceData", "StgIacQryWznmAppApp1NSequence", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmAppApp1NSequenceData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmAppApp1NSequenceData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("App1NSequence");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmAppApp1NSequenceData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
