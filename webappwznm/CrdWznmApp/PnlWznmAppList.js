/**
  * \file PnlWznmAppList.js
  * web client functionality for panel PnlWznmAppList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmAppList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmAppList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmAppList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmAppList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmAppList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmAppList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmAppList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoOwnWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoShoWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoTitWidth"));
		var TcoTrgWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoTrgWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppList", "TcoVerWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmAppList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTrg"); doc.widths.push(TcoTrgWidth); doc.fs.push("trg2"); doc.tos.push(0); wtot += TcoTrgWidth;
			} else if (i == 6) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmAppList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmAppList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQAppList
		rownode = getRownode(srcdoc, "ListWznmQAppList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmAppList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmAppList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmAppList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmAppList", "ListWznmQAppList", retrieveTi(srcdoc, "TagWznmAppList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmAppList", "TxtRecord2"),
				["Grp","Own","Sho","Tit","","Trg","Ver"],
				["grp","own","sho","tit","trg","trg2","ver"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmAppList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmAppList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmAppListData", scrJref, "StgIacQryWznmAppList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmAppList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmAppListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmAppList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmAppList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmAppListData", scrJref, "ContIacWznmAppList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "ContIacWznmAppList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "ContInfWznmAppList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "StatShrWznmAppList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "StgIacWznmAppList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "TagWznmAppList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "ListWznmQAppList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "StatAppQryWznmAppList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "StatShrQryWznmAppList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmAppListData", "StgIacQryWznmAppList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmAppListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmAppListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmAppListData") {
				mask = mergeDpchEngData(dom);
