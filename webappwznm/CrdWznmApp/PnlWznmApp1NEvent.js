/**
  * \file PnlWznmApp1NEvent.js
  * web client functionality for panel PnlWznmApp1NEvent
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NEvent", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NEvent_side").setAttribute("height", "30");
	doc.getElementById("1NEvent_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NEvent_side").src = "./PnlWznmApp1NEvent_aside.html";
	doc.getElementById("1NEvent_cont").src = "./PnlWznmApp1NEvent_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NEvent", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NEvent_side").setAttribute("height", "255");
	doc.getElementById("1NEvent_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NEvent_side").src = "./PnlWznmApp1NEvent_bside.html";
	doc.getElementById("1NEvent_cont").src = "./PnlWznmApp1NEvent_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NEvent_side").contentDocument;
	contdoc = doc.getElementById("1NEvent_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NEvent_side").contentDocument;
	contdoc = doc.getElementById("1NEvent_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NEvent_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NEvent_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NEvent_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmApp1NEvent", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmApp1NEvent", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmApp1NEvent", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmApp1NEvent", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmApp1NEvent", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmApp1NEvent", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmApp1NEvent", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmApp1NEvent", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmApp1NEvent", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmApp1NEvent", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmApp1NEvent", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQApp1NEvent
		rownode = getRownode(srcdoc, "ListWznmQApp1NEvent", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmApp1NEvent", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmApp1NEvent", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmApp1NEvent", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NEvent", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NEvent_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NEvent_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NEvent_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NEvent_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmApp1NEvent", "ListWznmQApp1NEvent", retrieveTi(srcdoc, "TagWznmApp1NEvent", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmApp1NEvent", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmApp1NEvent", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmApp1NEvent", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmApp1NEvent", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmApp1NEvent", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmApp1NEventData", scrJref, "StgIacQryWznmApp1NEvent");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmApp1NEvent", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmApp1NEvent", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmApp1NEvent", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmApp1NEvent", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmApp1NEventDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "ContInfWznmApp1NEvent", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StatAppWznmApp1NEvent", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StatShrWznmApp1NEvent", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StgIacWznmApp1NEvent", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "TagWznmApp1NEvent", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "ListWznmQApp1NEvent", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StatAppQryWznmApp1NEvent", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StatShrQryWznmApp1NEvent", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmApp1NEventData", "StgIacQryWznmApp1NEvent", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmApp1NEventData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmApp1NEventData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NEvent");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmApp1NEventData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
