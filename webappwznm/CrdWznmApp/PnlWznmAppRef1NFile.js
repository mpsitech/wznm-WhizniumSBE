/**
  * \file PnlWznmAppRef1NFile.js
  * web client functionality for panel PnlWznmAppRef1NFile
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWznmAppRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWznmAppRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlWznmAppRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlWznmAppRef1NFile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmAppRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmAppRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmAppRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmAppRef1NFile", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmAppRef1NFile", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmAppRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQAppRef1NFile
		rownode = getRownode(srcdoc, "ListWznmQAppRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmAppRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmAppRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmAppRef1NFile", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmAppRef1NFile", "ListWznmQAppRef1NFile", retrieveTi(srcdoc, "TagWznmAppRef1NFile", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmAppRef1NFile", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmAppRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmAppRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmAppRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmAppRef1NFileData", scrJref, "StgIacQryWznmAppRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmAppRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmAppRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmAppRef1NFile", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmAppRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmAppRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "ContInfWznmAppRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StatAppWznmAppRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StatShrWznmAppRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StgIacWznmAppRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "TagWznmAppRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "ListWznmQAppRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StatAppQryWznmAppRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StatShrQryWznmAppRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmAppRef1NFileData", "StgIacQryWznmAppRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmAppRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmAppRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmAppRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
