/**
  * \file PnlWznmOpkARetval.js
  * web client functionality for panel PnlWznmOpkARetval
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("ARetval", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ARetval_side").setAttribute("height", "30");
	doc.getElementById("ARetval_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ARetval_side").src = "./PnlWznmOpkARetval_aside.html";
	doc.getElementById("ARetval_cont").src = "./PnlWznmOpkARetval_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("ARetval", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ARetval_side").setAttribute("height", "255");
	doc.getElementById("ARetval_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ARetval_side").src = "./PnlWznmOpkARetval_bside.html";
	doc.getElementById("ARetval_cont").src = "./PnlWznmOpkARetval_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ARetval_side").contentDocument;
	contdoc = doc.getElementById("ARetval_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ARetval_side").contentDocument;
	contdoc = doc.getElementById("ARetval_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ARetval_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ARetval_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ARetval_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpkARetval", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmOpkARetval", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpkARetval", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmOpkARetval", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkARetval", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkARetval", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkARetval", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkARetval", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpkARetval", "TcoSrfWidth"));
		var TcoVtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpkARetval", "TcoVtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpkARetval", "TcoVecWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpkARetval", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmOpkARetval", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmOpkARetval", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQOpkARetval
		rownode = getRownode(srcdoc, "ListWznmQOpkARetval", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmOpkARetval", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmOpkARetval", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmOpkARetval", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ARetval", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ARetval_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ARetval_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ARetval_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ARetval_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmOpkARetval", "ListWznmQOpkARetval", retrieveTi(srcdoc, "TagWznmOpkARetval", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmOpkARetval", "TxtRecord2"),
				["Srf","Vty","","Vec","Cmt"],
				["srf","vty","vty2","vec","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkARetval", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkARetval", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmOpkARetval", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmOpkARetval", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmOpkARetvalData", scrJref, "StgIacQryWznmOpkARetval");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkARetval", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmOpkARetval", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmOpkARetval", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkARetval", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmOpkARetvalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "ContInfWznmOpkARetval", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StatAppWznmOpkARetval", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StatShrWznmOpkARetval", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StgIacWznmOpkARetval", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "TagWznmOpkARetval", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "ListWznmQOpkARetval", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StatAppQryWznmOpkARetval", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StatShrQryWznmOpkARetval", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmOpkARetvalData", "StgIacQryWznmOpkARetval", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmOpkARetvalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmOpkARetvalData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ARetval");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmOpkARetvalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
