/**
  * \file PnlWznmOpkRef1NBlock.js
  * web client functionality for panel PnlWznmOpkRef1NBlock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NBlock", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NBlock_side").setAttribute("height", "30");
	doc.getElementById("Ref1NBlock_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NBlock_side").src = "./PnlWznmOpkRef1NBlock_aside.html";
	doc.getElementById("Ref1NBlock_cont").src = "./PnlWznmOpkRef1NBlock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NBlock", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NBlock_side").setAttribute("height", "255");
	doc.getElementById("Ref1NBlock_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NBlock_side").src = "./PnlWznmOpkRef1NBlock_bside.html";
	doc.getElementById("Ref1NBlock_cont").src = "./PnlWznmOpkRef1NBlock_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NBlock_side").contentDocument;
	contdoc = doc.getElementById("Ref1NBlock_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NBlock_side").contentDocument;
	contdoc = doc.getElementById("Ref1NBlock_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NBlock_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NBlock_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NBlock_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmOpkRef1NBlock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkRef1NBlock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkRef1NBlock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkRef1NBlock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkRef1NBlock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpkRef1NBlock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQOpkRef1NBlock
		rownode = getRownode(srcdoc, "ListWznmQOpkRef1NBlock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmOpkRef1NBlock", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NBlock", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NBlock_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NBlock_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NBlock_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NBlock_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmOpkRef1NBlock", "ListWznmQOpkRef1NBlock", retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmOpkRef1NBlock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkRef1NBlock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpkRef1NBlock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmOpkRef1NBlock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmOpkRef1NBlock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmOpkRef1NBlockData", scrJref, "StgIacQryWznmOpkRef1NBlock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmOpkRef1NBlock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmOpkRef1NBlock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpkRef1NBlock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmOpkRef1NBlockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "ContInfWznmOpkRef1NBlock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StatAppWznmOpkRef1NBlock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StatShrWznmOpkRef1NBlock", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StgIacWznmOpkRef1NBlock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "TagWznmOpkRef1NBlock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "ListWznmQOpkRef1NBlock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StatAppQryWznmOpkRef1NBlock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StatShrQryWznmOpkRef1NBlock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmOpkRef1NBlockData", "StgIacQryWznmOpkRef1NBlock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmOpkRef1NBlockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmOpkRef1NBlockData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NBlock");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmOpkRef1NBlockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
