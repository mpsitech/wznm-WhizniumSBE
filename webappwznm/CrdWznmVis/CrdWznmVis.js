function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmVis", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmVis", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmVis", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmVis", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmVis", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmVisHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmVisList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmVisRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmVis", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmVis", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmVis", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmVis", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmVis", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmVis", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmVis", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWznmVis", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdNewAvail") == "true");
	MitCrdIstAvail = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdIstAvail") == "true");
	MitCrdIstActive = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdIstActive") == "true");
	MitCrdCmfAvail = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdCmfAvail") == "true");
	MitCrdCmfActive = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdCmfActive") == "true");
	MitCrdWrcAvail = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdWrcAvail") == "true");
	MitCrdWrcActive = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdWrcActive") == "true");
	MitCrdEstActive = (retrieveSi(srcdoc, "StatShrWznmVis", "MitCrdEstActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmVis", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdVis"), retrieveCi(srcdoc, "ContInfWznmVis", "MtxCrdVis"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagWznmVis", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdIst"), retrieveTi(srcdoc, "TagWznmVis", "MitCrdIst"));
	setMitActive("MitCrdIst", MitCrdIstActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdCmf"), retrieveTi(srcdoc, "TagWznmVis", "MitCrdCmf"));
	setMitActive("MitCrdCmf", MitCrdCmfActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdWrc"), retrieveTi(srcdoc, "TagWznmVis", "MitCrdWrc"));
	setMitActive("MitCrdWrc", MitCrdWrcActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdEst"), retrieveTi(srcdoc, "TagWznmVis", "MitCrdEst"));
	setMitActive("MitCrdEst", MitCrdEstActive);

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdNew", MitCrdNewAvail, 20);
	height -= setMitMspAvail("MitCrdIst", MitCrdIstAvail, 20);
	height -= setMitMspAvail("MitCrdCmf", MitCrdCmfAvail, 20);
	height -= setMitMspAvail("MitCrdWrc", MitCrdWrcAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWznmVisImpstr") || (sref == "DlgWznmVisWrite") || (sref == "DlgWznmVisExpstr")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgexpstr = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefDlgexpstr");
	var scrJrefDlgimpstr = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefDlgimpstr");
	var scrJrefDlgmissfeat = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefDlgmissfeat");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefDlgnew");
	var scrJrefDlgwrite = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefDlgwrite");

	if (scrJrefDlgexpstr != "") {
		if (scrJrefDlg != scrJrefDlgexpstr) showDlg("DlgWznmVisExpstr", scrJrefDlgexpstr);
	} else if (scrJrefDlgimpstr != "") {
		if (scrJrefDlg != scrJrefDlgimpstr) showDlg("DlgWznmVisImpstr", scrJrefDlgimpstr);
	} else if (scrJrefDlgmissfeat != "") {
		if (scrJrefDlg != scrJrefDlgmissfeat) showDlg("DlgWznmVisMissfeat", scrJrefDlgmissfeat);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgWznmVisNew", scrJrefDlgnew);
	} else if (scrJrefDlgwrite != "") {
		if (scrJrefDlg != scrJrefDlgwrite) showDlg("DlgWznmVisWrite", scrJrefDlgwrite);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWznmVis", "MtxCrdVis") + " - WhizniumSBE v1.1.13";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVisDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVisDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmVis", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVisData", "ContInfWznmVis", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVisData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmVisData", "StatAppWznmVis", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVisData", "StatShrWznmVis", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVisData", "TagWznmVis", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVisData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmVis", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmVisData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmVis", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmVis", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmVis", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmVis", "latency"));
