...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Vis1NSheet", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Vis1NSheet_side").setAttribute("height", "30");
	doc.getElementById("Vis1NSheet_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Vis1NSheet_side").src = "./PnlWznmVisVis1NSheet_aside.html";
	doc.getElementById("Vis1NSheet_cont").src = "./PnlWznmVisVis1NSheet_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Vis1NSheet", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Vis1NSheet_side").setAttribute("height", "255");
	doc.getElementById("Vis1NSheet_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Vis1NSheet_side").src = "./PnlWznmVisVis1NSheet_bside.html";
	doc.getElementById("Vis1NSheet_cont").src = "./PnlWznmVisVis1NSheet_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Vis1NSheet_side").contentDocument;
	contdoc = doc.getElementById("Vis1NSheet_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Vis1NSheet_side").contentDocument;
	contdoc = doc.getElementById("Vis1NSheet_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Vis1NSheet_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Vis1NSheet_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Vis1NSheet_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVisVis1NSheet", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVisVis1NSheet", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVisVis1NSheet", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVisVis1NSheet", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVisVis1NSheet", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVisVis1NSheet", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVisVis1NSheet", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVisVis1NSheet
		rownode = getRownode(srcdoc, "ListWznmQVisVis1NSheet", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVisVis1NSheet", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Vis1NSheet", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Vis1NSheet_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Vis1NSheet_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Vis1NSheet_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Vis1NSheet_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVisVis1NSheet", "ListWznmQVisVis1NSheet", retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVisVis1NSheet", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVisVis1NSheet", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVisVis1NSheet", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVisVis1NSheet", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVisVis1NSheet", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVisVis1NSheetData", scrJref, "StgIacQryWznmVisVis1NSheet");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVisVis1NSheet", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVisVis1NSheet", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVisVis1NSheet", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVisVis1NSheetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "ContInfWznmVisVis1NSheet", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StatAppWznmVisVis1NSheet", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StatShrWznmVisVis1NSheet", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StgIacWznmVisVis1NSheet", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "TagWznmVisVis1NSheet", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "ListWznmQVisVis1NSheet", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StatAppQryWznmVisVis1NSheet", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StatShrQryWznmVisVis1NSheet", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVisVis1NSheetData", "StgIacQryWznmVisVis1NSheet", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVisVis1NSheetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVisVis1NSheetData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Vis1NSheet");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVisVis1NSheetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
