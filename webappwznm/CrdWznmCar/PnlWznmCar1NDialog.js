/**
  * \file PnlWznmCar1NDialog.js
  * web client functionality for panel PnlWznmCar1NDialog
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NDialog", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NDialog_side").setAttribute("height", "30");
	doc.getElementById("1NDialog_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NDialog_side").src = "./PnlWznmCar1NDialog_aside.html";
	doc.getElementById("1NDialog_cont").src = "./PnlWznmCar1NDialog_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NDialog", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NDialog_side").setAttribute("height", "255");
	doc.getElementById("1NDialog_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NDialog_side").src = "./PnlWznmCar1NDialog_bside.html";
	doc.getElementById("1NDialog_cont").src = "./PnlWznmCar1NDialog_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NDialog_side").contentDocument;
	contdoc = doc.getElementById("1NDialog_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NDialog_side").contentDocument;
	contdoc = doc.getElementById("1NDialog_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NDialog_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NDialog_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NDialog_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCar1NDialog", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCar1NDialog", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCar1NDialog", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCar1NDialog", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCar1NDialog", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCar1NDialog", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCar1NDialog", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCar1NDialog", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCar1NDialog", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCar1NDialog", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCar1NDialog", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCar1NDialog
		rownode = getRownode(srcdoc, "ListWznmQCar1NDialog", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCar1NDialog", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCar1NDialog", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCar1NDialog", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NDialog", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NDialog_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NDialog_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NDialog_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NDialog_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCar1NDialog", "ListWznmQCar1NDialog", retrieveTi(srcdoc, "TagWznmCar1NDialog", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCar1NDialog", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCar1NDialog", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCar1NDialog", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCar1NDialog", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCar1NDialog", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCar1NDialogData", scrJref, "StgIacQryWznmCar1NDialog");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCar1NDialog", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCar1NDialog", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCar1NDialog", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCar1NDialog", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCar1NDialogDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "ContInfWznmCar1NDialog", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StatAppWznmCar1NDialog", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StatShrWznmCar1NDialog", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StgIacWznmCar1NDialog", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "TagWznmCar1NDialog", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "ListWznmQCar1NDialog", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StatAppQryWznmCar1NDialog", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StatShrQryWznmCar1NDialog", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCar1NDialogData", "StgIacQryWznmCar1NDialog", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCar1NDialogData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCar1NDialogData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NDialog");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCar1NDialogData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
