/**
  * \file PnlWznmCarCar1NPanel.js
  * web client functionality for panel PnlWznmCarCar1NPanel
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Car1NPanel", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Car1NPanel_side").setAttribute("height", "30");
	doc.getElementById("Car1NPanel_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Car1NPanel_side").src = "./PnlWznmCarCar1NPanel_aside.html";
	doc.getElementById("Car1NPanel_cont").src = "./PnlWznmCarCar1NPanel_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Car1NPanel", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Car1NPanel_side").setAttribute("height", "255");
	doc.getElementById("Car1NPanel_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Car1NPanel_side").src = "./PnlWznmCarCar1NPanel_bside.html";
	doc.getElementById("Car1NPanel_cont").src = "./PnlWznmCarCar1NPanel_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Car1NPanel_side").contentDocument;
	contdoc = doc.getElementById("Car1NPanel_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Car1NPanel_side").contentDocument;
	contdoc = doc.getElementById("Car1NPanel_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Car1NPanel_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Car1NPanel_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Car1NPanel_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCarCar1NPanel", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCarCar1NPanel", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCarCar1NPanel", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCarCar1NPanel", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCarCar1NPanel", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCarCar1NPanel", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCarCar1NPanel", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCarCar1NPanel
		rownode = getRownode(srcdoc, "ListWznmQCarCar1NPanel", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCarCar1NPanel", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Car1NPanel", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Car1NPanel_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Car1NPanel_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Car1NPanel_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Car1NPanel_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCarCar1NPanel", "ListWznmQCarCar1NPanel", retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCarCar1NPanel", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCarCar1NPanel", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCarCar1NPanel", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCarCar1NPanel", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCarCar1NPanel", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCarCar1NPanelData", scrJref, "StgIacQryWznmCarCar1NPanel");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCarCar1NPanel", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCarCar1NPanel", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCarCar1NPanel", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCarCar1NPanelDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "ContInfWznmCarCar1NPanel", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StatAppWznmCarCar1NPanel", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StatShrWznmCarCar1NPanel", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StgIacWznmCarCar1NPanel", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "TagWznmCarCar1NPanel", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "ListWznmQCarCar1NPanel", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StatAppQryWznmCarCar1NPanel", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StatShrQryWznmCarCar1NPanel", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCarCar1NPanelData", "StgIacQryWznmCarCar1NPanel", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCarCar1NPanelData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCarCar1NPanelData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Car1NPanel");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCarCar1NPanelData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
