/**
  * \file CrdWznmRls.js
  * web client functionality for card CrdWznmRls
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmRls", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmRls", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmRls", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmRls", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmRls", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmRlsHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmRlsList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmRlsRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmRls", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmRls", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmRls", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmRls", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmRls", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmRls", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmRls", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWznmRls", "MspCrd1Avail") == "true");
	MitCrdSrtAvail = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdSrtAvail") == "true");
	MitCrdSrtActive = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdSrtActive") == "true");
	MitCrdCrtAvail = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdCrtAvail") == "true");
	MitCrdCrtActive = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdCrtActive") == "true");
	MitCrdFrtAvail = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdFrtAvail") == "true");
	MitCrdFrtActive = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdFrtActive") == "true");
	MitCrdWcoAvail = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdWcoAvail") == "true");
	MitCrdWcoActive = (retrieveSi(srcdoc, "StatShrWznmRls", "MitCrdWcoActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmRls", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdRls"), retrieveCi(srcdoc, "ContInfWznmRls", "MtxCrdRls"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdSrt"), retrieveTi(srcdoc, "TagWznmRls", "MitCrdSrt"));
	setMitActive("MitCrdSrt", MitCrdSrtActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdCrt"), retrieveTi(srcdoc, "TagWznmRls", "MitCrdCrt"));
	setMitActive("MitCrdCrt", MitCrdCrtActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdFrt"), retrieveTi(srcdoc, "TagWznmRls", "MitCrdFrt"));
	setMitActive("MitCrdFrt", MitCrdFrtActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdWco"), retrieveTi(srcdoc, "TagWznmRls", "MitCrdWco"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWznmRlsStareptr") || (sref == "DlgWznmRlsFinreptr") || (sref == "DlgWznmRlsWrite")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgfinreptr = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefDlgfinreptr");
	var scrJrefDlgstareptr = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefDlgstareptr");
	var scrJrefDlgwrite = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefDlgwrite");

	if (scrJrefDlgfinreptr != "") {
		if (scrJrefDlg != scrJrefDlgfinreptr) showDlg("DlgWznmRlsFinreptr", scrJrefDlgfinreptr);
	} else if (scrJrefDlgstareptr != "") {
		if (scrJrefDlg != scrJrefDlgstareptr) showDlg("DlgWznmRlsStareptr", scrJrefDlgstareptr);
	} else if (scrJrefDlgwrite != "") {
		if (scrJrefDlg != scrJrefDlgwrite) showDlg("DlgWznmRlsWrite", scrJrefDlgwrite);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWznmRls", "MtxCrdRls") + " - WhizniumSBE v1.0.7";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmRlsDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmRlsDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmRls", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmRlsData", "ContInfWznmRls", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmRlsData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmRlsData", "StatAppWznmRls", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmRlsData", "StatShrWznmRls", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmRlsData", "TagWznmRls", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmRlsData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmRls", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmRlsData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmRls", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmRls", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmRls", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmRls", "latency"));
