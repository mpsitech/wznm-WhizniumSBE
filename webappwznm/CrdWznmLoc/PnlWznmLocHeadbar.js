...
function getMenCptwidth(short) {
	return(parseInt(retrieveSi(srcdoc, "StgInfWznmLocHeadbar", "Men" + short + "Cptwidth")));
};
...
function init() {
	...
	setTextContent(contdoc, contdoc.getElementById("MenApp"), "\\u00a0" + retrieveTi(srcdoc, "TagWznmLocHeadbar", "MenApp") + "\\u00a0");
	w = getMenCptwidth("Crd"); contdoc.getElementById("colMenCrd").setAttribute("width", "" + w); wFill -= w + 1;
	setTextContent(contdoc, contdoc.getElementById("MenCrd"), "\\u00a0" + retrieveTi(srcdoc, "TagWznmLocHeadbar", "MenCrd") + "\\u00a0");
...
function handleMenAppClick() {
	getCrdwnd().toggleMenu("App", getMenLeft("App"), parseInt(retrieveSi(srcdoc, "StgInfWznmLocHeadbar", "MenAppWidth")), 42);
};

function handleMenCrdClick() {
	getCrdwnd().toggleMenu("Crd", getMenLeft("Crd"), parseInt(retrieveSi(srcdoc, "StgInfWznmLocHeadbar", "MenCrdWidth")), 22);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLocHeadbarData", "StgInfWznmLocHeadbar", srcdoc)) mask.push("stginf");
	if (updateSrcblock(dom, "DpchEngWznmLocHeadbarData", "TagWznmLocHeadbar", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLocHeadbarData") {
