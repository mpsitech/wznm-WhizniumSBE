/**
  * \file PnlWznmLocList.js
  * web client functionality for panel PnlWznmLocList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmLocList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmLocList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmLocList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmLocList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLocList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLocList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmLocList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLocList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLocList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLocList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLocList", "TcoTitWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmLocList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmLocList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLocList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLocList
		rownode = getRownode(srcdoc, "ListWznmQLocList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLocList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLocList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLocList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLocList", "ListWznmQLocList", retrieveTi(srcdoc, "TagWznmLocList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLocList", "TxtRecord2"),
				["Srf","Tit"],
				["srf","tit"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLocList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLocList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLocListData", scrJref, "StgIacQryWznmLocList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLocList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLocListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmLocList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmLocList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLocListData", scrJref, "ContIacWznmLocList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "ContIacWznmLocList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "ContInfWznmLocList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "StatShrWznmLocList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "StgIacWznmLocList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "TagWznmLocList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "ListWznmQLocList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "StatAppQryWznmLocList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "StatShrQryWznmLocList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLocListData", "StgIacQryWznmLocList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmLocListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLocListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLocListData") {
				mask = mergeDpchEngData(dom);
