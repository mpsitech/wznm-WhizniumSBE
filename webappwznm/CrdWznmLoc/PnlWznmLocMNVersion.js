...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNVersion_side").src = "./PnlWznmLocMNVersion_aside.html";
	doc.getElementById("MNVersion_cont").src = "./PnlWznmLocMNVersion_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNVersion_side").src = "./PnlWznmLocMNVersion_bside.html";
	doc.getElementById("MNVersion_cont").src = "./PnlWznmLocMNVersion_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLocMNVersion", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmLocMNVersion", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmLocMNVersion", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmLocMNVersion", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocMNVersion", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocMNVersion", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocMNVersion", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocMNVersion", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmLocMNVersion", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmLocMNVersion", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmLocMNVersion", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQLocMNVersion
		rownode = getRownode(srcdoc, "ListWznmQLocMNVersion", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmLocMNVersion", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmLocMNVersion", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmLocMNVersion", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmLocMNVersion", "ListWznmQLocMNVersion", retrieveTi(srcdoc, "TagWznmLocMNVersion", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmLocMNVersion", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocMNVersion", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmLocMNVersion", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmLocMNVersion", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmLocMNVersion", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmLocMNVersionData", scrJref, "StgIacQryWznmLocMNVersion");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmLocMNVersion", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmLocMNVersion", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmLocMNVersion", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmLocMNVersion", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmLocMNVersionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "ContInfWznmLocMNVersion", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StatAppWznmLocMNVersion", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StatShrWznmLocMNVersion", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StgIacWznmLocMNVersion", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "TagWznmLocMNVersion", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "ListWznmQLocMNVersion", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StatAppQryWznmLocMNVersion", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StatShrQryWznmLocMNVersion", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmLocMNVersionData", "StgIacQryWznmLocMNVersion", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmLocMNVersionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmLocMNVersionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmLocMNVersionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
