/**
  * \file PnlWznmMty1NMachine.js
  * web client functionality for panel PnlWznmMty1NMachine
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NMachine", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NMachine_side").setAttribute("height", "30");
	doc.getElementById("1NMachine_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NMachine_side").src = "./PnlWznmMty1NMachine_aside.html";
	doc.getElementById("1NMachine_cont").src = "./PnlWznmMty1NMachine_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NMachine", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NMachine_side").setAttribute("height", "255");
	doc.getElementById("1NMachine_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NMachine_side").src = "./PnlWznmMty1NMachine_bside.html";
	doc.getElementById("1NMachine_cont").src = "./PnlWznmMty1NMachine_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NMachine_side").contentDocument;
	contdoc = doc.getElementById("1NMachine_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NMachine_side").contentDocument;
	contdoc = doc.getElementById("1NMachine_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NMachine_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NMachine_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NMachine_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMty1NMachine", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmMty1NMachine", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMty1NMachine", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmMty1NMachine", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMty1NMachine", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMty1NMachine", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMty1NMachine", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMty1NMachine", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmMty1NMachine", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmMty1NMachine", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmMty1NMachine", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQMty1NMachine
		rownode = getRownode(srcdoc, "ListWznmQMty1NMachine", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmMty1NMachine", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmMty1NMachine", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmMty1NMachine", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NMachine", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMachine_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMachine_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NMachine_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NMachine_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmMty1NMachine", "ListWznmQMty1NMachine", retrieveTi(srcdoc, "TagWznmMty1NMachine", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmMty1NMachine", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMty1NMachine", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMty1NMachine", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmMty1NMachine", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmMty1NMachine", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmMty1NMachineData", scrJref, "StgIacQryWznmMty1NMachine");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMty1NMachine", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmMty1NMachine", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmMty1NMachine", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMty1NMachine", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmMty1NMachineDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "ContInfWznmMty1NMachine", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StatAppWznmMty1NMachine", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StatShrWznmMty1NMachine", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StgIacWznmMty1NMachine", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "TagWznmMty1NMachine", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "ListWznmQMty1NMachine", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StatAppQryWznmMty1NMachine", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StatShrQryWznmMty1NMachine", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmMty1NMachineData", "StgIacQryWznmMty1NMachine", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmMty1NMachineData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmMty1NMachineData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NMachine");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmMty1NMachineData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
