/**
  * \file PnlWznmMtyList.js
  * web client functionality for panel PnlWznmMtyList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmMtyList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmMtyList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmMtyList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmMtyList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMtyList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmMtyList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmMtyList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmMtyList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmMtyList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMtyList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMtyList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMtyList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMtyList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmMtyList", "TcoSrfWidth"));
		var TcoAchWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmMtyList", "TcoAchWidth"));
		var TcoOsWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmMtyList", "TcoOsWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmMtyList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoAch"); doc.widths.push(TcoAchWidth); doc.fs.push("ach2"); doc.tos.push(0); wtot += TcoAchWidth;
			} else if (i == 3) {
				doc.cols.push("TcoOs"); doc.widths.push(TcoOsWidth); doc.fs.push("os2"); doc.tos.push(0); wtot += TcoOsWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmMtyList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmMtyList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQMtyList
		rownode = getRownode(srcdoc, "ListWznmQMtyList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmMtyList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmMtyList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmMtyList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmMtyList", "ListWznmQMtyList", retrieveTi(srcdoc, "TagWznmMtyList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmMtyList", "TxtRecord2"),
				["Srf","","Ach","","Os"],
				["srf","ach","ach2","os","os2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMtyList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmMtyList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmMtyList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmMtyList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmMtyListData", scrJref, "StgIacQryWznmMtyList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmMtyList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmMtyList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmMtyList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmMtyListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmMtyList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmMtyList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmMtyListData", scrJref, "ContIacWznmMtyList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "ContIacWznmMtyList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "ContInfWznmMtyList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "StatShrWznmMtyList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "StgIacWznmMtyList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "TagWznmMtyList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "ListWznmQMtyList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "StatAppQryWznmMtyList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "StatShrQryWznmMtyList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmMtyListData", "StgIacQryWznmMtyList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmMtyListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmMtyListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmMtyListData") {
				mask = mergeDpchEngData(dom);
