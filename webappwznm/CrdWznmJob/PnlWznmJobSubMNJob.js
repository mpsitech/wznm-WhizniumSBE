/**
  * \file PnlWznmJobSubMNJob.js
  * web client functionality for panel PnlWznmJobSubMNJob
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("SubMNJob", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("SubMNJob_side").setAttribute("height", "30");
	doc.getElementById("SubMNJob_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("SubMNJob_side").src = "./PnlWznmJobSubMNJob_aside.html";
	doc.getElementById("SubMNJob_cont").src = "./PnlWznmJobSubMNJob_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("SubMNJob", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("SubMNJob_side").setAttribute("height", "255");
	doc.getElementById("SubMNJob_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("SubMNJob_side").src = "./PnlWznmJobSubMNJob_bside.html";
	doc.getElementById("SubMNJob_cont").src = "./PnlWznmJobSubMNJob_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("SubMNJob_side").contentDocument;
	contdoc = doc.getElementById("SubMNJob_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("SubMNJob_side").contentDocument;
	contdoc = doc.getElementById("SubMNJob_cont").contentDocument;
	hdrdoc = contdoc.getElementById("SubMNJob_hdr").contentDocument;
	tbldoc = contdoc.getElementById("SubMNJob_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("SubMNJob_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobSubMNJob", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobSubMNJob", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobSubMNJob", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobSubMNJob", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSubMNJob", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSubMNJob", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSubMNJob", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSubMNJob", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSubMNJob", "TcoMrefWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSubMNJob", "TcoShoWidth"));
		var TcoMltWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSubMNJob", "TcoMltWidth"));
		var TcoCsaWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSubMNJob", "TcoCsaWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobSubMNJob", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobSubMNJob", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobSubMNJob
		rownode = getRownode(srcdoc, "ListWznmQJobSubMNJob", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobSubMNJob", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobSubMNJob", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobSubMNJob", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("SubMNJob", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("SubMNJob_side").setAttribute("height", "" + (height+55));
	doc.getElementById("SubMNJob_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("SubMNJob_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("SubMNJob_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobSubMNJob", "ListWznmQJobSubMNJob", retrieveTi(srcdoc, "TagWznmJobSubMNJob", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobSubMNJob", "TxtRecord2"),
				["Mref","Sho","Mlt","Csa",""],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSubMNJob", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSubMNJob", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobSubMNJob", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobSubMNJob", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobSubMNJobData", scrJref, "StgIacQryWznmJobSubMNJob");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSubMNJob", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobSubMNJob", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobSubMNJob", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSubMNJob", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobSubMNJobDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "ContInfWznmJobSubMNJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StatAppWznmJobSubMNJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StatShrWznmJobSubMNJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StgIacWznmJobSubMNJob", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "TagWznmJobSubMNJob", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "ListWznmQJobSubMNJob", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StatAppQryWznmJobSubMNJob", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StatShrQryWznmJobSubMNJob", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobSubMNJobData", "StgIacQryWznmJobSubMNJob", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobSubMNJobData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobSubMNJobData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("SubMNJob");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobSubMNJobData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
