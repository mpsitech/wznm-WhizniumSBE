/**
  * \file PnlWznmJobMNOppack.js
  * web client functionality for panel PnlWznmJobMNOppack
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNOppack", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNOppack_side").setAttribute("height", "30");
	doc.getElementById("MNOppack_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNOppack_side").src = "./PnlWznmJobMNOppack_aside.html";
	doc.getElementById("MNOppack_cont").src = "./PnlWznmJobMNOppack_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNOppack", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNOppack_side").setAttribute("height", "255");
	doc.getElementById("MNOppack_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNOppack_side").src = "./PnlWznmJobMNOppack_bside.html";
	doc.getElementById("MNOppack_cont").src = "./PnlWznmJobMNOppack_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNOppack_side").contentDocument;
	contdoc = doc.getElementById("MNOppack_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNOppack_side").contentDocument;
	contdoc = doc.getElementById("MNOppack_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNOppack_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNOppack_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNOppack_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobMNOppack", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobMNOppack", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobMNOppack", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobMNOppack", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOppack", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOppack", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOppack", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOppack", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobMNOppack", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobMNOppack", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobMNOppack", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobMNOppack
		rownode = getRownode(srcdoc, "ListWznmQJobMNOppack", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobMNOppack", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobMNOppack", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobMNOppack", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNOppack", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNOppack_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNOppack_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNOppack_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNOppack_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobMNOppack", "ListWznmQJobMNOppack", retrieveTi(srcdoc, "TagWznmJobMNOppack", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobMNOppack", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOppack", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOppack", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobMNOppack", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobMNOppack", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobMNOppackData", scrJref, "StgIacQryWznmJobMNOppack");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOppack", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobMNOppack", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobMNOppack", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOppack", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobMNOppackDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "ContInfWznmJobMNOppack", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StatAppWznmJobMNOppack", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StatShrWznmJobMNOppack", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StgIacWznmJobMNOppack", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "TagWznmJobMNOppack", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "ListWznmQJobMNOppack", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StatAppQryWznmJobMNOppack", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StatShrQryWznmJobMNOppack", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOppackData", "StgIacQryWznmJobMNOppack", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobMNOppackData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobMNOppackData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNOppack");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobMNOppackData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
