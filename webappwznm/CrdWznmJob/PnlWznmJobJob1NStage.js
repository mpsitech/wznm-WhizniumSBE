...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Job1NStage", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Job1NStage_side").setAttribute("height", "30");
	doc.getElementById("Job1NStage_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Job1NStage_side").src = "./PnlWznmJobJob1NStage_aside.html";
	doc.getElementById("Job1NStage_cont").src = "./PnlWznmJobJob1NStage_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Job1NStage", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Job1NStage_side").setAttribute("height", "255");
	doc.getElementById("Job1NStage_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Job1NStage_side").src = "./PnlWznmJobJob1NStage_bside.html";
	doc.getElementById("Job1NStage_cont").src = "./PnlWznmJobJob1NStage_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Job1NStage_side").contentDocument;
	contdoc = doc.getElementById("Job1NStage_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Job1NStage_side").contentDocument;
	contdoc = doc.getElementById("Job1NStage_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Job1NStage_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Job1NStage_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Job1NStage_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobJob1NStage", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobJob1NStage", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobJob1NStage", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobJob1NStage", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobJob1NStage", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobJob1NStage", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobJob1NStage", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobJob1NStage", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobJob1NStage", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobJob1NStage", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobJob1NStage", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobJob1NStage
		rownode = getRownode(srcdoc, "ListWznmQJobJob1NStage", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobJob1NStage", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobJob1NStage", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobJob1NStage", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Job1NStage", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Job1NStage_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Job1NStage_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Job1NStage_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Job1NStage_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobJob1NStage", "ListWznmQJobJob1NStage", retrieveTi(srcdoc, "TagWznmJobJob1NStage", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobJob1NStage", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobJob1NStage", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobJob1NStage", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobJob1NStage", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobJob1NStage", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobJob1NStageData", scrJref, "StgIacQryWznmJobJob1NStage");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobJob1NStage", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobJob1NStage", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobJob1NStage", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobJob1NStage", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobJob1NStageDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "ContInfWznmJobJob1NStage", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StatAppWznmJobJob1NStage", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StatShrWznmJobJob1NStage", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StgIacWznmJobJob1NStage", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "TagWznmJobJob1NStage", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "ListWznmQJobJob1NStage", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StatAppQryWznmJobJob1NStage", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StatShrQryWznmJobJob1NStage", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobJob1NStageData", "StgIacQryWznmJobJob1NStage", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobJob1NStageData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobJob1NStageData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Job1NStage");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobJob1NStageData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
