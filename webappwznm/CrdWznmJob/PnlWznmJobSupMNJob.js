/**
  * \file PnlWznmJobSupMNJob.js
  * web client functionality for panel PnlWznmJobSupMNJob
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("SupMNJob", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("SupMNJob_side").setAttribute("height", "30");
	doc.getElementById("SupMNJob_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("SupMNJob_side").src = "./PnlWznmJobSupMNJob_aside.html";
	doc.getElementById("SupMNJob_cont").src = "./PnlWznmJobSupMNJob_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("SupMNJob", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("SupMNJob_side").setAttribute("height", "255");
	doc.getElementById("SupMNJob_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("SupMNJob_side").src = "./PnlWznmJobSupMNJob_bside.html";
	doc.getElementById("SupMNJob_cont").src = "./PnlWznmJobSupMNJob_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("SupMNJob_side").contentDocument;
	contdoc = doc.getElementById("SupMNJob_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("SupMNJob_side").contentDocument;
	contdoc = doc.getElementById("SupMNJob_cont").contentDocument;
	hdrdoc = contdoc.getElementById("SupMNJob_hdr").contentDocument;
	tbldoc = contdoc.getElementById("SupMNJob_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("SupMNJob_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobSupMNJob", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobSupMNJob", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobSupMNJob", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobSupMNJob", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSupMNJob", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSupMNJob", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSupMNJob", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSupMNJob", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSupMNJob", "TcoMrefWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSupMNJob", "TcoShoWidth"));
		var TcoMltWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSupMNJob", "TcoMltWidth"));
		var TcoCsaWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobSupMNJob", "TcoCsaWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSho"); doc.widths.push(TcoShoWidth); doc.fs.push("sho"); wtot += TcoShoWidth;
			} else if (i == 3) {
				doc.cols.push("TcoMlt"); doc.widths.push(TcoMltWidth); doc.fs.push("mlt"); wtot += TcoMltWidth;
			} else if (i == 4) {
				doc.cols.push("TcoCsa"); doc.widths.push(TcoCsaWidth); doc.fs.push("csa"); wtot += TcoCsaWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobSupMNJob", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobSupMNJob", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobSupMNJob
		rownode = getRownode(srcdoc, "ListWznmQJobSupMNJob", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobSupMNJob", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobSupMNJob", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobSupMNJob", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("SupMNJob", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("SupMNJob_side").setAttribute("height", "" + (height+55));
	doc.getElementById("SupMNJob_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("SupMNJob_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("SupMNJob_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobSupMNJob", "ListWznmQJobSupMNJob", retrieveTi(srcdoc, "TagWznmJobSupMNJob", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobSupMNJob", "TxtRecord2"),
				["Mref","Sho","Mlt","Csa",""],
				["mref","sho","mlt","csa","csa2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSupMNJob", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobSupMNJob", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobSupMNJob", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobSupMNJob", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobSupMNJobData", scrJref, "StgIacQryWznmJobSupMNJob");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobSupMNJob", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobSupMNJob", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobSupMNJob", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobSupMNJob", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobSupMNJobDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "ContInfWznmJobSupMNJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StatAppWznmJobSupMNJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StatShrWznmJobSupMNJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StgIacWznmJobSupMNJob", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "TagWznmJobSupMNJob", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "ListWznmQJobSupMNJob", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StatAppQryWznmJobSupMNJob", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StatShrQryWznmJobSupMNJob", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobSupMNJobData", "StgIacQryWznmJobSupMNJob", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobSupMNJobData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobSupMNJobData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("SupMNJob");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobSupMNJobData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
