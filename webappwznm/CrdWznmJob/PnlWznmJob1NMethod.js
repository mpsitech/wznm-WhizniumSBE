/**
  * \file PnlWznmJob1NMethod.js
  * web client functionality for panel PnlWznmJob1NMethod
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NMethod", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NMethod_side").setAttribute("height", "30");
	doc.getElementById("1NMethod_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NMethod_side").src = "./PnlWznmJob1NMethod_aside.html";
	doc.getElementById("1NMethod_cont").src = "./PnlWznmJob1NMethod_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NMethod", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NMethod_side").setAttribute("height", "255");
	doc.getElementById("1NMethod_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NMethod_side").src = "./PnlWznmJob1NMethod_bside.html";
	doc.getElementById("1NMethod_cont").src = "./PnlWznmJob1NMethod_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NMethod_side").contentDocument;
	contdoc = doc.getElementById("1NMethod_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NMethod_side").contentDocument;
	contdoc = doc.getElementById("1NMethod_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NMethod_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NMethod_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NMethod_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJob1NMethod", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJob1NMethod", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJob1NMethod", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJob1NMethod", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NMethod", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NMethod", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NMethod", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NMethod", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJob1NMethod", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJob1NMethod", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJob1NMethod", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJob1NMethod
		rownode = getRownode(srcdoc, "ListWznmQJob1NMethod", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJob1NMethod", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJob1NMethod", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJob1NMethod", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NMethod", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMethod_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMethod_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NMethod_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NMethod_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJob1NMethod", "ListWznmQJob1NMethod", retrieveTi(srcdoc, "TagWznmJob1NMethod", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJob1NMethod", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NMethod", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NMethod", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJob1NMethod", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJob1NMethod", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJob1NMethodData", scrJref, "StgIacQryWznmJob1NMethod");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NMethod", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJob1NMethod", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJob1NMethod", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NMethod", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJob1NMethodDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "ContInfWznmJob1NMethod", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StatAppWznmJob1NMethod", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StatShrWznmJob1NMethod", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StgIacWznmJob1NMethod", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "TagWznmJob1NMethod", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "ListWznmQJob1NMethod", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StatAppQryWznmJob1NMethod", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StatShrQryWznmJob1NMethod", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJob1NMethodData", "StgIacQryWznmJob1NMethod", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJob1NMethodData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJob1NMethodData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NMethod");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJob1NMethodData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
