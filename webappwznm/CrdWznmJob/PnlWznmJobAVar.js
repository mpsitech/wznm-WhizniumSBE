/**
  * \file PnlWznmJobAVar.js
  * web client functionality for panel PnlWznmJobAVar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("AVar", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AVar_side").setAttribute("height", "30");
	doc.getElementById("AVar_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AVar_side").src = "./PnlWznmJobAVar_aside.html";
	doc.getElementById("AVar_cont").src = "./PnlWznmJobAVar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("AVar", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AVar_side").setAttribute("height", "255");
	doc.getElementById("AVar_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AVar_side").src = "./PnlWznmJobAVar_bside.html";
	doc.getElementById("AVar_cont").src = "./PnlWznmJobAVar_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AVar_side").contentDocument;
	contdoc = doc.getElementById("AVar_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AVar_side").contentDocument;
	contdoc = doc.getElementById("AVar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AVar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AVar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AVar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobAVar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobAVar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobAVar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobAVar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobAVar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobAVar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobAVar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobAVar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoSrfWidth"));
		var TcoVtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoVtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoVecWidth"));
		var TcoLenWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoLenWidth"));
		var TcoShrWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoShrWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobAVar", "TcoCmtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoShr"); doc.widths.push(TcoShrWidth); doc.fs.push("shr"); wtot += TcoShrWidth;
			} else if (i == 6) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobAVar", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobAVar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobAVar
		rownode = getRownode(srcdoc, "ListWznmQJobAVar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobAVar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobAVar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobAVar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AVar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AVar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AVar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AVar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AVar_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobAVar", "ListWznmQJobAVar", retrieveTi(srcdoc, "TagWznmJobAVar", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobAVar", "TxtRecord2"),
				["Srf","Vty","","Vec","Len","Shr","Cmt"],
				["srf","vty","vty2","vec","len","shr","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobAVar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobAVar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobAVar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobAVar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobAVarData", scrJref, "StgIacQryWznmJobAVar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobAVar", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobAVar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobAVar", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobAVar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobAVarDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "ContInfWznmJobAVar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StatAppWznmJobAVar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StatShrWznmJobAVar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StgIacWznmJobAVar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "TagWznmJobAVar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "ListWznmQJobAVar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StatAppQryWznmJobAVar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StatShrQryWznmJobAVar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobAVarData", "StgIacQryWznmJobAVar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobAVarData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobAVarData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AVar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobAVarData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
