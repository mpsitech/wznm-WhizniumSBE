/**
  * \file PnlWznmJob1NSensitivity.js
  * web client functionality for panel PnlWznmJob1NSensitivity
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NSensitivity", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NSensitivity_side").setAttribute("height", "30");
	doc.getElementById("1NSensitivity_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NSensitivity_side").src = "./PnlWznmJob1NSensitivity_aside.html";
	doc.getElementById("1NSensitivity_cont").src = "./PnlWznmJob1NSensitivity_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NSensitivity", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NSensitivity_side").setAttribute("height", "255");
	doc.getElementById("1NSensitivity_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NSensitivity_side").src = "./PnlWznmJob1NSensitivity_bside.html";
	doc.getElementById("1NSensitivity_cont").src = "./PnlWznmJob1NSensitivity_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("1NSensitivity_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NSensitivity_side").contentDocument;
	contdoc = doc.getElementById("1NSensitivity_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NSensitivity_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NSensitivity_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NSensitivity_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJob1NSensitivity", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NSensitivity", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NSensitivity", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NSensitivity", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NSensitivity", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJob1NSensitivity", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJob1NSensitivity", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJob1NSensitivity
		rownode = getRownode(srcdoc, "ListWznmQJob1NSensitivity", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJob1NSensitivity", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NSensitivity", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSensitivity_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSensitivity_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NSensitivity_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NSensitivity_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJob1NSensitivity", "ListWznmQJob1NSensitivity", retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJob1NSensitivity", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NSensitivity", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJob1NSensitivity", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJob1NSensitivity", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJob1NSensitivity", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJob1NSensitivityData", scrJref, "StgIacQryWznmJob1NSensitivity");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJob1NSensitivity", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJob1NSensitivity", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJob1NSensitivity", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJob1NSensitivityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "ContInfWznmJob1NSensitivity", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "StatAppWznmJob1NSensitivity", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "StgIacWznmJob1NSensitivity", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "TagWznmJob1NSensitivity", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "ListWznmQJob1NSensitivity", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "StatAppQryWznmJob1NSensitivity", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "StatShrQryWznmJob1NSensitivity", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJob1NSensitivityData", "StgIacQryWznmJob1NSensitivity", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJob1NSensitivityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJob1NSensitivityData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NSensitivity");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJob1NSensitivityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
