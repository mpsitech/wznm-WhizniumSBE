/**
  * \file PnlWznmJobMNOp.js
  * web client functionality for panel PnlWznmJobMNOp
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNOp", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNOp_side").setAttribute("height", "30");
	doc.getElementById("MNOp_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNOp_side").src = "./PnlWznmJobMNOp_aside.html";
	doc.getElementById("MNOp_cont").src = "./PnlWznmJobMNOp_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNOp", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNOp_side").setAttribute("height", "255");
	doc.getElementById("MNOp_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNOp_side").src = "./PnlWznmJobMNOp_bside.html";
	doc.getElementById("MNOp_cont").src = "./PnlWznmJobMNOp_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNOp_side").contentDocument;
	contdoc = doc.getElementById("MNOp_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNOp_side").contentDocument;
	contdoc = doc.getElementById("MNOp_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNOp_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNOp_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNOp_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobMNOp", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobMNOp", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobMNOp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobMNOp", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobMNOp", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobMNOp", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobMNOp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobMNOp
		rownode = getRownode(srcdoc, "ListWznmQJobMNOp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobMNOp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobMNOp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobMNOp", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNOp", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNOp_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNOp_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNOp_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNOp_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobMNOp", "ListWznmQJobMNOp", retrieveTi(srcdoc, "TagWznmJobMNOp", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobMNOp", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobMNOp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobMNOp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobMNOp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobMNOpData", scrJref, "StgIacQryWznmJobMNOp");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobMNOp", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobMNOp", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobMNOp", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobMNOp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobMNOpDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "ContInfWznmJobMNOp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StatAppWznmJobMNOp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StatShrWznmJobMNOp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StgIacWznmJobMNOp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "TagWznmJobMNOp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "ListWznmQJobMNOp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StatAppQryWznmJobMNOp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StatShrQryWznmJobMNOp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobMNOpData", "StgIacQryWznmJobMNOp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobMNOpData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobMNOpData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNOp");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobMNOpData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
