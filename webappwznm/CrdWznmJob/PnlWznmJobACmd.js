/**
  * \file PnlWznmJobACmd.js
  * web client functionality for panel PnlWznmJobACmd
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("ACmd", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ACmd_side").setAttribute("height", "30");
	doc.getElementById("ACmd_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ACmd_side").src = "./PnlWznmJobACmd_aside.html";
	doc.getElementById("ACmd_cont").src = "./PnlWznmJobACmd_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("ACmd", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ACmd_side").setAttribute("height", "255");
	doc.getElementById("ACmd_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ACmd_side").src = "./PnlWznmJobACmd_bside.html";
	doc.getElementById("ACmd_cont").src = "./PnlWznmJobACmd_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ACmd_side").contentDocument;
	contdoc = doc.getElementById("ACmd_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ACmd_side").contentDocument;
	contdoc = doc.getElementById("ACmd_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ACmd_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ACmd_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ACmd_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobACmd", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmJobACmd", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmJobACmd", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmJobACmd", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobACmd", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobACmd", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobACmd", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobACmd", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobACmd", "TcoSrfWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmJobACmd", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmJobACmd", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmJobACmd", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQJobACmd
		rownode = getRownode(srcdoc, "ListWznmQJobACmd", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmJobACmd", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmJobACmd", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmJobACmd", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ACmd", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ACmd_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ACmd_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ACmd_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ACmd_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmJobACmd", "ListWznmQJobACmd", retrieveTi(srcdoc, "TagWznmJobACmd", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmJobACmd", "TxtRecord2"),
				["Srf","Cmt"],
				["srf","cmt"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobACmd", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmJobACmd", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmJobACmd", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmJobACmd", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmJobACmdData", scrJref, "StgIacQryWznmJobACmd");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmJobACmd", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmJobACmd", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmJobACmd", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmJobACmd", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmJobACmdDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "ContInfWznmJobACmd", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StatAppWznmJobACmd", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StatShrWznmJobACmd", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StgIacWznmJobACmd", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "TagWznmJobACmd", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "ListWznmQJobACmd", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StatAppQryWznmJobACmd", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StatShrQryWznmJobACmd", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmJobACmdData", "StgIacQryWznmJobACmd", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmJobACmdData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmJobACmdData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ACmd");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmJobACmdData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
