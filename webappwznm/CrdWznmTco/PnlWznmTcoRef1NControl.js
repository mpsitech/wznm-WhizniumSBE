/**
  * \file PnlWznmTcoRef1NControl.js
  * web client functionality for panel PnlWznmTcoRef1NControl
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NControl", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NControl_side").setAttribute("height", "30");
	doc.getElementById("Ref1NControl_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NControl_side").src = "./PnlWznmTcoRef1NControl_aside.html";
	doc.getElementById("Ref1NControl_cont").src = "./PnlWznmTcoRef1NControl_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NControl", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NControl_side").setAttribute("height", "255");
	doc.getElementById("Ref1NControl_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NControl_side").src = "./PnlWznmTcoRef1NControl_bside.html";
	doc.getElementById("Ref1NControl_cont").src = "./PnlWznmTcoRef1NControl_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NControl_side").contentDocument;
	contdoc = doc.getElementById("Ref1NControl_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NControl_side").contentDocument;
	contdoc = doc.getElementById("Ref1NControl_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NControl_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NControl_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NControl_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NControl", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NControl", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NControl", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NControl", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NControl", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoRef1NControl", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTcoRef1NControl", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTcoRef1NControl
		rownode = getRownode(srcdoc, "ListWznmQTcoRef1NControl", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTcoRef1NControl", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NControl", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NControl_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NControl_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NControl_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NControl_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTcoRef1NControl", "ListWznmQTcoRef1NControl", retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTcoRef1NControl", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NControl", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NControl", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTcoRef1NControl", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTcoRef1NControl", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTcoRef1NControlData", scrJref, "StgIacQryWznmTcoRef1NControl");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTcoRef1NControl", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTcoRef1NControl", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NControl", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTcoRef1NControlDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "ContInfWznmTcoRef1NControl", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StatAppWznmTcoRef1NControl", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StatShrWznmTcoRef1NControl", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StgIacWznmTcoRef1NControl", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "TagWznmTcoRef1NControl", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "ListWznmQTcoRef1NControl", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StatAppQryWznmTcoRef1NControl", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StatShrQryWznmTcoRef1NControl", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NControlData", "StgIacQryWznmTcoRef1NControl", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTcoRef1NControlData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTcoRef1NControlData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NControl");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTcoRef1NControlData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
