/**
  * \file PnlWznmTcoRef1NQuerymod.js
  * web client functionality for panel PnlWznmTcoRef1NQuerymod
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NQuerymod_side").src = "./PnlWznmTcoRef1NQuerymod_aside.html";
	doc.getElementById("Ref1NQuerymod_cont").src = "./PnlWznmTcoRef1NQuerymod_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NQuerymod_side").src = "./PnlWznmTcoRef1NQuerymod_bside.html";
	doc.getElementById("Ref1NQuerymod_cont").src = "./PnlWznmTcoRef1NQuerymod_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NQuerymod", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NQuerymod", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NQuerymod", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NQuerymod", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NQuerymod", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoRef1NQuerymod", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTcoRef1NQuerymod
		rownode = getRownode(srcdoc, "ListWznmQTcoRef1NQuerymod", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTcoRef1NQuerymod", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTcoRef1NQuerymod", "ListWznmQTcoRef1NQuerymod", retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTcoRef1NQuerymod", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NQuerymod", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoRef1NQuerymod", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTcoRef1NQuerymod", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTcoRef1NQuerymod", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTcoRef1NQuerymodData", scrJref, "StgIacQryWznmTcoRef1NQuerymod");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTcoRef1NQuerymod", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTcoRef1NQuerymod", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoRef1NQuerymod", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTcoRef1NQuerymodDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "ContInfWznmTcoRef1NQuerymod", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StatAppWznmTcoRef1NQuerymod", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StatShrWznmTcoRef1NQuerymod", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StgIacWznmTcoRef1NQuerymod", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "TagWznmTcoRef1NQuerymod", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "ListWznmQTcoRef1NQuerymod", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StatAppQryWznmTcoRef1NQuerymod", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StatShrQryWznmTcoRef1NQuerymod", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoRef1NQuerymodData", "StgIacQryWznmTcoRef1NQuerymod", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTcoRef1NQuerymodData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTcoRef1NQuerymodData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTcoRef1NQuerymodData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
