...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmTcoList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmTcoList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmTcoList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmTcoList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTcoList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmTcoList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmTcoList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmTcoList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTcoList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTcoList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmTcoList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTcoList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmTcoList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmTcoList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoSrfWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoShoWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoTypWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoTblWidth"));
		var TcoFctWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoFctWidth"));
		var TcoStyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoStyWidth"));
		var TcoAxfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTcoList", "TcoAxfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmTcoList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoAxf"); doc.widths.push(TcoAxfWidth); doc.fs.push("axf2"); doc.tos.push(0); wtot += TcoAxfWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmTcoList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTcoList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTcoList
		rownode = getRownode(srcdoc, "ListWznmQTcoList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTcoList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTcoList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTcoList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTcoList", "ListWznmQTcoList", retrieveTi(srcdoc, "TagWznmTcoList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTcoList", "TxtRecord2"),
				["Srf","Sho","","Typ","Tbl","","Fct","","Sty","","Axf"],
				["srf","sho","typ","typ2","tbl","fct","fct2","sty","sty2","axf","axf2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTcoList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTcoList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTcoList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTcoListData", scrJref, "StgIacQryWznmTcoList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTcoList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTcoList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTcoList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTcoListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmTcoList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmTcoList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTcoListData", scrJref, "ContIacWznmTcoList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "ContIacWznmTcoList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "ContInfWznmTcoList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "StatShrWznmTcoList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "StgIacWznmTcoList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "TagWznmTcoList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "ListWznmQTcoList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "StatAppQryWznmTcoList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "StatShrQryWznmTcoList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTcoListData", "StgIacQryWznmTcoList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmTcoListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTcoListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTcoListData") {
				mask = mergeDpchEngData(dom);
