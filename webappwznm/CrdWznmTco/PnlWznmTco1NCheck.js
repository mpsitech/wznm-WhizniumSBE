/**
  * \file PnlWznmTco1NCheck.js
  * web client functionality for panel PnlWznmTco1NCheck
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NCheck", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NCheck_side").setAttribute("height", "30");
	doc.getElementById("1NCheck_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NCheck_side").src = "./PnlWznmTco1NCheck_aside.html";
	doc.getElementById("1NCheck_cont").src = "./PnlWznmTco1NCheck_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NCheck", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NCheck_side").setAttribute("height", "255");
	doc.getElementById("1NCheck_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NCheck_side").src = "./PnlWznmTco1NCheck_bside.html";
	doc.getElementById("1NCheck_cont").src = "./PnlWznmTco1NCheck_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NCheck_side").contentDocument;
	contdoc = doc.getElementById("1NCheck_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NCheck_side").contentDocument;
	contdoc = doc.getElementById("1NCheck_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NCheck_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NCheck_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NCheck_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTco1NCheck", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTco1NCheck", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTco1NCheck", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTco1NCheck", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTco1NCheck", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTco1NCheck", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTco1NCheck", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTco1NCheck", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTco1NCheck", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTco1NCheck", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTco1NCheck", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTco1NCheck
		rownode = getRownode(srcdoc, "ListWznmQTco1NCheck", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTco1NCheck", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTco1NCheck", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTco1NCheck", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NCheck", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCheck_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCheck_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NCheck_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NCheck_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTco1NCheck", "ListWznmQTco1NCheck", retrieveTi(srcdoc, "TagWznmTco1NCheck", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTco1NCheck", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTco1NCheck", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTco1NCheck", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTco1NCheck", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTco1NCheck", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTco1NCheckData", scrJref, "StgIacQryWznmTco1NCheck");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTco1NCheck", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTco1NCheck", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTco1NCheck", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTco1NCheck", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTco1NCheckDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "ContInfWznmTco1NCheck", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StatAppWznmTco1NCheck", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StatShrWznmTco1NCheck", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StgIacWznmTco1NCheck", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "TagWznmTco1NCheck", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "ListWznmQTco1NCheck", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StatAppQryWznmTco1NCheck", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StatShrQryWznmTco1NCheck", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTco1NCheckData", "StgIacQryWznmTco1NCheck", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTco1NCheckData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTco1NCheckData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NCheck");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTco1NCheckData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
