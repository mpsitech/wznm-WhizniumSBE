/**
  * \file CrdWznmUsr.js
  * web client functionality for card CrdWznmUsr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmUsr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmUsr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmUsr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmUsr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmUsr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmUsrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmUsrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmUsrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmUsr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmUsr", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmUsr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmUsr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmUsr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmUsr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmUsr", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MitCrdMkpAvail = (retrieveSi(srcdoc, "StatShrWznmUsr", "MitCrdMkpAvail") == "true");
	MitCrdMkpActive = (retrieveSi(srcdoc, "StatShrWznmUsr", "MitCrdMkpActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmUsr", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsr"), retrieveCi(srcdoc, "ContInfWznmUsr", "MtxCrdUsr"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdMkp"), retrieveTi(srcdoc, "TagWznmUsr", "MitCrdMkp"));
...
function refresh() {
	var scrJrefDlgmngkeypair = retrieveSi(srcdoc, "StatShrWznmUsr", "scrJrefDlgmngkeypair");

	if (scrJrefDlgmngkeypair != "") {
		if (scrJrefDlg != scrJrefDlgmngkeypair) showDlg("DlgWznmUsrMngkeypair", scrJrefDlgmngkeypair);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWznmUsr", "MtxCrdUsr") + " - WhizniumSBE v1.0.6";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmUsrDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmUsrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmUsr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmUsrData", "ContInfWznmUsr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmUsrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmUsrData", "StatAppWznmUsr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmUsrData", "StatShrWznmUsr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmUsrData", "TagWznmUsr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmUsrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmUsr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmUsr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmUsr", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmUsrData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmUsr", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmUsr", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmUsr", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmUsr", "latency"));
