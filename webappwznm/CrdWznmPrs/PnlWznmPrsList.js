/**
  * \file PnlWznmPrsList.js
  * web client functionality for panel PnlWznmPrsList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmPrsList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmPrsList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmPrsList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmPrsList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmPrsList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmPrsList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmPrsList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmPrsList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmPrsList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoOwnWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoTitWidth"));
		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoFnmWidth"));
		var TcoLnmWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoLnmWidth"));
		var TcoSexWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoSexWidth"));
		var TcoTelWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoTelWidth"));
		var TcoEmlWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsList", "TcoEmlWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmPrsList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmPrsList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmPrsList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQPrsList
		rownode = getRownode(srcdoc, "ListWznmQPrsList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmPrsList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmPrsList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmPrsList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmPrsList", "ListWznmQPrsList", retrieveTi(srcdoc, "TagWznmPrsList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmPrsList", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmPrsList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmPrsList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmPrsListData", scrJref, "StgIacQryWznmPrsList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmPrsList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmPrsListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmPrsList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmPrsList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmPrsListData", scrJref, "ContIacWznmPrsList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "ContIacWznmPrsList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "ContInfWznmPrsList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "StatShrWznmPrsList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "StgIacWznmPrsList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "TagWznmPrsList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "ListWznmQPrsList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "StatAppQryWznmPrsList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "StatShrQryWznmPrsList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmPrsListData", "StgIacQryWznmPrsList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmPrsListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmPrsListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmPrsListData") {
				mask = mergeDpchEngData(dom);
