/**
  * \file PnlWznmPrsADetail.js
  * web client functionality for panel PnlWznmPrsADetail
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlWznmPrsADetail_aside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWznmPrsADetail_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("ADetail_side").src = "./PnlWznmPrsADetail_bside.html";
	doc.getElementById("ADetail_cont").src = "./PnlWznmPrsADetail_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmPrsADetail", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmPrsADetail", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmPrsADetail", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmPrsADetail", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsADetail", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsADetail", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsADetail", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsADetail", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsADetail", "TcoTypWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmPrsADetail", "TcoValWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmPrsADetail", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmPrsADetail", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQPrsADetail
		rownode = getRownode(srcdoc, "ListWznmQPrsADetail", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmPrsADetail", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmPrsADetail", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmPrsADetail", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmPrsADetail", "ListWznmQPrsADetail", retrieveTi(srcdoc, "TagWznmPrsADetail", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmPrsADetail", "TxtRecord2"),
				["Typ","","Val"],
				["typ","typ2","val"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsADetail", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmPrsADetail", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmPrsADetail", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmPrsADetail", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmPrsADetailData", scrJref, "StgIacQryWznmPrsADetail");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmPrsADetail", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmPrsADetail", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmPrsADetail", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmPrsADetail", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmPrsADetailDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "ContInfWznmPrsADetail", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StatAppWznmPrsADetail", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StatShrWznmPrsADetail", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StgIacWznmPrsADetail", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "TagWznmPrsADetail", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "ListWznmQPrsADetail", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StatAppQryWznmPrsADetail", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StatShrQryWznmPrsADetail", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmPrsADetailData", "StgIacQryWznmPrsADetail", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmPrsADetailData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmPrsADetailData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmPrsADetailData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
