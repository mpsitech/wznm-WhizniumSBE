...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Tos1NRelation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Tos1NRelation_side").setAttribute("height", "30");
	doc.getElementById("Tos1NRelation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Tos1NRelation_side").src = "./PnlWznmSbsTos1NRelation_aside.html";
	doc.getElementById("Tos1NRelation_cont").src = "./PnlWznmSbsTos1NRelation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Tos1NRelation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Tos1NRelation_side").setAttribute("height", "255");
	doc.getElementById("Tos1NRelation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Tos1NRelation_side").src = "./PnlWznmSbsTos1NRelation_bside.html";
	doc.getElementById("Tos1NRelation_cont").src = "./PnlWznmSbsTos1NRelation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Tos1NRelation_side").contentDocument;
	contdoc = doc.getElementById("Tos1NRelation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Tos1NRelation_side").contentDocument;
	contdoc = doc.getElementById("Tos1NRelation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Tos1NRelation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Tos1NRelation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Tos1NRelation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmSbsTos1NRelation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmSbsTos1NRelation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmSbsTos1NRelation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmSbsTos1NRelation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmSbsTos1NRelation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmSbsTos1NRelation", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQSbsTos1NRelation
		rownode = getRownode(srcdoc, "ListWznmQSbsTos1NRelation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmSbsTos1NRelation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Tos1NRelation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Tos1NRelation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Tos1NRelation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Tos1NRelation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Tos1NRelation_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmSbsTos1NRelation", "ListWznmQSbsTos1NRelation", retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmSbsTos1NRelation", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmSbsTos1NRelation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmSbsTos1NRelation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmSbsTos1NRelation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmSbsTos1NRelation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmSbsTos1NRelationData", scrJref, "StgIacQryWznmSbsTos1NRelation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmSbsTos1NRelation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmSbsTos1NRelation", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmSbsTos1NRelation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmSbsTos1NRelationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "ContInfWznmSbsTos1NRelation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StatAppWznmSbsTos1NRelation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StatShrWznmSbsTos1NRelation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StgIacWznmSbsTos1NRelation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "TagWznmSbsTos1NRelation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "ListWznmQSbsTos1NRelation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StatAppQryWznmSbsTos1NRelation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StatShrQryWznmSbsTos1NRelation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmSbsTos1NRelationData", "StgIacQryWznmSbsTos1NRelation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmSbsTos1NRelationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmSbsTos1NRelationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Tos1NRelation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmSbsTos1NRelationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
