/**
  * \file PnlWznmImeList.js
  * web client functionality for panel PnlWznmImeList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmImeList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmImeList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmImeList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmImeList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmImeList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmImeList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmImeList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmImeList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmImeList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmImeList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmImeList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmImeList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmImeList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmImeList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmImeList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmImeList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmImeList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmImeList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmImeList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmImeList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmImeList", "TcoSrfWidth"));
		var TcoIexWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmImeList", "TcoIexWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmImeList", "TcoSupWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmImeList", "TcoTblWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmImeList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmImeList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmImeList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQImeList
		rownode = getRownode(srcdoc, "ListWznmQImeList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmImeList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmImeList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmImeList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmImeList", "ListWznmQImeList", retrieveTi(srcdoc, "TagWznmImeList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmImeList", "TxtRecord2"),
				["Srf","Iex","Sup","Tbl"],
				["srf","iex","sup","tbl"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmImeList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmImeList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmImeList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmImeList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmImeListData", scrJref, "StgIacQryWznmImeList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmImeList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmImeList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmImeList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmImeListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmImeList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmImeList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmImeListData", scrJref, "ContIacWznmImeList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "ContIacWznmImeList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "ContInfWznmImeList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "StatShrWznmImeList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "StgIacWznmImeList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "TagWznmImeList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "ListWznmQImeList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "StatAppQryWznmImeList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "StatShrQryWznmImeList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmImeListData", "StgIacQryWznmImeList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmImeListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmImeListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmImeListData") {
				mask = mergeDpchEngData(dom);
