/**
  * \file PnlWznmOpxRef1NBlock.js
  * web client functionality for panel PnlWznmOpxRef1NBlock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NBlock_side").src = "./PnlWznmOpxRef1NBlock_aside.html";
	doc.getElementById("Ref1NBlock_cont").src = "./PnlWznmOpxRef1NBlock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NBlock_side").src = "./PnlWznmOpxRef1NBlock_bside.html";
	doc.getElementById("Ref1NBlock_cont").src = "./PnlWznmOpxRef1NBlock_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmOpxRef1NBlock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxRef1NBlock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxRef1NBlock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxRef1NBlock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxRef1NBlock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxRef1NBlock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQOpxRef1NBlock
		rownode = getRownode(srcdoc, "ListWznmQOpxRef1NBlock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmOpxRef1NBlock", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmOpxRef1NBlock", "ListWznmQOpxRef1NBlock", retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmOpxRef1NBlock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxRef1NBlock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxRef1NBlock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmOpxRef1NBlock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmOpxRef1NBlock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmOpxRef1NBlockData", scrJref, "StgIacQryWznmOpxRef1NBlock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmOpxRef1NBlock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxRef1NBlock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxRef1NBlock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmOpxRef1NBlockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "ContInfWznmOpxRef1NBlock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StatAppWznmOpxRef1NBlock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StatShrWznmOpxRef1NBlock", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StgIacWznmOpxRef1NBlock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "TagWznmOpxRef1NBlock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "ListWznmQOpxRef1NBlock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StatAppQryWznmOpxRef1NBlock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StatShrQryWznmOpxRef1NBlock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxRef1NBlockData", "StgIacQryWznmOpxRef1NBlock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmOpxRef1NBlockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmOpxRef1NBlockData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmOpxRef1NBlockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
