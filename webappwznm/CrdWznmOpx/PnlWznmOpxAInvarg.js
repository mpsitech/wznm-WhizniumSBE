/**
  * \file PnlWznmOpxAInvarg.js
  * web client functionality for panel PnlWznmOpxAInvarg
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("AInvarg_side").src = "./PnlWznmOpxAInvarg_aside.html";
	doc.getElementById("AInvarg_cont").src = "./PnlWznmOpxAInvarg_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("AInvarg_side").src = "./PnlWznmOpxAInvarg_bside.html";
	doc.getElementById("AInvarg_cont").src = "./PnlWznmOpxAInvarg_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxAInvarg", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmOpxAInvarg", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxAInvarg", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmOpxAInvarg", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxAInvarg", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxAInvarg", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxAInvarg", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxAInvarg", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoSrfWidth"));
		var TcoVtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoVtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoVecWidth"));
		var TcoDvlWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoDvlWidth"));
		var TcoVitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoVitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxAInvarg", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmOpxAInvarg", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmOpxAInvarg", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQOpxAInvarg
		rownode = getRownode(srcdoc, "ListWznmQOpxAInvarg", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmOpxAInvarg", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmOpxAInvarg", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmOpxAInvarg", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmOpxAInvarg", "ListWznmQOpxAInvarg", retrieveTi(srcdoc, "TagWznmOpxAInvarg", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmOpxAInvarg", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxAInvarg", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxAInvarg", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmOpxAInvarg", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmOpxAInvarg", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmOpxAInvargData", scrJref, "StgIacQryWznmOpxAInvarg");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxAInvarg", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmOpxAInvarg", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxAInvarg", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxAInvarg", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmOpxAInvargDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "ContInfWznmOpxAInvarg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StatAppWznmOpxAInvarg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StatShrWznmOpxAInvarg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StgIacWznmOpxAInvarg", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "TagWznmOpxAInvarg", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "ListWznmQOpxAInvarg", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StatAppQryWznmOpxAInvarg", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StatShrQryWznmOpxAInvarg", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxAInvargData", "StgIacQryWznmOpxAInvarg", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmOpxAInvargData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmOpxAInvargData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmOpxAInvargData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
