/**
  * \file PnlWznmOpxARetval.js
  * web client functionality for panel PnlWznmOpxARetval
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("ARetval_side").src = "./PnlWznmOpxARetval_aside.html";
	doc.getElementById("ARetval_cont").src = "./PnlWznmOpxARetval_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("ARetval_side").src = "./PnlWznmOpxARetval_bside.html";
	doc.getElementById("ARetval_cont").src = "./PnlWznmOpxARetval_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxARetval", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmOpxARetval", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmOpxARetval", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmOpxARetval", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxARetval", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxARetval", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxARetval", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxARetval", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxARetval", "TcoSrfWidth"));
		var TcoVtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxARetval", "TcoVtyWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxARetval", "TcoVecWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmOpxARetval", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmOpxARetval", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmOpxARetval", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQOpxARetval
		rownode = getRownode(srcdoc, "ListWznmQOpxARetval", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmOpxARetval", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmOpxARetval", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmOpxARetval", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmOpxARetval", "ListWznmQOpxARetval", retrieveTi(srcdoc, "TagWznmOpxARetval", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmOpxARetval", "TxtRecord2"),
				["Srf","Vty","","Vec","Cmt"],
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxARetval", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmOpxARetval", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmOpxARetval", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmOpxARetval", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmOpxARetvalData", scrJref, "StgIacQryWznmOpxARetval");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmOpxARetval", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmOpxARetval", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmOpxARetval", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmOpxARetval", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmOpxARetvalDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "ContInfWznmOpxARetval", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StatAppWznmOpxARetval", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StatShrWznmOpxARetval", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StgIacWznmOpxARetval", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "TagWznmOpxARetval", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "ListWznmQOpxARetval", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StatAppQryWznmOpxARetval", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StatShrQryWznmOpxARetval", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmOpxARetvalData", "StgIacQryWznmOpxARetval", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmOpxARetvalData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmOpxARetvalData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmOpxARetvalData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
