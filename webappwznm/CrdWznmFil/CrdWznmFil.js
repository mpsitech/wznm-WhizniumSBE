/**
  * \file CrdWznmFil.js
  * web client functionality for card CrdWznmFil
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmFil", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmFil", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppWznmFil", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppWznmFil", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppWznmFil", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlWznmFilHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlWznmFilList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlWznmFilRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppWznmFil", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppWznmFil", "shortMenu", short);
	setSi(srcdoc, "StatAppWznmFil", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppWznmFil", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmFil", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagWznmFil", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagWznmFil", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrWznmFil", "MspCrd1Avail") == "true");
	MitCrdNewAvail = (retrieveSi(srcdoc, "StatShrWznmFil", "MitCrdNewAvail") == "true");
	MitCrdDldAvail = (retrieveSi(srcdoc, "StatShrWznmFil", "MitCrdDldAvail") == "true");
	MitCrdDldActive = (retrieveSi(srcdoc, "StatShrWznmFil", "MitCrdDldActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppWznmFil", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdFil"), retrieveCi(srcdoc, "ContInfWznmFil", "MtxCrdFil"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdNew"), retrieveTi(srcdoc, "TagWznmFil", "MitCrdNew"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdDld"), retrieveTi(srcdoc, "TagWznmFil", "MitCrdDld"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgWznmFilNew")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgdownload = retrieveSi(srcdoc, "StatShrWznmFil", "scrJrefDlgdownload");
	var scrJrefDlgnew = retrieveSi(srcdoc, "StatShrWznmFil", "scrJrefDlgnew");

	if (scrJrefDlgdownload != "") {
		if (scrJrefDlg != scrJrefDlgdownload) showDlg("DlgWznmFilDownload", scrJrefDlgdownload);
	} else if (scrJrefDlgnew != "") {
		if (scrJrefDlg != scrJrefDlgnew) showDlg("DlgWznmFilNew", scrJrefDlgnew);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfWznmFil", "MtxCrdFil") + " - WhizniumSBE v1.0.7";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmFilDo", scrJref, "close");
	sendReq(str, doc, null, true);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmFilDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfWznmFil", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmFilData", "ContInfWznmFil", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmFilData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngWznmFilData", "StatAppWznmFil", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmFilData", "StatShrWznmFil", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmFilData", "TagWznmFil", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmFilData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrWznmFil", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrWznmFil", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrWznmFil", "scrJrefRec");
		...
		if (blk) {
			...
			if (_scrJref != "") {
				if (_scrJref == scrJref) {
					...
					} else if (blk.nodeName == "DpchEngWznmFilData") {
						mergeDpchEngData(dom);
						refresh();
					};
...
function restartNotify() {
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmFil", "srefIxWznmVReqitmode");

	if (srefIxWznmVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxWznmVReqitmode = "notify";
		setSi(srcdoc, "StatAppWznmFil", "srefIxWznmVReqitmode", srefIxWznmVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxWznmVReqitmode = retrieveSi(srcdoc, "StatAppWznmFil", "srefIxWznmVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppWznmFil", "latency"));
