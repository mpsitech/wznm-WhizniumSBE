/**
  * \file PnlWznmBlkAItem.js
  * web client functionality for panel PnlWznmBlkAItem
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("AItem", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AItem_side").setAttribute("height", "30");
	doc.getElementById("AItem_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AItem_side").src = "./PnlWznmBlkAItem_aside.html";
	doc.getElementById("AItem_cont").src = "./PnlWznmBlkAItem_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("AItem", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AItem_side").setAttribute("height", "255");
	doc.getElementById("AItem_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AItem_side").src = "./PnlWznmBlkAItem_bside.html";
	doc.getElementById("AItem_cont").src = "./PnlWznmBlkAItem_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AItem_side").contentDocument;
	contdoc = doc.getElementById("AItem_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AItem_side").contentDocument;
	contdoc = doc.getElementById("AItem_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AItem_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AItem_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AItem_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBlkAItem", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmBlkAItem", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBlkAItem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBlkAItem", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkAItem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkAItem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkAItem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkAItem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoTypWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoSrfWidth"));
		var TcoVtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoVtyWidth"));
		var TcoConWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoConWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoVecWidth"));
		var TcoFedWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoFedWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoTblWidth"));
		var TcoBl2Width = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoBl2Width"));
		var TcoDfvWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoDfvWidth"));
		var TcoVitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoVitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkAItem", "TcoCmtWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoVty"); doc.widths.push(TcoVtyWidth); doc.fs.push("vty"); wtot += TcoVtyWidth;
			} else if (i == 4) {
				doc.cols.push("TcoCon"); doc.widths.push(TcoConWidth); doc.fs.push("con"); wtot += TcoConWidth;
			} else if (i == 5) {
				doc.cols.push("TcoVec"); doc.widths.push(TcoVecWidth); doc.fs.push("vec"); wtot += TcoVecWidth;
			} else if (i == 6) {
				doc.cols.push("TcoFed"); doc.widths.push(TcoFedWidth); doc.fs.push("fed"); wtot += TcoFedWidth;
			} else if (i == 7) {
				doc.cols.push("TcoTbl"); doc.widths.push(TcoTblWidth); doc.fs.push("tbl"); wtot += TcoTblWidth;
			} else if (i == 8) {
				doc.cols.push("TcoBl2"); doc.widths.push(TcoBl2Width); doc.fs.push("bl2"); wtot += TcoBl2Width;
			} else if (i == 9) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmBlkAItem", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmBlkAItem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQBlkAItem
		rownode = getRownode(srcdoc, "ListWznmQBlkAItem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmBlkAItem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmBlkAItem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmBlkAItem", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AItem", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AItem_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AItem_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AItem_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AItem_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmBlkAItem", "ListWznmQBlkAItem", retrieveTi(srcdoc, "TagWznmBlkAItem", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmBlkAItem", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkAItem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkAItem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmBlkAItem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmBlkAItem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBlkAItemData", scrJref, "StgIacQryWznmBlkAItem");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkAItem", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmBlkAItem", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmBlkAItem", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkAItem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmBlkAItemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "ContInfWznmBlkAItem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StatAppWznmBlkAItem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StatShrWznmBlkAItem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StgIacWznmBlkAItem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "TagWznmBlkAItem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "ListWznmQBlkAItem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StatAppQryWznmBlkAItem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StatShrQryWznmBlkAItem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmBlkAItemData", "StgIacQryWznmBlkAItem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmBlkAItemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmBlkAItemData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AItem");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmBlkAItemData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
