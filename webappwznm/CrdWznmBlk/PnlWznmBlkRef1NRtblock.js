/**
  * \file PnlWznmBlkRef1NRtblock.js
  * web client functionality for panel PnlWznmBlkRef1NRtblock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NRtblock", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NRtblock_side").setAttribute("height", "30");
	doc.getElementById("Ref1NRtblock_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NRtblock_side").src = "./PnlWznmBlkRef1NRtblock_aside.html";
	doc.getElementById("Ref1NRtblock_cont").src = "./PnlWznmBlkRef1NRtblock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NRtblock", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NRtblock_side").setAttribute("height", "255");
	doc.getElementById("Ref1NRtblock_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NRtblock_side").src = "./PnlWznmBlkRef1NRtblock_bside.html";
	doc.getElementById("Ref1NRtblock_cont").src = "./PnlWznmBlkRef1NRtblock_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NRtblock_side").contentDocument;
	contdoc = doc.getElementById("Ref1NRtblock_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NRtblock_side").contentDocument;
	contdoc = doc.getElementById("Ref1NRtblock_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NRtblock_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NRtblock_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NRtblock_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmBlkRef1NRtblock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkRef1NRtblock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkRef1NRtblock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkRef1NRtblock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkRef1NRtblock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmBlkRef1NRtblock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQBlkRef1NRtblock
		rownode = getRownode(srcdoc, "ListWznmQBlkRef1NRtblock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmBlkRef1NRtblock", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NRtblock", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NRtblock_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NRtblock_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NRtblock_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NRtblock_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmBlkRef1NRtblock", "ListWznmQBlkRef1NRtblock", retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmBlkRef1NRtblock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkRef1NRtblock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmBlkRef1NRtblock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmBlkRef1NRtblock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmBlkRef1NRtblock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmBlkRef1NRtblockData", scrJref, "StgIacQryWznmBlkRef1NRtblock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmBlkRef1NRtblock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmBlkRef1NRtblock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmBlkRef1NRtblock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmBlkRef1NRtblockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "ContInfWznmBlkRef1NRtblock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "StatAppWznmBlkRef1NRtblock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "StgIacWznmBlkRef1NRtblock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "TagWznmBlkRef1NRtblock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "ListWznmQBlkRef1NRtblock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "StatAppQryWznmBlkRef1NRtblock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "StatShrQryWznmBlkRef1NRtblock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmBlkRef1NRtblockData", "StgIacQryWznmBlkRef1NRtblock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmBlkRef1NRtblockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmBlkRef1NRtblockData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NRtblock");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmBlkRef1NRtblockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
