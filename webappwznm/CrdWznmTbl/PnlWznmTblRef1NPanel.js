/**
  * \file PnlWznmTblRef1NPanel.js
  * web client functionality for panel PnlWznmTblRef1NPanel
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmTblRef1NPanel_aside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmTblRef1NPanel_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NPanel_side").src = "./PnlWznmTblRef1NPanel_bside.html";
	doc.getElementById("Ref1NPanel_cont").src = "./PnlWznmTblRef1NPanel_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblRef1NPanel", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NPanel", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NPanel", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NPanel", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NPanel", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblRef1NPanel", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblRef1NPanel", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblRef1NPanel
		rownode = getRownode(srcdoc, "ListWznmQTblRef1NPanel", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblRef1NPanel", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblRef1NPanel", "ListWznmQTblRef1NPanel", retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblRef1NPanel", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NPanel", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NPanel", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblRef1NPanel", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblRef1NPanel", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblRef1NPanelData", scrJref, "StgIacQryWznmTblRef1NPanel");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblRef1NPanel", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NPanel", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NPanel", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblRef1NPanelDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "ContInfWznmTblRef1NPanel", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StatAppWznmTblRef1NPanel", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StatShrWznmTblRef1NPanel", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StgIacWznmTblRef1NPanel", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "TagWznmTblRef1NPanel", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "ListWznmQTblRef1NPanel", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StatAppQryWznmTblRef1NPanel", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StatShrQryWznmTblRef1NPanel", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NPanelData", "StgIacQryWznmTblRef1NPanel", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblRef1NPanelData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblRef1NPanelData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblRef1NPanelData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
