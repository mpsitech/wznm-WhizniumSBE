/**
  * \file PnlWznmTblATitle.js
  * web client functionality for panel PnlWznmTblATitle
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("ATitle", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ATitle_side").setAttribute("height", "30");
	doc.getElementById("ATitle_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ATitle_side").src = "./PnlWznmTblATitle_aside.html";
	doc.getElementById("ATitle_cont").src = "./PnlWznmTblATitle_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("ATitle", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ATitle_side").setAttribute("height", "255");
	doc.getElementById("ATitle_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ATitle_side").src = "./PnlWznmTblATitle_bside.html";
	doc.getElementById("ATitle_cont").src = "./PnlWznmTblATitle_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ATitle_side").contentDocument;
	contdoc = doc.getElementById("ATitle_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ATitle_side").contentDocument;
	contdoc = doc.getElementById("ATitle_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ATitle_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ATitle_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ATitle_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblATitle", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblATitle", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblATitle", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblATitle", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblATitle", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblATitle", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblATitle", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblATitle", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblATitle", "TcoTypWidth"));
		var TcoLocWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblATitle", "TcoLocWidth"));
		var TcoGndWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblATitle", "TcoGndWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblATitle", "TcoTitWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoLoc"); doc.widths.push(TcoLocWidth); doc.fs.push("loc"); wtot += TcoLocWidth;
			} else if (i == 3) {
				doc.cols.push("TcoGnd"); doc.widths.push(TcoGndWidth); doc.fs.push("gnd"); wtot += TcoGndWidth;
			} else if (i == 4) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblATitle", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblATitle", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblATitle
		rownode = getRownode(srcdoc, "ListWznmQTblATitle", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblATitle", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblATitle", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblATitle", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ATitle", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ATitle_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ATitle_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ATitle_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ATitle_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblATitle", "ListWznmQTblATitle", retrieveTi(srcdoc, "TagWznmTblATitle", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblATitle", "TxtRecord2"),
				["Typ","","Loc","Gnd","","Tit"],
				["typ","typ2","loc","gnd","gnd2","tit"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblATitle", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblATitle", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblATitle", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblATitle", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblATitleData", scrJref, "StgIacQryWznmTblATitle");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblATitle", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblATitle", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblATitle", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblATitle", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblATitleDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "ContInfWznmTblATitle", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StatAppWznmTblATitle", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StatShrWznmTblATitle", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StgIacWznmTblATitle", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "TagWznmTblATitle", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "ListWznmQTblATitle", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StatAppQryWznmTblATitle", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StatShrQryWznmTblATitle", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblATitleData", "StgIacQryWznmTblATitle", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblATitleData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblATitleData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ATitle");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblATitleData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
