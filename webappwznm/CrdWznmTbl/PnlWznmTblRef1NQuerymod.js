/**
  * \file PnlWznmTblRef1NQuerymod.js
  * web client functionality for panel PnlWznmTblRef1NQuerymod
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ref1NQuerymod", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ref1NQuerymod_side").setAttribute("height", "30");
	doc.getElementById("Ref1NQuerymod_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ref1NQuerymod_side").src = "./PnlWznmTblRef1NQuerymod_aside.html";
	doc.getElementById("Ref1NQuerymod_cont").src = "./PnlWznmTblRef1NQuerymod_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ref1NQuerymod", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ref1NQuerymod_side").setAttribute("height", "255");
	doc.getElementById("Ref1NQuerymod_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ref1NQuerymod_side").src = "./PnlWznmTblRef1NQuerymod_bside.html";
	doc.getElementById("Ref1NQuerymod_cont").src = "./PnlWznmTblRef1NQuerymod_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ref1NQuerymod_side").contentDocument;
	contdoc = doc.getElementById("Ref1NQuerymod_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ref1NQuerymod_side").contentDocument;
	contdoc = doc.getElementById("Ref1NQuerymod_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ref1NQuerymod_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ref1NQuerymod_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ref1NQuerymod_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblRef1NQuerymod", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NQuerymod", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NQuerymod", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NQuerymod", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NQuerymod", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblRef1NQuerymod", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblRef1NQuerymod
		rownode = getRownode(srcdoc, "ListWznmQTblRef1NQuerymod", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblRef1NQuerymod", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ref1NQuerymod", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NQuerymod_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ref1NQuerymod_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ref1NQuerymod_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ref1NQuerymod_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblRef1NQuerymod", "ListWznmQTblRef1NQuerymod", retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblRef1NQuerymod", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NQuerymod", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NQuerymod", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblRef1NQuerymod", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblRef1NQuerymod", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblRef1NQuerymodData", scrJref, "StgIacQryWznmTblRef1NQuerymod");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblRef1NQuerymod", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NQuerymod", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NQuerymod", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblRef1NQuerymodDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "ContInfWznmTblRef1NQuerymod", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StatAppWznmTblRef1NQuerymod", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StatShrWznmTblRef1NQuerymod", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StgIacWznmTblRef1NQuerymod", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "TagWznmTblRef1NQuerymod", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "ListWznmQTblRef1NQuerymod", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StatAppQryWznmTblRef1NQuerymod", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StatShrQryWznmTblRef1NQuerymod", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NQuerymodData", "StgIacQryWznmTblRef1NQuerymod", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblRef1NQuerymodData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblRef1NQuerymodData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ref1NQuerymod");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblRef1NQuerymodData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
