/**
  * \file PnlWznmTblPst1NQuerymod.js
  * web client functionality for panel PnlWznmTblPst1NQuerymod
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Pst1NQuerymod", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Pst1NQuerymod_side").setAttribute("height", "30");
	doc.getElementById("Pst1NQuerymod_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Pst1NQuerymod_side").src = "./PnlWznmTblPst1NQuerymod_aside.html";
	doc.getElementById("Pst1NQuerymod_cont").src = "./PnlWznmTblPst1NQuerymod_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Pst1NQuerymod", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Pst1NQuerymod_side").setAttribute("height", "255");
	doc.getElementById("Pst1NQuerymod_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Pst1NQuerymod_side").src = "./PnlWznmTblPst1NQuerymod_bside.html";
	doc.getElementById("Pst1NQuerymod_cont").src = "./PnlWznmTblPst1NQuerymod_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Pst1NQuerymod_side").contentDocument;
	contdoc = doc.getElementById("Pst1NQuerymod_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Pst1NQuerymod_side").contentDocument;
	contdoc = doc.getElementById("Pst1NQuerymod_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Pst1NQuerymod_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Pst1NQuerymod_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Pst1NQuerymod_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblPst1NQuerymod", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblPst1NQuerymod", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblPst1NQuerymod", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblPst1NQuerymod", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblPst1NQuerymod", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblPst1NQuerymod", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblPst1NQuerymod
		rownode = getRownode(srcdoc, "ListWznmQTblPst1NQuerymod", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblPst1NQuerymod", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Pst1NQuerymod", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Pst1NQuerymod_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Pst1NQuerymod_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Pst1NQuerymod_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Pst1NQuerymod_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblPst1NQuerymod", "ListWznmQTblPst1NQuerymod", retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblPst1NQuerymod", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblPst1NQuerymod", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblPst1NQuerymod", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblPst1NQuerymod", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblPst1NQuerymod", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblPst1NQuerymodData", scrJref, "StgIacQryWznmTblPst1NQuerymod");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblPst1NQuerymod", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblPst1NQuerymod", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblPst1NQuerymod", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblPst1NQuerymodDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "ContInfWznmTblPst1NQuerymod", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StatAppWznmTblPst1NQuerymod", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StatShrWznmTblPst1NQuerymod", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StgIacWznmTblPst1NQuerymod", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "TagWznmTblPst1NQuerymod", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "ListWznmQTblPst1NQuerymod", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StatAppQryWznmTblPst1NQuerymod", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StatShrQryWznmTblPst1NQuerymod", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblPst1NQuerymodData", "StgIacQryWznmTblPst1NQuerymod", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblPst1NQuerymodData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblPst1NQuerymodData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Pst1NQuerymod");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblPst1NQuerymodData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
