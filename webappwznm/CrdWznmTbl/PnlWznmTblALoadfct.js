/**
  * \file PnlWznmTblALoadfct.js
  * web client functionality for panel PnlWznmTblALoadfct
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("ALoadfct", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("ALoadfct_side").setAttribute("height", "30");
	doc.getElementById("ALoadfct_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("ALoadfct_side").src = "./PnlWznmTblALoadfct_aside.html";
	doc.getElementById("ALoadfct_cont").src = "./PnlWznmTblALoadfct_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("ALoadfct", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("ALoadfct_side").setAttribute("height", "255");
	doc.getElementById("ALoadfct_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("ALoadfct_side").src = "./PnlWznmTblALoadfct_bside.html";
	doc.getElementById("ALoadfct_cont").src = "./PnlWznmTblALoadfct_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("ALoadfct_side").contentDocument;
	contdoc = doc.getElementById("ALoadfct_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("ALoadfct_side").contentDocument;
	contdoc = doc.getElementById("ALoadfct_cont").contentDocument;
	hdrdoc = contdoc.getElementById("ALoadfct_hdr").contentDocument;
	tbldoc = contdoc.getElementById("ALoadfct_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("ALoadfct_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblALoadfct", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblALoadfct", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblALoadfct", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblALoadfct", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblALoadfct", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblALoadfct", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblALoadfct", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblALoadfct", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "ncol"));

		var TcoLotWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoLotWidth"));
		var TcoFnaWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoFnaWidth"));
		var TcoLdxWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoLdxWidth"));
		var TcoLbyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoLbyWidth"));
		var TcoOrdWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoOrdWidth"));
		var TcoLitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblALoadfct", "TcoLitWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoLot"); doc.widths.push(TcoLotWidth); doc.fs.push("lot"); wtot += TcoLotWidth;
			} else if (i == 2) {
				doc.cols.push("TcoFna"); doc.widths.push(TcoFnaWidth); doc.fs.push("fna"); wtot += TcoFnaWidth;
			} else if (i == 3) {
				doc.cols.push("TcoLdx"); doc.widths.push(TcoLdxWidth); doc.fs.push("ldx"); wtot += TcoLdxWidth;
			} else if (i == 4) {
				doc.cols.push("TcoLby"); doc.widths.push(TcoLbyWidth); doc.fs.push("lby"); wtot += TcoLbyWidth;
			} else if (i == 5) {
				doc.cols.push("TcoOrd"); doc.widths.push(TcoOrdWidth); doc.fs.push("ord"); wtot += TcoOrdWidth;
			} else if (i == 6) {
				doc.cols.push("TcoLit"); doc.widths.push(TcoLitWidth); doc.fs.push("lit"); wtot += TcoLitWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblALoadfct", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblALoadfct", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblALoadfct
		rownode = getRownode(srcdoc, "ListWznmQTblALoadfct", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblALoadfct", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblALoadfct", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblALoadfct", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("ALoadfct", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("ALoadfct_side").setAttribute("height", "" + (height+55));
	doc.getElementById("ALoadfct_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("ALoadfct_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("ALoadfct_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblALoadfct", "ListWznmQTblALoadfct", retrieveTi(srcdoc, "TagWznmTblALoadfct", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblALoadfct", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblALoadfct", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblALoadfct", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblALoadfct", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblALoadfct", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblALoadfctData", scrJref, "StgIacQryWznmTblALoadfct");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblALoadfct", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblALoadfct", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblALoadfct", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblALoadfct", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblALoadfctDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "ContInfWznmTblALoadfct", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StatAppWznmTblALoadfct", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StatShrWznmTblALoadfct", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StgIacWznmTblALoadfct", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "TagWznmTblALoadfct", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "ListWznmQTblALoadfct", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StatAppQryWznmTblALoadfct", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StatShrQryWznmTblALoadfct", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblALoadfctData", "StgIacQryWznmTblALoadfct", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblALoadfctData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblALoadfctData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("ALoadfct");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblALoadfctData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
