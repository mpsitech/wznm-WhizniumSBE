/**
  * \file PnlWznmTbl1NCheck.js
  * web client functionality for panel PnlWznmTbl1NCheck
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NCheck_side").src = "./PnlWznmTbl1NCheck_aside.html";
	doc.getElementById("1NCheck_cont").src = "./PnlWznmTbl1NCheck_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NCheck_side").src = "./PnlWznmTbl1NCheck_bside.html";
	doc.getElementById("1NCheck_cont").src = "./PnlWznmTbl1NCheck_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NCheck", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTbl1NCheck", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NCheck", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTbl1NCheck", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NCheck", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NCheck", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NCheck", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NCheck", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTbl1NCheck", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTbl1NCheck", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTbl1NCheck", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTbl1NCheck
		rownode = getRownode(srcdoc, "ListWznmQTbl1NCheck", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTbl1NCheck", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTbl1NCheck", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTbl1NCheck", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTbl1NCheck", "ListWznmQTbl1NCheck", retrieveTi(srcdoc, "TagWznmTbl1NCheck", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTbl1NCheck", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NCheck", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NCheck", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTbl1NCheck", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTbl1NCheck", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTbl1NCheckData", scrJref, "StgIacQryWznmTbl1NCheck");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NCheck", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTbl1NCheck", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NCheck", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NCheck", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTbl1NCheckDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "ContInfWznmTbl1NCheck", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StatAppWznmTbl1NCheck", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StatShrWznmTbl1NCheck", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StgIacWznmTbl1NCheck", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "TagWznmTbl1NCheck", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "ListWznmQTbl1NCheck", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StatAppQryWznmTbl1NCheck", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StatShrQryWznmTbl1NCheck", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NCheckData", "StgIacQryWznmTbl1NCheck", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTbl1NCheckData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTbl1NCheckData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTbl1NCheckData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
