/**
  * \file PnlWznmTbl1NSubset.js
  * web client functionality for panel PnlWznmTbl1NSubset
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NSubset", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NSubset_side").setAttribute("height", "30");
	doc.getElementById("1NSubset_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NSubset_side").src = "./PnlWznmTbl1NSubset_aside.html";
	doc.getElementById("1NSubset_cont").src = "./PnlWznmTbl1NSubset_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NSubset", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NSubset_side").setAttribute("height", "255");
	doc.getElementById("1NSubset_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NSubset_side").src = "./PnlWznmTbl1NSubset_bside.html";
	doc.getElementById("1NSubset_cont").src = "./PnlWznmTbl1NSubset_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NSubset_side").contentDocument;
	contdoc = doc.getElementById("1NSubset_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NSubset_side").contentDocument;
	contdoc = doc.getElementById("1NSubset_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NSubset_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NSubset_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NSubset_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NSubset", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTbl1NSubset", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NSubset", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTbl1NSubset", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NSubset", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NSubset", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NSubset", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NSubset", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTbl1NSubset", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTbl1NSubset", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTbl1NSubset", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTbl1NSubset
		rownode = getRownode(srcdoc, "ListWznmQTbl1NSubset", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTbl1NSubset", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTbl1NSubset", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTbl1NSubset", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NSubset", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSubset_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NSubset_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NSubset_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NSubset_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTbl1NSubset", "ListWznmQTbl1NSubset", retrieveTi(srcdoc, "TagWznmTbl1NSubset", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTbl1NSubset", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NSubset", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NSubset", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTbl1NSubset", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTbl1NSubset", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTbl1NSubsetData", scrJref, "StgIacQryWznmTbl1NSubset");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NSubset", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTbl1NSubset", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NSubset", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NSubset", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTbl1NSubsetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "ContInfWznmTbl1NSubset", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StatAppWznmTbl1NSubset", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StatShrWznmTbl1NSubset", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StgIacWznmTbl1NSubset", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "TagWznmTbl1NSubset", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "ListWznmQTbl1NSubset", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StatAppQryWznmTbl1NSubset", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StatShrQryWznmTbl1NSubset", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NSubsetData", "StgIacQryWznmTbl1NSubset", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTbl1NSubsetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTbl1NSubsetData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NSubset");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTbl1NSubsetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
