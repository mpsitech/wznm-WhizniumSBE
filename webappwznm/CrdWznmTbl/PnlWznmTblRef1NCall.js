/**
  * \file PnlWznmTblRef1NCall.js
  * web client functionality for panel PnlWznmTblRef1NCall
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NCall_side").src = "./PnlWznmTblRef1NCall_aside.html";
	doc.getElementById("Ref1NCall_cont").src = "./PnlWznmTblRef1NCall_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NCall_side").src = "./PnlWznmTblRef1NCall_bside.html";
	doc.getElementById("Ref1NCall_cont").src = "./PnlWznmTblRef1NCall_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NCall", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblRef1NCall", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NCall", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblRef1NCall", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NCall", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NCall", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NCall", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NCall", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblRef1NCall", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblRef1NCall", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblRef1NCall", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblRef1NCall
		rownode = getRownode(srcdoc, "ListWznmQTblRef1NCall", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblRef1NCall", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblRef1NCall", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblRef1NCall", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblRef1NCall", "ListWznmQTblRef1NCall", retrieveTi(srcdoc, "TagWznmTblRef1NCall", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblRef1NCall", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NCall", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NCall", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblRef1NCall", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblRef1NCall", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblRef1NCallData", scrJref, "StgIacQryWznmTblRef1NCall");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NCall", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblRef1NCall", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NCall", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NCall", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblRef1NCallDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "ContInfWznmTblRef1NCall", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "StatAppWznmTblRef1NCall", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "StgIacWznmTblRef1NCall", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "TagWznmTblRef1NCall", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "ListWznmQTblRef1NCall", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "StatAppQryWznmTblRef1NCall", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "StatShrQryWznmTblRef1NCall", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NCallData", "StgIacQryWznmTblRef1NCall", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblRef1NCallData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblRef1NCallData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblRef1NCallData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
