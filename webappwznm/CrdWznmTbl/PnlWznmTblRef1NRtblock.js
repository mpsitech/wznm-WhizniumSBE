/**
  * \file PnlWznmTblRef1NRtblock.js
  * web client functionality for panel PnlWznmTblRef1NRtblock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NRtblock_side").src = "./PnlWznmTblRef1NRtblock_aside.html";
	doc.getElementById("Ref1NRtblock_cont").src = "./PnlWznmTblRef1NRtblock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NRtblock_side").src = "./PnlWznmTblRef1NRtblock_bside.html";
	doc.getElementById("Ref1NRtblock_cont").src = "./PnlWznmTblRef1NRtblock_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblRef1NRtblock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NRtblock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NRtblock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NRtblock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NRtblock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblRef1NRtblock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblRef1NRtblock
		rownode = getRownode(srcdoc, "ListWznmQTblRef1NRtblock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblRef1NRtblock", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblRef1NRtblock", "ListWznmQTblRef1NRtblock", retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblRef1NRtblock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NRtblock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NRtblock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblRef1NRtblock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblRef1NRtblock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblRef1NRtblockData", scrJref, "StgIacQryWznmTblRef1NRtblock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblRef1NRtblock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NRtblock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NRtblock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblRef1NRtblockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "ContInfWznmTblRef1NRtblock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "StatAppWznmTblRef1NRtblock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "StgIacWznmTblRef1NRtblock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "TagWznmTblRef1NRtblock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "ListWznmQTblRef1NRtblock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "StatAppQryWznmTblRef1NRtblock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "StatShrQryWznmTblRef1NRtblock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NRtblockData", "StgIacQryWznmTblRef1NRtblock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblRef1NRtblockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblRef1NRtblockData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblRef1NRtblockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
