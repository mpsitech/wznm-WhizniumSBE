/**
  * \file PnlWznmTblSrc1NFeed.js
  * web client functionality for panel PnlWznmTblSrc1NFeed
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Src1NFeed", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Src1NFeed_side").setAttribute("height", "30");
	doc.getElementById("Src1NFeed_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Src1NFeed_side").src = "./PnlWznmTblSrc1NFeed_aside.html";
	doc.getElementById("Src1NFeed_cont").src = "./PnlWznmTblSrc1NFeed_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Src1NFeed", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Src1NFeed_side").setAttribute("height", "255");
	doc.getElementById("Src1NFeed_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Src1NFeed_side").src = "./PnlWznmTblSrc1NFeed_bside.html";
	doc.getElementById("Src1NFeed_cont").src = "./PnlWznmTblSrc1NFeed_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Src1NFeed_side").contentDocument;
	contdoc = doc.getElementById("Src1NFeed_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Src1NFeed_side").contentDocument;
	contdoc = doc.getElementById("Src1NFeed_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Src1NFeed_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Src1NFeed_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Src1NFeed_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblSrc1NFeed", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblSrc1NFeed", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblSrc1NFeed", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblSrc1NFeed", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblSrc1NFeed", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblSrc1NFeed", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblSrc1NFeed
		rownode = getRownode(srcdoc, "ListWznmQTblSrc1NFeed", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblSrc1NFeed", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Src1NFeed", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Src1NFeed_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Src1NFeed_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Src1NFeed_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Src1NFeed_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblSrc1NFeed", "ListWznmQTblSrc1NFeed", retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblSrc1NFeed", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblSrc1NFeed", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblSrc1NFeed", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblSrc1NFeed", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblSrc1NFeed", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblSrc1NFeedData", scrJref, "StgIacQryWznmTblSrc1NFeed");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblSrc1NFeed", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblSrc1NFeed", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblSrc1NFeed", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblSrc1NFeedDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "ContInfWznmTblSrc1NFeed", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StatAppWznmTblSrc1NFeed", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StatShrWznmTblSrc1NFeed", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StgIacWznmTblSrc1NFeed", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "TagWznmTblSrc1NFeed", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "ListWznmQTblSrc1NFeed", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StatAppQryWznmTblSrc1NFeed", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StatShrQryWznmTblSrc1NFeed", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblSrc1NFeedData", "StgIacQryWznmTblSrc1NFeed", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblSrc1NFeedData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblSrc1NFeedData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Src1NFeed");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblSrc1NFeedData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
