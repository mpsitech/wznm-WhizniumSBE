/**
  * \file PnlWznmTbl1NStub.js
  * web client functionality for panel PnlWznmTbl1NStub
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NStub", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NStub_side").setAttribute("height", "30");
	doc.getElementById("1NStub_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NStub_side").src = "./PnlWznmTbl1NStub_aside.html";
	doc.getElementById("1NStub_cont").src = "./PnlWznmTbl1NStub_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NStub", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NStub_side").setAttribute("height", "255");
	doc.getElementById("1NStub_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NStub_side").src = "./PnlWznmTbl1NStub_bside.html";
	doc.getElementById("1NStub_cont").src = "./PnlWznmTbl1NStub_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NStub_side").contentDocument;
	contdoc = doc.getElementById("1NStub_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NStub_side").contentDocument;
	contdoc = doc.getElementById("1NStub_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NStub_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NStub_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NStub_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NStub", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTbl1NStub", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTbl1NStub", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTbl1NStub", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NStub", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NStub", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NStub", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NStub", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTbl1NStub", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTbl1NStub", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTbl1NStub", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTbl1NStub
		rownode = getRownode(srcdoc, "ListWznmQTbl1NStub", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTbl1NStub", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTbl1NStub", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTbl1NStub", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NStub", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStub_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NStub_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NStub_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NStub_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTbl1NStub", "ListWznmQTbl1NStub", retrieveTi(srcdoc, "TagWznmTbl1NStub", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTbl1NStub", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NStub", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTbl1NStub", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTbl1NStub", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTbl1NStub", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTbl1NStubData", scrJref, "StgIacQryWznmTbl1NStub");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTbl1NStub", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTbl1NStub", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTbl1NStub", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTbl1NStub", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTbl1NStubDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "ContInfWznmTbl1NStub", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StatAppWznmTbl1NStub", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StatShrWznmTbl1NStub", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StgIacWznmTbl1NStub", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "TagWznmTbl1NStub", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "ListWznmQTbl1NStub", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StatAppQryWznmTbl1NStub", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StatShrQryWznmTbl1NStub", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTbl1NStubData", "StgIacQryWznmTbl1NStub", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTbl1NStubData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTbl1NStubData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NStub");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTbl1NStubData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
