/**
  * \file PnlWznmTblRef1NDialog.js
  * web client functionality for panel PnlWznmTblRef1NDialog
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NDialog_side").src = "./PnlWznmTblRef1NDialog_aside.html";
	doc.getElementById("Ref1NDialog_cont").src = "./PnlWznmTblRef1NDialog_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NDialog_side").src = "./PnlWznmTblRef1NDialog_bside.html";
	doc.getElementById("Ref1NDialog_cont").src = "./PnlWznmTblRef1NDialog_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblRef1NDialog", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NDialog", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NDialog", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NDialog", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NDialog", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblRef1NDialog", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblRef1NDialog", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblRef1NDialog
		rownode = getRownode(srcdoc, "ListWznmQTblRef1NDialog", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblRef1NDialog", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblRef1NDialog", "ListWznmQTblRef1NDialog", retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblRef1NDialog", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NDialog", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblRef1NDialog", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblRef1NDialog", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblRef1NDialog", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblRef1NDialogData", scrJref, "StgIacQryWznmTblRef1NDialog");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblRef1NDialog", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblRef1NDialog", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblRef1NDialog", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblRef1NDialogDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "ContInfWznmTblRef1NDialog", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StatAppWznmTblRef1NDialog", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StatShrWznmTblRef1NDialog", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StgIacWznmTblRef1NDialog", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "TagWznmTblRef1NDialog", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "ListWznmQTblRef1NDialog", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StatAppQryWznmTblRef1NDialog", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StatShrQryWznmTblRef1NDialog", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblRef1NDialogData", "StgIacQryWznmTblRef1NDialog", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblRef1NDialogData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblRef1NDialogData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblRef1NDialogData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
