/**
  * \file PnlWznmTblTo1NRelation.js
  * web client functionality for panel PnlWznmTblTo1NRelation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("To1NRelation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("To1NRelation_side").setAttribute("height", "30");
	doc.getElementById("To1NRelation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("To1NRelation_side").src = "./PnlWznmTblTo1NRelation_aside.html";
	doc.getElementById("To1NRelation_cont").src = "./PnlWznmTblTo1NRelation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("To1NRelation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("To1NRelation_side").setAttribute("height", "255");
	doc.getElementById("To1NRelation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("To1NRelation_side").src = "./PnlWznmTblTo1NRelation_bside.html";
	doc.getElementById("To1NRelation_cont").src = "./PnlWznmTblTo1NRelation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("To1NRelation_side").contentDocument;
	contdoc = doc.getElementById("To1NRelation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("To1NRelation_side").contentDocument;
	contdoc = doc.getElementById("To1NRelation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("To1NRelation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("To1NRelation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("To1NRelation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblTo1NRelation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblTo1NRelation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblTo1NRelation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblTo1NRelation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblTo1NRelation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblTo1NRelation", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblTo1NRelation", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblTo1NRelation
		rownode = getRownode(srcdoc, "ListWznmQTblTo1NRelation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblTo1NRelation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("To1NRelation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("To1NRelation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("To1NRelation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("To1NRelation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("To1NRelation_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblTo1NRelation", "ListWznmQTblTo1NRelation", retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblTo1NRelation", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblTo1NRelation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblTo1NRelation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblTo1NRelation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblTo1NRelation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblTo1NRelationData", scrJref, "StgIacQryWznmTblTo1NRelation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblTo1NRelation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblTo1NRelation", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblTo1NRelation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblTo1NRelationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "ContInfWznmTblTo1NRelation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StatAppWznmTblTo1NRelation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StatShrWznmTblTo1NRelation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StgIacWznmTblTo1NRelation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "TagWznmTblTo1NRelation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "ListWznmQTblTo1NRelation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StatAppQryWznmTblTo1NRelation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StatShrQryWznmTblTo1NRelation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblTo1NRelationData", "StgIacQryWznmTblTo1NRelation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblTo1NRelationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblTo1NRelationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("To1NRelation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblTo1NRelationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
