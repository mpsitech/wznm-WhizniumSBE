/**
  * \file PnlWznmTblMNVector.js
  * web client functionality for panel PnlWznmTblMNVector
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNVector", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNVector_side").setAttribute("height", "30");
	doc.getElementById("MNVector_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNVector_side").src = "./PnlWznmTblMNVector_aside.html";
	doc.getElementById("MNVector_cont").src = "./PnlWznmTblMNVector_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNVector", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNVector_side").setAttribute("height", "255");
	doc.getElementById("MNVector_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNVector_side").src = "./PnlWznmTblMNVector_bside.html";
	doc.getElementById("MNVector_cont").src = "./PnlWznmTblMNVector_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNVector_side").contentDocument;
	contdoc = doc.getElementById("MNVector_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNVector_side").contentDocument;
	contdoc = doc.getElementById("MNVector_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNVector_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNVector_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNVector_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblMNVector", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmTblMNVector", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmTblMNVector", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmTblMNVector", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblMNVector", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblMNVector", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblMNVector", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblMNVector", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblMNVector", "TcoMrefWidth"));
		var TcoSbsWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmTblMNVector", "TcoSbsWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoSbs"); doc.widths.push(TcoSbsWidth); doc.fs.push("sbs"); wtot += TcoSbsWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmTblMNVector", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmTblMNVector", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQTblMNVector
		rownode = getRownode(srcdoc, "ListWznmQTblMNVector", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmTblMNVector", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmTblMNVector", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmTblMNVector", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNVector", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNVector_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNVector_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNVector_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNVector_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmTblMNVector", "ListWznmQTblMNVector", retrieveTi(srcdoc, "TagWznmTblMNVector", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmTblMNVector", "TxtRecord2"),
				["Mref","Sbs"],
				["mref","sbs"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblMNVector", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmTblMNVector", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmTblMNVector", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmTblMNVector", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmTblMNVectorData", scrJref, "StgIacQryWznmTblMNVector");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmTblMNVector", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmTblMNVector", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmTblMNVector", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmTblMNVector", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmTblMNVectorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "ContInfWznmTblMNVector", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StatAppWznmTblMNVector", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StatShrWznmTblMNVector", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StgIacWznmTblMNVector", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "TagWznmTblMNVector", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "ListWznmQTblMNVector", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StatAppQryWznmTblMNVector", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StatShrQryWznmTblMNVector", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmTblMNVectorData", "StgIacQryWznmTblMNVector", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmTblMNVectorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmTblMNVectorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNVector");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmTblMNVectorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
