/**
  * \file PnlWznmIexList.js
  * web client functionality for panel PnlWznmIexList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmIexList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmIexList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmIexList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmIexList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIexList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmIexList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmIexList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmIexList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmIexList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIexList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmIexList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmIexList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmIexList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmIexList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIexList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIexList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIexList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIexList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmIexList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmIexList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIexList", "TcoSrfWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIexList", "TcoShoWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIexList", "TcoTitWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIexList", "TcoVerWidth"));
		var TcoMivWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmIexList", "TcoMivWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmIexList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoMiv"); doc.widths.push(TcoMivWidth); doc.fs.push("miv"); doc.tos.push(0); wtot += TcoMivWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmIexList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmIexList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQIexList
		rownode = getRownode(srcdoc, "ListWznmQIexList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmIexList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmIexList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmIexList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmIexList", "ListWznmQIexList", retrieveTi(srcdoc, "TagWznmIexList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmIexList", "TxtRecord2"),
				["Srf","Sho","Tit","Ver","Miv"],
				["srf","sho","tit","ver","miv"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIexList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmIexList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmIexList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmIexList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmIexListData", scrJref, "StgIacQryWznmIexList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmIexList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmIexList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmIexList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmIexListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmIexList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmIexList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmIexListData", scrJref, "ContIacWznmIexList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "ContIacWznmIexList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "ContInfWznmIexList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "StatShrWznmIexList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "StgIacWznmIexList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "TagWznmIexList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "ListWznmQIexList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "StatAppQryWznmIexList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "StatShrQryWznmIexList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmIexListData", "StgIacQryWznmIexList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmIexListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmIexListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmIexListData") {
				mask = mergeDpchEngData(dom);
