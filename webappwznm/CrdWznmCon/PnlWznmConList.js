...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmConList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmConList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmConList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmConList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmConList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmConList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmConList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmConList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmConList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmConList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmConList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmConList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmConList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoTitWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoTypWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoHkuWidth"));
		var TcoSctWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoSctWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoReuWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoSupWidth"));
		var TcoStyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConList", "TcoStyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmConList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoReu"); doc.widths.push(TcoReuWidth); doc.fs.push("reu"); doc.tos.push(7); wtot += TcoReuWidth;
			} else if (i == 9) {
				doc.cols.push("TcoSup"); doc.widths.push(TcoSupWidth); doc.fs.push("sup"); doc.tos.push(8); wtot += TcoSupWidth;
			} else if (i == 10) {
				doc.cols.push("TcoSty"); doc.widths.push(TcoStyWidth); doc.fs.push("sty2"); doc.tos.push(0); wtot += TcoStyWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmConList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmConList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQConList
		rownode = getRownode(srcdoc, "ListWznmQConList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmConList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmConList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmConList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmConList", "ListWznmQConList", retrieveTi(srcdoc, "TagWznmConList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmConList", "TxtRecord2"),
				["Srf","Tit","","Typ","","Hkt","Hku","","Sct","","Ret","Reu","Sup","","Sty"],
				["srf","tit","typ","typ2","hkt","hkt2","hku","sct","sct2","ret","ret2","reu","sup","sty","sty2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmConList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmConList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmConListData", scrJref, "StgIacQryWznmConList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmConList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmConListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmConList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmConList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmConListData", scrJref, "ContIacWznmConList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmConListData", "ContIacWznmConList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "ContInfWznmConList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "StatShrWznmConList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "StgIacWznmConList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "TagWznmConList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "ListWznmQConList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "StatAppQryWznmConList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "StatShrQryWznmConList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmConListData", "StgIacQryWznmConList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmConListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmConListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmConListData") {
				mask = mergeDpchEngData(dom);
