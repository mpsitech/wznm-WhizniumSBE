/**
  * \file PnlWznmConFedRef1NRtblock.js
  * web client functionality for panel PnlWznmConFedRef1NRtblock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("FedRef1NRtblock", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("FedRef1NRtblock_side").setAttribute("height", "30");
	doc.getElementById("FedRef1NRtblock_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("FedRef1NRtblock_side").src = "./PnlWznmConFedRef1NRtblock_aside.html";
	doc.getElementById("FedRef1NRtblock_cont").src = "./PnlWznmConFedRef1NRtblock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("FedRef1NRtblock", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("FedRef1NRtblock_side").setAttribute("height", "255");
	doc.getElementById("FedRef1NRtblock_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("FedRef1NRtblock_side").src = "./PnlWznmConFedRef1NRtblock_bside.html";
	doc.getElementById("FedRef1NRtblock_cont").src = "./PnlWznmConFedRef1NRtblock_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("FedRef1NRtblock_side").contentDocument;
	contdoc = doc.getElementById("FedRef1NRtblock_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("FedRef1NRtblock_side").contentDocument;
	contdoc = doc.getElementById("FedRef1NRtblock_cont").contentDocument;
	hdrdoc = contdoc.getElementById("FedRef1NRtblock_hdr").contentDocument;
	tbldoc = contdoc.getElementById("FedRef1NRtblock_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("FedRef1NRtblock_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmConFedRef1NRtblock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConFedRef1NRtblock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConFedRef1NRtblock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConFedRef1NRtblock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConFedRef1NRtblock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConFedRef1NRtblock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQConFedRef1NRtblock
		rownode = getRownode(srcdoc, "ListWznmQConFedRef1NRtblock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmConFedRef1NRtblock", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("FedRef1NRtblock", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("FedRef1NRtblock_side").setAttribute("height", "" + (height+55));
	doc.getElementById("FedRef1NRtblock_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("FedRef1NRtblock_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("FedRef1NRtblock_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmConFedRef1NRtblock", "ListWznmQConFedRef1NRtblock", retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmConFedRef1NRtblock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConFedRef1NRtblock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConFedRef1NRtblock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmConFedRef1NRtblock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmConFedRef1NRtblock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmConFedRef1NRtblockData", scrJref, "StgIacQryWznmConFedRef1NRtblock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmConFedRef1NRtblock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmConFedRef1NRtblock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConFedRef1NRtblock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmConFedRef1NRtblockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "ContInfWznmConFedRef1NRtblock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "StatAppWznmConFedRef1NRtblock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "StgIacWznmConFedRef1NRtblock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "TagWznmConFedRef1NRtblock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "ListWznmQConFedRef1NRtblock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "StatAppQryWznmConFedRef1NRtblock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "StatShrQryWznmConFedRef1NRtblock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmConFedRef1NRtblockData", "StgIacQryWznmConFedRef1NRtblock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmConFedRef1NRtblockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmConFedRef1NRtblockData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("FedRef1NRtblock");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmConFedRef1NRtblockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
