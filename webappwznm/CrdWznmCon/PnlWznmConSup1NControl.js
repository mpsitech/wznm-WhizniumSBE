/**
  * \file PnlWznmConSup1NControl.js
  * web client functionality for panel PnlWznmConSup1NControl
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NControl", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NControl_side").setAttribute("height", "30");
	doc.getElementById("Sup1NControl_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NControl_side").src = "./PnlWznmConSup1NControl_aside.html";
	doc.getElementById("Sup1NControl_cont").src = "./PnlWznmConSup1NControl_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NControl", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NControl_side").setAttribute("height", "255");
	doc.getElementById("Sup1NControl_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NControl_side").src = "./PnlWznmConSup1NControl_bside.html";
	doc.getElementById("Sup1NControl_cont").src = "./PnlWznmConSup1NControl_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NControl_side").contentDocument;
	contdoc = doc.getElementById("Sup1NControl_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NControl_side").contentDocument;
	contdoc = doc.getElementById("Sup1NControl_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NControl_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NControl_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NControl_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConSup1NControl", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmConSup1NControl", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmConSup1NControl", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmConSup1NControl", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConSup1NControl", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConSup1NControl", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConSup1NControl", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConSup1NControl", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmConSup1NControl", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmConSup1NControl", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmConSup1NControl", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQConSup1NControl
		rownode = getRownode(srcdoc, "ListWznmQConSup1NControl", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmConSup1NControl", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmConSup1NControl", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmConSup1NControl", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NControl", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NControl_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NControl_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NControl_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NControl_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmConSup1NControl", "ListWznmQConSup1NControl", retrieveTi(srcdoc, "TagWznmConSup1NControl", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmConSup1NControl", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConSup1NControl", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmConSup1NControl", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmConSup1NControl", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmConSup1NControl", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmConSup1NControlData", scrJref, "StgIacQryWznmConSup1NControl");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmConSup1NControl", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmConSup1NControl", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmConSup1NControl", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmConSup1NControl", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmConSup1NControlDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "ContInfWznmConSup1NControl", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StatAppWznmConSup1NControl", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StatShrWznmConSup1NControl", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StgIacWznmConSup1NControl", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "TagWznmConSup1NControl", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "ListWznmQConSup1NControl", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StatAppQryWznmConSup1NControl", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StatShrQryWznmConSup1NControl", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmConSup1NControlData", "StgIacQryWznmConSup1NControl", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmConSup1NControlData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmConSup1NControlData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NControl");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmConSup1NControlData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
