/**
  * \file PnlWznmQryMNPanel.js
  * web client functionality for panel PnlWznmQryMNPanel
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNPanel", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNPanel_side").setAttribute("height", "30");
	doc.getElementById("MNPanel_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNPanel_side").src = "./PnlWznmQryMNPanel_aside.html";
	doc.getElementById("MNPanel_cont").src = "./PnlWznmQryMNPanel_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNPanel", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNPanel_side").setAttribute("height", "255");
	doc.getElementById("MNPanel_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNPanel_side").src = "./PnlWznmQryMNPanel_bside.html";
	doc.getElementById("MNPanel_cont").src = "./PnlWznmQryMNPanel_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNPanel_side").contentDocument;
	contdoc = doc.getElementById("MNPanel_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNPanel_side").contentDocument;
	contdoc = doc.getElementById("MNPanel_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNPanel_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNPanel_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNPanel_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryMNPanel", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQryMNPanel", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryMNPanel", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryMNPanel", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNPanel", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNPanel", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNPanel", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNPanel", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryMNPanel", "TcoMrefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQryMNPanel", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQryMNPanel", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQryMNPanel
		rownode = getRownode(srcdoc, "ListWznmQQryMNPanel", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQryMNPanel", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQryMNPanel", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQryMNPanel", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNPanel", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNPanel_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNPanel_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNPanel_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNPanel_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQryMNPanel", "ListWznmQQryMNPanel", retrieveTi(srcdoc, "TagWznmQryMNPanel", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQryMNPanel", "TxtRecord2"),
				["Mref"],
				["mref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNPanel", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNPanel", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQryMNPanel", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQryMNPanel", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryMNPanelData", scrJref, "StgIacQryWznmQryMNPanel");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNPanel", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQryMNPanel", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQryMNPanel", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNPanel", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQryMNPanelDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "ContInfWznmQryMNPanel", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StatAppWznmQryMNPanel", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StatShrWznmQryMNPanel", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StgIacWznmQryMNPanel", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "TagWznmQryMNPanel", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "ListWznmQQryMNPanel", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StatAppQryWznmQryMNPanel", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StatShrQryWznmQryMNPanel", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQryMNPanelData", "StgIacQryWznmQryMNPanel", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQryMNPanelData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQryMNPanelData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNPanel");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQryMNPanelData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
