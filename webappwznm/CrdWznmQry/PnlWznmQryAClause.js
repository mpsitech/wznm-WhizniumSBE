/**
  * \file PnlWznmQryAClause.js
  * web client functionality for panel PnlWznmQryAClause
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("AClause", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AClause_side").setAttribute("height", "30");
	doc.getElementById("AClause_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AClause_side").src = "./PnlWznmQryAClause_aside.html";
	doc.getElementById("AClause_cont").src = "./PnlWznmQryAClause_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("AClause", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AClause_side").setAttribute("height", "255");
	doc.getElementById("AClause_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AClause_side").src = "./PnlWznmQryAClause_bside.html";
	doc.getElementById("AClause_cont").src = "./PnlWznmQryAClause_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AClause_side").contentDocument;
	contdoc = doc.getElementById("AClause_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AClause_side").contentDocument;
	contdoc = doc.getElementById("AClause_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AClause_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AClause_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AClause_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryAClause", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQryAClause", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryAClause", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryAClause", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAClause", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAClause", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAClause", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAClause", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "ncol"));

		var TcoQmdWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoQmdWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoTypWidth"));
		var TcoCseWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoCseWidth"));
		var TcoPstWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoPstWidth"));
		var TcoVecWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoVecWidth"));
		var TcoVitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAClause", "TcoVitWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoQmd"); doc.widths.push(TcoQmdWidth); doc.fs.push("qmd"); wtot += TcoQmdWidth;
			} else if (i == 2) {
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ"); wtot += TcoTypWidth;
			} else if (i == 3) {
				doc.cols.push("TcoCse"); doc.widths.push(TcoCseWidth); doc.fs.push("cse"); wtot += TcoCseWidth;
			} else if (i == 4) {
				doc.cols.push("TcoPst"); doc.widths.push(TcoPstWidth); doc.fs.push("pst"); wtot += TcoPstWidth;
			} else if (i == 5) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQryAClause", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQryAClause", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQryAClause
		rownode = getRownode(srcdoc, "ListWznmQQryAClause", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQryAClause", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQryAClause", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQryAClause", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AClause", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AClause_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AClause_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AClause_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AClause_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQryAClause", "ListWznmQQryAClause", retrieveTi(srcdoc, "TagWznmQryAClause", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQryAClause", "TxtRecord2"),
				["Qmd","Typ","","Cse","Pst","Vec","Vit"],
				["qmd","typ","typ2","cse","pst","vec","vit"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAClause", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAClause", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQryAClause", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQryAClause", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryAClauseData", scrJref, "StgIacQryWznmQryAClause");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAClause", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQryAClause", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQryAClause", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAClause", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQryAClauseDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "ContInfWznmQryAClause", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StatAppWznmQryAClause", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StatShrWznmQryAClause", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StgIacWznmQryAClause", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "TagWznmQryAClause", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "ListWznmQQryAClause", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StatAppQryWznmQryAClause", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StatShrQryWznmQryAClause", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQryAClauseData", "StgIacQryWznmQryAClause", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQryAClauseData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQryAClauseData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AClause");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQryAClauseData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
