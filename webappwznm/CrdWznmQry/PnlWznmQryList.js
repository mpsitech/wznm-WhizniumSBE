/**
  * \file PnlWznmQryList.js
  * web client functionality for panel PnlWznmQryList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmQryList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQryList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmQryList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQryList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQryList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmQryList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmQryList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQryList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQryList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQryList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQryList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoSrfWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoTypWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoVerWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoSupWidth"));
		var TcoSrlWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoSrlWidth"));
		var TcoTblWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoTblWidth"));
		var TcoQtbWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryList", "TcoQtbWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQryList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTbl"); doc.widths.push(TcoTblWidth); doc.fs.push("tbl"); doc.tos.push(6); wtot += TcoTblWidth;
			} else if (i == 7) {
				doc.cols.push("TcoQtb"); doc.widths.push(TcoQtbWidth); doc.fs.push("qtb"); doc.tos.push(7); wtot += TcoQtbWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmQryList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQryList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQryList
		rownode = getRownode(srcdoc, "ListWznmQQryList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQryList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQryList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQryList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQryList", "ListWznmQQryList", retrieveTi(srcdoc, "TagWznmQryList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQryList", "TxtRecord2"),
				["Srf","","Typ","Ver","Sup","","Srl","Tbl","Qtb"],
				["srf","typ","typ2","ver","sup","srl","srl2","tbl","qtb"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQryList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQryList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryListData", scrJref, "StgIacQryWznmQryList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQryList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQryListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQryList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmQryList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryListData", scrJref, "ContIacWznmQryList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "ContIacWznmQryList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "ContInfWznmQryList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "StatShrWznmQryList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "StgIacWznmQryList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "TagWznmQryList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "ListWznmQQryList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "StatAppQryWznmQryList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "StatShrQryWznmQryList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQryListData", "StgIacQryWznmQryList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmQryListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQryListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQryListData") {
				mask = mergeDpchEngData(dom);
