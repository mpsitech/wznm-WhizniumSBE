/**
  * \file PnlWznmQryAOrder.js
  * web client functionality for panel PnlWznmQryAOrder
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("AOrder", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("AOrder_side").setAttribute("height", "30");
	doc.getElementById("AOrder_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("AOrder_side").src = "./PnlWznmQryAOrder_aside.html";
	doc.getElementById("AOrder_cont").src = "./PnlWznmQryAOrder_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("AOrder", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("AOrder_side").setAttribute("height", "255");
	doc.getElementById("AOrder_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("AOrder_side").src = "./PnlWznmQryAOrder_bside.html";
	doc.getElementById("AOrder_cont").src = "./PnlWznmQryAOrder_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AOrder_side").contentDocument;
	contdoc = doc.getElementById("AOrder_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("AOrder_side").contentDocument;
	contdoc = doc.getElementById("AOrder_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AOrder_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AOrder_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AOrder_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryAOrder", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQryAOrder", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryAOrder", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryAOrder", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAOrder", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAOrder", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAOrder", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAOrder", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "ncol"));

		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAOrder", "TcoShoWidth"));
		var TcoTcoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryAOrder", "TcoTcoWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTco"); doc.widths.push(TcoTcoWidth); doc.fs.push("tco"); wtot += TcoTcoWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQryAOrder", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQryAOrder", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQryAOrder
		rownode = getRownode(srcdoc, "ListWznmQQryAOrder", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQryAOrder", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQryAOrder", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQryAOrder", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AOrder", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AOrder_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AOrder_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AOrder_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AOrder_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQryAOrder", "ListWznmQQryAOrder", retrieveTi(srcdoc, "TagWznmQryAOrder", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQryAOrder", "TxtRecord2"),
				["Sho","Tco"],
				["sho","tco"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAOrder", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryAOrder", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQryAOrder", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQryAOrder", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryAOrderData", scrJref, "StgIacQryWznmQryAOrder");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryAOrder", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQryAOrder", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQryAOrder", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryAOrder", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQryAOrderDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "ContInfWznmQryAOrder", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StatAppWznmQryAOrder", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StatShrWznmQryAOrder", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StgIacWznmQryAOrder", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "TagWznmQryAOrder", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "ListWznmQQryAOrder", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StatAppQryWznmQryAOrder", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StatShrQryWznmQryAOrder", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQryAOrderData", "StgIacQryWznmQryAOrder", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQryAOrderData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQryAOrderData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AOrder");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQryAOrderData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
