/**
  * \file PnlWznmQrySup1NQuery.js
  * web client functionality for panel PnlWznmQrySup1NQuery
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Sup1NQuery", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Sup1NQuery_side").setAttribute("height", "30");
	doc.getElementById("Sup1NQuery_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Sup1NQuery_side").src = "./PnlWznmQrySup1NQuery_aside.html";
	doc.getElementById("Sup1NQuery_cont").src = "./PnlWznmQrySup1NQuery_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Sup1NQuery", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Sup1NQuery_side").setAttribute("height", "255");
	doc.getElementById("Sup1NQuery_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Sup1NQuery_side").src = "./PnlWznmQrySup1NQuery_bside.html";
	doc.getElementById("Sup1NQuery_cont").src = "./PnlWznmQrySup1NQuery_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NQuery_side").contentDocument;
	contdoc = doc.getElementById("Sup1NQuery_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Sup1NQuery_side").contentDocument;
	contdoc = doc.getElementById("Sup1NQuery_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NQuery_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NQuery_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NQuery_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQrySup1NQuery", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQrySup1NQuery", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQrySup1NQuery", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQrySup1NQuery", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQrySup1NQuery", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQrySup1NQuery", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQrySup1NQuery", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQrySup1NQuery
		rownode = getRownode(srcdoc, "ListWznmQQrySup1NQuery", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQrySup1NQuery", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NQuery", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NQuery_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NQuery_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NQuery_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NQuery_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQrySup1NQuery", "ListWznmQQrySup1NQuery", retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQrySup1NQuery", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQrySup1NQuery", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQrySup1NQuery", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQrySup1NQuery", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQrySup1NQuery", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQrySup1NQueryData", scrJref, "StgIacQryWznmQrySup1NQuery");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQrySup1NQuery", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQrySup1NQuery", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQrySup1NQuery", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQrySup1NQueryDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "ContInfWznmQrySup1NQuery", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StatAppWznmQrySup1NQuery", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StatShrWznmQrySup1NQuery", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StgIacWznmQrySup1NQuery", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "TagWznmQrySup1NQuery", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "ListWznmQQrySup1NQuery", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StatAppQryWznmQrySup1NQuery", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StatShrQryWznmQrySup1NQuery", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQrySup1NQueryData", "StgIacQryWznmQrySup1NQuery", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQrySup1NQueryData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQrySup1NQueryData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NQuery");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQrySup1NQueryData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
