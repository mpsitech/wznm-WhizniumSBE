/**
  * \file PnlWznmQryMNTable.js
  * web client functionality for panel PnlWznmQryMNTable
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("MNTable", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("MNTable_side").setAttribute("height", "30");
	doc.getElementById("MNTable_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("MNTable_side").src = "./PnlWznmQryMNTable_aside.html";
	doc.getElementById("MNTable_cont").src = "./PnlWznmQryMNTable_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("MNTable", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("MNTable_side").setAttribute("height", "255");
	doc.getElementById("MNTable_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("MNTable_side").src = "./PnlWznmQryMNTable_bside.html";
	doc.getElementById("MNTable_cont").src = "./PnlWznmQryMNTable_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNTable_side").contentDocument;
	contdoc = doc.getElementById("MNTable_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("MNTable_side").contentDocument;
	contdoc = doc.getElementById("MNTable_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNTable_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNTable_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNTable_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryMNTable", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQryMNTable", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQryMNTable", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQryMNTable", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNTable", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNTable", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNTable", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNTable", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryMNTable", "TcoMrefWidth"));
		var TcoTqmdWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryMNTable", "TcoTqmdWidth"));
		var TcoSrcWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryMNTable", "TcoSrcWidth"));
		var TcoPfxWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQryMNTable", "TcoPfxWidth"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTqmd"); doc.widths.push(TcoTqmdWidth); doc.fs.push("tqmd"); wtot += TcoTqmdWidth;
			} else if (i == 3) {
				doc.cols.push("TcoSrc"); doc.widths.push(TcoSrcWidth); doc.fs.push("src"); wtot += TcoSrcWidth;
			} else if (i == 4) {
				doc.cols.push("TcoPfx"); doc.widths.push(TcoPfxWidth); doc.fs.push("pfx"); wtot += TcoPfxWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQryMNTable", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQryMNTable", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQryMNTable
		rownode = getRownode(srcdoc, "ListWznmQQryMNTable", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQryMNTable", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQryMNTable", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQryMNTable", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNTable", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNTable_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNTable_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNTable_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNTable_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQryMNTable", "ListWznmQQryMNTable", retrieveTi(srcdoc, "TagWznmQryMNTable", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQryMNTable", "TxtRecord2"),
				["Mref","Tqmd","Src","Pfx"],
				["mref","tqmd","src","pfx"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNTable", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQryMNTable", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQryMNTable", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQryMNTable", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQryMNTableData", scrJref, "StgIacQryWznmQryMNTable");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQryMNTable", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQryMNTable", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQryMNTable", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQryMNTable", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQryMNTableDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "ContInfWznmQryMNTable", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StatAppWznmQryMNTable", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StatShrWznmQryMNTable", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StgIacWznmQryMNTable", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "TagWznmQryMNTable", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "ListWznmQQryMNTable", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StatAppQryWznmQryMNTable", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StatShrQryWznmQryMNTable", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQryMNTableData", "StgIacQryWznmQryMNTable", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQryMNTableData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQryMNTableData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNTable");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQryMNTableData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
