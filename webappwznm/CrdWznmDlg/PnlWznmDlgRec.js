/**
  * \file PnlWznmDlgRec.js
  * web client functionality for panel PnlWznmDlgRec
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppWznmDlgRec", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppWznmDlgRec", "initdone" + pnlshort, "true");
...
function checkInitdone() {
	...
	var initdoneAPar = (retrieveSi(srcdoc, "StatAppWznmDlgRec", "initdoneAPar") == "true");
...
function minimize() {
	...
	doc.getElementById("Rec_side").src = "./PnlWznmDlgRec_aside.html";
	doc.getElementById("Rec_cont").src = "./PnlWznmDlgRec_a.html";
};
...
function regularize() {
	...
	doc.getElementById("Rec_side").src = "./PnlWznmDlgRec_bside.html";
	doc.getElementById("Rec_cont").src = "./PnlWznmDlgRec_b.html";
};
...
function initA() {
	...
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmDlgRec", "Cpt"));
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmDlgRec", "Cpt"));

	checkInitdone();
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmDlgRec", "srefIxWznmVExpstate");
...
function refreshAside() {
	...
	ButRegularizeActive = (retrieveSi(srcdoc, "StatShrWznmDlgRec", "ButRegularizeActive") == "true");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWznmDlgRec", "TxtRef"));
};

function refreshB() {

	setTextContent(hdrdoc, hdrdoc.getElementById("TxtRef"), retrieveCi(srcdoc, "ContInfWznmDlgRec", "TxtRef"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmDlgRecDo", scrJref, ctlsref + "Click");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmDlgRecData", "ContInfWznmDlgRec", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmDlgRecData", "StatAppWznmDlgRec", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmDlgRecData", "StatShrWznmDlgRec", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmDlgRecData", "TagWznmDlgRec", srcdoc)) mask.push("tag");
...
function handleDpchEng(dom, dpch) {
	...
	srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmDlgRec", "srefIxWznmVExpstate");

	if (_scrJref == scrJref) {
		if (dpch == "DpchEngWznmDlgRecData") {
			mask = mergeDpchEngData(dom);

			if (mask.indexOf("statshr") != -1) {
				newSrefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmDlgRec", "srefIxWznmVExpstate");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmDlgRecData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmDlgRecData") {
				srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmDlgRec", "srefIxWznmVExpstate");
