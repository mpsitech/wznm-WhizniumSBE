/**
  * \file PnlWznmQcoList.js
  * web client functionality for panel PnlWznmQcoList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmQcoList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQcoList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmQcoList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmQcoList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQcoList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQcoList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmQcoList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmQcoList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQcoList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQcoList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQcoList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQcoList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQcoList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQcoList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmQcoList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmQcoList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoList", "TcoSrfWidth"));
		var TcoShoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoList", "TcoShoWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoList", "TcoTypWidth"));
		var TcoQryWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoList", "TcoQryWidth"));
		var TcoTcoWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoList", "TcoTcoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQcoList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoQry"); doc.widths.push(TcoQryWidth); doc.fs.push("qry"); doc.tos.push(3); wtot += TcoQryWidth;
			} else if (i == 5) {
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmQcoList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQcoList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQcoList
		rownode = getRownode(srcdoc, "ListWznmQQcoList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQcoList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQcoList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQcoList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQcoList", "ListWznmQQcoList", retrieveTi(srcdoc, "TagWznmQcoList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQcoList", "TxtRecord2"),
				["Srf","Sho","","Typ","Qry","Tco"],
				["srf","sho","typ","typ2","qry","tco"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQcoList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQcoList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQcoListData", scrJref, "StgIacQryWznmQcoList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQcoList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQcoListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmQcoList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmQcoList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQcoListData", scrJref, "ContIacWznmQcoList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "ContIacWznmQcoList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "ContInfWznmQcoList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "StatShrWznmQcoList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "StgIacWznmQcoList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "TagWznmQcoList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "ListWznmQQcoList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "StatAppQryWznmQcoList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "StatShrQryWznmQcoList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQcoListData", "StgIacQryWznmQcoList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmQcoListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQcoListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQcoListData") {
				mask = mergeDpchEngData(dom);
