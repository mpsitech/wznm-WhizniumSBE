/**
  * \file PnlWznmQcoRef1NControl.js
  * web client functionality for panel PnlWznmQcoRef1NControl
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NControl_side").src = "./PnlWznmQcoRef1NControl_aside.html";
	doc.getElementById("Ref1NControl_cont").src = "./PnlWznmQcoRef1NControl_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NControl_side").src = "./PnlWznmQcoRef1NControl_bside.html";
	doc.getElementById("Ref1NControl_cont").src = "./PnlWznmQcoRef1NControl_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmQcoRef1NControl", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoRef1NControl", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoRef1NControl", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoRef1NControl", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoRef1NControl", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmQcoRef1NControl", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmQcoRef1NControl", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQQcoRef1NControl
		rownode = getRownode(srcdoc, "ListWznmQQcoRef1NControl", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmQcoRef1NControl", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmQcoRef1NControl", "ListWznmQQcoRef1NControl", retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmQcoRef1NControl", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoRef1NControl", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmQcoRef1NControl", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmQcoRef1NControl", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmQcoRef1NControl", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmQcoRef1NControlData", scrJref, "StgIacQryWznmQcoRef1NControl");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmQcoRef1NControl", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmQcoRef1NControl", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmQcoRef1NControl", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmQcoRef1NControlDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "ContInfWznmQcoRef1NControl", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StatAppWznmQcoRef1NControl", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StatShrWznmQcoRef1NControl", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StgIacWznmQcoRef1NControl", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "TagWznmQcoRef1NControl", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "ListWznmQQcoRef1NControl", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StatAppQryWznmQcoRef1NControl", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StatShrQryWznmQcoRef1NControl", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmQcoRef1NControlData", "StgIacQryWznmQcoRef1NControl", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmQcoRef1NControlData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmQcoRef1NControlData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmQcoRef1NControlData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
