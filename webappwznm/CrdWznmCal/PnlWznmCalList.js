/**
  * \file PnlWznmCalList.js
  * web client functionality for panel PnlWznmCalList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmCalList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmCalList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmCalList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmCalList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCalList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCalList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmCalList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmCalList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmCalList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCalList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCalList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCalList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmCalList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCalList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCalList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCalList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCalList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCalList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmCalList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmCalList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoSrfWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoTypWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoVerWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoReuWidth"));
		var TcoIatWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoIatWidth"));
		var TcoRatWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCalList", "TcoRatWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmCalList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoIat"); doc.widths.push(TcoIatWidth); doc.fs.push("iat"); doc.tos.push(2); wtot += TcoIatWidth;
			} else if (i == 7) {
				doc.cols.push("TcoRat"); doc.widths.push(TcoRatWidth); doc.fs.push("rat"); doc.tos.push(1); wtot += TcoRatWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmCalList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCalList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCalList
		rownode = getRownode(srcdoc, "ListWznmQCalList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCalList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCalList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCalList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCalList", "ListWznmQCalList", retrieveTi(srcdoc, "TagWznmCalList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCalList", "TxtRecord2"),
				["Srf","","Typ","Ver","","Ret","Reu","Iat","","Rat",""],
				["srf","typ","typ2","ver","ret","ret2","reu","iat","iat2","rat","rat2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCalList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCalList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCalList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCalList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCalListData", scrJref, "StgIacQryWznmCalList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCalList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCalList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCalList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCalListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmCalList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmCalList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCalListData", scrJref, "ContIacWznmCalList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "ContIacWznmCalList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "ContInfWznmCalList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "StatShrWznmCalList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "StgIacWznmCalList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "TagWznmCalList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "ListWznmQCalList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "StatAppQryWznmCalList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "StatShrQryWznmCalList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCalListData", "StgIacQryWznmCalList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmCalListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCalListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCalListData") {
				mask = mergeDpchEngData(dom);
