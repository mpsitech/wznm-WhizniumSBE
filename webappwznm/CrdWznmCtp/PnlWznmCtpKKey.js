/**
  * \file PnlWznmCtpKKey.js
  * web client functionality for panel PnlWznmCtpKKey
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("KKey", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("KKey_side").setAttribute("height", "30");
	doc.getElementById("KKey_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("KKey_side").src = "./PnlWznmCtpKKey_aside.html";
	doc.getElementById("KKey_cont").src = "./PnlWznmCtpKKey_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("KKey", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("KKey_side").setAttribute("height", "255");
	doc.getElementById("KKey_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("KKey_side").src = "./PnlWznmCtpKKey_bside.html";
	doc.getElementById("KKey_cont").src = "./PnlWznmCtpKKey_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KKey_side").contentDocument;
	contdoc = doc.getElementById("KKey_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("KKey_side").contentDocument;
	contdoc = doc.getElementById("KKey_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KKey_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KKey_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KKey_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpKKey", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCtpKKey", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpKKey", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCtpKKey", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpKKey", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpKKey", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpKKey", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpKKey", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpKKey", "TcoCmtWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCtpKKey", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCtpKKey", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCtpKKey
		rownode = getRownode(srcdoc, "ListWznmQCtpKKey", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCtpKKey", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCtpKKey", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCtpKKey", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KKey", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KKey_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KKey_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KKey_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KKey_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCtpKKey", "ListWznmQCtpKKey", retrieveTi(srcdoc, "TagWznmCtpKKey", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCtpKKey", "TxtRecord2"),
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpKKey", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpKKey", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCtpKKey", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCtpKKey", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCtpKKeyData", scrJref, "StgIacQryWznmCtpKKey");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpKKey", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCtpKKey", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCtpKKey", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpKKey", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCtpKKeyDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "ContInfWznmCtpKKey", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StatAppWznmCtpKKey", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StatShrWznmCtpKKey", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StgIacWznmCtpKKey", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "TagWznmCtpKKey", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "ListWznmQCtpKKey", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StatAppQryWznmCtpKKey", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StatShrQryWznmCtpKKey", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpKKeyData", "StgIacQryWznmCtpKKey", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCtpKKeyData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCtpKKeyData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KKey");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCtpKKeyData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
