/**
  * \file PnlWznmCtpTpl1NCapability.js
  * web client functionality for panel PnlWznmCtpTpl1NCapability
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Tpl1NCapability", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Tpl1NCapability_side").setAttribute("height", "30");
	doc.getElementById("Tpl1NCapability_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Tpl1NCapability_side").src = "./PnlWznmCtpTpl1NCapability_aside.html";
	doc.getElementById("Tpl1NCapability_cont").src = "./PnlWznmCtpTpl1NCapability_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Tpl1NCapability", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Tpl1NCapability_side").setAttribute("height", "255");
	doc.getElementById("Tpl1NCapability_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Tpl1NCapability_side").src = "./PnlWznmCtpTpl1NCapability_bside.html";
	doc.getElementById("Tpl1NCapability_cont").src = "./PnlWznmCtpTpl1NCapability_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Tpl1NCapability_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NCapability_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Tpl1NCapability_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NCapability_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Tpl1NCapability_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Tpl1NCapability_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Tpl1NCapability_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCtpTpl1NCapability", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpTpl1NCapability", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpTpl1NCapability", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpTpl1NCapability", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpTpl1NCapability", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpTpl1NCapability", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCtpTpl1NCapability
		rownode = getRownode(srcdoc, "ListWznmQCtpTpl1NCapability", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCtpTpl1NCapability", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Tpl1NCapability", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NCapability_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NCapability_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Tpl1NCapability_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Tpl1NCapability_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCtpTpl1NCapability", "ListWznmQCtpTpl1NCapability", retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCtpTpl1NCapability", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpTpl1NCapability", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpTpl1NCapability", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCtpTpl1NCapability", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCtpTpl1NCapability", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCtpTpl1NCapabilityData", scrJref, "StgIacQryWznmCtpTpl1NCapability");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCtpTpl1NCapability", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCtpTpl1NCapability", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpTpl1NCapability", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCtpTpl1NCapabilityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "ContInfWznmCtpTpl1NCapability", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StatAppWznmCtpTpl1NCapability", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StatShrWznmCtpTpl1NCapability", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StgIacWznmCtpTpl1NCapability", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "TagWznmCtpTpl1NCapability", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "ListWznmQCtpTpl1NCapability", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StatAppQryWznmCtpTpl1NCapability", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StatShrQryWznmCtpTpl1NCapability", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpTpl1NCapabilityData", "StgIacQryWznmCtpTpl1NCapability", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCtpTpl1NCapabilityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCtpTpl1NCapabilityData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Tpl1NCapability");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCtpTpl1NCapabilityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
