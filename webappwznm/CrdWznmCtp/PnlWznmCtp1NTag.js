/**
  * \file PnlWznmCtp1NTag.js
  * web client functionality for panel PnlWznmCtp1NTag
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NTag", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NTag_side").setAttribute("height", "30");
	doc.getElementById("1NTag_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NTag_side").src = "./PnlWznmCtp1NTag_aside.html";
	doc.getElementById("1NTag_cont").src = "./PnlWznmCtp1NTag_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NTag", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NTag_side").setAttribute("height", "255");
	doc.getElementById("1NTag_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NTag_side").src = "./PnlWznmCtp1NTag_bside.html";
	doc.getElementById("1NTag_cont").src = "./PnlWznmCtp1NTag_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NTag_side").contentDocument;
	contdoc = doc.getElementById("1NTag_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NTag_side").contentDocument;
	contdoc = doc.getElementById("1NTag_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NTag_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NTag_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NTag_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtp1NTag", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCtp1NTag", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtp1NTag", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCtp1NTag", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtp1NTag", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtp1NTag", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtp1NTag", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtp1NTag", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtp1NTag", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmCtp1NTag", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCtp1NTag", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCtp1NTag
		rownode = getRownode(srcdoc, "ListWznmQCtp1NTag", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCtp1NTag", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCtp1NTag", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCtp1NTag", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NTag", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTag_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NTag_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NTag_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NTag_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCtp1NTag", "ListWznmQCtp1NTag", retrieveTi(srcdoc, "TagWznmCtp1NTag", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCtp1NTag", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtp1NTag", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtp1NTag", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCtp1NTag", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCtp1NTag", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCtp1NTagData", scrJref, "StgIacQryWznmCtp1NTag");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtp1NTag", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCtp1NTag", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmCtp1NTag", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtp1NTag", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCtp1NTagDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "ContInfWznmCtp1NTag", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StatAppWznmCtp1NTag", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StatShrWznmCtp1NTag", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StgIacWznmCtp1NTag", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "TagWznmCtp1NTag", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "ListWznmQCtp1NTag", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StatAppQryWznmCtp1NTag", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StatShrQryWznmCtp1NTag", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCtp1NTagData", "StgIacQryWznmCtp1NTag", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmCtp1NTagData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCtp1NTagData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NTag");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCtp1NTagData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
