/**
  * \file PnlWznmCtpList.js
  * web client functionality for panel PnlWznmCtpList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmCtpList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmCtpList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmCtpList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmCtpList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmCtpList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmCtpList", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmCtpList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmCtpList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpList", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpList", "TcoTitWidth"));
		var TcoVerWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpList", "TcoVerWidth"));
		var TcoAtfWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmCtpList", "TcoAtfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmCtpList", "numFTos"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmCtpList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmCtpList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQCtpList
		rownode = getRownode(srcdoc, "ListWznmQCtpList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmCtpList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmCtpList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmCtpList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmCtpList", "ListWznmQCtpList", retrieveTi(srcdoc, "TagWznmCtpList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmCtpList", "TxtRecord2"),
				["Srf","Tit","Ver","Atf",""],
				["srf","tit","ver","atf","atf2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmCtpList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmCtpList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmCtpList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCtpListData", scrJref, "StgIacQryWznmCtpList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmCtpList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmCtpList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmCtpList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmCtpListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmCtpList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmCtpList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmCtpListData", scrJref, "ContIacWznmCtpList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "ContIacWznmCtpList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "ContInfWznmCtpList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "StatShrWznmCtpList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "StgIacWznmCtpList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "TagWznmCtpList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "ListWznmQCtpList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "StatAppQryWznmCtpList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "StatShrQryWznmCtpList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmCtpListData", "StgIacQryWznmCtpList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmCtpListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmCtpListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmCtpListData") {
				mask = mergeDpchEngData(dom);
