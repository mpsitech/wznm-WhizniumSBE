/**
  * \file PnlWznmVer1NRelation.js
  * web client functionality for panel PnlWznmVer1NRelation
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NRelation", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NRelation_side").setAttribute("height", "30");
	doc.getElementById("1NRelation_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NRelation_side").src = "./PnlWznmVer1NRelation_aside.html";
	doc.getElementById("1NRelation_cont").src = "./PnlWznmVer1NRelation_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NRelation", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NRelation_side").setAttribute("height", "255");
	doc.getElementById("1NRelation_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NRelation_side").src = "./PnlWznmVer1NRelation_bside.html";
	doc.getElementById("1NRelation_cont").src = "./PnlWznmVer1NRelation_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NRelation_side").contentDocument;
	contdoc = doc.getElementById("1NRelation_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NRelation_side").contentDocument;
	contdoc = doc.getElementById("1NRelation_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NRelation_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NRelation_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NRelation_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NRelation", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NRelation", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NRelation", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NRelation", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NRelation", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NRelation", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NRelation", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NRelation", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NRelation", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NRelation", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NRelation", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NRelation
		rownode = getRownode(srcdoc, "ListWznmQVer1NRelation", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NRelation", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NRelation", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NRelation", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NRelation", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRelation_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NRelation_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NRelation_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NRelation_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NRelation", "ListWznmQVer1NRelation", retrieveTi(srcdoc, "TagWznmVer1NRelation", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NRelation", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NRelation", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NRelation", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NRelation", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NRelation", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NRelationData", scrJref, "StgIacQryWznmVer1NRelation");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NRelation", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NRelation", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NRelation", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NRelation", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NRelationDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "ContInfWznmVer1NRelation", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StatAppWznmVer1NRelation", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StatShrWznmVer1NRelation", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StgIacWznmVer1NRelation", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "TagWznmVer1NRelation", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "ListWznmQVer1NRelation", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StatAppQryWznmVer1NRelation", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StatShrQryWznmVer1NRelation", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NRelationData", "StgIacQryWznmVer1NRelation", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NRelationData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NRelationData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NRelation");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NRelationData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
