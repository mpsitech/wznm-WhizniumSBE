/**
  * \file PnlWznmVer1NJob.js
  * web client functionality for panel PnlWznmVer1NJob
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NJob", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NJob_side").setAttribute("height", "30");
	doc.getElementById("1NJob_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NJob_side").src = "./PnlWznmVer1NJob_aside.html";
	doc.getElementById("1NJob_cont").src = "./PnlWznmVer1NJob_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NJob", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NJob_side").setAttribute("height", "255");
	doc.getElementById("1NJob_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NJob_side").src = "./PnlWznmVer1NJob_bside.html";
	doc.getElementById("1NJob_cont").src = "./PnlWznmVer1NJob_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NJob_side").contentDocument;
	contdoc = doc.getElementById("1NJob_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NJob_side").contentDocument;
	contdoc = doc.getElementById("1NJob_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NJob_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NJob_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NJob_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NJob", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NJob", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NJob", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NJob", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NJob", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NJob", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NJob", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NJob", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NJob", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NJob", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NJob", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NJob
		rownode = getRownode(srcdoc, "ListWznmQVer1NJob", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NJob", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NJob", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NJob", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NJob", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NJob_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NJob_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NJob_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NJob_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NJob", "ListWznmQVer1NJob", retrieveTi(srcdoc, "TagWznmVer1NJob", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NJob", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NJob", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NJob", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NJob", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NJob", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NJobData", scrJref, "StgIacQryWznmVer1NJob");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NJob", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NJob", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NJob", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NJob", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NJobDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "ContInfWznmVer1NJob", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StatAppWznmVer1NJob", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StatShrWznmVer1NJob", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StgIacWznmVer1NJob", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "TagWznmVer1NJob", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "ListWznmQVer1NJob", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StatAppQryWznmVer1NJob", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StatShrQryWznmVer1NJob", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NJobData", "StgIacQryWznmVer1NJob", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NJobData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NJobData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NJob");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NJobData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
