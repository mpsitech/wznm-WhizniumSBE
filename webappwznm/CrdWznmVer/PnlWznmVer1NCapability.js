/**
  * \file PnlWznmVer1NCapability.js
  * web client functionality for panel PnlWznmVer1NCapability
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NCapability", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NCapability_side").setAttribute("height", "30");
	doc.getElementById("1NCapability_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NCapability_side").src = "./PnlWznmVer1NCapability_aside.html";
	doc.getElementById("1NCapability_cont").src = "./PnlWznmVer1NCapability_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NCapability", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NCapability_side").setAttribute("height", "255");
	doc.getElementById("1NCapability_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NCapability_side").src = "./PnlWznmVer1NCapability_bside.html";
	doc.getElementById("1NCapability_cont").src = "./PnlWznmVer1NCapability_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NCapability_side").contentDocument;
	contdoc = doc.getElementById("1NCapability_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NCapability_side").contentDocument;
	contdoc = doc.getElementById("1NCapability_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NCapability_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NCapability_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NCapability_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NCapability", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NCapability", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NCapability", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NCapability", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCapability", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCapability", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCapability", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCapability", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NCapability", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NCapability", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NCapability", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NCapability
		rownode = getRownode(srcdoc, "ListWznmQVer1NCapability", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NCapability", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NCapability", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NCapability", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NCapability", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCapability_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCapability_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NCapability_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NCapability_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NCapability", "ListWznmQVer1NCapability", retrieveTi(srcdoc, "TagWznmVer1NCapability", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NCapability", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCapability", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCapability", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NCapability", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NCapability", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NCapabilityData", scrJref, "StgIacQryWznmVer1NCapability");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCapability", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NCapability", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NCapability", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCapability", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NCapabilityDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "ContInfWznmVer1NCapability", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StatAppWznmVer1NCapability", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StatShrWznmVer1NCapability", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StgIacWznmVer1NCapability", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "TagWznmVer1NCapability", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "ListWznmQVer1NCapability", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StatAppQryWznmVer1NCapability", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StatShrQryWznmVer1NCapability", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCapabilityData", "StgIacQryWznmVer1NCapability", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NCapabilityData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NCapabilityData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NCapability");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NCapabilityData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
