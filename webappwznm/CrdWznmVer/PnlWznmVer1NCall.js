/**
  * \file PnlWznmVer1NCall.js
  * web client functionality for panel PnlWznmVer1NCall
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NCall", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NCall_side").setAttribute("height", "30");
	doc.getElementById("1NCall_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NCall_side").src = "./PnlWznmVer1NCall_aside.html";
	doc.getElementById("1NCall_cont").src = "./PnlWznmVer1NCall_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NCall", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NCall_side").setAttribute("height", "255");
	doc.getElementById("1NCall_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NCall_side").src = "./PnlWznmVer1NCall_bside.html";
	doc.getElementById("1NCall_cont").src = "./PnlWznmVer1NCall_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NCall_side").contentDocument;
	contdoc = doc.getElementById("1NCall_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NCall_side").contentDocument;
	contdoc = doc.getElementById("1NCall_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NCall_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NCall_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NCall_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NCall", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NCall", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NCall", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NCall", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCall", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCall", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCall", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCall", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NCall", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NCall", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NCall", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NCall
		rownode = getRownode(srcdoc, "ListWznmQVer1NCall", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NCall", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NCall", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NCall", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NCall", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCall_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NCall_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NCall_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NCall_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NCall", "ListWznmQVer1NCall", retrieveTi(srcdoc, "TagWznmVer1NCall", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NCall", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCall", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NCall", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NCall", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NCall", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NCallData", scrJref, "StgIacQryWznmVer1NCall");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NCall", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NCall", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NCall", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NCall", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NCallDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "ContInfWznmVer1NCall", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StatAppWznmVer1NCall", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StatShrWznmVer1NCall", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StgIacWznmVer1NCall", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "TagWznmVer1NCall", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "ListWznmQVer1NCall", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StatAppQryWznmVer1NCall", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StatShrQryWznmVer1NCall", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NCallData", "StgIacQryWznmVer1NCall", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NCallData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NCallData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NCall");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NCallData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
