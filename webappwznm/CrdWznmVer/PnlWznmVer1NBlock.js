/**
  * \file PnlWznmVer1NBlock.js
  * web client functionality for panel PnlWznmVer1NBlock
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NBlock", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NBlock_side").setAttribute("height", "30");
	doc.getElementById("1NBlock_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NBlock_side").src = "./PnlWznmVer1NBlock_aside.html";
	doc.getElementById("1NBlock_cont").src = "./PnlWznmVer1NBlock_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NBlock", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NBlock_side").setAttribute("height", "255");
	doc.getElementById("1NBlock_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NBlock_side").src = "./PnlWznmVer1NBlock_bside.html";
	doc.getElementById("1NBlock_cont").src = "./PnlWznmVer1NBlock_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NBlock_side").contentDocument;
	contdoc = doc.getElementById("1NBlock_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NBlock_side").contentDocument;
	contdoc = doc.getElementById("1NBlock_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NBlock_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NBlock_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NBlock_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NBlock", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NBlock", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NBlock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NBlock", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NBlock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NBlock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NBlock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NBlock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NBlock", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NBlock", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NBlock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NBlock
		rownode = getRownode(srcdoc, "ListWznmQVer1NBlock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NBlock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NBlock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NBlock", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NBlock", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NBlock_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NBlock_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NBlock_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NBlock_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NBlock", "ListWznmQVer1NBlock", retrieveTi(srcdoc, "TagWznmVer1NBlock", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NBlock", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NBlock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NBlock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NBlock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NBlock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NBlockData", scrJref, "StgIacQryWznmVer1NBlock");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NBlock", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NBlock", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NBlock", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NBlock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NBlockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "ContInfWznmVer1NBlock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StatAppWznmVer1NBlock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StatShrWznmVer1NBlock", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StgIacWznmVer1NBlock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "TagWznmVer1NBlock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "ListWznmQVer1NBlock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StatAppQryWznmVer1NBlock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StatShrQryWznmVer1NBlock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NBlockData", "StgIacQryWznmVer1NBlock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NBlockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NBlockData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NBlock");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NBlockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
