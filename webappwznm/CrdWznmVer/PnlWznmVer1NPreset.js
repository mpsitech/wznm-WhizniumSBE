/**
  * \file PnlWznmVer1NPreset.js
  * web client functionality for panel PnlWznmVer1NPreset
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NPreset", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NPreset_side").setAttribute("height", "30");
	doc.getElementById("1NPreset_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NPreset_side").src = "./PnlWznmVer1NPreset_aside.html";
	doc.getElementById("1NPreset_cont").src = "./PnlWznmVer1NPreset_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NPreset", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NPreset_side").setAttribute("height", "255");
	doc.getElementById("1NPreset_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NPreset_side").src = "./PnlWznmVer1NPreset_bside.html";
	doc.getElementById("1NPreset_cont").src = "./PnlWznmVer1NPreset_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NPreset_side").contentDocument;
	contdoc = doc.getElementById("1NPreset_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NPreset_side").contentDocument;
	contdoc = doc.getElementById("1NPreset_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NPreset_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NPreset_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NPreset_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NPreset", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NPreset", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NPreset", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NPreset", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NPreset", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NPreset", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NPreset", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NPreset", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NPreset", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NPreset", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NPreset", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NPreset
		rownode = getRownode(srcdoc, "ListWznmQVer1NPreset", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NPreset", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NPreset", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NPreset", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NPreset", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NPreset_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NPreset_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NPreset_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NPreset_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NPreset", "ListWznmQVer1NPreset", retrieveTi(srcdoc, "TagWznmVer1NPreset", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NPreset", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NPreset", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NPreset", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NPreset", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NPreset", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NPresetData", scrJref, "StgIacQryWznmVer1NPreset");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NPreset", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NPreset", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NPreset", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NPreset", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NPresetDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "ContInfWznmVer1NPreset", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StatAppWznmVer1NPreset", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StatShrWznmVer1NPreset", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StgIacWznmVer1NPreset", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "TagWznmVer1NPreset", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "ListWznmQVer1NPreset", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StatAppQryWznmVer1NPreset", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StatShrQryWznmVer1NPreset", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NPresetData", "StgIacQryWznmVer1NPreset", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NPresetData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NPresetData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NPreset");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NPresetData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
