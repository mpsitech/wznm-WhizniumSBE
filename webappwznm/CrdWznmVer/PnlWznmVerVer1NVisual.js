...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ver1NVisual", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ver1NVisual_side").setAttribute("height", "30");
	doc.getElementById("Ver1NVisual_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ver1NVisual_side").src = "./PnlWznmVerVer1NVisual_aside.html";
	doc.getElementById("Ver1NVisual_cont").src = "./PnlWznmVerVer1NVisual_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ver1NVisual", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ver1NVisual_side").setAttribute("height", "255");
	doc.getElementById("Ver1NVisual_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ver1NVisual_side").src = "./PnlWznmVerVer1NVisual_bside.html";
	doc.getElementById("Ver1NVisual_cont").src = "./PnlWznmVerVer1NVisual_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ver1NVisual_side").contentDocument;
	contdoc = doc.getElementById("Ver1NVisual_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ver1NVisual_side").contentDocument;
	contdoc = doc.getElementById("Ver1NVisual_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ver1NVisual_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ver1NVisual_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ver1NVisual_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVerVer1NVisual", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NVisual", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NVisual", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NVisual", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NVisual", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerVer1NVisual", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVerVer1NVisual", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVerVer1NVisual
		rownode = getRownode(srcdoc, "ListWznmQVerVer1NVisual", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVerVer1NVisual", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ver1NVisual", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NVisual_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NVisual_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ver1NVisual_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ver1NVisual_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVerVer1NVisual", "ListWznmQVerVer1NVisual", retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVerVer1NVisual", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NVisual", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NVisual", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVerVer1NVisual", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVerVer1NVisual", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVerVer1NVisualData", scrJref, "StgIacQryWznmVerVer1NVisual");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVerVer1NVisual", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NVisual", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NVisual", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVerVer1NVisualDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "ContInfWznmVerVer1NVisual", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StatAppWznmVerVer1NVisual", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StatShrWznmVerVer1NVisual", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StgIacWznmVerVer1NVisual", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "TagWznmVerVer1NVisual", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "ListWznmQVerVer1NVisual", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StatAppQryWznmVerVer1NVisual", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StatShrQryWznmVerVer1NVisual", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NVisualData", "StgIacQryWznmVerVer1NVisual", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVerVer1NVisualData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVerVer1NVisualData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ver1NVisual");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVerVer1NVisualData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
