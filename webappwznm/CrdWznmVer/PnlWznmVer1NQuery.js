/**
  * \file PnlWznmVer1NQuery.js
  * web client functionality for panel PnlWznmVer1NQuery
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NQuery", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NQuery_side").setAttribute("height", "30");
	doc.getElementById("1NQuery_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NQuery_side").src = "./PnlWznmVer1NQuery_aside.html";
	doc.getElementById("1NQuery_cont").src = "./PnlWznmVer1NQuery_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NQuery", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NQuery_side").setAttribute("height", "255");
	doc.getElementById("1NQuery_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NQuery_side").src = "./PnlWznmVer1NQuery_bside.html";
	doc.getElementById("1NQuery_cont").src = "./PnlWznmVer1NQuery_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NQuery_side").contentDocument;
	contdoc = doc.getElementById("1NQuery_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NQuery_side").contentDocument;
	contdoc = doc.getElementById("1NQuery_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NQuery_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NQuery_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NQuery_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NQuery", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NQuery", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NQuery", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NQuery", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NQuery", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NQuery", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NQuery", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NQuery", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NQuery", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NQuery", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NQuery", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NQuery
		rownode = getRownode(srcdoc, "ListWznmQVer1NQuery", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NQuery", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NQuery", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NQuery", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NQuery", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NQuery_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NQuery_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NQuery_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NQuery_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NQuery", "ListWznmQVer1NQuery", retrieveTi(srcdoc, "TagWznmVer1NQuery", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NQuery", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NQuery", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NQuery", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NQuery", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NQuery", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NQueryData", scrJref, "StgIacQryWznmVer1NQuery");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NQuery", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NQuery", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NQuery", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NQuery", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NQueryDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "ContInfWznmVer1NQuery", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StatAppWznmVer1NQuery", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StatShrWznmVer1NQuery", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StgIacWznmVer1NQuery", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "TagWznmVer1NQuery", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "ListWznmQVer1NQuery", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StatAppQryWznmVer1NQuery", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StatShrQryWznmVer1NQuery", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NQueryData", "StgIacQryWznmVer1NQuery", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NQueryData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NQueryData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NQuery");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NQueryData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
