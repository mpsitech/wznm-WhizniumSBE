...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlWznmVerList_aside.html";
	doc.getElementById("List_cont").src = "./PnlWznmVerList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlWznmVerList_bside.html";
	doc.getElementById("List_cont").src = "./PnlWznmVerList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVerList", "Trs"));
...
function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatShrWznmVerList", "srefIxWznmVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfWznmVerList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmVerList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVerList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmVerList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVerList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagWznmVerList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfWznmVerList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "ncol"));

		var TcoGrpWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoGrpWidth"));
		var TcoOwnWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoOwnWidth"));
		var TcoPrjWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoPrjWidth"));
		var TcoMajWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoMajWidth"));
		var TcoMinWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoMinWidth"));
		var TcoSubWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoSubWidth"));
		var TcoBvrWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoBvrWidth"));
		var TcoLocWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoLocWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoSteWidth"));
		var TcoDtyWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerList", "TcoDtyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmVerList", "numFTos"));
		...
		for (i = firstcol, ix = 0; i <= ncol; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoLoc"); doc.widths.push(TcoLocWidth); doc.fs.push("loc"); doc.tos.push(3); wtot += TcoLocWidth;
			} else if (i == 9) {
				doc.cols.push("TcoSte"); doc.widths.push(TcoSteWidth); doc.fs.push("ste2"); doc.tos.push(0); wtot += TcoSteWidth;
			} else if (i == 10) {
				doc.cols.push("TcoDty"); doc.widths.push(TcoDtyWidth); doc.fs.push("dty"); doc.tos.push(0); wtot += TcoDtyWidth;
			} else {
				ix--;
			};
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagWznmVerList", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVerList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVerList
		rownode = getRownode(srcdoc, "ListWznmQVerList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVerList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVerList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVerList", "numFCsiQst"));
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVerList", "ListWznmQVerList", retrieveTi(srcdoc, "TagWznmVerList", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVerList", "TxtRecord2"),
				["Grp","Own","Prj","Maj","Min","Sub","Bvr","Loc","","Ste","Dty",""],
				["grp","own","prj","maj","min","sub","bvr","loc","ste","ste2","dty","dty2"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVerList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVerList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVerListData", scrJref, "StgIacQryWznmVerList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerList", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVerList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVerListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacWznmVerList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacWznmVerList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVerListData", scrJref, "ContIacWznmVerList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "ContIacWznmVerList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "ContInfWznmVerList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "StatShrWznmVerList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "StgIacWznmVerList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "TagWznmVerList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "ListWznmQVerList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "StatAppQryWznmVerList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "StatShrQryWznmVerList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVerListData", "StgIacQryWznmVerList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngWznmVerListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVerListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVerListData") {
				mask = mergeDpchEngData(dom);
