/**
  * \file PnlWznmVerVer1NApp.js
  * web client functionality for panel PnlWznmVerVer1NApp
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ver1NApp", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ver1NApp_side").setAttribute("height", "30");
	doc.getElementById("Ver1NApp_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ver1NApp_side").src = "./PnlWznmVerVer1NApp_aside.html";
	doc.getElementById("Ver1NApp_cont").src = "./PnlWznmVerVer1NApp_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ver1NApp", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ver1NApp_side").setAttribute("height", "255");
	doc.getElementById("Ver1NApp_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ver1NApp_side").src = "./PnlWznmVerVer1NApp_bside.html";
	doc.getElementById("Ver1NApp_cont").src = "./PnlWznmVerVer1NApp_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ver1NApp_side").contentDocument;
	contdoc = doc.getElementById("Ver1NApp_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ver1NApp_side").contentDocument;
	contdoc = doc.getElementById("Ver1NApp_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ver1NApp_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ver1NApp_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ver1NApp_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NApp", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVerVer1NApp", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NApp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVerVer1NApp", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NApp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NApp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NApp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NApp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerVer1NApp", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVerVer1NApp", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVerVer1NApp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVerVer1NApp
		rownode = getRownode(srcdoc, "ListWznmQVerVer1NApp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVerVer1NApp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVerVer1NApp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVerVer1NApp", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ver1NApp", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NApp_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NApp_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ver1NApp_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ver1NApp_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVerVer1NApp", "ListWznmQVerVer1NApp", retrieveTi(srcdoc, "TagWznmVerVer1NApp", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVerVer1NApp", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NApp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NApp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVerVer1NApp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVerVer1NApp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVerVer1NAppData", scrJref, "StgIacQryWznmVerVer1NApp");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NApp", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVerVer1NApp", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NApp", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NApp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVerVer1NAppDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "ContInfWznmVerVer1NApp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StatAppWznmVerVer1NApp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StatShrWznmVerVer1NApp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StgIacWznmVerVer1NApp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "TagWznmVerVer1NApp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "ListWznmQVerVer1NApp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StatAppQryWznmVerVer1NApp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StatShrQryWznmVerVer1NApp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NAppData", "StgIacQryWznmVerVer1NApp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVerVer1NAppData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVerVer1NAppData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ver1NApp");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVerVer1NAppData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
