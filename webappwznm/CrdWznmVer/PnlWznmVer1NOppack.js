/**
  * \file PnlWznmVer1NOppack.js
  * web client functionality for panel PnlWznmVer1NOppack
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NOppack", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NOppack_side").setAttribute("height", "30");
	doc.getElementById("1NOppack_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NOppack_side").src = "./PnlWznmVer1NOppack_aside.html";
	doc.getElementById("1NOppack_cont").src = "./PnlWznmVer1NOppack_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NOppack", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NOppack_side").setAttribute("height", "255");
	doc.getElementById("1NOppack_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NOppack_side").src = "./PnlWznmVer1NOppack_bside.html";
	doc.getElementById("1NOppack_cont").src = "./PnlWznmVer1NOppack_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NOppack_side").contentDocument;
	contdoc = doc.getElementById("1NOppack_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NOppack_side").contentDocument;
	contdoc = doc.getElementById("1NOppack_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NOppack_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NOppack_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NOppack_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NOppack", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NOppack", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NOppack", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NOppack", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NOppack", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NOppack", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NOppack", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NOppack", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NOppack", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NOppack", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NOppack", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NOppack
		rownode = getRownode(srcdoc, "ListWznmQVer1NOppack", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NOppack", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NOppack", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NOppack", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NOppack", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NOppack_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NOppack_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NOppack_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NOppack_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NOppack", "ListWznmQVer1NOppack", retrieveTi(srcdoc, "TagWznmVer1NOppack", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NOppack", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NOppack", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NOppack", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NOppack", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NOppack", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NOppackData", scrJref, "StgIacQryWznmVer1NOppack");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NOppack", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NOppack", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NOppack", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NOppack", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NOppackDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "ContInfWznmVer1NOppack", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StatAppWznmVer1NOppack", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StatShrWznmVer1NOppack", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StgIacWznmVer1NOppack", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "TagWznmVer1NOppack", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "ListWznmQVer1NOppack", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StatAppQryWznmVer1NOppack", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StatShrQryWznmVer1NOppack", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NOppackData", "StgIacQryWznmVer1NOppack", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NOppackData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NOppackData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NOppack");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NOppackData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
