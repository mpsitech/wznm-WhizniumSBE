...
function minimize(updh) {
	...
	setSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("Ver1NError", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("Ver1NError_side").setAttribute("height", "30");
	doc.getElementById("Ver1NError_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("Ver1NError_side").src = "./PnlWznmVerVer1NError_aside.html";
	doc.getElementById("Ver1NError_cont").src = "./PnlWznmVerVer1NError_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("Ver1NError", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("Ver1NError_side").setAttribute("height", "255");
	doc.getElementById("Ver1NError_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("Ver1NError_side").src = "./PnlWznmVerVer1NError_bside.html";
	doc.getElementById("Ver1NError_cont").src = "./PnlWznmVerVer1NError_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Ver1NError_side").contentDocument;
	contdoc = doc.getElementById("Ver1NError_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("Ver1NError_side").contentDocument;
	contdoc = doc.getElementById("Ver1NError_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Ver1NError_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Ver1NError_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Ver1NError_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NError", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVerVer1NError", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVerVer1NError", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVerVer1NError", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NError", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NError", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NError", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NError", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVerVer1NError", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVerVer1NError", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVerVer1NError", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVerVer1NError
		rownode = getRownode(srcdoc, "ListWznmQVerVer1NError", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVerVer1NError", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVerVer1NError", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVerVer1NError", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Ver1NError", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NError_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Ver1NError_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Ver1NError_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Ver1NError_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVerVer1NError", "ListWznmQVerVer1NError", retrieveTi(srcdoc, "TagWznmVerVer1NError", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVerVer1NError", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NError", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVerVer1NError", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVerVer1NError", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVerVer1NError", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVerVer1NErrorData", scrJref, "StgIacQryWznmVerVer1NError");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVerVer1NError", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVerVer1NError", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVerVer1NError", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVerVer1NError", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVerVer1NErrorDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "ContInfWznmVerVer1NError", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StatAppWznmVerVer1NError", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StatShrWznmVerVer1NError", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StgIacWznmVerVer1NError", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "TagWznmVerVer1NError", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "ListWznmQVerVer1NError", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StatAppQryWznmVerVer1NError", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StatShrQryWznmVerVer1NError", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVerVer1NErrorData", "StgIacQryWznmVerVer1NError", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVerVer1NErrorData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVerVer1NErrorData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Ver1NError");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVerVer1NErrorData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
