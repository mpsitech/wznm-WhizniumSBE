/**
  * \file PnlWznmVer1NImpexpcplx.js
  * web client functionality for panel PnlWznmVer1NImpexpcplx
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate", "mind");

	getRecwnd().changeHeight("1NImpexpcplx", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NImpexpcplx_side").setAttribute("height", "30");
	doc.getElementById("1NImpexpcplx_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NImpexpcplx_side").src = "./PnlWznmVer1NImpexpcplx_aside.html";
	doc.getElementById("1NImpexpcplx_cont").src = "./PnlWznmVer1NImpexpcplx_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate", "regd");

	getRecwnd().changeHeight("1NImpexpcplx", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NImpexpcplx_side").setAttribute("height", "255");
	doc.getElementById("1NImpexpcplx_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NImpexpcplx_side").src = "./PnlWznmVer1NImpexpcplx_bside.html";
	doc.getElementById("1NImpexpcplx_cont").src = "./PnlWznmVer1NImpexpcplx_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NImpexpcplx_side").contentDocument;
	contdoc = doc.getElementById("1NImpexpcplx_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NImpexpcplx_side").contentDocument;
	contdoc = doc.getElementById("1NImpexpcplx_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NImpexpcplx_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NImpexpcplx_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NImpexpcplx_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxWznmVExpstate = retrieveSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryWznmVer1NImpexpcplx", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NImpexpcplx", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NImpexpcplx", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NImpexpcplx", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NImpexpcplx", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacWznmVer1NImpexpcplx", "TcoRefWidth"));
		...
		for (var j = 0; j < doc.cols.length; j++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", doc.cols[j]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (var i = 1; i <= ndisp; i++) {
		...
		// find element in ListWznmQVer1NImpexpcplx
		rownode = getRownode(srcdoc, "ListWznmQVer1NImpexpcplx", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfWznmVer1NImpexpcplx", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NImpexpcplx", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NImpexpcplx_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NImpexpcplx_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NImpexpcplx_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NImpexpcplx_src").contentDocument;
...
function handleButClipboardClick() {
	copyToClipboard(document, srcdoc, "WznmVer1NImpexpcplx", "ListWznmQVer1NImpexpcplx", retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "TxtRecord1"), retrieveTi(srcdoc, "TagWznmVer1NImpexpcplx", "TxtRecord2"),
				["Ref"],
				["ref"]);
};
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NImpexpcplx", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryWznmVer1NImpexpcplx", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryWznmVer1NImpexpcplx", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryWznmVer1NImpexpcplx", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppWznmVer1NImpexpcplxData", scrJref, "StgIacQryWznmVer1NImpexpcplx");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "firstcol"));

	setSi(srcdoc, "StatAppQryWznmVer1NImpexpcplx", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppWznmVer1NImpexpcplx", "srefIxWznmVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryWznmVer1NImpexpcplx", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppWznmVer1NImpexpcplxDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "ContInfWznmVer1NImpexpcplx", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StatAppWznmVer1NImpexpcplx", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StatShrWznmVer1NImpexpcplx", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StgIacWznmVer1NImpexpcplx", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "TagWznmVer1NImpexpcplx", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "ListWznmQVer1NImpexpcplx", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StatAppQryWznmVer1NImpexpcplx", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StatShrQryWznmVer1NImpexpcplx", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngWznmVer1NImpexpcplxData", "StgIacQryWznmVer1NImpexpcplx", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngWznmVer1NImpexpcplxData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngWznmVer1NImpexpcplxData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NImpexpcplx");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngWznmVer1NImpexpcplxData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
